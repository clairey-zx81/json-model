//
// Generated by JSON Model Compiler version 2
// see https://github.com/clairey-zx81/json-model
//

import json_model.*;
import java.util.Map;
import java.util.Set;
import java.util.HashMap;
import java.util.HashSet;
import java.util.Iterator;
import java.util.regex.Pattern;
import java.util.regex.Matcher;

@SuppressWarnings("unchecked")
public class trafo_2 extends ModelChecker
{
    static public final String VERSION = "2";

    public Map<String, Checker> trafo_2_map_pmap;

    // check $Zz (.'$Zz')
    public boolean json_model_5(Object val, Path path, Report rep)
    {
        // .'$Zz'
        boolean res = json_model_6(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected value for model \"$zero\" [.'$Zz']", path);
        }
        return res;
    }

    // check $z (.'$z')
    public boolean json_model_3(Object val, Path path, Report rep)
    {
        // .'$z'
        boolean res = json_model_6(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected value for model \"$Zz#zero\" [.'$z']", path);
        }
        return res;
    }

    // check $l (.'$l')
    public boolean json_model_4(Object val, Path path, Report rep)
    {
        // .'$l'
        boolean res = json.isArray(val);
        if (res)
        {
            int arr_0_idx = -1;
            Iterator<Object> arr_0_item_loop = json.arrayIterator(val);
            while (arr_0_item_loop.hasNext())
            {
                arr_0_idx++;
                Object arr_0_item = arr_0_item_loop.next();
                Path arr_0_lpath = new Path(arr_0_idx, path);
                // .'$l'.0
                res = json_model_6(arr_0_item, (path != null ? arr_0_lpath : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected value for model \"$z\" [.'$l'.0]", (path != null ? arr_0_lpath : null));
                    break;
                }
            }
        }
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$l']", path);
        }
        return res;
    }

    // check $ (.)
    public boolean json_model_1(Object val, Path path, Report rep)
    {
        // .
        boolean res = json_model_4(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected value for model \"$l\" [.]", path);
        }
        return res;
    }

    // check $Zz#zero (.'$Zz#zero')
    public boolean json_model_6(Object val, Path path, Report rep)
    {
        // .'$Zz#zero'
        // check close must only props
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Zz#zero']", path);
            return false;
        }
        if (json.objectSize(val) != 2)
        {
            if (rep != null) rep.addEntry("bad property count [.'$Zz#zero']", path);
            return false;
        }
        Path lpath;
        Object pval;
        boolean res;
        if (! ((pval = json.objectValue(val, "za")) != null))
        {
            if (rep != null) rep.addEntry("missing mandatory prop <za> [.'$Zz#zero']", path);
            return false;
        }
        lpath = new Path("za", path);
        // .'$Zz#zero'.za
        res = json.isInteger(pval) && json.asLong(pval) >= 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("not a 0 strict int [.'$Zz#zero'.za]", (path != null ? lpath : null));
            if (rep != null) rep.addEntry("unexpected value for mandatory prop <za> [.'$Zz#zero']", (path != null ? lpath : null));
            return false;
        }
        if (! ((pval = json.objectValue(val, "zb")) != null))
        {
            if (rep != null) rep.addEntry("missing mandatory prop <zb> [.'$Zz#zero']", path);
            return false;
        }
        lpath = new Path("zb", path);
        // .'$Zz#zero'.zb
        res = json.isInteger(pval) && json.asLong(pval) >= 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("not a 0 strict int [.'$Zz#zero'.zb]", (path != null ? lpath : null));
            if (rep != null) rep.addEntry("unexpected value for mandatory prop <zb> [.'$Zz#zero']", (path != null ? lpath : null));
            return false;
        }
        return true;
    }


    public void init(JSON json)
    {
        if (!initialized)
        {
            try {
            trafo_2_map_pmap = new HashMap<String, Checker>();
            trafo_2_map_pmap.put("", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_4(o, p, r);} });
            trafo_2_map_pmap.put("Zz", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_6(o, p, r);} });
            trafo_2_map_pmap.put("z", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_6(o, p, r);} });
            trafo_2_map_pmap.put("l", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_4(o, p, r);} });
                super.init(json);
            }
            catch (Exception e) {
                throw new Error("cannot initialized model checker: " + e);
            }
        }
    }

    public void free()
    {
        if (initialized)
        {
            super.free();
            trafo_2_map_pmap = null;
        }
    }

    public Checker get(String name)
    {
        return trafo_2_map_pmap.get(name);
    }

    public Set<String> models()
    {
        return trafo_2_map_pmap.keySet();
    }

    static public void main(String[] args) throws Exception
    {
        ModelChecker checker = new trafo_2();
        Main.main("trafo_2", checker, VERSION, args);
    }
}
