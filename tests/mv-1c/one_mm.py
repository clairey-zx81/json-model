...
def _jm_obj_0(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    for prop, pval in val.items():
        ...
        if prop == "must":
            ...
            if not res:
                rep is None or rep.append(("invalid mandatory property value [.must]", lpath_0 if path is not None else None))
                return False
        elif prop == "may":
            ...
            if not res:
                rep is None or rep.append(("invalid may property value [.may]", lpath_0 if path is not None else None))
                return False
        else:
    ...
    if must_count != 1:
        if rep is not None:
            if not "must" in val:
                rep is None or rep.append(("missing mandatory prop <must> []", path))
        return False
    return True
