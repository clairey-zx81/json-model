...
static bool json_model_6(const json_t* val, Path* path, Report* rep)
{
    ...
    if (res)
    {
        ...
        json_array_foreach(val, arr_0_idx, arr_0_item)
        {
            ...
            // $.'$D#l'.0
            res = json_model_10(arr_0_item, path, rep);
            if (! res)
            {
...
// check _jm_obj_0_must_ua ($.'$D#U#un'.ua)
static bool _jm_f_0(const json_t* val, Path* path, Report* rep)
{
    bool res;
    // $.'$D#U#un'.ua
    ...
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a 0 strict int [$.'$D#U#un'.ua]", path);
    }
    return res;
}

// check _jm_obj_0_must_ub ($.'$D#U#un'.ub)
static bool _jm_f_1(const json_t* val, Path* path, Report* rep)
{
    bool res;
    // $.'$D#U#un'.ub
    ...
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a 0 strict int [$.'$D#U#un'.ub]", path);
    }
    return res;
}
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if ((pfun = _jm_obj_0_must(prop)))
        {
            ...
            if (pfun != NULL)
            {
                ...
                {
                    if (rep) jm_report_add_entry(rep, "invalid must property value [$.'$D#U#un']", (path ? &lpath_0 : NULL));
                    return false;
                }
            }
        }
...
char *CHECK_init(void)
{
    if (!initialized)
    {
        ...
        _jm_obj_0_must_tab[0] = (propmap_t) { "ua", _jm_f_0 };
        _jm_obj_0_must_tab[1] = (propmap_t) { "ub", _jm_f_1 };
        jm_sort_propmap(_jm_obj_0_must_tab, 2);
        check_model_map_tab[0] = (propmap_t) { "", json_model_1 };
        check_model_map_tab[1] = (propmap_t) { "D", json_model_3 };
