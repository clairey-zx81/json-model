...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        ...
        if not res:
            rep is None or rep.append(("unexpected value for model \"=1\" [.'|'.0]", path))
            # .'|'.1
            res = isinstance(val, int) and not isinstance(val, bool) and val == 2
            if not res:
                rep is None or rep.append(("unexpected value for model \"=2\" [.'|'.1]", path))
                # .'|'.2
                res = isinstance(val, int) and not isinstance(val, bool) and val == 3
                if not res:
                    rep is None or rep.append(("unexpected value for model \"=3\" [.'|'.2]", path))
        if res:
