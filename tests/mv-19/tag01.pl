...
sub json_model_2($$$)
{
    ...
    unless ($res)
    {
        push @$rep, ["unexpected value for model \"=true\" [.'\$Aa'.t]", defined $path ? $lpath : undef] if defined $rep;
...
sub json_model_3($$$)
{
    ...
    unless ($res)
    {
        push @$rep, ["unexpected value for model \"=false\" [.'\$Bb'.t]", defined $path ? $lpath : undef] if defined $rep;
