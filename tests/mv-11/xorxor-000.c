...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    if (unlikely(! res))
    {
        if (rep) jm_report_add_entry(rep, "unexpected value for model \"$Xx\" [.'|'.0]", path);
        // .'|'.1
        ...
        if (likely(res))
        {
            ...
            json_array_foreach(val, arr_0_idx, arr_0_item)
            {
                ...
                if (unlikely(! res))
                {
                    if (rep) jm_report_add_entry(rep, "unexpected value for model \"$Xx\" [.'|'.1.0]", (path ? &arr_0_lpath : NULL));
                    break;
                }
            }
        }
