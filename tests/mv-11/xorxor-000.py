...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if not res:
        rep is None or rep.append(("unexpected value for model \"$Xx\" [.'|'.0]", path))
        # .'|'.1
        ...
        if res:
            for arr_0_idx, arr_0_item in enumerate(val):
                ...
                if not res:
                    rep is None or rep.append(("unexpected value for model \"$Xx\" [.'|'.1.0]", arr_0_lpath if path is not None else None))
                    break
        if not res:
