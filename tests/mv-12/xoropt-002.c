...
static bool _jm_obj_0(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (strcmp(prop, "c") == 0)
        {
            ...
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "not a 0 strict int [.'|'.2.c]", (path ? &lpath_0 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid mandatory property value [.'|'.2.c]", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else
