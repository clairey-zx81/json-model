...
def _jm_obj_0(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    for prop, pval in val.items():
        ...
        if prop == "c":
            ...
            if not res:
                rep is None or rep.append(("not a 0 strict int [.'|'.2.c]", lpath_0 if path is not None else None))
            if not res:
                rep is None or rep.append(("invalid must property value [.'|'.2.c]", lpath_0 if path is not None else None))
                return False
        else:
