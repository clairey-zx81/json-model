...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    if (likely(res))
    {
        res = json_model_2(json_array_get(val, 0), NULL, NULL);
        if (likely(res))
            res = json_model_2(json_array_get(val, 1), NULL, NULL);
    }
    return res;
}
...
const char *check_model_init(void)
{
    if (!initialized)
    {
        ...
        check_model_map_tab[1] = (jm_propmap_t) { "ident", json_model_2 };
