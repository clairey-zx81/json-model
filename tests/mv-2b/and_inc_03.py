...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    for prop, pval in val.items():
        if prop == "a":
            res = isinstance(pval, str) and _jm_re_1(pval, None, None) and _jm_re_0(pval, None, None)
            if not res:
                return False
            continue
        elif prop == "b":
            res = isinstance(pval, str) and _jm_re_3(pval, None, None) and _jm_re_2(pval, None, None)
            if not res:
                return False
            continue
        return False
