...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (unlikely(jm_str_eq_4(prop, 0x006d6f6e)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.nom]", (path ? &lpath_0 : NULL));
        ...
        else if (likely(jm_str_eq_7(prop, 0x00006d6f6e657270LL)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.prenom]", (path ? &lpath_0 : NULL));
                if (rep) jm_report_add_entry(rep, "invalid mandatory prop value [.prenom]", (path ? &lpath_0 : NULL));
                return false;
            }
            continue;
        }
        if (likely(jm_str_eq_4(prop, 0x00656761)))
        {
