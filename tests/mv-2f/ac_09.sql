...
BEGIN
  ...
  FOR prop, pval IN SELECT * FROM JSONB_EACH(val) LOOP
    IF _jm_re_0(prop, NULL, NULL) THEN
      res := JSONB_TYPEOF(pval) = 'string' AND STARTS_WITH(JSON_VALUE(pval, '$' RETURNING TEXT), 'bla');
      IF NOT res THEN
        RETURN FALSE;
      END IF;
    ELSE
...
BEGIN
  ...
  IF res THEN
    FOR arr_0_idx IN 0 .. JSONB_ARRAY_LENGTH(val) - 1 LOOP
      ...
      res := JSONB_TYPEOF(arr_0_item) = 'string' AND STARTS_WITH(JSON_VALUE(arr_0_item, '$' RETURNING TEXT), 'foo');
      IF NOT res THEN
        EXIT;
      END IF;
    END LOOP;
