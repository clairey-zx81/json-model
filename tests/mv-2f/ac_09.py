...
def _jm_obj_0(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    for prop, pval in val.items():
        if _jm_re_0(prop, None, None):
            res = isinstance(pval, str) and pval.startswith("bla")
            if not res:
                return False
        else:
            return False
...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        for arr_0_idx, arr_0_item in enumerate(val):
            res = isinstance(arr_0_item, str) and arr_0_item.startswith("foo")
            if not res:
                break
...
def check_model_init():
    ...
    if not initialized:
        ...
        _jm_re_0_reco = re.compile("^[a-zA-Z]+$")
