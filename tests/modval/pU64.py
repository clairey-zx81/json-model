...
def json_model_1(value: Jsonable, path: str, rep: Report = None) -> bool:
    # $
    result = isinstance(value, int) and not isinstance(value, bool) and value >= 0 or _rep(f"invalid $U64 at {path}", rep)
    if not result:
        rep is None or rep.append(f"not an expected $U64 at {path} [$]")
    return result
