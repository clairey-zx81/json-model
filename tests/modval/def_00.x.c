...
static propmap_t _check_model_map_tab[2];
...
static bool json_model_2(const json_t* val, Path* path, Report* rep)
{
    ...
    if (! res)
    {
        if (rep) report_add_entry(rep, "not a 0.0 float [$.'$pos']", path);
    }
    return res;
}
...
static bool json_model_1(const json_t* val, Path* path, Report* rep)
{
    ...
    if (! res)
    {
        if (rep) report_add_entry(rep, "unexpected $pos [$]", path);
    }
    return res;
}
...
{
    return search_propmap(pname, _check_model_map_tab, 2);
}
...
char *check_model_init(void)
{
    if (!initialized)
    {
        ...
        _check_model_map_tab[1] = (propmap_t) { "pos", json_model_2 };
        sort_propmap(_check_model_map_tab, 2);
    }
    return NULL;
}
