...
static bool _jm_f_0(const json_t* val, Path* path, Report* rep)
{
    ...
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $A [$.'$R'.a]", path);
    }
    return res;
}
...
static bool _jm_obj_0(const json_t* val, Path* path, Report* rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if ((pfun = _jm_obj_0_may(prop)))
        {
            ...
            {
                if (rep) jm_report_add_entry(rep, "invalid may property value [$.'$R']", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else
...
static bool _jm_f_1(const json_t* val, Path* path, Report* rep)
{
    ...
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $R [$.RA]", path);
    }
    return res;
}
...
char *CHECK_init(void)
{
    if (!initialized)
    {
        ...
        _jm_obj_0_may_tab[0] = (propmap_t) { "a", _jm_f_0 };
        jm_sort_propmap(_jm_obj_0_may_tab, 1);
        _jm_obj_1_must_tab[0] = (propmap_t) { "RA", _jm_f_1 };
        jm_sort_propmap(_jm_obj_1_must_tab, 1);
        _jm_obj_1_may_tab[0] = (propmap_t) { "a", _jm_f_2 };
        _jm_obj_1_may_tab[1] = (propmap_t) { "b", _jm_f_3 };
