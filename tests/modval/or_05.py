...
# regex "/[a-z]/"
_jm_re_1 = re.compile("[a-z]").search
# regex "/[A-Z]/"
_jm_re_2 = re.compile("[A-Z]").search
...
def json_model_1(value: Jsonable, path: str, rep: Report = None) -> bool:
    ...
    if result:
        ...
        if not result:
            # $.'|'.1
            # "/[a-z]/"
            result = _jm_re_1(value) is not None or _rep(f"does not match FESC at {path}", rep)
            if not result:
                rep is None or rep.append(f"not an expected REGEX at {path} [$.'|'.1]")
            if not result:
                # $.'|'.2
                # "/[A-Z]/"
                result = _jm_re_2(value) is not None or _rep(f"does not match FESC at {path}", rep)
                if not result:
                    rep is None or rep.append(f"not an expected REGEX at {path} [$.'|'.2]")
        if not result:
