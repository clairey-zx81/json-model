...
def json_model_1(value: Jsonable, path: str, rep: Report = None) -> bool:
    ...
    if result:
        for array_0_idx, array_0_item in enumerate(value):
            ...
            # $.0
            result = isinstance(array_0_item, list)
            if result:
                for array_1_idx, array_1_item in enumerate(array_0_item):
                    lpath = lpath + '.' + str(array_1_idx)
                    # $.0.0
                    result = isinstance(array_1_item, str)
                    if not result:
                        rep is None or rep.append(f"not an expected string at {lpath} [$.0.0]")
                    if not result:
                        break
            if not result:
