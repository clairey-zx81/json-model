...
static bool json_model_1(const json_t* val, Path* path, Report* rep)
{
    ...
    if (res)
    {
        ...
        if (! res)
        {
            if (rep) report_add_entry(rep, "unexpected REGEX [$.'&'.0]", path);
        }
        if (res)
        {
            // $.'&'.1
            ...
            if (! res)
            {
                if (rep) report_add_entry(rep, "unexpected REGEX [$.'&'.1]", path);
            }
        }
    }
    if (! res)
    {
        if (rep) report_add_entry(rep, "not all model match [$.'&']", path);
    }
    return res;
}
