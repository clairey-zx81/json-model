...
def _jm_f_0(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if not res:
        rep is None or rep.append(("not a 0 int [$.'|'.1.b]", path))
    return res
...
def _jm_f_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    res = False
    if not res:
        rep is None or rep.append(("unexpected $NONE [$.'|'.1.a]", path))
    return res
    ...
    for prop, pval in val.items():
        ...
        if pfun := _jm_obj_0_must.get(prop):
            ...
            if pfun != UNDEFINED and not pfun(pval, lpath_0 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'|'.1]", lpath_0 if path is not None else None))
                return False
        else:
            # handle other props
            # $.'|'.1.''
            ...
            if not res:
                rep is None or rep.append(("not a 0 int [$.'|'.1.'']", lpath_0 if path is not None else None))
            if not res:
                return False
    if must_count != 1:
...
def _jm_f_3(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    res = False
    if not res:
        rep is None or rep.append(("unexpected $NONE [$.'|'.0.b]", path))
    return res
    ...
    for prop, pval in val.items():
        ...
        if pfun := _jm_obj_1_must.get(prop):
            ...
            if pfun != UNDEFINED and not pfun(pval, lpath_1 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'|'.0]", lpath_1 if path is not None else None))
                return False
        else:
            # handle other props
            # $.'|'.0.''
            res = isinstance(pval, int) and not isinstance(pval, bool) and pval >= 0
            if not res:
                rep is None or rep.append(("not a 0 int [$.'|'.0.'']", lpath_1 if path is not None else None))
            if not res:
                return False
    if must_count != 1:
