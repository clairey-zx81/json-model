...
def _jm_f_0(value: Jsonable, path: str, rep: Report = None) -> bool:
    # $.'|'.0.t
    result = isinstance(value, float) and value == 3.1415927
    if not result:
        rep is None or rep.append(f"not an expected =3.1415927 at {path} [$.'|'.0.t]")
    return result

# define "_jm_obj_0_must_pi" ($.'|'.0.pi)
def _jm_f_1(value: Jsonable, path: str, rep: Report = None) -> bool:
    # $.'|'.0.pi
    result = isinstance(value, str)
    if not result:
        rep is None or rep.append(f"not an expected string at {path} [$.'|'.0.pi]")
    return result
...
def _jm_f_2(value: Jsonable, path: str, rep: Report = None) -> bool:
    ...
    if not result:
        rep is None or rep.append(f"not an expected =1.0 at {path} [$.'|'.1.t]")
    return result

# define "_jm_obj_1_must_one" ($.'|'.1.one)
def _jm_f_3(value: Jsonable, path: str, rep: Report = None) -> bool:
    # $.'|'.1.one
    result = isinstance(value, str)
    if not result:
        rep is None or rep.append(f"not an expected string at {path} [$.'|'.1.one]")
    return result

# define "_jm_obj_2_must_t" ($.'|'.2.t)
def _jm_f_4(value: Jsonable, path: str, rep: Report = None) -> bool:
    # $.'|'.2.t
    result = isinstance(value, float) and value == 0.5
    if not result:
        rep is None or rep.append(f"not an expected =0.5 at {path} [$.'|'.2.t]")
    return result

# define "_jm_obj_2_must_half" ($.'|'.2.half)
def _jm_f_5(value: Jsonable, path: str, rep: Report = None) -> bool:
    # $.'|'.2.half
    result = isinstance(value, str)
    if not result:
        rep is None or rep.append(f"not an expected string at {path} [$.'|'.2.half]")
    return result
