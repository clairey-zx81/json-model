...
def json_model_1(value: Jsonable, path: str, rep: Report = None) -> bool:
    # $
    result = not isinstance(value, (list, dict)) and value in {'Susie'}
    if not result:
    ...
    if not result:
        ...
        if not result:
            ...
            if not result:
                rep is None or rep.append(f"not null at {path} [$.'|'.1]")
            if not result:
                # $.'|'.2
                result = isinstance(value, float) and value == 3.1415927
                if not result:
                    rep is None or rep.append(f"not an expected =3.1415927 at {path} [$.'|'.2]")
        if not result:
