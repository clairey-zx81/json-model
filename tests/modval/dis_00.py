...
def _jm_f_0(value: Jsonable, path: str, rep: Report = None) -> bool:
    ...
    if not result:
        rep is None or rep.append(f"not an expected a at {path} [$.'|'.0.tag]")
    return result
...
def _jm_f_2(value: Jsonable, path: str, rep: Report = None) -> bool:
    ...
    if not result:
        rep is None or rep.append(f"not an expected b at {path} [$.'|'.1.tag]")
    return result
...
def _jm_f_4(value: Jsonable, path: str, rep: Report = None) -> bool:
    # $.'|'.2.tag
    result = isinstance(value, str) and value == "c"
    if not result:
        rep is None or rep.append(f"not an expected c at {path} [$.'|'.2.tag]")
    return result
