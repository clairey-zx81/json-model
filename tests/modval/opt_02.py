# Generated for model: ./opt_02.model.json
#
...
def json_model_0(value: Jsonable, path: str, rep: Report = None) -> bool:
    ...
    if not result:
        rep is None or rep.append(f"not a 0 int at {path} [$.'|'.0]")
    if not result:
        # $.'|'.1
        result = isinstance(value, float) and value >= 0.0
        if not result:
            rep is None or rep.append(f"not a 0.0 float at {path} [$.'|'.1]")
        if not result:
            # $.'|'.2
            result = isinstance(value, bool)
            if not result:
                rep is None or rep.append(f"not a bool at {path} [$.'|'.2]")
    if not result:
