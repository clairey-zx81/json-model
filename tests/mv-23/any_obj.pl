...
sub json_model_2($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Dd'
    my $res = jm_is_string($val) && $val eq "d";
    push @$rep, ["unexpected value for model \"_d\" [.'\$Dd']", $path] if defined $rep and not $res;
    return $res;
}
...
sub json_model_1($$$)
{
    ...
    unless ($res)
    {
        push @$rep, ["unexpected value for model \"\" [.a]", defined $path ? $lpath : undef] if defined $rep;
