//
// Generated by JSON Model Compiler version 2
// see https://github.com/clairey-zx81/json-model
//
import 'dart:convert';
import 'dart:io';
import 'dart:core';
// Import the helper library generated nearby
import 'package:json_model_runtime/json_model.dart' as runtime;
import 'package:json_model_runtime/main.dart';

//
// Generated JSON Model Validator
//
const String JSON_MODEL_VERSION = "2";

var check_model_map = <String, Function>{};

// object .'$a'.''.'|'.1
bool _jm_obj_0(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    if (! (val is Map))
    {
        rep?.add(["not an object [.'\$a'.''.'|'.1]", path]);
        return false;
    }
    if (val.length == 0)
    {
        return true;
    }
    else
    {
        rep?.add(["expecting empty object [.'\$a'.''.'|'.1]", path]);
        return false;
    }
}

// check $a (.'$a')
bool json_model_2(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    // .'$a'
    if (! (val is Map))
    {
        rep?.add(["not an object [.'\$a']", path]);
        return false;
    }
    bool res;
    for (var entry in val.entries)
    {
        var prop = entry.key;
        var pval = entry.value;
        List<dynamic>? lpath_0 = (path != null ? [...?path, prop] : null);
        // handle other props
        // .'$a'.''
        // .'$a'.''.'|'.0
        res = json_model_3(pval, (path != null ? lpath_0 : null), rep);
        if (! res)
        {
            rep?.add(["unexpected \$r [.'\$a'.''.'|'.0]", (path != null ? lpath_0 : null)]);
            // .'$a'.''.'|'.1
            res = _jm_obj_0(pval, (path != null ? lpath_0 : null), rep);
            if (! res)
            {
                rep?.add(["unexpected element [.'\$a'.''.'|'.1]", (path != null ? lpath_0 : null)]);
            }
        }
        if (res)
        {
            rep?.clear();
        }
        else
        {
            rep?.add(["no model matched [.'\$a'.''.'|']", (path != null ? lpath_0 : null)]);
            return false;
        }
    }
    return true;
}

// object .'$r'.'|'.1
bool _jm_obj_1(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    if (! (val is Map))
    {
        rep?.add(["not an object [.'\$r'.'|'.1]", path]);
        return false;
    }
    if (val.length == 0)
    {
        return true;
    }
    else
    {
        rep?.add(["expecting empty object [.'\$r'.'|'.1]", path]);
        return false;
    }
}

// check $r (.'$r')
bool json_model_3(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    // .'$r'
    // .'$r'.'|'.0
    bool res = json_model_2(val, path, rep);
    if (! res)
    {
        rep?.add(["unexpected \$a [.'\$r'.'|'.0]", path]);
        // .'$r'.'|'.1
        res = _jm_obj_1(val, path, rep);
        if (! res)
        {
            rep?.add(["unexpected element [.'\$r'.'|'.1]", path]);
        }
    }
    if (res)
    {
        rep?.clear();
    }
    else
    {
        rep?.add(["no model matched [.'\$r'.'|']", path]);
    }
    return res;
}

// check $ (.)
bool json_model_1(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    // .
    bool res = json_model_3(val, path, rep);
    if (! res)
    {
        rep?.add(["unexpected \$r [.]", path]);
    }
    return res;
}


// dart_init.dart
class GlobalState {
  static bool initializer = false;
}

Function? check_model_fun(String name) {
  return check_model_map[name];
}


void check_model_init() {
  if (!GlobalState.initializer){
    GlobalState.initializer = true;
  }
        check_model_map[""] = json_model_3;
        check_model_map["a"] = json_model_2;
        check_model_map["r"] = json_model_3;
}

// dart_free.dart
void check_model_free() {
  GlobalState.initializer = true;
}


void main(List<String> args) {
  runValidation(
    args,
    check_model_init,
    check_model_fun,
    check_model_free,
  );
}
