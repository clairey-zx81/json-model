...
sub json_model_2($$$)
{
    ...
    for my $prop (sort keys %$val)
    {
        ...
        unless ($res)
        {
            push @$rep, ["unexpected value for model \"\$r\" [.'\$a'.'']", defined $path ? $lpath_0 : undef] if defined $rep;
            return 0;
        }
    }
    return 1;
...
sub json_model_3($$$)
{
    ...
    push @$rep, ["unexpected value for model \"\$a\" [.'\$r']", $path] if defined $rep and not $res;
    return $res;
}
