...
{
    // .'$Dd'
    bool res = json_is_string(val) && jm_str_eq_2(json_string_value(val), 0x00000064);
    if (unlikely(! res))
    {
        if (rep) jm_report_add_entry(rep, "unexpected value for model \"_d\" [.'$Dd']", path);
    }
    return res;
}
...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (jm_str_eq_2(prop, 0x00000061))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.a]", (path ? &lpath_0 : NULL));
...
const char *check_model_init(void)
{
    if (!initialized)
    {
        ...
        check_model_map_tab[1] = (jm_propmap_t) { "Dd", json_model_2 };
