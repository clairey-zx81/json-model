...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        ...
        res = ival_0 <= 2
        if not res:
            rep is None or rep.append(("constraints failed [.'|'.0]", path))
    else:
        rep is None or rep.append(("not a -1 loose int [.'|'.0.'@']", path))
    if not res:
        # .'|'.1
        # .'|'.1.'@'
        ...
        if res:
            ...
            rep is None or rep.append(("not a -1.0 loose float [.'|'.1.'@']", path))
    if res:
