#! /bin/env python
#
# Model: ./json-schema-draft-01.model.json
#
    ...
    if not result:
        ...
        if result:
            for array_1_idx, array_1_item in enumerate(value):
                ...
                # $.items.'|'.1.0
                result = json_model_3(array_1_item, path, rep)
                if not result:
...
# define "json_model_1_may_maxDecimal" ($.maxDecimal)
    ...
    if not result:
        ...
        if result:
            for array_4_idx, array_4_item in enumerate(value):
                ...
                # $.extends.'|'.1.0
                result = json_model_3(array_4_item, path, rep)
                if not result:
