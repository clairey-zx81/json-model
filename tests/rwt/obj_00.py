# Generated for model: ./obj_00.model.json
#
...
def json_model_1(value: Jsonable, path: str, rep: Report = None) -> bool:
    ...
    if not result:
        rep is None or rep.append(f"not an expected $BOOLEAN at {path} [$.'$foo']")
    return result
...
def json_model_0(value: Jsonable, path: str, rep: Report = None) -> bool:
    ...
    result = isinstance(value, bool) or _rep(f"invalid $BOOLEAN at {path}", rep)
    if not result:
        rep is None or rep.append(f"not an expected $BOOLEAN at {path} [$]")
    return result
