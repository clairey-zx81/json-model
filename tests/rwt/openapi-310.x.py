#! /bin/env python
#
# Generated by JSON Model Compiler version 2.0a0
# see https://github.com/clairey-zx81/json-model
#
from typing import Callable
import re2 as re
from json_model.runtime import *
__version__ = "2.0a0"

def check_model_fun(name: str) -> CheckFun:
    """Return check function for JSON model name."""
    return check_model_map[name]

# entry point for generated checkers
def check_model(val: Jsonable, name: str = "", rep: Report = None) -> bool:
    """Check val validity agains JSON Model name."""
    if name not in check_model_map:
        raise Exception(f"unexpected model name: {name}")
    checker = check_model_map[name]
    return checker(val, [], rep)

_jm_obj_0_must: PropMap
_jm_obj_0_may: PropMap
_jm_re_0_search: Callable
_jm_re_0: RegexFun
_jm_obj_2_must: PropMap
_jm_obj_2_may: PropMap
_jm_obj_3_may: PropMap
_jm_obj_4_must: PropMap
_jm_obj_5_must: PropMap
_jm_obj_6_must: PropMap
_jm_obj_6_may: PropMap
_jm_obj_8_must: PropMap
_jm_obj_8_may: PropMap
_jm_obj_9_may: PropMap
_jm_re_1_search: Callable
_jm_re_1: RegexFun
_jm_obj_21_may: PropMap
_jm_obj_23_may: PropMap
_jm_obj_27_must: PropMap
_jm_obj_27_may: PropMap
_jm_cst_0: set[str]
_jm_obj_28_may: PropMap
_jm_cst_1: set[str]
_jm_obj_31_must: PropMap
_jm_cst_2: set[str]
_jm_obj_31_may: PropMap
_jm_obj_34_may: PropMap
_jm_obj_36_may: PropMap
_jm_obj_39_may: PropMap
_jm_obj_41_may: PropMap
_jm_re_2_search: Callable
_jm_re_2: RegexFun
_jm_obj_42_may: PropMap
_jm_obj_47_may: PropMap
_jm_re_3_search: Callable
_jm_re_3: RegexFun
_jm_obj_48_must: PropMap
_jm_obj_48_may: PropMap
_jm_obj_50_must: PropMap
_jm_obj_50_may: PropMap
_jm_obj_52_must: PropMap
_jm_obj_52_may: PropMap
_jm_obj_53_must: PropMap
_jm_obj_53_may: PropMap
_jm_obj_54_may: PropMap
_jm_obj_61_must: PropMap
_jm_obj_61_may: PropMap
_jm_obj_63_may: PropMap
_jm_obj_64_must: PropMap
_jm_obj_64_may: PropMap
_jm_obj_65_may: PropMap
_jm_obj_66_must: PropMap
_jm_obj_66_may: PropMap
_jm_cst_3: set[str]
_jm_obj_70_may: PropMap
check_model_map: PropMap

# check _jm_obj_0_must_info ($.'$OpenAPI'.info)
def _jm_f_0(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.info
    res = json_model_3(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Info [$.'$OpenAPI'.info]", path))
    return res

# check _jm_obj_0_must_openapi ($.'$OpenAPI'.openapi)
def _jm_f_1(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.openapi
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$OpenAPI'.openapi]", path))
    return res


# check _jm_obj_0_may_component ($.'$OpenAPI'.component)
def _jm_f_2(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.component
    res = json_model_8(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Components [$.'$OpenAPI'.component]", path))
    return res

# check _jm_obj_0_may_externalDocs ($.'$OpenAPI'.externalDocs)
def _jm_f_3(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.externalDocs
    res = json_model_12(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $ExternalDocumentation [$.'$OpenAPI'.externalDocs]", path))
    return res

# check _jm_obj_0_may_jsonSchemaDialect ($.'$OpenAPI'.jsonSchemaDialect)
def _jm_f_4(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.jsonSchemaDialect
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$OpenAPI'.jsonSchemaDialect]", path))
    return res

# check _jm_obj_0_may_paths ($.'$OpenAPI'.paths)
def _jm_f_5(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.paths
    res = json_model_9(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Paths [$.'$OpenAPI'.paths]", path))
    return res

# check _jm_obj_0_may_security ($.'$OpenAPI'.security)
def _jm_f_6(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.security
    res = json_model_34(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $SecurityRequirement [$.'$OpenAPI'.security]", path))
    return res

# check _jm_obj_0_may_servers ($.'$OpenAPI'.servers)
def _jm_f_7(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.servers
    res = isinstance(val, list)
    if res:
        for arr_0_idx, arr_0_item in enumerate(val):
            arr_0_lpath: Path = (path + [ arr_0_idx ]) if path is not None else None
            # $.'$OpenAPI'.servers.0
            res = json_model_6(arr_0_item, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Server [$.'$OpenAPI'.servers.0]", arr_0_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$OpenAPI'.servers]", path))
    return res

# check _jm_obj_0_may_tags ($.'$OpenAPI'.tags)
def _jm_f_8(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.tags
    res = isinstance(val, list)
    if res:
        for arr_1_idx, arr_1_item in enumerate(val):
            arr_1_lpath: Path = (path + [ arr_1_idx ]) if path is not None else None
            # $.'$OpenAPI'.tags.0
            res = json_model_25(arr_1_item, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Tag [$.'$OpenAPI'.tags.0]", arr_1_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$OpenAPI'.tags]", path))
    return res

# object $.'$OpenAPI'.webhooks
def _jm_obj_1(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$OpenAPI'.webhooks]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_1: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$OpenAPI'.webhooks.''
        # $.'$OpenAPI'.webhooks.''.'|'.0
        res = json_model_10(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $PathItem [$.'$OpenAPI'.webhooks.''.'|'.0]", lpath_1 if path is not None else None))
        if not res:
            # $.'$OpenAPI'.webhooks.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$OpenAPI'.webhooks.''.'|'.1]", lpath_1 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$OpenAPI'.webhooks.''.'|']", lpath_1 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_0_may_webhooks ($.'$OpenAPI'.webhooks)
def _jm_f_9(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'.webhooks
    res = _jm_obj_1(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$OpenAPI'.webhooks]", path))
    return res



# object $.'$OpenAPI'
def _jm_obj_0(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$OpenAPI']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_0: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_0_must.get(prop):
            # handle 2 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_0 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$OpenAPI']", lpath_0 if path is not None else None))
                    return False
        elif pfun := _jm_obj_0_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_0 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$OpenAPI']", lpath_0 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$OpenAPI'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$OpenAPI'.'/^x\\\\-.*$/']", lpath_0 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$OpenAPI']", lpath_0 if path is not None else None))
            return False
    if must_count != 2:
        rep is None or rep.append(("missing must prop [$.'$OpenAPI']", path))
        return False
    return True

# check $OpenAPI ($.'$OpenAPI')
def json_model_2(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OpenAPI'
    res = _jm_obj_0(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$OpenAPI']", path))
    return res

# check _jm_obj_2_must_title ($.'$Info'.title)
def _jm_f_10(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Info'.title
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Info'.title]", path))
    return res

# check _jm_obj_2_must_version ($.'$Info'.version)
def _jm_f_11(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Info'.version
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Info'.version]", path))
    return res


# check _jm_obj_2_may_contact ($.'$Info'.contact)
def _jm_f_12(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Info'.contact
    res = json_model_4(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Contact [$.'$Info'.contact]", path))
    return res

# check _jm_obj_2_may_description ($.'$Info'.description)
def _jm_f_13(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Info'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Info'.description]", path))
    return res

# check _jm_obj_2_may_license ($.'$Info'.license)
def _jm_f_14(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Info'.license
    res = json_model_5(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $License [$.'$Info'.license]", path))
    return res

# check _jm_obj_2_may_summary ($.'$Info'.summary)
def _jm_f_15(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Info'.summary
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Info'.summary]", path))
    return res

# check _jm_obj_2_may_termsOfService ($.'$Info'.termsOfService)
def _jm_f_16(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Info'.termsOfService
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Info'.termsOfService]", path))
    return res


# object $.'$Info'
def _jm_obj_2(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Info']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_2: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_2_must.get(prop):
            # handle 2 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_2 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$Info']", lpath_2 if path is not None else None))
                    return False
        elif pfun := _jm_obj_2_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_2 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Info']", lpath_2 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Info'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Info'.'/^x\\\\-.*$/']", lpath_2 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Info']", lpath_2 if path is not None else None))
            return False
    if must_count != 2:
        rep is None or rep.append(("missing must prop [$.'$Info']", path))
        return False
    return True

# check $Info ($.'$Info')
def json_model_3(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Info'
    res = _jm_obj_2(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Info']", path))
    return res

# check _jm_obj_3_may_email ($.'$Contact'.email)
def _jm_f_17(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Contact'.email
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Contact'.email]", path))
    return res

# check _jm_obj_3_may_name ($.'$Contact'.name)
def _jm_f_18(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Contact'.name
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Contact'.name]", path))
    return res

# check _jm_obj_3_may_url ($.'$Contact'.url)
def _jm_f_19(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Contact'.url
    res = is_valid_url(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URL [$.'$Contact'.url]", path))
    return res


# object $.'$Contact'
def _jm_obj_3(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Contact']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_3: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_3_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_3 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Contact']", lpath_3 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Contact'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Contact'.'/^x\\\\-.*$/']", lpath_3 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Contact']", lpath_3 if path is not None else None))
            return False
    return True

# check $Contact ($.'$Contact')
def json_model_4(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Contact'
    res = _jm_obj_3(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Contact']", path))
    return res

# check _jm_obj_4_must_name ($.'$License'.'|'.1.name)
def _jm_f_20(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$License'.'|'.1.name
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$License'.'|'.1.name]", path))
    return res

# check _jm_obj_4_must_url ($.'$License'.'|'.1.url)
def _jm_f_21(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$License'.'|'.1.url
    res = is_valid_url(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URL [$.'$License'.'|'.1.url]", path))
    return res


# object $.'$License'.'|'.1
def _jm_obj_4(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$License'.'|'.1]", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_4: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_4_must.get(prop):
            # handle 2 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_4 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$License'.'|'.1]", lpath_4 if path is not None else None))
                    return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$License'.'|'.1.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$License'.'|'.1.'/^x\\\\-.*$/']", lpath_4 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$License'.'|'.1]", lpath_4 if path is not None else None))
            return False
    if must_count != 2:
        rep is None or rep.append(("missing must prop [$.'$License'.'|'.1]", path))
        return False
    return True

# check _jm_obj_5_must_identifier ($.'$License'.'|'.0.identifier)
def _jm_f_22(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$License'.'|'.0.identifier
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$License'.'|'.0.identifier]", path))
    return res

# check _jm_obj_5_must_name ($.'$License'.'|'.0.name)
def _jm_f_23(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$License'.'|'.0.name
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$License'.'|'.0.name]", path))
    return res


# object $.'$License'.'|'.0
def _jm_obj_5(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$License'.'|'.0]", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_5: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_5_must.get(prop):
            # handle 2 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_5 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$License'.'|'.0]", lpath_5 if path is not None else None))
                    return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$License'.'|'.0.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$License'.'|'.0.'/^x\\\\-.*$/']", lpath_5 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$License'.'|'.0]", lpath_5 if path is not None else None))
            return False
    if must_count != 2:
        rep is None or rep.append(("missing must prop [$.'$License'.'|'.0]", path))
        return False
    return True

# check $License ($.'$License')
def json_model_5(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$License'
    res = isinstance(val, dict)
    if not res:
        rep is None or rep.append(("unexpected type at [$.'$License'.'|']", path))
    if res:
        # $.'$License'.'|'.0
        res = _jm_obj_5(val, path, rep)
        if not res:
            rep is None or rep.append(("not an expected object at [$.'$License'.'|'.0]", path))
        if not res:
            # $.'$License'.'|'.1
            res = _jm_obj_4(val, path, rep)
            if not res:
                rep is None or rep.append(("not an expected object at [$.'$License'.'|'.1]", path))
        if not res:
            rep is None or rep.append(("no model matched [$.'$License'.'|']", path))
    return res

# check _jm_obj_6_must_url ($.'$Server'.url)
def _jm_f_24(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Server'.url
    res = is_valid_url(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URL [$.'$Server'.url]", path))
    return res


# check _jm_obj_6_may_description ($.'$Server'.description)
def _jm_f_25(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Server'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Server'.description]", path))
    return res

# object $.'$Server'.variables
def _jm_obj_7(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Server'.variables]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_7: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Server'.variables.''
        res = json_model_7(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $ServerVariable [$.'$Server'.variables.'']", lpath_7 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_6_may_variables ($.'$Server'.variables)
def _jm_f_26(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Server'.variables
    res = _jm_obj_7(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Server'.variables]", path))
    return res


# object $.'$Server'
def _jm_obj_6(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Server']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_6: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_6_must.get(prop):
            # handle 1 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_6 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$Server']", lpath_6 if path is not None else None))
                    return False
        elif pfun := _jm_obj_6_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_6 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Server']", lpath_6 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Server'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Server'.'/^x\\\\-.*$/']", lpath_6 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Server']", lpath_6 if path is not None else None))
            return False
    if must_count != 1:
        rep is None or rep.append(("missing must prop [$.'$Server']", path))
        return False
    return True

# check $Server ($.'$Server')
def json_model_6(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Server'
    res = _jm_obj_6(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Server']", path))
    return res

# check _jm_obj_8_must_default ($.'$ServerVariable'.default)
def _jm_f_27(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$ServerVariable'.default
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$ServerVariable'.default]", path))
    return res


# check _jm_obj_8_may_description ($.'$ServerVariable'.description)
def _jm_f_28(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$ServerVariable'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$ServerVariable'.description]", path))
    return res

# check _jm_obj_8_may_enum ($.'$ServerVariable'.enum)
def _jm_f_29(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$ServerVariable'.enum
    res = isinstance(val, list)
    if res:
        for arr_2_idx, arr_2_item in enumerate(val):
            arr_2_lpath: Path = (path + [ arr_2_idx ]) if path is not None else None
            # $.'$ServerVariable'.enum.0
            res = isinstance(arr_2_item, str)
            if not res:
                rep is None or rep.append(("unexpected string [$.'$ServerVariable'.enum.0]", arr_2_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$ServerVariable'.enum]", path))
    return res


# object $.'$ServerVariable'
def _jm_obj_8(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$ServerVariable']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_8: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_8_must.get(prop):
            # handle 1 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_8 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$ServerVariable']", lpath_8 if path is not None else None))
                    return False
        elif pfun := _jm_obj_8_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_8 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$ServerVariable']", lpath_8 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$ServerVariable'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$ServerVariable'.'/^x\\\\-.*$/']", lpath_8 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$ServerVariable']", lpath_8 if path is not None else None))
            return False
    if must_count != 1:
        rep is None or rep.append(("missing must prop [$.'$ServerVariable']", path))
        return False
    return True

# check $ServerVariable ($.'$ServerVariable')
def json_model_7(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$ServerVariable'
    res = _jm_obj_8(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$ServerVariable']", path))
    return res

# object $.'$Components'.callbacks
def _jm_obj_10(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.callbacks]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_10: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.callbacks.''
        # $.'$Components'.callbacks.''.'|'.0
        res = json_model_20(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Callback [$.'$Components'.callbacks.''.'|'.0]", lpath_10 if path is not None else None))
        if not res:
            # $.'$Components'.callbacks.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.callbacks.''.'|'.1]", lpath_10 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.callbacks.''.'|']", lpath_10 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_callbacks ($.'$Components'.callbacks)
def _jm_f_30(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.callbacks
    res = _jm_obj_10(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.callbacks]", path))
    return res

# object $.'$Components'.examples
def _jm_obj_11(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.examples]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_11: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.examples.''
        # $.'$Components'.examples.''.'|'.0
        res = json_model_21(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Example [$.'$Components'.examples.''.'|'.0]", lpath_11 if path is not None else None))
        if not res:
            # $.'$Components'.examples.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.examples.''.'|'.1]", lpath_11 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.examples.''.'|']", lpath_11 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_examples ($.'$Components'.examples)
def _jm_f_31(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.examples
    res = _jm_obj_11(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.examples]", path))
    return res

# object $.'$Components'.headers
def _jm_obj_12(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.headers]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_12: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.headers.''
        # $.'$Components'.headers.''.'|'.0
        res = json_model_24(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Header [$.'$Components'.headers.''.'|'.0]", lpath_12 if path is not None else None))
        if not res:
            # $.'$Components'.headers.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.headers.''.'|'.1]", lpath_12 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.headers.''.'|']", lpath_12 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_headers ($.'$Components'.headers)
def _jm_f_32(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.headers
    res = _jm_obj_12(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.headers]", path))
    return res

# object $.'$Components'.links
def _jm_obj_13(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.links]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_13: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.links.''
        # $.'$Components'.links.''.'|'.0
        res = json_model_23(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Link [$.'$Components'.links.''.'|'.0]", lpath_13 if path is not None else None))
        if not res:
            # $.'$Components'.links.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.links.''.'|'.1]", lpath_13 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.links.''.'|']", lpath_13 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_links ($.'$Components'.links)
def _jm_f_33(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.links
    res = _jm_obj_13(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.links]", path))
    return res

# object $.'$Components'.parameters
def _jm_obj_14(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.parameters]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_14: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.parameters.''
        # $.'$Components'.parameters.''.'|'.0
        res = json_model_14(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Parameter [$.'$Components'.parameters.''.'|'.0]", lpath_14 if path is not None else None))
        if not res:
            # $.'$Components'.parameters.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.parameters.''.'|'.1]", lpath_14 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.parameters.''.'|']", lpath_14 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_parameters ($.'$Components'.parameters)
def _jm_f_34(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.parameters
    res = _jm_obj_14(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.parameters]", path))
    return res

# object $.'$Components'.pathItems
def _jm_obj_15(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.pathItems]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_15: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.pathItems.''
        # $.'$Components'.pathItems.''.'|'.0
        res = json_model_10(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $PathItem [$.'$Components'.pathItems.''.'|'.0]", lpath_15 if path is not None else None))
        if not res:
            # $.'$Components'.pathItems.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.pathItems.''.'|'.1]", lpath_15 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.pathItems.''.'|']", lpath_15 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_pathItems ($.'$Components'.pathItems)
def _jm_f_35(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.pathItems
    res = _jm_obj_15(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.pathItems]", path))
    return res

# object $.'$Components'.requestBodies
def _jm_obj_16(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.requestBodies]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_16: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.requestBodies.''
        # $.'$Components'.requestBodies.''.'|'.0
        res = json_model_15(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $RequestBody [$.'$Components'.requestBodies.''.'|'.0]", lpath_16 if path is not None else None))
        if not res:
            # $.'$Components'.requestBodies.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.requestBodies.''.'|'.1]", lpath_16 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.requestBodies.''.'|']", lpath_16 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_requestBodies ($.'$Components'.requestBodies)
def _jm_f_36(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.requestBodies
    res = _jm_obj_16(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.requestBodies]", path))
    return res

# object $.'$Components'.responses
def _jm_obj_17(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.responses]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_17: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.responses.''
        # $.'$Components'.responses.''.'|'.0
        res = json_model_19(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Response [$.'$Components'.responses.''.'|'.0]", lpath_17 if path is not None else None))
        if not res:
            # $.'$Components'.responses.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.responses.''.'|'.1]", lpath_17 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.responses.''.'|']", lpath_17 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_responses ($.'$Components'.responses)
def _jm_f_37(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.responses
    res = _jm_obj_17(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.responses]", path))
    return res

# object $.'$Components'.schemas
def _jm_obj_18(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.schemas]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_18: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.schemas.''
        res = json_model_28(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Schema [$.'$Components'.schemas.'']", lpath_18 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_schemas ($.'$Components'.schemas)
def _jm_f_38(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.schemas
    res = _jm_obj_18(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.schemas]", path))
    return res

# object $.'$Components'.securitySchemes
def _jm_obj_19(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components'.securitySchemes]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_19: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Components'.securitySchemes.''
        # $.'$Components'.securitySchemes.''.'|'.0
        res = json_model_31(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $SecurityScheme [$.'$Components'.securitySchemes.''.'|'.0]", lpath_19 if path is not None else None))
        if not res:
            # $.'$Components'.securitySchemes.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Components'.securitySchemes.''.'|'.1]", lpath_19 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Components'.securitySchemes.''.'|']", lpath_19 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_9_may_securitySchemes ($.'$Components'.securitySchemes)
def _jm_f_39(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'.securitySchemes
    res = _jm_obj_19(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components'.securitySchemes]", path))
    return res


# object $.'$Components'
def _jm_obj_9(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Components']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_9: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_9_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_9 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Components']", lpath_9 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Components'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Components'.'/^x\\\\-.*$/']", lpath_9 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Components']", lpath_9 if path is not None else None))
            return False
    return True

# check $Components ($.'$Components')
def json_model_8(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Components'
    res = _jm_obj_9(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Components']", path))
    return res


# object $.'$Paths'
def _jm_obj_20(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Paths']", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_20: Path = (path + [ prop ]) if path is not None else None
        if _jm_re_1(prop):
            # handle {len(regs)} re props
            # $.'$Paths'.'/^//'
            res = json_model_10(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $PathItem [$.'$Paths'.'/^//']", lpath_20 if path is not None else None))
            if not res:
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Paths'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Paths'.'/^x\\\\-.*$/']", lpath_20 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Paths']", lpath_20 if path is not None else None))
            return False
    return True

# check $Paths ($.'$Paths')
def json_model_9(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Paths'
    res = _jm_obj_20(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Paths']", path))
    return res

# check _jm_obj_21_may_$ref ($.'$PathItem'.'$ref')
def _jm_f_40(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.'$ref'
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$PathItem'.'$ref']", path))
    return res

# check _jm_obj_21_may_description ($.'$PathItem'.description)
def _jm_f_41(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$PathItem'.description]", path))
    return res

# check _jm_obj_21_may_get ($.'$PathItem'.get)
def _jm_f_42(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.get
    res = json_model_11(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Operation [$.'$PathItem'.get]", path))
    return res

# check _jm_obj_21_may_head ($.'$PathItem'.head)
def _jm_f_43(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.head
    res = json_model_11(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Operation [$.'$PathItem'.head]", path))
    return res

# check _jm_obj_21_may_options ($.'$PathItem'.options)
def _jm_f_44(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.options
    res = json_model_11(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Operation [$.'$PathItem'.options]", path))
    return res

# object $.'$PathItem'.parameters
def _jm_obj_22(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$PathItem'.parameters]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_22: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$PathItem'.parameters.''
        # $.'$PathItem'.parameters.''.'|'.0
        res = json_model_14(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Parameter [$.'$PathItem'.parameters.''.'|'.0]", lpath_22 if path is not None else None))
        if not res:
            # $.'$PathItem'.parameters.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$PathItem'.parameters.''.'|'.1]", lpath_22 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$PathItem'.parameters.''.'|']", lpath_22 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_21_may_parameters ($.'$PathItem'.parameters)
def _jm_f_45(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.parameters
    res = _jm_obj_22(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$PathItem'.parameters]", path))
    return res

# check _jm_obj_21_may_patch ($.'$PathItem'.patch)
def _jm_f_46(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.patch
    res = json_model_11(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Operation [$.'$PathItem'.patch]", path))
    return res

# check _jm_obj_21_may_post ($.'$PathItem'.post)
def _jm_f_47(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.post
    res = json_model_11(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Operation [$.'$PathItem'.post]", path))
    return res

# check _jm_obj_21_may_put ($.'$PathItem'.put)
def _jm_f_48(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.put
    res = json_model_11(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Operation [$.'$PathItem'.put]", path))
    return res

# check _jm_obj_21_may_servers ($.'$PathItem'.servers)
def _jm_f_49(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.servers
    res = json_model_11(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Operation [$.'$PathItem'.servers]", path))
    return res

# check _jm_obj_21_may_summary ($.'$PathItem'.summary)
def _jm_f_50(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.summary
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$PathItem'.summary]", path))
    return res

# check _jm_obj_21_may_trace ($.'$PathItem'.trace)
def _jm_f_51(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'.trace
    res = json_model_11(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Operation [$.'$PathItem'.trace]", path))
    return res


# object $.'$PathItem'
def _jm_obj_21(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$PathItem']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_21: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_21_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_21 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$PathItem']", lpath_21 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$PathItem'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$PathItem'.'/^x\\\\-.*$/']", lpath_21 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$PathItem']", lpath_21 if path is not None else None))
            return False
    return True

# check $PathItem ($.'$PathItem')
def json_model_10(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$PathItem'
    res = _jm_obj_21(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$PathItem']", path))
    return res

# object $.'$Operation'.callbacks
def _jm_obj_24(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Operation'.callbacks]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_24: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Operation'.callbacks.''
        # $.'$Operation'.callbacks.''.'|'.0
        res = json_model_20(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Callback [$.'$Operation'.callbacks.''.'|'.0]", lpath_24 if path is not None else None))
        if not res:
            # $.'$Operation'.callbacks.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Operation'.callbacks.''.'|'.1]", lpath_24 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Operation'.callbacks.''.'|']", lpath_24 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_23_may_callbacks ($.'$Operation'.callbacks)
def _jm_f_52(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.callbacks
    res = _jm_obj_24(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Operation'.callbacks]", path))
    return res

# check _jm_obj_23_may_deprecated ($.'$Operation'.deprecated)
def _jm_f_53(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.deprecated
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Operation'.deprecated]", path))
    return res

# check _jm_obj_23_may_description ($.'$Operation'.description)
def _jm_f_54(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Operation'.description]", path))
    return res

# check _jm_obj_23_may_externalDocs ($.'$Operation'.externalDocs)
def _jm_f_55(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.externalDocs
    res = json_model_12(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $ExternalDocumentation [$.'$Operation'.externalDocs]", path))
    return res

# check _jm_obj_23_may_operationId ($.'$Operation'.operationId)
def _jm_f_56(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.operationId
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Operation'.operationId]", path))
    return res

# object $.'$Operation'.parameters
def _jm_obj_25(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Operation'.parameters]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_25: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Operation'.parameters.''
        # $.'$Operation'.parameters.''.'|'.0
        res = json_model_14(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Parameter [$.'$Operation'.parameters.''.'|'.0]", lpath_25 if path is not None else None))
        if not res:
            # $.'$Operation'.parameters.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Operation'.parameters.''.'|'.1]", lpath_25 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Operation'.parameters.''.'|']", lpath_25 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_23_may_parameters ($.'$Operation'.parameters)
def _jm_f_57(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.parameters
    res = _jm_obj_25(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Operation'.parameters]", path))
    return res

# object $.'$Operation'.requestBody
def _jm_obj_26(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Operation'.requestBody]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_26: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Operation'.requestBody.''
        # $.'$Operation'.requestBody.''.'|'.0
        res = json_model_15(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $RequestBody [$.'$Operation'.requestBody.''.'|'.0]", lpath_26 if path is not None else None))
        if not res:
            # $.'$Operation'.requestBody.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Operation'.requestBody.''.'|'.1]", lpath_26 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Operation'.requestBody.''.'|']", lpath_26 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_23_may_requestBody ($.'$Operation'.requestBody)
def _jm_f_58(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.requestBody
    res = _jm_obj_26(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Operation'.requestBody]", path))
    return res

# check _jm_obj_23_may_responses ($.'$Operation'.responses)
def _jm_f_59(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.responses
    res = json_model_18(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Responses [$.'$Operation'.responses]", path))
    return res

# check _jm_obj_23_may_security ($.'$Operation'.security)
def _jm_f_60(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.security
    res = isinstance(val, list)
    if res:
        for arr_3_idx, arr_3_item in enumerate(val):
            arr_3_lpath: Path = (path + [ arr_3_idx ]) if path is not None else None
            # $.'$Operation'.security.0
            res = json_model_34(arr_3_item, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $SecurityRequirement [$.'$Operation'.security.0]", arr_3_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$Operation'.security]", path))
    return res

# check _jm_obj_23_may_servers ($.'$Operation'.servers)
def _jm_f_61(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.servers
    res = isinstance(val, list)
    if res:
        for arr_4_idx, arr_4_item in enumerate(val):
            arr_4_lpath: Path = (path + [ arr_4_idx ]) if path is not None else None
            # $.'$Operation'.servers.0
            res = json_model_6(arr_4_item, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Server [$.'$Operation'.servers.0]", arr_4_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$Operation'.servers]", path))
    return res

# check _jm_obj_23_may_summary ($.'$Operation'.summary)
def _jm_f_62(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.summary
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Operation'.summary]", path))
    return res

# check _jm_obj_23_may_tags ($.'$Operation'.tags)
def _jm_f_63(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'.tags
    res = isinstance(val, list)
    if res:
        for arr_5_idx, arr_5_item in enumerate(val):
            arr_5_lpath: Path = (path + [ arr_5_idx ]) if path is not None else None
            # $.'$Operation'.tags.0
            res = isinstance(arr_5_item, str)
            if not res:
                rep is None or rep.append(("unexpected string [$.'$Operation'.tags.0]", arr_5_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$Operation'.tags]", path))
    return res


# object $.'$Operation'
def _jm_obj_23(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Operation']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_23: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_23_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_23 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Operation']", lpath_23 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Operation'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Operation'.'/^x\\\\-.*$/']", lpath_23 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Operation']", lpath_23 if path is not None else None))
            return False
    return True

# check $Operation ($.'$Operation')
def json_model_11(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Operation'
    res = _jm_obj_23(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Operation']", path))
    return res

# check _jm_obj_27_must_url ($.'$ExternalDocumentation'.url)
def _jm_f_64(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$ExternalDocumentation'.url
    res = is_valid_url(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URL [$.'$ExternalDocumentation'.url]", path))
    return res


# check _jm_obj_27_may_description ($.'$ExternalDocumentation'.description)
def _jm_f_65(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$ExternalDocumentation'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$ExternalDocumentation'.description]", path))
    return res


# object $.'$ExternalDocumentation'
def _jm_obj_27(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$ExternalDocumentation']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_27: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_27_must.get(prop):
            # handle 1 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_27 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$ExternalDocumentation']", lpath_27 if path is not None else None))
                    return False
        elif pfun := _jm_obj_27_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_27 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$ExternalDocumentation']", lpath_27 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$ExternalDocumentation'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$ExternalDocumentation'.'/^x\\\\-.*$/']", lpath_27 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$ExternalDocumentation']", lpath_27 if path is not None else None))
            return False
    if must_count != 1:
        rep is None or rep.append(("missing must prop [$.'$ExternalDocumentation']", path))
        return False
    return True

# check $ExternalDocumentation ($.'$ExternalDocumentation')
def json_model_12(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$ExternalDocumentation'
    res = _jm_obj_27(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$ExternalDocumentation']", path))
    return res

# check _jm_obj_28_may_allowEmptyValue ($.'$parameterShare'.allowEmptyValue)
def _jm_f_66(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.allowEmptyValue
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$parameterShare'.allowEmptyValue]", path))
    return res

# check _jm_obj_28_may_allowReserved ($.'$parameterShare'.allowReserved)
def _jm_f_67(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.allowReserved
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$parameterShare'.allowReserved]", path))
    return res

# object $.'$parameterShare'.content
def _jm_obj_29(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$parameterShare'.content]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_29: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$parameterShare'.content.''
        res = json_model_16(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $MediaType [$.'$parameterShare'.content.'']", lpath_29 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_28_may_content ($.'$parameterShare'.content)
def _jm_f_68(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.content
    res = _jm_obj_29(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$parameterShare'.content]", path))
    return res

# check _jm_obj_28_may_deprecated ($.'$parameterShare'.deprecated)
def _jm_f_69(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.deprecated
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$parameterShare'.deprecated]", path))
    return res

# check _jm_obj_28_may_description ($.'$parameterShare'.description)
def _jm_f_70(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$parameterShare'.description]", path))
    return res

# check _jm_obj_28_may_example ($.'$parameterShare'.example)
def _jm_f_71(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.example
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$parameterShare'.example]", path))
    return res

# object $.'$parameterShare'.examples
def _jm_obj_30(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$parameterShare'.examples]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_30: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$parameterShare'.examples.''
        # $.'$parameterShare'.examples.''.'|'.0
        res = json_model_21(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Example [$.'$parameterShare'.examples.''.'|'.0]", lpath_30 if path is not None else None))
        if not res:
            # $.'$parameterShare'.examples.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$parameterShare'.examples.''.'|'.1]", lpath_30 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$parameterShare'.examples.''.'|']", lpath_30 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_28_may_examples ($.'$parameterShare'.examples)
def _jm_f_72(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.examples
    res = _jm_obj_30(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$parameterShare'.examples]", path))
    return res

# check _jm_obj_28_may_explode ($.'$parameterShare'.explode)
def _jm_f_73(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.explode
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$parameterShare'.explode]", path))
    return res

# check _jm_obj_28_may_required ($.'$parameterShare'.required)
def _jm_f_74(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.required
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$parameterShare'.required]", path))
    return res

# check _jm_obj_28_may_schema ($.'$parameterShare'.schema)
def _jm_f_75(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.schema
    res = json_model_28(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$parameterShare'.schema]", path))
    return res


# check _jm_obj_28_may_style ($.'$parameterShare'.style)
def _jm_f_76(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'.style
    res = (val is None or isinstance(val, (bool, int, float, str))) and val in _jm_cst_0
    if not res:
        rep is None or rep.append(("value not in enum [$.'$parameterShare'.style.'|']", path))
    return res


# object $.'$parameterShare'
def _jm_obj_28(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$parameterShare']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_28: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_28_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_28 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$parameterShare']", lpath_28 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$parameterShare'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$parameterShare'.'/^x\\\\-.*$/']", lpath_28 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$parameterShare']", lpath_28 if path is not None else None))
            return False
    return True

# check $parameterShare ($.'$parameterShare')
def json_model_13(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$parameterShare'
    res = _jm_obj_28(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$parameterShare']", path))
    return res


# check _jm_obj_31_must_in ($.'$Parameter'.in)
def _jm_f_77(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.in
    res = (val is None or isinstance(val, (bool, int, float, str))) and val in _jm_cst_1
    if not res:
        rep is None or rep.append(("value not in enum [$.'$Parameter'.in.'|']", path))
    return res

# check _jm_obj_31_must_name ($.'$Parameter'.name)
def _jm_f_78(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.name
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Parameter'.name]", path))
    return res


# check _jm_obj_31_may_allowEmptyValue ($.'$Parameter'.allowEmptyValue)
def _jm_f_79(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.allowEmptyValue
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Parameter'.allowEmptyValue]", path))
    return res

# check _jm_obj_31_may_allowReserved ($.'$Parameter'.allowReserved)
def _jm_f_80(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.allowReserved
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Parameter'.allowReserved]", path))
    return res

# object $.'$Parameter'.content
def _jm_obj_32(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Parameter'.content]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_32: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Parameter'.content.''
        res = json_model_16(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $MediaType [$.'$Parameter'.content.'']", lpath_32 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_31_may_content ($.'$Parameter'.content)
def _jm_f_81(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.content
    res = _jm_obj_32(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Parameter'.content]", path))
    return res

# check _jm_obj_31_may_deprecated ($.'$Parameter'.deprecated)
def _jm_f_82(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.deprecated
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Parameter'.deprecated]", path))
    return res

# check _jm_obj_31_may_description ($.'$Parameter'.description)
def _jm_f_83(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Parameter'.description]", path))
    return res

# check _jm_obj_31_may_example ($.'$Parameter'.example)
def _jm_f_84(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.example
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$Parameter'.example]", path))
    return res

# object $.'$Parameter'.examples
def _jm_obj_33(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Parameter'.examples]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_33: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Parameter'.examples.''
        # $.'$Parameter'.examples.''.'|'.0
        res = json_model_21(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Example [$.'$Parameter'.examples.''.'|'.0]", lpath_33 if path is not None else None))
        if not res:
            # $.'$Parameter'.examples.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Parameter'.examples.''.'|'.1]", lpath_33 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Parameter'.examples.''.'|']", lpath_33 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_31_may_examples ($.'$Parameter'.examples)
def _jm_f_85(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.examples
    res = _jm_obj_33(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Parameter'.examples]", path))
    return res

# check _jm_obj_31_may_explode ($.'$Parameter'.explode)
def _jm_f_86(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.explode
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Parameter'.explode]", path))
    return res

# check _jm_obj_31_may_required ($.'$Parameter'.required)
def _jm_f_87(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.required
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Parameter'.required]", path))
    return res

# check _jm_obj_31_may_schema ($.'$Parameter'.schema)
def _jm_f_88(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.schema
    res = json_model_28(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$Parameter'.schema]", path))
    return res


# check _jm_obj_31_may_style ($.'$Parameter'.style)
def _jm_f_89(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'.style
    res = (val is None or isinstance(val, (bool, int, float, str))) and val in _jm_cst_2
    if not res:
        rep is None or rep.append(("value not in enum [$.'$Parameter'.style.'|']", path))
    return res


# object $.'$Parameter'
def _jm_obj_31(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Parameter']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_31: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_31_must.get(prop):
            # handle 2 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_31 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$Parameter']", lpath_31 if path is not None else None))
                    return False
        elif pfun := _jm_obj_31_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_31 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Parameter']", lpath_31 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Parameter'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Parameter'.'/^x\\\\-.*$/']", lpath_31 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Parameter']", lpath_31 if path is not None else None))
            return False
    if must_count != 2:
        rep is None or rep.append(("missing must prop [$.'$Parameter']", path))
        return False
    return True

# check $Parameter ($.'$Parameter')
def json_model_14(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Parameter'
    res = _jm_obj_31(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Parameter']", path))
    return res

# object $.'$RequestBody'.content
def _jm_obj_35(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$RequestBody'.content]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_35: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$RequestBody'.content.''
        res = json_model_16(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $MediaType [$.'$RequestBody'.content.'']", lpath_35 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_34_may_content ($.'$RequestBody'.content)
def _jm_f_90(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$RequestBody'.content
    res = _jm_obj_35(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$RequestBody'.content]", path))
    return res

# check _jm_obj_34_may_description ($.'$RequestBody'.description)
def _jm_f_91(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$RequestBody'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$RequestBody'.description]", path))
    return res

# check _jm_obj_34_may_required ($.'$RequestBody'.required)
def _jm_f_92(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$RequestBody'.required
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$RequestBody'.required]", path))
    return res


# object $.'$RequestBody'
def _jm_obj_34(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$RequestBody']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_34: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_34_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_34 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$RequestBody']", lpath_34 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$RequestBody'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$RequestBody'.'/^x\\\\-.*$/']", lpath_34 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$RequestBody']", lpath_34 if path is not None else None))
            return False
    return True

# check $RequestBody ($.'$RequestBody')
def json_model_15(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$RequestBody'
    res = _jm_obj_34(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$RequestBody']", path))
    return res

# object $.'$MediaType'.encoding
def _jm_obj_37(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$MediaType'.encoding]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_37: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$MediaType'.encoding.''
        res = json_model_17(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Encoding [$.'$MediaType'.encoding.'']", lpath_37 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_36_may_encoding ($.'$MediaType'.encoding)
def _jm_f_93(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$MediaType'.encoding
    res = _jm_obj_37(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$MediaType'.encoding]", path))
    return res

# check _jm_obj_36_may_example ($.'$MediaType'.example)
def _jm_f_94(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$MediaType'.example
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$MediaType'.example]", path))
    return res

# object $.'$MediaType'.examples
def _jm_obj_38(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$MediaType'.examples]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_38: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$MediaType'.examples.''
        # $.'$MediaType'.examples.''.'|'.0
        res = json_model_21(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Example [$.'$MediaType'.examples.''.'|'.0]", lpath_38 if path is not None else None))
        if not res:
            # $.'$MediaType'.examples.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$MediaType'.examples.''.'|'.1]", lpath_38 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$MediaType'.examples.''.'|']", lpath_38 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_36_may_examples ($.'$MediaType'.examples)
def _jm_f_95(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$MediaType'.examples
    res = _jm_obj_38(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$MediaType'.examples]", path))
    return res

# check _jm_obj_36_may_schema ($.'$MediaType'.schema)
def _jm_f_96(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$MediaType'.schema
    res = json_model_28(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$MediaType'.schema]", path))
    return res


# object $.'$MediaType'
def _jm_obj_36(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$MediaType']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_36: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_36_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_36 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$MediaType']", lpath_36 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$MediaType'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$MediaType'.'/^x\\\\-.*$/']", lpath_36 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$MediaType']", lpath_36 if path is not None else None))
            return False
    return True

# check $MediaType ($.'$MediaType')
def json_model_16(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$MediaType'
    res = _jm_obj_36(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$MediaType']", path))
    return res

# check _jm_obj_39_may_allowReserved ($.'$Encoding'.allowReserved)
def _jm_f_97(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Encoding'.allowReserved
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Encoding'.allowReserved]", path))
    return res

# check _jm_obj_39_may_contentType ($.'$Encoding'.contentType)
def _jm_f_98(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Encoding'.contentType
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Encoding'.contentType]", path))
    return res

# check _jm_obj_39_may_explode ($.'$Encoding'.explode)
def _jm_f_99(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Encoding'.explode
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Encoding'.explode]", path))
    return res

# object $.'$Encoding'.headers
def _jm_obj_40(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Encoding'.headers]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_40: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Encoding'.headers.''
        # $.'$Encoding'.headers.''.'|'.0
        res = json_model_24(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Header [$.'$Encoding'.headers.''.'|'.0]", lpath_40 if path is not None else None))
        if not res:
            # $.'$Encoding'.headers.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Encoding'.headers.''.'|'.1]", lpath_40 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Encoding'.headers.''.'|']", lpath_40 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_39_may_headers ($.'$Encoding'.headers)
def _jm_f_100(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Encoding'.headers
    res = _jm_obj_40(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Encoding'.headers]", path))
    return res

# check _jm_obj_39_may_style ($.'$Encoding'.style)
def _jm_f_101(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Encoding'.style
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Encoding'.style]", path))
    return res


# object $.'$Encoding'
def _jm_obj_39(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Encoding']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_39: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_39_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_39 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Encoding']", lpath_39 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Encoding'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Encoding'.'/^x\\\\-.*$/']", lpath_39 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Encoding']", lpath_39 if path is not None else None))
            return False
    return True

# check $Encoding ($.'$Encoding')
def json_model_17(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Encoding'
    res = _jm_obj_39(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Encoding']", path))
    return res

# check _jm_obj_41_may_default ($.'$Responses'.default)
def _jm_f_102(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Responses'.default
    # $.'$Responses'.default.'|'.0
    res = json_model_19(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Response [$.'$Responses'.default.'|'.0]", path))
    if not res:
        # $.'$Responses'.default.'|'.1
        res = json_model_26(val, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Reference [$.'$Responses'.default.'|'.1]", path))
    if not res:
        rep is None or rep.append(("no model matched [$.'$Responses'.default.'|']", path))
    return res



# object $.'$Responses'
def _jm_obj_41(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Responses']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_41: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_41_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_41 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Responses']", lpath_41 if path is not None else None))
                return False
        elif _jm_re_2(prop):
            # handle {len(regs)} re props
            # $.'$Responses'.'/^[1-5](\\d\\d|XX)$/'
            # $.'$Responses'.'/^[1-5](\\d\\d|XX)$/'.'|'.0
            res = json_model_19(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Response [$.'$Responses'.'/^[1-5](\\\\d\\\\d|XX)$/'.'|'.0]", lpath_41 if path is not None else None))
            if not res:
                # $.'$Responses'.'/^[1-5](\\d\\d|XX)$/'.'|'.1
                res = json_model_26(pval, path, rep)
                if not res:
                    rep is None or rep.append(("unexpected $Reference [$.'$Responses'.'/^[1-5](\\\\d\\\\d|XX)$/'.'|'.1]", lpath_41 if path is not None else None))
            if not res:
                rep is None or rep.append(("no model matched [$.'$Responses'.'/^[1-5](\\\\d\\\\d|XX)$/'.'|']", lpath_41 if path is not None else None))
            if not res:
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Responses'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Responses'.'/^x\\\\-.*$/']", lpath_41 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Responses']", lpath_41 if path is not None else None))
            return False
    return True

# check $Responses ($.'$Responses')
def json_model_18(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Responses'
    res = _jm_obj_41(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Responses']", path))
    return res

# object $.'$Response'.content
def _jm_obj_43(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Response'.content]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_43: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Response'.content.''
        res = json_model_16(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $MediaType [$.'$Response'.content.'']", lpath_43 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_42_may_content ($.'$Response'.content)
def _jm_f_103(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Response'.content
    res = _jm_obj_43(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Response'.content]", path))
    return res

# check _jm_obj_42_may_description ($.'$Response'.description)
def _jm_f_104(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Response'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Response'.description]", path))
    return res

# object $.'$Response'.headers
def _jm_obj_44(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Response'.headers]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_44: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Response'.headers.''
        # $.'$Response'.headers.''.'|'.0
        res = json_model_24(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Header [$.'$Response'.headers.''.'|'.0]", lpath_44 if path is not None else None))
        if not res:
            # $.'$Response'.headers.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Response'.headers.''.'|'.1]", lpath_44 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Response'.headers.''.'|']", lpath_44 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_42_may_headers ($.'$Response'.headers)
def _jm_f_105(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Response'.headers
    res = _jm_obj_44(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Response'.headers]", path))
    return res

# object $.'$Response'.links
def _jm_obj_45(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Response'.links]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_45: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Response'.links.''
        # $.'$Response'.links.''.'|'.0
        res = json_model_23(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Link [$.'$Response'.links.''.'|'.0]", lpath_45 if path is not None else None))
        if not res:
            # $.'$Response'.links.''.'|'.1
            res = json_model_26(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Reference [$.'$Response'.links.''.'|'.1]", lpath_45 if path is not None else None))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Response'.links.''.'|']", lpath_45 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_42_may_links ($.'$Response'.links)
def _jm_f_106(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Response'.links
    res = _jm_obj_45(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Response'.links]", path))
    return res


# object $.'$Response'
def _jm_obj_42(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Response']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_42: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_42_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_42 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Response']", lpath_42 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Response'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Response'.'/^x\\\\-.*$/']", lpath_42 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Response']", lpath_42 if path is not None else None))
            return False
    return True

# check $Response ($.'$Response')
def json_model_19(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Response'
    res = _jm_obj_42(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Response']", path))
    return res

# object $.'$Callback'
def _jm_obj_46(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Callback']", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_46: Path = (path + [ prop ]) if path is not None else None
        if json_model_22(prop, lpath_46 if path is not None else None, rep):
            # handle {len(defs)} key props
            # $.'$Callback'.'$Expression'
            # $.'$Callback'.'$Expression'.'|'.0
            res = json_model_10(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $PathItem [$.'$Callback'.'$Expression'.'|'.0]", lpath_46 if path is not None else None))
            if not res:
                # $.'$Callback'.'$Expression'.'|'.1
                res = json_model_26(pval, path, rep)
                if not res:
                    rep is None or rep.append(("unexpected $Reference [$.'$Callback'.'$Expression'.'|'.1]", lpath_46 if path is not None else None))
            if not res:
                rep is None or rep.append(("no model matched [$.'$Callback'.'$Expression'.'|']", lpath_46 if path is not None else None))
            if not res:
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Callback'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Callback'.'/^x\\\\-.*$/']", lpath_46 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Callback']", lpath_46 if path is not None else None))
            return False
    return True

# check $Callback ($.'$Callback')
def json_model_20(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Callback'
    res = _jm_obj_46(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Callback']", path))
    return res

# check _jm_obj_47_may_description ($.'$Example'.description)
def _jm_f_107(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Example'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Example'.description]", path))
    return res

# check _jm_obj_47_may_externalValue ($.'$Example'.externalValue)
def _jm_f_108(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Example'.externalValue
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Example'.externalValue]", path))
    return res

# check _jm_obj_47_may_summary ($.'$Example'.summary)
def _jm_f_109(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Example'.summary
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Example'.summary]", path))
    return res

# check _jm_obj_47_may_value ($.'$Example'.value)
def _jm_f_110(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Example'.value
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$Example'.value]", path))
    return res


# object $.'$Example'
def _jm_obj_47(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Example']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_47: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_47_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_47 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Example']", lpath_47 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Example'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Example'.'/^x\\\\-.*$/']", lpath_47 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Example']", lpath_47 if path is not None else None))
            return False
    return True

# check $Example ($.'$Example')
def json_model_21(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Example'
    res = _jm_obj_47(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Example']", path))
    return res


# check $Expression ($.'$Expression')
def json_model_22(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Expression'
    # "/\\{.*\\}/"
    res = isinstance(val, str) and _jm_re_3(val)
    if not res:
        rep is None or rep.append(("unexpected REGEX [$.'$Expression']", path))
    return res

# check _jm_obj_48_must_operationId ($.'$Link'.'|'.1.operationId)
def _jm_f_111(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.1.operationId
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Link'.'|'.1.operationId]", path))
    return res


# check _jm_obj_48_may_description ($.'$Link'.'|'.1.description)
def _jm_f_112(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.1.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Link'.'|'.1.description]", path))
    return res

# object $.'$Link'.'|'.1.parameters
def _jm_obj_49(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Link'.'|'.1.parameters]", path))
        return False
    # accept any object
    return True

# check _jm_obj_48_may_parameters ($.'$Link'.'|'.1.parameters)
def _jm_f_113(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.1.parameters
    res = _jm_obj_49(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Link'.'|'.1.parameters]", path))
    return res

# check _jm_obj_48_may_requestBody ($.'$Link'.'|'.1.requestBody)
def _jm_f_114(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.1.requestBody
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$Link'.'|'.1.requestBody]", path))
    return res

# check _jm_obj_48_may_server ($.'$Link'.'|'.1.server)
def _jm_f_115(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.1.server
    res = json_model_6(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Server [$.'$Link'.'|'.1.server]", path))
    return res


# object $.'$Link'.'|'.1
def _jm_obj_48(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Link'.'|'.1]", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_48: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_48_must.get(prop):
            # handle 1 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_48 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$Link'.'|'.1]", lpath_48 if path is not None else None))
                    return False
        elif pfun := _jm_obj_48_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_48 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Link'.'|'.1]", lpath_48 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Link'.'|'.1.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Link'.'|'.1.'/^x\\\\-.*$/']", lpath_48 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Link'.'|'.1]", lpath_48 if path is not None else None))
            return False
    if must_count != 1:
        rep is None or rep.append(("missing must prop [$.'$Link'.'|'.1]", path))
        return False
    return True

# check _jm_obj_50_must_operationRef ($.'$Link'.'|'.0.operationRef)
def _jm_f_116(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.0.operationRef
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Link'.'|'.0.operationRef]", path))
    return res


# check _jm_obj_50_may_description ($.'$Link'.'|'.0.description)
def _jm_f_117(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.0.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Link'.'|'.0.description]", path))
    return res

# object $.'$Link'.'|'.0.parameters
def _jm_obj_51(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Link'.'|'.0.parameters]", path))
        return False
    # accept any object
    return True

# check _jm_obj_50_may_parameters ($.'$Link'.'|'.0.parameters)
def _jm_f_118(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.0.parameters
    res = _jm_obj_51(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Link'.'|'.0.parameters]", path))
    return res

# check _jm_obj_50_may_requestBody ($.'$Link'.'|'.0.requestBody)
def _jm_f_119(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.0.requestBody
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$Link'.'|'.0.requestBody]", path))
    return res

# check _jm_obj_50_may_server ($.'$Link'.'|'.0.server)
def _jm_f_120(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'.'|'.0.server
    res = json_model_6(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Server [$.'$Link'.'|'.0.server]", path))
    return res


# object $.'$Link'.'|'.0
def _jm_obj_50(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Link'.'|'.0]", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_49: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_50_must.get(prop):
            # handle 1 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_49 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$Link'.'|'.0]", lpath_49 if path is not None else None))
                    return False
        elif pfun := _jm_obj_50_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_49 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Link'.'|'.0]", lpath_49 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Link'.'|'.0.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Link'.'|'.0.'/^x\\\\-.*$/']", lpath_49 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Link'.'|'.0]", lpath_49 if path is not None else None))
            return False
    if must_count != 1:
        rep is None or rep.append(("missing must prop [$.'$Link'.'|'.0]", path))
        return False
    return True

# check $Link ($.'$Link')
def json_model_23(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Link'
    res = isinstance(val, dict)
    if not res:
        rep is None or rep.append(("unexpected type at [$.'$Link'.'|']", path))
    if res:
        # $.'$Link'.'|'.0
        res = _jm_obj_50(val, path, rep)
        if not res:
            rep is None or rep.append(("not an expected object at [$.'$Link'.'|'.0]", path))
        if not res:
            # $.'$Link'.'|'.1
            res = _jm_obj_48(val, path, rep)
            if not res:
                rep is None or rep.append(("not an expected object at [$.'$Link'.'|'.1]", path))
        if not res:
            rep is None or rep.append(("no model matched [$.'$Link'.'|']", path))
    return res

# check $Header ($.'$Header')
def json_model_24(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Header'
    res = json_model_13(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $parameterShare [$.'$Header']", path))
    return res

# check _jm_obj_52_must_name ($.'$Tag'.name)
def _jm_f_121(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Tag'.name
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Tag'.name]", path))
    return res


# check _jm_obj_52_may_description ($.'$Tag'.description)
def _jm_f_122(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Tag'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Tag'.description]", path))
    return res

# check _jm_obj_52_may_externalDocs ($.'$Tag'.externalDocs)
def _jm_f_123(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Tag'.externalDocs
    res = json_model_12(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $ExternalDocumentation [$.'$Tag'.externalDocs]", path))
    return res


# object $.'$Tag'
def _jm_obj_52(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Tag']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_50: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_52_must.get(prop):
            # handle 1 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_50 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$Tag']", lpath_50 if path is not None else None))
                    return False
        elif pfun := _jm_obj_52_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_50 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Tag']", lpath_50 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Tag'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Tag'.'/^x\\\\-.*$/']", lpath_50 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Tag']", lpath_50 if path is not None else None))
            return False
    if must_count != 1:
        rep is None or rep.append(("missing must prop [$.'$Tag']", path))
        return False
    return True

# check $Tag ($.'$Tag')
def json_model_25(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Tag'
    res = _jm_obj_52(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Tag']", path))
    return res

# check _jm_obj_53_must_$ref ($.'$Reference'.'$ref')
def _jm_f_124(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Reference'.'$ref'
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Reference'.'$ref']", path))
    return res


# check _jm_obj_53_may_description ($.'$Reference'.description)
def _jm_f_125(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Reference'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Reference'.description]", path))
    return res

# check _jm_obj_53_may_summary ($.'$Reference'.summary)
def _jm_f_126(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Reference'.summary
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Reference'.summary]", path))
    return res


# object $.'$Reference'
def _jm_obj_53(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Reference']", path))
        return False
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_51: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_53_must.get(prop):
            # handle 1 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_51 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$Reference']", lpath_51 if path is not None else None))
                    return False
        elif pfun := _jm_obj_53_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_51 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Reference']", lpath_51 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Reference']", lpath_51 if path is not None else None))
            return False
    if must_count != 1:
        rep is None or rep.append(("missing must prop [$.'$Reference']", path))
        return False
    return True

# check $Reference ($.'$Reference')
def json_model_26(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Reference'
    res = _jm_obj_53(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Reference']", path))
    return res

# check $schema ($.'$schema')
def json_model_36(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema'
    # $.'$schema'.'@'
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema'.'@']", path))
    return res

# check _jm_obj_54_may_$anchor ($.'$Schema'.'$anchor')
def _jm_f_127(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$anchor'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#URI-REFERENCE [$.'$Schema'.'$anchor']", path))
    return res

# check _jm_obj_54_may_$comment ($.'$Schema'.'$comment')
def _jm_f_128(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$comment'
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Schema'.'$comment']", path))
    return res

# object $.'$Schema'.'$defs'
def _jm_obj_55(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Schema'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_53: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Schema'.'$defs'.''
        res = json_model_52(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.'$defs'.'']", lpath_53 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_54_may_$defs ($.'$Schema'.'$defs')
def _jm_f_129(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$defs'
    res = _jm_obj_55(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Schema'.'$defs']", path))
    return res

# check _jm_obj_54_may_$dynamicAnchor ($.'$Schema'.'$dynamicAnchor')
def _jm_f_130(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$dynamicAnchor'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#URI-REFERENCE [$.'$Schema'.'$dynamicAnchor']", path))
    return res

# check _jm_obj_54_may_$dynamicRef ($.'$Schema'.'$dynamicRef')
def _jm_f_131(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$dynamicRef'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#URI-REFERENCE [$.'$Schema'.'$dynamicRef']", path))
    return res

# check _jm_obj_54_may_$id ($.'$Schema'.'$id')
def _jm_f_132(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$id'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#URI-REFERENCE [$.'$Schema'.'$id']", path))
    return res

# check _jm_obj_54_may_$ref ($.'$Schema'.'$ref')
def _jm_f_133(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$ref'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#URI-REFERENCE [$.'$Schema'.'$ref']", path))
    return res

# check _jm_obj_54_may_$schema ($.'$Schema'.'$schema')
def _jm_f_134(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$schema'
    res = isinstance(val, str) and val == "https://json-schema.org/draft/2020-12/schema"
    if not res:
        rep is None or rep.append(("unexpected _https://json-schema.org/draft/2020-12/schema [$.'$Schema'.'$schema']", path))
    return res

# object $.'$Schema'.'$vocabulary'
def _jm_obj_56(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Schema'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_54: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_54 if path is not None else None, rep):
            # handle {len(defs)} key props
            # $.'$Schema'.'$vocabulary'.'$URL'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [$.'$Schema'.'$vocabulary'.'$URL']", lpath_54 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Schema'.'$vocabulary']", lpath_54 if path is not None else None))
            return False
    return True

# check _jm_obj_54_may_$vocabulary ($.'$Schema'.'$vocabulary')
def _jm_f_135(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.'$vocabulary'
    res = _jm_obj_56(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Schema'.'$vocabulary']", path))
    return res

# check _jm_obj_54_may_additionalProperties ($.'$Schema'.additionalProperties)
def _jm_f_136(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.additionalProperties
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.additionalProperties]", path))
    return res

# check _jm_obj_54_may_allOf ($.'$Schema'.allOf)
def _jm_f_137(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.allOf
    res = json_model_49(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#schemaArray [$.'$Schema'.allOf]", path))
    return res

# check _jm_obj_54_may_anyOf ($.'$Schema'.anyOf)
def _jm_f_138(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.anyOf
    res = json_model_49(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#schemaArray [$.'$Schema'.anyOf]", path))
    return res

# check _jm_obj_54_may_const ($.'$Schema'.const)
def _jm_f_139(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.const
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$Schema'.const]", path))
    return res

# check _jm_obj_54_may_contains ($.'$Schema'.contains)
def _jm_f_140(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.contains
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.contains]", path))
    return res

# check _jm_obj_54_may_contentEncoding ($.'$Schema'.contentEncoding)
def _jm_f_141(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.contentEncoding
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Schema'.contentEncoding]", path))
    return res

# check _jm_obj_54_may_contentMediaType ($.'$Schema'.contentMediaType)
def _jm_f_142(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.contentMediaType
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Schema'.contentMediaType]", path))
    return res

# check _jm_obj_54_may_contentSchema ($.'$Schema'.contentSchema)
def _jm_f_143(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.contentSchema
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.contentSchema]", path))
    return res

# check _jm_obj_54_may_default ($.'$Schema'.default)
def _jm_f_144(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.default
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$Schema'.default]", path))
    return res

# object $.'$Schema'.dependentRequired
def _jm_obj_57(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Schema'.dependentRequired]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_55: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Schema'.dependentRequired.''
        res = json_model_46(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $schema#stringArray [$.'$Schema'.dependentRequired.'']", lpath_55 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_54_may_dependentRequired ($.'$Schema'.dependentRequired)
def _jm_f_145(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.dependentRequired
    res = _jm_obj_57(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Schema'.dependentRequired]", path))
    return res

# object $.'$Schema'.dependentSchemas
def _jm_obj_58(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Schema'.dependentSchemas]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_56: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Schema'.dependentSchemas.''
        res = json_model_52(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.dependentSchemas.'']", lpath_56 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_54_may_dependentSchemas ($.'$Schema'.dependentSchemas)
def _jm_f_146(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.dependentSchemas
    res = _jm_obj_58(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Schema'.dependentSchemas]", path))
    return res

# check _jm_obj_54_may_deprecated ($.'$Schema'.deprecated)
def _jm_f_147(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.deprecated
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Schema'.deprecated]", path))
    return res

# check _jm_obj_54_may_description ($.'$Schema'.description)
def _jm_f_148(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Schema'.description]", path))
    return res

# check _jm_obj_54_may_discriminator ($.'$Schema'.discriminator)
def _jm_f_149(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.discriminator
    res = json_model_29(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Discriminator [$.'$Schema'.discriminator]", path))
    return res

# check _jm_obj_54_may_else ($.'$Schema'.else)
def _jm_f_150(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.else
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.else]", path))
    return res

# check _jm_obj_54_may_enum ($.'$Schema'.enum)
def _jm_f_151(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.enum
    res = isinstance(val, list)
    if res:
        # accept any array
        pass
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$Schema'.enum]", path))
    return res

# check _jm_obj_54_may_examples ($.'$Schema'.examples)
def _jm_f_152(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.examples
    res = isinstance(val, list)
    if res:
        # accept any array
        pass
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$Schema'.examples]", path))
    return res

# check _jm_obj_54_may_exclusiveMaximum ($.'$Schema'.exclusiveMaximum)
def _jm_f_153(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.exclusiveMaximum
    res = isinstance(val, float)
    if not res:
        rep is None or rep.append(("not a -1.0 strict float [$.'$Schema'.exclusiveMaximum]", path))
    return res

# check _jm_obj_54_may_exclusiveMinimum ($.'$Schema'.exclusiveMinimum)
def _jm_f_154(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.exclusiveMinimum
    res = isinstance(val, float)
    if not res:
        rep is None or rep.append(("not a -1.0 strict float [$.'$Schema'.exclusiveMinimum]", path))
    return res

# check _jm_obj_54_may_externalDocs ($.'$Schema'.externalDocs)
def _jm_f_155(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.externalDocs
    res = json_model_12(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $ExternalDocumentation [$.'$Schema'.externalDocs]", path))
    return res

# check _jm_obj_54_may_format ($.'$Schema'.format)
def _jm_f_156(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.format
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Schema'.format]", path))
    return res

# check _jm_obj_54_may_if ($.'$Schema'.if)
def _jm_f_157(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.if
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.if]", path))
    return res

# check _jm_obj_54_may_items ($.'$Schema'.items)
def _jm_f_158(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.items
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.items]", path))
    return res

# check _jm_obj_54_may_maxContains ($.'$Schema'.maxContains)
def _jm_f_159(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.maxContains
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#nonNegativeInteger [$.'$Schema'.maxContains]", path))
    return res

# check _jm_obj_54_may_maxItems ($.'$Schema'.maxItems)
def _jm_f_160(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.maxItems
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#nonNegativeInteger [$.'$Schema'.maxItems]", path))
    return res

# check _jm_obj_54_may_maxLength ($.'$Schema'.maxLength)
def _jm_f_161(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.maxLength
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#nonNegativeInteger [$.'$Schema'.maxLength]", path))
    return res

# check _jm_obj_54_may_maxProperties ($.'$Schema'.maxProperties)
def _jm_f_162(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.maxProperties
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#nonNegativeInteger [$.'$Schema'.maxProperties]", path))
    return res

# check _jm_obj_54_may_maximum ($.'$Schema'.maximum)
def _jm_f_163(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.maximum
    res = isinstance(val, float)
    if not res:
        rep is None or rep.append(("not a -1.0 strict float [$.'$Schema'.maximum]", path))
    return res

# check _jm_obj_54_may_minContains ($.'$Schema'.minContains)
def _jm_f_164(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.minContains
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#nonNegativeInteger [$.'$Schema'.minContains]", path))
    return res

# check _jm_obj_54_may_minItems ($.'$Schema'.minItems)
def _jm_f_165(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.minItems
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#nonNegativeInteger [$.'$Schema'.minItems]", path))
    return res

# check _jm_obj_54_may_minLength ($.'$Schema'.minLength)
def _jm_f_166(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.minLength
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#nonNegativeInteger [$.'$Schema'.minLength]", path))
    return res

# check _jm_obj_54_may_minProperties ($.'$Schema'.minProperties)
def _jm_f_167(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.minProperties
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#nonNegativeInteger [$.'$Schema'.minProperties]", path))
    return res

# check _jm_obj_54_may_minimum ($.'$Schema'.minimum)
def _jm_f_168(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.minimum
    res = isinstance(val, float)
    if not res:
        rep is None or rep.append(("not a -1.0 strict float [$.'$Schema'.minimum]", path))
    return res

# check _jm_obj_54_may_multipleOf ($.'$Schema'.multipleOf)
def _jm_f_169(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.multipleOf
    res = isinstance(val, float) and val > 0.0
    if not res:
        rep is None or rep.append(("not a 1.0 strict float [$.'$Schema'.multipleOf]", path))
    return res

# check _jm_obj_54_may_not ($.'$Schema'.not)
def _jm_f_170(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.not
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.not]", path))
    return res

# check _jm_obj_54_may_oneOf ($.'$Schema'.oneOf)
def _jm_f_171(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.oneOf
    res = json_model_49(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#schemaArray [$.'$Schema'.oneOf]", path))
    return res

# check _jm_obj_54_may_pattern ($.'$Schema'.pattern)
def _jm_f_172(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.pattern
    res = is_valid_regex(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $REGEX [$.'$Schema'.pattern]", path))
    return res

# object $.'$Schema'.patternProperties
def _jm_obj_59(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Schema'.patternProperties]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_57: Path = (path + [ prop ]) if path is not None else None
        if is_valid_regex(prop, lpath_57 if path is not None else None, rep):
            # handle {len(defs)} key props
            # $.'$Schema'.patternProperties.'$REGEX'
            res = json_model_52(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.patternProperties.'$REGEX']", lpath_57 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Schema'.patternProperties]", lpath_57 if path is not None else None))
            return False
    return True

# check _jm_obj_54_may_patternProperties ($.'$Schema'.patternProperties)
def _jm_f_173(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.patternProperties
    res = _jm_obj_59(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Schema'.patternProperties]", path))
    return res

# check _jm_obj_54_may_prefixItems ($.'$Schema'.prefixItems)
def _jm_f_174(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.prefixItems
    res = json_model_49(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#schemaArray [$.'$Schema'.prefixItems]", path))
    return res

# object $.'$Schema'.properties
def _jm_obj_60(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Schema'.properties]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_58: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Schema'.properties.''
        res = json_model_52(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.properties.'']", lpath_58 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_54_may_properties ($.'$Schema'.properties)
def _jm_f_175(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.properties
    res = _jm_obj_60(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Schema'.properties]", path))
    return res

# check _jm_obj_54_may_propertyNames ($.'$Schema'.propertyNames)
def _jm_f_176(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.propertyNames
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.propertyNames]", path))
    return res

# check _jm_obj_54_may_readOnly ($.'$Schema'.readOnly)
def _jm_f_177(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.readOnly
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Schema'.readOnly]", path))
    return res

# check _jm_obj_54_may_required ($.'$Schema'.required)
def _jm_f_178(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.required
    res = json_model_46(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#stringArray [$.'$Schema'.required]", path))
    return res

# check _jm_obj_54_may_then ($.'$Schema'.then)
def _jm_f_179(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.then
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.then]", path))
    return res

# check _jm_obj_54_may_title ($.'$Schema'.title)
def _jm_f_180(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.title
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Schema'.title]", path))
    return res

# check _jm_obj_54_may_type ($.'$Schema'.type)
def _jm_f_181(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.type
    # $.'$Schema'.type.'|'.0
    res = json_model_44(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#simpleTypes [$.'$Schema'.type.'|'.0]", path))
    if not res:
        # $.'$Schema'.type.'|'.1
        res = json_model_45(val, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $schema#simpleTypesArray [$.'$Schema'.type.'|'.1]", path))
    if not res:
        rep is None or rep.append(("no model matched [$.'$Schema'.type.'|']", path))
    return res

# check _jm_obj_54_may_unevaluatedItems ($.'$Schema'.unevaluatedItems)
def _jm_f_182(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.unevaluatedItems
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.unevaluatedItems]", path))
    return res

# check _jm_obj_54_may_unevaluatedProperties ($.'$Schema'.unevaluatedProperties)
def _jm_f_183(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.unevaluatedProperties
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schema#Schema [$.'$Schema'.unevaluatedProperties]", path))
    return res

# check _jm_obj_54_may_uniqueItems ($.'$Schema'.uniqueItems)
def _jm_f_184(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.uniqueItems
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Schema'.uniqueItems]", path))
    return res

# check _jm_obj_54_may_writeOnly ($.'$Schema'.writeOnly)
def _jm_f_185(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.writeOnly
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$Schema'.writeOnly]", path))
    return res

# check _jm_obj_54_may_xml ($.'$Schema'.xml)
def _jm_f_186(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'.xml
    res = json_model_30(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $XML [$.'$Schema'.xml]", path))
    return res


# object $.'$Schema'
def _jm_obj_54(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Schema']", path))
        return False
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_52: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_54_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_52 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Schema']", lpath_52 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Schema']", lpath_52 if path is not None else None))
            return False
    return True

# check $Schema ($.'$Schema')
def json_model_28(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Schema'
    res = _jm_obj_54(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Schema']", path))
    return res

# check _jm_obj_61_must_propertyName ($.'$Discriminator'.propertyName)
def _jm_f_187(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Discriminator'.propertyName
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$Discriminator'.propertyName]", path))
    return res


# object $.'$Discriminator'.mapping
def _jm_obj_62(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Discriminator'.mapping]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_60: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$Discriminator'.mapping.''
        res = isinstance(pval, str)
        if not res:
            rep is None or rep.append(("unexpected string [$.'$Discriminator'.mapping.'']", lpath_60 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_61_may_mapping ($.'$Discriminator'.mapping)
def _jm_f_188(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Discriminator'.mapping
    res = _jm_obj_62(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Discriminator'.mapping]", path))
    return res


# object $.'$Discriminator'
def _jm_obj_61(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Discriminator']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_59: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_61_must.get(prop):
            # handle 1 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_59 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$Discriminator']", lpath_59 if path is not None else None))
                    return False
        elif pfun := _jm_obj_61_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_59 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$Discriminator']", lpath_59 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Discriminator'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Discriminator'.'/^x\\\\-.*$/']", lpath_59 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Discriminator']", lpath_59 if path is not None else None))
            return False
    if must_count != 1:
        rep is None or rep.append(("missing must prop [$.'$Discriminator']", path))
        return False
    return True

# check $Discriminator ($.'$Discriminator')
def json_model_29(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Discriminator'
    res = _jm_obj_61(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Discriminator']", path))
    return res

# check _jm_obj_63_may_attribute ($.'$XML'.attribute)
def _jm_f_189(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$XML'.attribute
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$XML'.attribute]", path))
    return res

# check _jm_obj_63_may_name ($.'$XML'.name)
def _jm_f_190(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$XML'.name
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$XML'.name]", path))
    return res

# check _jm_obj_63_may_namespace ($.'$XML'.namespace)
def _jm_f_191(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$XML'.namespace
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$XML'.namespace]", path))
    return res

# check _jm_obj_63_may_prefix ($.'$XML'.prefix)
def _jm_f_192(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$XML'.prefix
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$XML'.prefix]", path))
    return res

# check _jm_obj_63_may_wrapped ($.'$XML'.wrapped)
def _jm_f_193(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$XML'.wrapped
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$XML'.wrapped]", path))
    return res


# object $.'$XML'
def _jm_obj_63(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$XML']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_61: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_63_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_61 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$XML']", lpath_61 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$XML'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$XML'.'/^x\\\\-.*$/']", lpath_61 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$XML']", lpath_61 if path is not None else None))
            return False
    return True

# check $XML ($.'$XML')
def json_model_30(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$XML'
    res = _jm_obj_63(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$XML']", path))
    return res

# check _jm_obj_64_must_flows ($.'$SecurityScheme'.flows)
def _jm_f_194(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'.flows
    res = json_model_32(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $OAuthFlows [$.'$SecurityScheme'.flows]", path))
    return res

# check _jm_obj_64_must_in ($.'$SecurityScheme'.in)
def _jm_f_195(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'.in
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$SecurityScheme'.in]", path))
    return res

# check _jm_obj_64_must_name ($.'$SecurityScheme'.name)
def _jm_f_196(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'.name
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$SecurityScheme'.name]", path))
    return res

# check _jm_obj_64_must_openIdConnectUrl ($.'$SecurityScheme'.openIdConnectUrl)
def _jm_f_197(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'.openIdConnectUrl
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$SecurityScheme'.openIdConnectUrl]", path))
    return res

# check _jm_obj_64_must_scheme ($.'$SecurityScheme'.scheme)
def _jm_f_198(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'.scheme
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$SecurityScheme'.scheme]", path))
    return res

# check _jm_obj_64_must_type ($.'$SecurityScheme'.type)
def _jm_f_199(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'.type
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$SecurityScheme'.type]", path))
    return res


# check _jm_obj_64_may_bearerFormat ($.'$SecurityScheme'.bearerFormat)
def _jm_f_200(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'.bearerFormat
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$SecurityScheme'.bearerFormat]", path))
    return res

# check _jm_obj_64_may_description ($.'$SecurityScheme'.description)
def _jm_f_201(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$SecurityScheme'.description]", path))
    return res


# object $.'$SecurityScheme'
def _jm_obj_64(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$SecurityScheme']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_62: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_64_must.get(prop):
            # handle 6 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_62 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$SecurityScheme']", lpath_62 if path is not None else None))
                    return False
        elif pfun := _jm_obj_64_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_62 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$SecurityScheme']", lpath_62 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$SecurityScheme'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$SecurityScheme'.'/^x\\\\-.*$/']", lpath_62 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$SecurityScheme']", lpath_62 if path is not None else None))
            return False
    if must_count != 6:
        rep is None or rep.append(("missing must prop [$.'$SecurityScheme']", path))
        return False
    return True

# check $SecurityScheme ($.'$SecurityScheme')
def json_model_31(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityScheme'
    res = _jm_obj_64(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$SecurityScheme']", path))
    return res

# check _jm_obj_65_may_authorizationCode ($.'$OAuthFlows'.authorizationCode)
def _jm_f_202(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlows'.authorizationCode
    res = json_model_33(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $OAuthFlow [$.'$OAuthFlows'.authorizationCode]", path))
    return res

# check _jm_obj_65_may_clientCredentials ($.'$OAuthFlows'.clientCredentials)
def _jm_f_203(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlows'.clientCredentials
    res = json_model_33(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $OAuthFlow [$.'$OAuthFlows'.clientCredentials]", path))
    return res

# check _jm_obj_65_may_implicit ($.'$OAuthFlows'.implicit)
def _jm_f_204(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlows'.implicit
    res = json_model_33(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $OAuthFlow [$.'$OAuthFlows'.implicit]", path))
    return res

# check _jm_obj_65_may_password ($.'$OAuthFlows'.password)
def _jm_f_205(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlows'.password
    res = json_model_33(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $OAuthFlow [$.'$OAuthFlows'.password]", path))
    return res


# object $.'$OAuthFlows'
def _jm_obj_65(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$OAuthFlows']", path))
        return False
    res: bool
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_63: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_65_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_63 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$OAuthFlows']", lpath_63 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$OAuthFlows'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$OAuthFlows'.'/^x\\\\-.*$/']", lpath_63 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$OAuthFlows']", lpath_63 if path is not None else None))
            return False
    return True

# check $OAuthFlows ($.'$OAuthFlows')
def json_model_32(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlows'
    res = _jm_obj_65(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$OAuthFlows']", path))
    return res

# check _jm_obj_66_must_authorizationUrl ($.'$OAuthFlow'.authorizationUrl)
def _jm_f_206(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlow'.authorizationUrl
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$OAuthFlow'.authorizationUrl]", path))
    return res

# object $.'$OAuthFlow'.scopes
def _jm_obj_67(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$OAuthFlow'.scopes]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_65: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$OAuthFlow'.scopes.''
        res = isinstance(pval, str)
        if not res:
            rep is None or rep.append(("unexpected string [$.'$OAuthFlow'.scopes.'']", lpath_65 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_66_must_scopes ($.'$OAuthFlow'.scopes)
def _jm_f_207(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlow'.scopes
    res = _jm_obj_67(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$OAuthFlow'.scopes]", path))
    return res

# check _jm_obj_66_must_tokenUrl ($.'$OAuthFlow'.tokenUrl)
def _jm_f_208(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlow'.tokenUrl
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$OAuthFlow'.tokenUrl]", path))
    return res


# check _jm_obj_66_may_refreshUrl ($.'$OAuthFlow'.refreshUrl)
def _jm_f_209(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlow'.refreshUrl
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$OAuthFlow'.refreshUrl]", path))
    return res


# object $.'$OAuthFlow'
def _jm_obj_66(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$OAuthFlow']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_64: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_66_must.get(prop):
            # handle 3 must props
            if pfun != UNDEFINED:
                must_count += 1
                if not pfun(pval, lpath_64 if path is not None else None, rep):
                    rep is None or rep.append(("invalid must property value [$.'$OAuthFlow']", lpath_64 if path is not None else None))
                    return False
        elif pfun := _jm_obj_66_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_64 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$OAuthFlow']", lpath_64 if path is not None else None))
                return False
        elif _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$OAuthFlow'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$OAuthFlow'.'/^x\\\\-.*$/']", lpath_64 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$OAuthFlow']", lpath_64 if path is not None else None))
            return False
    if must_count != 3:
        rep is None or rep.append(("missing must prop [$.'$OAuthFlow']", path))
        return False
    return True

# check $OAuthFlow ($.'$OAuthFlow')
def json_model_33(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$OAuthFlow'
    res = _jm_obj_66(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$OAuthFlow']", path))
    return res

# object $.'$SecurityRequirement'
def _jm_obj_68(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$SecurityRequirement']", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_66: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$SecurityRequirement'.''
        res = isinstance(pval, list)
        if res:
            for arr_6_idx, arr_6_item in enumerate(pval):
                arr_6_lpath: Path = ((lpath_66 if path is not None else None) + [ arr_6_idx ]) if (lpath_66 if path is not None else None) is not None else None
                # $.'$SecurityRequirement'.''.0
                res = isinstance(arr_6_item, str)
                if not res:
                    rep is None or rep.append(("unexpected string [$.'$SecurityRequirement'.''.0]", arr_6_lpath if (lpath_66 if path is not None else None) is not None else None))
                if not res:
                    break
        if not res:
            rep is None or rep.append(("not array or unexpected array [$.'$SecurityRequirement'.'']", lpath_66 if path is not None else None))
        if not res:
            return False
    return True

# check $SecurityRequirement ($.'$SecurityRequirement')
def json_model_34(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$SecurityRequirement'
    res = _jm_obj_68(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$SecurityRequirement']", path))
    return res

# object $.'$Extension'
def _jm_obj_69(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$Extension']", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_67: Path = (path + [ prop ]) if path is not None else None
        if _jm_re_0(prop):
            # handle {len(regs)} re props
            # $.'$Extension'.'/^x\\-.*$/'
            res = True
            if not res:
                rep is None or rep.append(("unexpected $ANY [$.'$Extension'.'/^x\\\\-.*$/']", lpath_67 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$Extension']", lpath_67 if path is not None else None))
            return False
    return True

# check $Extension ($.'$Extension')
def json_model_35(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$Extension'
    res = _jm_obj_69(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$Extension']", path))
    return res

# check $ ($)
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $
    res = json_model_2(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $OpenAPI [$]", path))
    return res

# check $schema#URI-REFERENCE ($.'$schema#URI-REFERENCE')
def json_model_37(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#URI-REFERENCE'
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$schema#URI-REFERENCE']", path))
    return res


# check $schema#simpleTypes ($.'$schema#simpleTypes')
def json_model_44(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#simpleTypes'
    res = (val is None or isinstance(val, (bool, int, float, str))) and val in _jm_cst_3
    if not res:
        rep is None or rep.append(("value not in enum [$.'$schema#simpleTypes'.'|']", path))
    return res

# check $schema#simpleTypesArray ($.'$schema#simpleTypesArray')
def json_model_45(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#simpleTypesArray'
    # $.'$schema#simpleTypesArray'.'@'
    res = isinstance(val, list)
    if res:
        for arr_7_idx, arr_7_item in enumerate(val):
            arr_7_lpath: Path = (path + [ arr_7_idx ]) if path is not None else None
            # $.'$schema#simpleTypesArray'.'@'.0
            res = json_model_44(arr_7_item, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $simpleTypes [$.'$schema#simpleTypesArray'.'@'.0]", arr_7_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$schema#simpleTypesArray'.'@']", path))
    if res:
        ival_0: int = len(val)
        res = is_unique_array(val, path, rep) and ival_0 >= 1
    if not res:
        rep is None or rep.append(("constraints failed at [$.'$schema#simpleTypesArray']", path))
    return res

# check $schema#stringArray ($.'$schema#stringArray')
def json_model_46(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#stringArray'
    # $.'$schema#stringArray'.'@'
    res = isinstance(val, list)
    if res:
        for arr_8_idx, arr_8_item in enumerate(val):
            arr_8_lpath: Path = (path + [ arr_8_idx ]) if path is not None else None
            # $.'$schema#stringArray'.'@'.0
            res = isinstance(arr_8_item, str)
            if not res:
                rep is None or rep.append(("unexpected string [$.'$schema#stringArray'.'@'.0]", arr_8_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$schema#stringArray'.'@']", path))
    if res:
        res = is_unique_array(val, path, rep)
    if not res:
        rep is None or rep.append(("constraints failed at [$.'$schema#stringArray']", path))
    return res

# check $schema#nonNegativeInteger ($.'$schema#nonNegativeInteger')
def json_model_47(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#nonNegativeInteger'
    res = (isinstance(val, int) and not isinstance(val, bool) or isinstance(val, float) and val == int(val)) and val >= 0
    if not res:
        rep is None or rep.append(("not a 0 loose int [$.'$schema#nonNegativeInteger']", path))
    return res

# check $schema#schemaArray ($.'$schema#schemaArray')
def json_model_49(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#schemaArray'
    # $.'$schema#schemaArray'.'@'
    res = isinstance(val, list)
    if res:
        for arr_9_idx, arr_9_item in enumerate(val):
            arr_9_lpath: Path = (path + [ arr_9_idx ]) if path is not None else None
            # $.'$schema#schemaArray'.'@'.0
            res = json_model_52(arr_9_item, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Schema [$.'$schema#schemaArray'.'@'.0]", arr_9_lpath if path is not None else None))
            if not res:
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$schema#schemaArray'.'@']", path))
    if res:
        ival_1: int = len(val)
        res = ival_1 >= 1
    if not res:
        rep is None or rep.append(("constraints failed at [$.'$schema#schemaArray']", path))
    return res

# check $schema#Schema ($.'$schema#Schema')
def json_model_52(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#Schema'
    # $.'$schema#Schema'.'|'.0
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$schema#Schema'.'|'.0]", path))
    if not res:
        # $.'$schema#Schema'.'|'.1
        res = json_model_51(val, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $ObjectSchema [$.'$schema#Schema'.'|'.1]", path))
    if not res:
        rep is None or rep.append(("no model matched [$.'$schema#Schema'.'|']", path))
    return res

# check _jm_obj_70_may_$anchor ($.'$schema#ObjectSchema'.'$anchor')
def _jm_f_210(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$anchor'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URI-REFERENCE [$.'$schema#ObjectSchema'.'$anchor']", path))
    return res

# check _jm_obj_70_may_$comment ($.'$schema#ObjectSchema'.'$comment')
def _jm_f_211(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$comment'
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$schema#ObjectSchema'.'$comment']", path))
    return res

# object $.'$schema#ObjectSchema'.'$defs'
def _jm_obj_71(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$schema#ObjectSchema'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_69: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$schema#ObjectSchema'.'$defs'.''
        res = json_model_52(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.'$defs'.'']", lpath_69 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_70_may_$defs ($.'$schema#ObjectSchema'.'$defs')
def _jm_f_212(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$defs'
    res = _jm_obj_71(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$schema#ObjectSchema'.'$defs']", path))
    return res

# check _jm_obj_70_may_$dynamicAnchor ($.'$schema#ObjectSchema'.'$dynamicAnchor')
def _jm_f_213(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$dynamicAnchor'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URI-REFERENCE [$.'$schema#ObjectSchema'.'$dynamicAnchor']", path))
    return res

# check _jm_obj_70_may_$dynamicRef ($.'$schema#ObjectSchema'.'$dynamicRef')
def _jm_f_214(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$dynamicRef'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URI-REFERENCE [$.'$schema#ObjectSchema'.'$dynamicRef']", path))
    return res

# check _jm_obj_70_may_$id ($.'$schema#ObjectSchema'.'$id')
def _jm_f_215(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$id'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URI-REFERENCE [$.'$schema#ObjectSchema'.'$id']", path))
    return res

# check _jm_obj_70_may_$ref ($.'$schema#ObjectSchema'.'$ref')
def _jm_f_216(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$ref'
    res = json_model_37(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $URI-REFERENCE [$.'$schema#ObjectSchema'.'$ref']", path))
    return res

# check _jm_obj_70_may_$schema ($.'$schema#ObjectSchema'.'$schema')
def _jm_f_217(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$schema'
    res = isinstance(val, str) and val == "https://json-schema.org/draft/2020-12/schema"
    if not res:
        rep is None or rep.append(("unexpected _https://json-schema.org/draft/2020-12/schema [$.'$schema#ObjectSchema'.'$schema']", path))
    return res

# object $.'$schema#ObjectSchema'.'$vocabulary'
def _jm_obj_72(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$schema#ObjectSchema'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_70: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_70 if path is not None else None, rep):
            # handle {len(defs)} key props
            # $.'$schema#ObjectSchema'.'$vocabulary'.'$URL'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [$.'$schema#ObjectSchema'.'$vocabulary'.'$URL']", lpath_70 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$schema#ObjectSchema'.'$vocabulary']", lpath_70 if path is not None else None))
            return False
    return True

# check _jm_obj_70_may_$vocabulary ($.'$schema#ObjectSchema'.'$vocabulary')
def _jm_f_218(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.'$vocabulary'
    res = _jm_obj_72(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$schema#ObjectSchema'.'$vocabulary']", path))
    return res

# check _jm_obj_70_may_additionalProperties ($.'$schema#ObjectSchema'.additionalProperties)
def _jm_f_219(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.additionalProperties
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.additionalProperties]", path))
    return res

# check _jm_obj_70_may_allOf ($.'$schema#ObjectSchema'.allOf)
def _jm_f_220(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.allOf
    res = json_model_49(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schemaArray [$.'$schema#ObjectSchema'.allOf]", path))
    return res

# check _jm_obj_70_may_anyOf ($.'$schema#ObjectSchema'.anyOf)
def _jm_f_221(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.anyOf
    res = json_model_49(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schemaArray [$.'$schema#ObjectSchema'.anyOf]", path))
    return res

# check _jm_obj_70_may_const ($.'$schema#ObjectSchema'.const)
def _jm_f_222(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.const
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$schema#ObjectSchema'.const]", path))
    return res

# check _jm_obj_70_may_contains ($.'$schema#ObjectSchema'.contains)
def _jm_f_223(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.contains
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.contains]", path))
    return res

# check _jm_obj_70_may_contentEncoding ($.'$schema#ObjectSchema'.contentEncoding)
def _jm_f_224(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.contentEncoding
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$schema#ObjectSchema'.contentEncoding]", path))
    return res

# check _jm_obj_70_may_contentMediaType ($.'$schema#ObjectSchema'.contentMediaType)
def _jm_f_225(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.contentMediaType
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$schema#ObjectSchema'.contentMediaType]", path))
    return res

# check _jm_obj_70_may_contentSchema ($.'$schema#ObjectSchema'.contentSchema)
def _jm_f_226(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.contentSchema
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.contentSchema]", path))
    return res

# check _jm_obj_70_may_default ($.'$schema#ObjectSchema'.default)
def _jm_f_227(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.default
    res = True
    if not res:
        rep is None or rep.append(("unexpected $ANY [$.'$schema#ObjectSchema'.default]", path))
    return res

# object $.'$schema#ObjectSchema'.dependentRequired
def _jm_obj_73(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$schema#ObjectSchema'.dependentRequired]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_71: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$schema#ObjectSchema'.dependentRequired.''
        res = json_model_46(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $stringArray [$.'$schema#ObjectSchema'.dependentRequired.'']", lpath_71 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_70_may_dependentRequired ($.'$schema#ObjectSchema'.dependentRequired)
def _jm_f_228(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.dependentRequired
    res = _jm_obj_73(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$schema#ObjectSchema'.dependentRequired]", path))
    return res

# object $.'$schema#ObjectSchema'.dependentSchemas
def _jm_obj_74(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$schema#ObjectSchema'.dependentSchemas]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_72: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$schema#ObjectSchema'.dependentSchemas.''
        res = json_model_52(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.dependentSchemas.'']", lpath_72 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_70_may_dependentSchemas ($.'$schema#ObjectSchema'.dependentSchemas)
def _jm_f_229(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.dependentSchemas
    res = _jm_obj_74(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$schema#ObjectSchema'.dependentSchemas]", path))
    return res

# check _jm_obj_70_may_deprecated ($.'$schema#ObjectSchema'.deprecated)
def _jm_f_230(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.deprecated
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$schema#ObjectSchema'.deprecated]", path))
    return res

# check _jm_obj_70_may_description ($.'$schema#ObjectSchema'.description)
def _jm_f_231(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.description
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$schema#ObjectSchema'.description]", path))
    return res

# check _jm_obj_70_may_else ($.'$schema#ObjectSchema'.else)
def _jm_f_232(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.else
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.else]", path))
    return res

# check _jm_obj_70_may_enum ($.'$schema#ObjectSchema'.enum)
def _jm_f_233(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.enum
    res = isinstance(val, list)
    if res:
        # accept any array
        pass
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$schema#ObjectSchema'.enum]", path))
    return res

# check _jm_obj_70_may_examples ($.'$schema#ObjectSchema'.examples)
def _jm_f_234(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.examples
    res = isinstance(val, list)
    if res:
        # accept any array
        pass
    if not res:
        rep is None or rep.append(("not array or unexpected array [$.'$schema#ObjectSchema'.examples]", path))
    return res

# check _jm_obj_70_may_exclusiveMaximum ($.'$schema#ObjectSchema'.exclusiveMaximum)
def _jm_f_235(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.exclusiveMaximum
    res = isinstance(val, (int, float)) and not isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a -1.0 loose float [$.'$schema#ObjectSchema'.exclusiveMaximum]", path))
    return res

# check _jm_obj_70_may_exclusiveMinimum ($.'$schema#ObjectSchema'.exclusiveMinimum)
def _jm_f_236(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.exclusiveMinimum
    res = isinstance(val, (int, float)) and not isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a -1.0 loose float [$.'$schema#ObjectSchema'.exclusiveMinimum]", path))
    return res

# check _jm_obj_70_may_format ($.'$schema#ObjectSchema'.format)
def _jm_f_237(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.format
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$schema#ObjectSchema'.format]", path))
    return res

# check _jm_obj_70_may_if ($.'$schema#ObjectSchema'.if)
def _jm_f_238(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.if
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.if]", path))
    return res

# check _jm_obj_70_may_items ($.'$schema#ObjectSchema'.items)
def _jm_f_239(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.items
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.items]", path))
    return res

# check _jm_obj_70_may_maxContains ($.'$schema#ObjectSchema'.maxContains)
def _jm_f_240(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.maxContains
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $nonNegativeInteger [$.'$schema#ObjectSchema'.maxContains]", path))
    return res

# check _jm_obj_70_may_maxItems ($.'$schema#ObjectSchema'.maxItems)
def _jm_f_241(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.maxItems
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $nonNegativeInteger [$.'$schema#ObjectSchema'.maxItems]", path))
    return res

# check _jm_obj_70_may_maxLength ($.'$schema#ObjectSchema'.maxLength)
def _jm_f_242(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.maxLength
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $nonNegativeInteger [$.'$schema#ObjectSchema'.maxLength]", path))
    return res

# check _jm_obj_70_may_maxProperties ($.'$schema#ObjectSchema'.maxProperties)
def _jm_f_243(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.maxProperties
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $nonNegativeInteger [$.'$schema#ObjectSchema'.maxProperties]", path))
    return res

# check _jm_obj_70_may_maximum ($.'$schema#ObjectSchema'.maximum)
def _jm_f_244(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.maximum
    res = isinstance(val, (int, float)) and not isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a -1.0 loose float [$.'$schema#ObjectSchema'.maximum]", path))
    return res

# check _jm_obj_70_may_minContains ($.'$schema#ObjectSchema'.minContains)
def _jm_f_245(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.minContains
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $nonNegativeInteger [$.'$schema#ObjectSchema'.minContains]", path))
    return res

# check _jm_obj_70_may_minItems ($.'$schema#ObjectSchema'.minItems)
def _jm_f_246(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.minItems
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $nonNegativeInteger [$.'$schema#ObjectSchema'.minItems]", path))
    return res

# check _jm_obj_70_may_minLength ($.'$schema#ObjectSchema'.minLength)
def _jm_f_247(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.minLength
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $nonNegativeInteger [$.'$schema#ObjectSchema'.minLength]", path))
    return res

# check _jm_obj_70_may_minProperties ($.'$schema#ObjectSchema'.minProperties)
def _jm_f_248(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.minProperties
    res = json_model_47(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $nonNegativeInteger [$.'$schema#ObjectSchema'.minProperties]", path))
    return res

# check _jm_obj_70_may_minimum ($.'$schema#ObjectSchema'.minimum)
def _jm_f_249(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.minimum
    res = isinstance(val, (int, float)) and not isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a -1.0 loose float [$.'$schema#ObjectSchema'.minimum]", path))
    return res

# check _jm_obj_70_may_multipleOf ($.'$schema#ObjectSchema'.multipleOf)
def _jm_f_250(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.multipleOf
    res = isinstance(val, (int, float)) and not isinstance(val, bool) and val > 0.0
    if not res:
        rep is None or rep.append(("not a 1.0 loose float [$.'$schema#ObjectSchema'.multipleOf]", path))
    return res

# check _jm_obj_70_may_not ($.'$schema#ObjectSchema'.not)
def _jm_f_251(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.not
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.not]", path))
    return res

# check _jm_obj_70_may_oneOf ($.'$schema#ObjectSchema'.oneOf)
def _jm_f_252(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.oneOf
    res = json_model_49(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schemaArray [$.'$schema#ObjectSchema'.oneOf]", path))
    return res

# check _jm_obj_70_may_pattern ($.'$schema#ObjectSchema'.pattern)
def _jm_f_253(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.pattern
    res = is_valid_regex(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $REGEX [$.'$schema#ObjectSchema'.pattern]", path))
    return res

# object $.'$schema#ObjectSchema'.patternProperties
def _jm_obj_75(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$schema#ObjectSchema'.patternProperties]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_73: Path = (path + [ prop ]) if path is not None else None
        if is_valid_regex(prop, lpath_73 if path is not None else None, rep):
            # handle {len(defs)} key props
            # $.'$schema#ObjectSchema'.patternProperties.'$REGEX'
            res = json_model_52(pval, path, rep)
            if not res:
                rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.patternProperties.'$REGEX']", lpath_73 if path is not None else None))
            if not res:
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$schema#ObjectSchema'.patternProperties]", lpath_73 if path is not None else None))
            return False
    return True

# check _jm_obj_70_may_patternProperties ($.'$schema#ObjectSchema'.patternProperties)
def _jm_f_254(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.patternProperties
    res = _jm_obj_75(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$schema#ObjectSchema'.patternProperties]", path))
    return res

# check _jm_obj_70_may_prefixItems ($.'$schema#ObjectSchema'.prefixItems)
def _jm_f_255(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.prefixItems
    res = json_model_49(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $schemaArray [$.'$schema#ObjectSchema'.prefixItems]", path))
    return res

# object $.'$schema#ObjectSchema'.properties
def _jm_obj_76(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$schema#ObjectSchema'.properties]", path))
        return False
    res: bool
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_74: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # $.'$schema#ObjectSchema'.properties.''
        res = json_model_52(pval, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.properties.'']", lpath_74 if path is not None else None))
        if not res:
            return False
    return True

# check _jm_obj_70_may_properties ($.'$schema#ObjectSchema'.properties)
def _jm_f_256(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.properties
    res = _jm_obj_76(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$schema#ObjectSchema'.properties]", path))
    return res

# check _jm_obj_70_may_propertyNames ($.'$schema#ObjectSchema'.propertyNames)
def _jm_f_257(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.propertyNames
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.propertyNames]", path))
    return res

# check _jm_obj_70_may_readOnly ($.'$schema#ObjectSchema'.readOnly)
def _jm_f_258(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.readOnly
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$schema#ObjectSchema'.readOnly]", path))
    return res

# check _jm_obj_70_may_required ($.'$schema#ObjectSchema'.required)
def _jm_f_259(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.required
    res = json_model_46(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $stringArray [$.'$schema#ObjectSchema'.required]", path))
    return res

# check _jm_obj_70_may_then ($.'$schema#ObjectSchema'.then)
def _jm_f_260(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.then
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.then]", path))
    return res

# check _jm_obj_70_may_title ($.'$schema#ObjectSchema'.title)
def _jm_f_261(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.title
    res = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected string [$.'$schema#ObjectSchema'.title]", path))
    return res

# check _jm_obj_70_may_type ($.'$schema#ObjectSchema'.type)
def _jm_f_262(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.type
    # $.'$schema#ObjectSchema'.type.'|'.0
    res = json_model_44(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $simpleTypes [$.'$schema#ObjectSchema'.type.'|'.0]", path))
    if not res:
        # $.'$schema#ObjectSchema'.type.'|'.1
        res = json_model_45(val, path, rep)
        if not res:
            rep is None or rep.append(("unexpected $simpleTypesArray [$.'$schema#ObjectSchema'.type.'|'.1]", path))
    if not res:
        rep is None or rep.append(("no model matched [$.'$schema#ObjectSchema'.type.'|']", path))
    return res

# check _jm_obj_70_may_unevaluatedItems ($.'$schema#ObjectSchema'.unevaluatedItems)
def _jm_f_263(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.unevaluatedItems
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.unevaluatedItems]", path))
    return res

# check _jm_obj_70_may_unevaluatedProperties ($.'$schema#ObjectSchema'.unevaluatedProperties)
def _jm_f_264(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.unevaluatedProperties
    res = json_model_52(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected $Schema [$.'$schema#ObjectSchema'.unevaluatedProperties]", path))
    return res

# check _jm_obj_70_may_uniqueItems ($.'$schema#ObjectSchema'.uniqueItems)
def _jm_f_265(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.uniqueItems
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$schema#ObjectSchema'.uniqueItems]", path))
    return res

# check _jm_obj_70_may_writeOnly ($.'$schema#ObjectSchema'.writeOnly)
def _jm_f_266(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'.writeOnly
    res = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [$.'$schema#ObjectSchema'.writeOnly]", path))
    return res


# object $.'$schema#ObjectSchema'
def _jm_obj_70(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [$.'$schema#ObjectSchema']", path))
        return False
    pfun: CheckFun
    for prop, pval in val.items():
        assert isinstance(prop, str)
        lpath_68: Path = (path + [ prop ]) if path is not None else None
        if pfun := _jm_obj_70_may.get(prop):
            # handle {len(may)} may props
            if pfun != UNDEFINED and not pfun(pval, lpath_68 if path is not None else None, rep):
                rep is None or rep.append(("invalid may property value [$.'$schema#ObjectSchema']", lpath_68 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("no other prop expected [$.'$schema#ObjectSchema']", lpath_68 if path is not None else None))
            return False
    return True

# check $schema#ObjectSchema ($.'$schema#ObjectSchema')
def json_model_51(val: Jsonable, path: Path, rep: Report) -> bool:
    res: bool
    # $.'$schema#ObjectSchema'
    res = _jm_obj_70(val, path, rep)
    if not res:
        rep is None or rep.append(("not an expected object at [$.'$schema#ObjectSchema']", path))
    return res


# initialization guard
initialized: bool = False

# differed module initializations
def check_model_init():
    global initialized
    if not initialized:
        initialized = True
        global _jm_obj_0_must
        _jm_obj_0_must = {
            "info": _jm_f_0,
            "openapi": _jm_f_1,
        }
        global _jm_obj_0_may
        _jm_obj_0_may = {
            "component": _jm_f_2,
            "externalDocs": _jm_f_3,
            "jsonSchemaDialect": _jm_f_4,
            "paths": _jm_f_5,
            "security": _jm_f_6,
            "servers": _jm_f_7,
            "tags": _jm_f_8,
            "webhooks": _jm_f_9,
        }
        global _jm_re_0_search, _jm_re_0
        _jm_re_0_search = re.compile("^x\\-.*$").search
        _jm_re_0 = lambda s: _jm_re_0_search(s) is not None
        global _jm_obj_2_must
        _jm_obj_2_must = {
            "title": _jm_f_10,
            "version": _jm_f_11,
        }
        global _jm_obj_2_may
        _jm_obj_2_may = {
            "contact": _jm_f_12,
            "description": _jm_f_13,
            "license": _jm_f_14,
            "summary": _jm_f_15,
            "termsOfService": _jm_f_16,
        }
        global _jm_obj_3_may
        _jm_obj_3_may = {
            "email": _jm_f_17,
            "name": _jm_f_18,
            "url": _jm_f_19,
        }
        global _jm_obj_4_must
        _jm_obj_4_must = {
            "name": _jm_f_20,
            "url": _jm_f_21,
        }
        global _jm_obj_5_must
        _jm_obj_5_must = {
            "identifier": _jm_f_22,
            "name": _jm_f_23,
        }
        global _jm_obj_6_must
        _jm_obj_6_must = {
            "url": _jm_f_24,
        }
        global _jm_obj_6_may
        _jm_obj_6_may = {
            "description": _jm_f_25,
            "variables": _jm_f_26,
        }
        global _jm_obj_8_must
        _jm_obj_8_must = {
            "default": _jm_f_27,
        }
        global _jm_obj_8_may
        _jm_obj_8_may = {
            "description": _jm_f_28,
            "enum": _jm_f_29,
        }
        global _jm_obj_9_may
        _jm_obj_9_may = {
            "callbacks": _jm_f_30,
            "examples": _jm_f_31,
            "headers": _jm_f_32,
            "links": _jm_f_33,
            "parameters": _jm_f_34,
            "pathItems": _jm_f_35,
            "requestBodies": _jm_f_36,
            "responses": _jm_f_37,
            "schemas": _jm_f_38,
            "securitySchemes": _jm_f_39,
        }
        global _jm_re_1_search, _jm_re_1
        _jm_re_1_search = re.compile("^/").search
        _jm_re_1 = lambda s: _jm_re_1_search(s) is not None
        global _jm_obj_21_may
        _jm_obj_21_may = {
            "$ref": _jm_f_40,
            "description": _jm_f_41,
            "get": _jm_f_42,
            "head": _jm_f_43,
            "options": _jm_f_44,
            "parameters": _jm_f_45,
            "patch": _jm_f_46,
            "post": _jm_f_47,
            "put": _jm_f_48,
            "servers": _jm_f_49,
            "summary": _jm_f_50,
            "trace": _jm_f_51,
        }
        global _jm_obj_23_may
        _jm_obj_23_may = {
            "callbacks": _jm_f_52,
            "deprecated": _jm_f_53,
            "description": _jm_f_54,
            "externalDocs": _jm_f_55,
            "operationId": _jm_f_56,
            "parameters": _jm_f_57,
            "requestBody": _jm_f_58,
            "responses": _jm_f_59,
            "security": _jm_f_60,
            "servers": _jm_f_61,
            "summary": _jm_f_62,
            "tags": _jm_f_63,
        }
        global _jm_obj_27_must
        _jm_obj_27_must = {
            "url": _jm_f_64,
        }
        global _jm_obj_27_may
        _jm_obj_27_may = {
            "description": _jm_f_65,
        }
        global _jm_cst_0
        _jm_cst_0 = {'deepObject', 'form', 'label', 'matrix', 'pipeDelimited', 'simple', 'spaceDelimited'}
        global _jm_obj_28_may
        _jm_obj_28_may = {
            "allowEmptyValue": _jm_f_66,
            "allowReserved": _jm_f_67,
            "content": _jm_f_68,
            "deprecated": _jm_f_69,
            "description": _jm_f_70,
            "example": _jm_f_71,
            "examples": _jm_f_72,
            "explode": _jm_f_73,
            "required": _jm_f_74,
            "schema": _jm_f_75,
            "style": _jm_f_76,
        }
        global _jm_cst_1
        _jm_cst_1 = {'cookie', 'header', 'path', 'query'}
        global _jm_obj_31_must
        _jm_obj_31_must = {
            "in": _jm_f_77,
            "name": _jm_f_78,
        }
        global _jm_cst_2
        _jm_cst_2 = {'deepObject', 'form', 'label', 'matrix', 'pipeDelimited', 'simple', 'spaceDelimited'}
        global _jm_obj_31_may
        _jm_obj_31_may = {
            "allowEmptyValue": _jm_f_79,
            "allowReserved": _jm_f_80,
            "content": _jm_f_81,
            "deprecated": _jm_f_82,
            "description": _jm_f_83,
            "example": _jm_f_84,
            "examples": _jm_f_85,
            "explode": _jm_f_86,
            "required": _jm_f_87,
            "schema": _jm_f_88,
            "style": _jm_f_89,
        }
        global _jm_obj_34_may
        _jm_obj_34_may = {
            "content": _jm_f_90,
            "description": _jm_f_91,
            "required": _jm_f_92,
        }
        global _jm_obj_36_may
        _jm_obj_36_may = {
            "encoding": _jm_f_93,
            "example": _jm_f_94,
            "examples": _jm_f_95,
            "schema": _jm_f_96,
        }
        global _jm_obj_39_may
        _jm_obj_39_may = {
            "allowReserved": _jm_f_97,
            "contentType": _jm_f_98,
            "explode": _jm_f_99,
            "headers": _jm_f_100,
            "style": _jm_f_101,
        }
        global _jm_obj_41_may
        _jm_obj_41_may = {
            "default": _jm_f_102,
        }
        global _jm_re_2_search, _jm_re_2
        _jm_re_2_search = re.compile("^[1-5](\\d\\d|XX)$").search
        _jm_re_2 = lambda s: _jm_re_2_search(s) is not None
        global _jm_obj_42_may
        _jm_obj_42_may = {
            "content": _jm_f_103,
            "description": _jm_f_104,
            "headers": _jm_f_105,
            "links": _jm_f_106,
        }
        global _jm_obj_47_may
        _jm_obj_47_may = {
            "description": _jm_f_107,
            "externalValue": _jm_f_108,
            "summary": _jm_f_109,
            "value": _jm_f_110,
        }
        global _jm_re_3_search, _jm_re_3
        _jm_re_3_search = re.compile("\\{.*\\}").search
        _jm_re_3 = lambda s: _jm_re_3_search(s) is not None
        global _jm_obj_48_must
        _jm_obj_48_must = {
            "operationId": _jm_f_111,
        }
        global _jm_obj_48_may
        _jm_obj_48_may = {
            "description": _jm_f_112,
            "parameters": _jm_f_113,
            "requestBody": _jm_f_114,
            "server": _jm_f_115,
        }
        global _jm_obj_50_must
        _jm_obj_50_must = {
            "operationRef": _jm_f_116,
        }
        global _jm_obj_50_may
        _jm_obj_50_may = {
            "description": _jm_f_117,
            "parameters": _jm_f_118,
            "requestBody": _jm_f_119,
            "server": _jm_f_120,
        }
        global _jm_obj_52_must
        _jm_obj_52_must = {
            "name": _jm_f_121,
        }
        global _jm_obj_52_may
        _jm_obj_52_may = {
            "description": _jm_f_122,
            "externalDocs": _jm_f_123,
        }
        global _jm_obj_53_must
        _jm_obj_53_must = {
            "$ref": _jm_f_124,
        }
        global _jm_obj_53_may
        _jm_obj_53_may = {
            "description": _jm_f_125,
            "summary": _jm_f_126,
        }
        global _jm_obj_54_may
        _jm_obj_54_may = {
            "$anchor": _jm_f_127,
            "$comment": _jm_f_128,
            "$defs": _jm_f_129,
            "$dynamicAnchor": _jm_f_130,
            "$dynamicRef": _jm_f_131,
            "$id": _jm_f_132,
            "$ref": _jm_f_133,
            "$schema": _jm_f_134,
            "$vocabulary": _jm_f_135,
            "additionalProperties": _jm_f_136,
            "allOf": _jm_f_137,
            "anyOf": _jm_f_138,
            "const": _jm_f_139,
            "contains": _jm_f_140,
            "contentEncoding": _jm_f_141,
            "contentMediaType": _jm_f_142,
            "contentSchema": _jm_f_143,
            "default": _jm_f_144,
            "dependentRequired": _jm_f_145,
            "dependentSchemas": _jm_f_146,
            "deprecated": _jm_f_147,
            "description": _jm_f_148,
            "discriminator": _jm_f_149,
            "else": _jm_f_150,
            "enum": _jm_f_151,
            "examples": _jm_f_152,
            "exclusiveMaximum": _jm_f_153,
            "exclusiveMinimum": _jm_f_154,
            "externalDocs": _jm_f_155,
            "format": _jm_f_156,
            "if": _jm_f_157,
            "items": _jm_f_158,
            "maxContains": _jm_f_159,
            "maxItems": _jm_f_160,
            "maxLength": _jm_f_161,
            "maxProperties": _jm_f_162,
            "maximum": _jm_f_163,
            "minContains": _jm_f_164,
            "minItems": _jm_f_165,
            "minLength": _jm_f_166,
            "minProperties": _jm_f_167,
            "minimum": _jm_f_168,
            "multipleOf": _jm_f_169,
            "not": _jm_f_170,
            "oneOf": _jm_f_171,
            "pattern": _jm_f_172,
            "patternProperties": _jm_f_173,
            "prefixItems": _jm_f_174,
            "properties": _jm_f_175,
            "propertyNames": _jm_f_176,
            "readOnly": _jm_f_177,
            "required": _jm_f_178,
            "then": _jm_f_179,
            "title": _jm_f_180,
            "type": _jm_f_181,
            "unevaluatedItems": _jm_f_182,
            "unevaluatedProperties": _jm_f_183,
            "uniqueItems": _jm_f_184,
            "writeOnly": _jm_f_185,
            "xml": _jm_f_186,
        }
        global _jm_obj_61_must
        _jm_obj_61_must = {
            "propertyName": _jm_f_187,
        }
        global _jm_obj_61_may
        _jm_obj_61_may = {
            "mapping": _jm_f_188,
        }
        global _jm_obj_63_may
        _jm_obj_63_may = {
            "attribute": _jm_f_189,
            "name": _jm_f_190,
            "namespace": _jm_f_191,
            "prefix": _jm_f_192,
            "wrapped": _jm_f_193,
        }
        global _jm_obj_64_must
        _jm_obj_64_must = {
            "flows": _jm_f_194,
            "in": _jm_f_195,
            "name": _jm_f_196,
            "openIdConnectUrl": _jm_f_197,
            "scheme": _jm_f_198,
            "type": _jm_f_199,
        }
        global _jm_obj_64_may
        _jm_obj_64_may = {
            "bearerFormat": _jm_f_200,
            "description": _jm_f_201,
        }
        global _jm_obj_65_may
        _jm_obj_65_may = {
            "authorizationCode": _jm_f_202,
            "clientCredentials": _jm_f_203,
            "implicit": _jm_f_204,
            "password": _jm_f_205,
        }
        global _jm_obj_66_must
        _jm_obj_66_must = {
            "authorizationUrl": _jm_f_206,
            "scopes": _jm_f_207,
            "tokenUrl": _jm_f_208,
        }
        global _jm_obj_66_may
        _jm_obj_66_may = {
            "refreshUrl": _jm_f_209,
        }
        global _jm_cst_3
        _jm_cst_3 = {'array', 'boolean', 'integer', 'null', 'number', 'object', 'string'}
        global _jm_obj_70_may
        _jm_obj_70_may = {
            "$anchor": _jm_f_210,
            "$comment": _jm_f_211,
            "$defs": _jm_f_212,
            "$dynamicAnchor": _jm_f_213,
            "$dynamicRef": _jm_f_214,
            "$id": _jm_f_215,
            "$ref": _jm_f_216,
            "$schema": _jm_f_217,
            "$vocabulary": _jm_f_218,
            "additionalProperties": _jm_f_219,
            "allOf": _jm_f_220,
            "anyOf": _jm_f_221,
            "const": _jm_f_222,
            "contains": _jm_f_223,
            "contentEncoding": _jm_f_224,
            "contentMediaType": _jm_f_225,
            "contentSchema": _jm_f_226,
            "default": _jm_f_227,
            "dependentRequired": _jm_f_228,
            "dependentSchemas": _jm_f_229,
            "deprecated": _jm_f_230,
            "description": _jm_f_231,
            "else": _jm_f_232,
            "enum": _jm_f_233,
            "examples": _jm_f_234,
            "exclusiveMaximum": _jm_f_235,
            "exclusiveMinimum": _jm_f_236,
            "format": _jm_f_237,
            "if": _jm_f_238,
            "items": _jm_f_239,
            "maxContains": _jm_f_240,
            "maxItems": _jm_f_241,
            "maxLength": _jm_f_242,
            "maxProperties": _jm_f_243,
            "maximum": _jm_f_244,
            "minContains": _jm_f_245,
            "minItems": _jm_f_246,
            "minLength": _jm_f_247,
            "minProperties": _jm_f_248,
            "minimum": _jm_f_249,
            "multipleOf": _jm_f_250,
            "not": _jm_f_251,
            "oneOf": _jm_f_252,
            "pattern": _jm_f_253,
            "patternProperties": _jm_f_254,
            "prefixItems": _jm_f_255,
            "properties": _jm_f_256,
            "propertyNames": _jm_f_257,
            "readOnly": _jm_f_258,
            "required": _jm_f_259,
            "then": _jm_f_260,
            "title": _jm_f_261,
            "type": _jm_f_262,
            "unevaluatedItems": _jm_f_263,
            "unevaluatedProperties": _jm_f_264,
            "uniqueItems": _jm_f_265,
            "writeOnly": _jm_f_266,
        }
        global check_model_map
        check_model_map = {
            "": json_model_1,
            "OpenAPI": json_model_2,
            "Info": json_model_3,
            "Contact": json_model_4,
            "License": json_model_5,
            "Server": json_model_6,
            "ServerVariable": json_model_7,
            "Components": json_model_8,
            "Paths": json_model_9,
            "PathItem": json_model_10,
            "Operation": json_model_11,
            "ExternalDocumentation": json_model_12,
            "parameterShare": json_model_13,
            "Parameter": json_model_14,
            "RequestBody": json_model_15,
            "MediaType": json_model_16,
            "Encoding": json_model_17,
            "Responses": json_model_18,
            "Response": json_model_19,
            "Callback": json_model_20,
            "Example": json_model_21,
            "Expression": json_model_22,
            "Link": json_model_23,
            "Header": json_model_24,
            "Tag": json_model_25,
            "Reference": json_model_26,
            "schema": json_model_36,
            "Schema": json_model_28,
            "Discriminator": json_model_29,
            "XML": json_model_30,
            "SecurityScheme": json_model_31,
            "OAuthFlows": json_model_32,
            "OAuthFlow": json_model_33,
            "SecurityRequirement": json_model_34,
            "Extension": json_model_35,
        }

# differed module cleanup
def check_model_free():
    global initialized
    if initialized:
        initialized = False
        global _jm_re_0_search, _jm_re_0
        _jm_re_0_search = None
        _jm_re_0 = None
        global _jm_re_1_search, _jm_re_1
        _jm_re_1_search = None
        _jm_re_1 = None
        global _jm_re_2_search, _jm_re_2
        _jm_re_2_search = None
        _jm_re_2 = None
        global _jm_re_3_search, _jm_re_3
        _jm_re_3_search = None
        _jm_re_3 = None


if __name__ == "__main__":
    check_model_init()
    main(check_model_fun, check_model_map, __version__)
    check_model_free()
