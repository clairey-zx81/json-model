...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    if (unlikely(! res))
    {
        if (rep) jm_report_add_entry(rep, "not a 1 strict int [.b]", (path ? &lpath : NULL));
    ...
    if (unlikely(! res))
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.c]", (path ? &lpath : NULL));
    ...
    if (unlikely(! res))
    {
        if (rep) jm_report_add_entry(rep, "not a -1 strict int [.d]", (path ? &lpath : NULL));
