...
def _jm_obj_0(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    for prop, pval in val.items():
        ...
        if prop == "a":
            ...
            if not res:
                rep is None or rep.append(("not a bool [.c]", lpath_0 if path is not None else None))
            if not res:
            ...
            if not res:
                rep is None or rep.append(("not a -1 strict int [.d]", lpath_0 if path is not None else None))
            if not res:
