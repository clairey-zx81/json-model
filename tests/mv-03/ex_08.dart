//
// Generated by JSON Model Compiler version 2
// see https://github.com/clairey-zx81/json-model
//
import 'dart:convert';
import 'dart:io';
import 'dart:core';
// Import the helper library generated nearby
import 'package:json_model_runtime/json_model.dart' as runtime;
import 'package:json_model_runtime/main.dart';

//
// Generated JSON Model Validator
//
const String JSON_MODEL_VERSION = "2";

var check_model_map = <String, Function>{};

// check $Val (.'$Val')
bool json_model_2(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    // .'$Val'
    bool res = (val is bool);
    if (! res)
    {
        rep?.add(["not a bool [.'\$Val']", path]);
    }
    return res;
}

// check $Key (.'$Key')
bool json_model_3(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    // .'$Key'
    bool res = runtime.jm_is_valid_url(val);
    if (! res)
    {
        rep?.add(["unexpected \$URL [.'\$Key']", path]);
    }
    return res;
}

// check $map (.'$map')
bool json_model_4(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    // .'$map'
    if (! (val is Map))
    {
        rep?.add(["not an object [.'\$map']", path]);
        return false;
    }
    bool res;
    for (var entry in val.entries)
    {
        var prop = entry.key;
        var pval = entry.value;
        List<dynamic>? lpath_0 = (path != null ? [...?path, prop] : null);
        if (runtime.jm_is_valid_url(prop))
        {
            // handle 1 key props
            // .'$map'.'$URL'
            res = json_model_2(pval, (path != null ? lpath_0 : null), rep);
            if (! res)
            {
                rep?.add(["unexpected \$Val [.'\$map'.'\$URL']", (path != null ? lpath_0 : null)]);
                return false;
            }
        }
        else
        {
            rep?.add(["unexpected prop [.'\$map']", (path != null ? lpath_0 : null)]);
            return false;
        }
    }
    return true;
}

// check $Ex08 (.'$Ex08')
bool json_model_5(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    // .'$Ex08'
    // .'$Ex08'.'|'.0
    bool res = json_model_4(val, path, rep);
    if (! res)
    {
        rep?.add(["unexpected \$map [.'\$Ex08'.'|'.0]", path]);
        // .'$Ex08'.'|'.1
        res = runtime.jm_is_valid_url(val);
        if (! res)
        {
            rep?.add(["unexpected \$URL [.'\$Ex08'.'|'.1]", path]);
            // .'$Ex08'.'|'.2
            res = json_model_2(val, path, rep);
            if (! res)
            {
                rep?.add(["unexpected \$Val [.'\$Ex08'.'|'.2]", path]);
            }
        }
    }
    if (res)
    {
        rep?.clear();
    }
    else
    {
        rep?.add(["no model matched [.'\$Ex08'.'|']", path]);
    }
    return res;
}

// check $ (.)
bool json_model_1(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    // .
    bool res = json_model_5(val, path, rep);
    if (! res)
    {
        rep?.add(["unexpected \$Ex08 [.]", path]);
    }
    return res;
}


// dart_init.dart
class GlobalState {
  static bool initializer = false;
}

Function? check_model_fun(String name) {
  return check_model_map[name];
}


void check_model_init() {
  if (!GlobalState.initializer){
    GlobalState.initializer = true;
  }
        check_model_map[""] = json_model_5;
        check_model_map["Val"] = json_model_2;
        check_model_map["Key"] = json_model_3;
        check_model_map["map"] = json_model_4;
        check_model_map["Ex08"] = json_model_5;
}

// dart_free.dart
void check_model_free() {
  GlobalState.initializer = true;
}


void main(List<String> args) {
  runValidation(
    args,
    check_model_init,
    check_model_fun,
    check_model_free,
  );
}
