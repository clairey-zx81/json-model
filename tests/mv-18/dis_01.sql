...
BEGIN
  ...
  FOR prop, pval IN SELECT * FROM JSONB_EACH(val) LOOP
    IF prop = 'tag' THEN
      ...
      -- .'|'.0.tag
      res := JSONB_TYPEOF(pval) = 'number' AND (pval)::INT8 = (pval)::FLOAT8 AND (pval)::INT8 = 18;
      IF NOT res THEN
        RETURN FALSE;
      END IF;
    ELSEIF prop = 'a' THEN
...
BEGIN
  ...
  FOR prop, pval IN SELECT * FROM JSONB_EACH(val) LOOP
    IF prop = 'tag' THEN
      ...
      must_count := must_count + 1;
      -- .'|'.1.tag
      res := JSONB_TYPEOF(pval) = 'number' AND (pval)::INT8 = (pval)::FLOAT8 AND (pval)::INT8 = 42;
      IF NOT res THEN
        RETURN FALSE;
      END IF;
    ELSEIF prop = 'b' THEN
...
BEGIN
  ...
  FOR prop, pval IN SELECT * FROM JSONB_EACH(val) LOOP
    IF prop = 'tag' THEN
      ...
      must_count := must_count + 1;
      -- .'|'.2.tag
      res := JSONB_TYPEOF(pval) = 'number' AND (pval)::INT8 = (pval)::FLOAT8 AND (pval)::INT8 = 1024;
      IF NOT res THEN
        RETURN FALSE;
      END IF;
    ELSEIF prop = 'c' THEN
...
INSERT INTO jm_constant_maps(mapname, tagval, value) VALUES
  ('_jm_map_0', JSONB '18', '_jm_obj_0'),
  ('_jm_map_0', JSONB '42', '_jm_obj_1'),
  ('_jm_map_0', JSONB '1024', '_jm_obj_2')
;
