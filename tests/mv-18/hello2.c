...
static bool _jm_obj_0(const json_t* val, Path* path, Report* rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (strcmp(prop, "hello") == 0)
        {
            ...
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "not a 0 strict int [.hello]", (path ? &lpath_0 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid must property value [.hello]", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else if (strcmp(prop, "world") == 0)
        {
            ...
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "not a bool [.world]", (path ? &lpath_0 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid must property value [.world]", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else
    ...
    if (must_count != 2)
    {
        if (rep != NULL)
        {
            ...
            {
                if (rep) jm_report_add_entry(rep, "missing must prop <hello> []", path);
            }
            if (! (json_object_get(val, "world") != NULL))
            {
                if (rep) jm_report_add_entry(rep, "missing must prop <world> []", path);
            }
        }
        return false;
    }
...
char *CHECK_init(void)
{
    if (!initialized)
    {
        ...
        check_model_map_tab[0] = (propmap_t) { "", json_model_2 };
