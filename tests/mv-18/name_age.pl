...
sub json_model_1($$$)
{
    ...
    unless (exists $$val{"age"})
    {
        push @$rep, ["missing mandatory prop <age> [.]", $path] if defined $rep;
        return 0;
    }
    $lpath = defined $path ? [@{$path}, "age"] : undef;
    $pval = $$val{"age"};
    # .age
    $res = jm_is_integer($pval) && $pval >= 0;
    unless ($res)
    {
        push @$rep, ["not a 0 strict int [.age]", defined $path ? $lpath : undef] if defined $rep;
        push @$rep, ["unexpected value for mandatory prop <age> [.]", defined $path ? $lpath : undef] if defined $rep;
        return 0;
    }
    return 1;
}
