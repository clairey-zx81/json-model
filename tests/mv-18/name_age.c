...
// check _jm_obj_0_must_name ($.name)
static bool _jm_f_1(const json_t* val, Path* path, Report* rep)
{
    bool res;
    // $.name
    ...
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [$.name]", path);
    }
    return res;
}
...
char *CHECK_init(void)
{
    if (!initialized)
    {
        ...
        _jm_obj_0_must_tab[0] = (propmap_t) { "age", _jm_f_0 };
        _jm_obj_0_must_tab[1] = (propmap_t) { "name", _jm_f_1 };
