...
static INLINE bool _jm_obj_2(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (jm_str_eq_2(prop, 0x00000074))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"ru\" [.movie.'|'.2.t]", (path ? &lpath_3 : NULL));
        ...
        else if (likely(jm_str_eq_5(prop, 0x0000000069747573LL)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.movie.'|'.2.suti]", (path ? &lpath_3 : NULL));
...
const char *check_model_init(void)
{
    if (!initialized)
    {
        ...
        _jm_map_0_tab[2] = (jm_constmap_t) { (jm_constant_t) { 3, { .s = "ru" } }, _jm_obj_2 };
