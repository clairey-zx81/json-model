...
def _jm_obj_2(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if not res:
        rep is None or rep.append(("unexpected value for model \"ru\" [.movie.'|'.2.t]", lpath if path is not None else None))
    ...
    res = isinstance(pval, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.movie.'|'.2.suti]", lpath if path is not None else None))
