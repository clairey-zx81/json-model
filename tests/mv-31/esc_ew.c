...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    // .
    // "/hello\\.world\\?$/"
    bool res = json_is_string(val) && strlen(json_string_value(val)) >= strlen("hello.world?") && strcmp(json_string_value(val) + strlen(json_string_value(val)) - strlen("hello.world?"), "hello.world?") == 0;
    if (unlikely(! res))
    {
