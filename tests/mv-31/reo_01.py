...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        ...
        if not res:
            rep is None or rep.append(("unexpected value for model \"/^abcdefghij/\" [.'|'.0]", path))
            # .'|'.1
            # "/^klmnopqrst$/"
            res = val == "klmnopqrst"
            if not res:
                rep is None or rep.append(("unexpected value for model \"/^klmnopqrst$/\" [.'|'.1]", path))
                # .'|'.2
                # "/qrstuvwxyz$/"
                res = val.endswith("qrstuvwxyz")
                if not res:
                    rep is None or rep.append(("unexpected value for model \"/qrstuvwxyz$/\" [.'|'.2]", path))
        if res:
