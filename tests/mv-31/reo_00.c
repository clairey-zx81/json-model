...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    if (likely(res))
    {
        // .'|'.0
        // "/^hello/"
        res = jm_str_eq_5(json_string_value(val), 0x0000006f6c6c6568LL);
        if (unlikely(! res))
        {
            if (rep) jm_report_add_entry(rep, "unexpected value for model \"/^hello/\" [.'|'.0]", path);
            // .'|'.1
            // "/^world$/"
            res = jm_str_eq_6(json_string_value(val), 0x000000646c726f77LL);
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"/^world$/\" [.'|'.1]", path);
                // .'|'.2
                // "/!$/"
                res = strlen(json_string_value(val)) >= strlen("!") && strcmp(json_string_value(val) + strlen(json_string_value(val)) - strlen("!"), "!") == 0;
                if (unlikely(! res))
                {
                    if (rep) jm_report_add_entry(rep, "unexpected value for model \"/!$/\" [.'|'.2]", path);
                }
            }
        }
        if (likely(res))
