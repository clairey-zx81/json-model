...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        ...
        if not res:
            rep is None or rep.append(("unexpected value for model \"/^hello/\" [.'|'.0]", path))
            # .'|'.1
            # "/^world$/"
            res = val == "world"
            if not res:
                rep is None or rep.append(("unexpected value for model \"/^world$/\" [.'|'.1]", path))
                # .'|'.2
                # "/!$/"
                res = val.endswith("!")
                if not res:
                    rep is None or rep.append(("unexpected value for model \"/!$/\" [.'|'.2]", path))
        if res:
