    ...
    IF prop = '|' THEN
      ...
      IF res THEN
        FOR arr_9_idx IN 0 .. JSONB_ARRAY_LENGTH(pval) - 1 LOOP
          ...
          -- .'$Model#Root'.'|'.1.'|'.0
          res := json_model_20(arr_9_item, NULL, rep);
          IF NOT res THEN
            EXIT;
          END IF;
        END LOOP;
...
BEGIN
  ...
  -- .'$Model#Root'.'|'.3
  -- .'$Model#Root'.'|'.4
  RETURN JSONB_TYPEOF(val) = 'object' AND (_jm_obj_17(val, path, rep) OR _jm_obj_14(val, path, rep) OR _jm_obj_11(val, path, rep) OR _jm_obj_8(val, path, rep) OR _jm_obj_5(val, path, rep));
END;
$$ LANGUAGE PLpgSQL;
...
BEGIN
  ...
  -- .'$Model#Identifier'.'&'.1
  -- "/[^A-Z0-9]/"
  RETURN JSONB_TYPEOF(val) = 'string' AND _jm_re_10(JSON_VALUE(val, '$' RETURNING TEXT), path, rep) AND _jm_re_9(JSON_VALUE(val, '$' RETURNING TEXT), path, rep);
END;
$$ LANGUAGE PLpgSQL;
