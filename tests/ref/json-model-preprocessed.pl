#! /usr/bin/perl
#
# Generated by JSON Model Compiler version 2
# see https://github.com/clairey-zx81/json-model
#
use utf8;
use strict;
use warnings;
no warnings "uninitialized";
use re::engine::RE2;
use JSON::JsonModel;
use constant JMC_VERSION => "2";


sub json_model_3($$$);
sub json_model_1($$$);
sub json_model_42($$$);
sub json_model_21($$$);
sub json_model_20($$$);
sub json_model_7($$$);
sub json_model_34($$$);
sub json_model_16($$$);
sub json_model_17($$$);
my %_jm_cst_0;
sub json_model_5($$$);
sub json_model_9($$$);
sub json_model_10($$$);
sub json_model_11($$$);
sub _jm_xre_0($$$);
sub _jm_xre_1($$$);
sub json_model_12($$$);
sub json_model_35($$$);
sub _jm_obj_0($$$);
sub _jm_obj_1($$$);
sub _jm_obj_2($$$);
sub _jm_obj_3($$$);
sub _jm_obj_4($$$);
sub json_model_31($$$);
sub json_model_14($$$);
sub json_model_15($$$);
sub json_model_28($$$);
sub json_model_13($$$);
sub _jm_obj_6($$$);
sub _jm_obj_7($$$);
sub _jm_obj_5($$$);
sub _jm_obj_9($$$);
sub _jm_obj_10($$$);
sub _jm_obj_8($$$);
sub _jm_obj_12($$$);
sub _jm_obj_13($$$);
sub _jm_obj_11($$$);
sub _jm_obj_15($$$);
sub _jm_obj_16($$$);
sub _jm_obj_14($$$);
sub _jm_obj_18($$$);
sub _jm_obj_19($$$);
sub _jm_obj_17($$$);
sub json_model_41($$$);
sub json_model_4($$$);
sub json_model_6($$$);
my %check_model_map;

# check $Model (.'$Model')
sub json_model_3($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model'
    return json_model_42($val, undef, undef);
}

# check $ (.)
sub json_model_1($$$)
{
    my ($val, $path, $rep) = @_;
    # .
    return json_model_42($val, undef, undef);
}

# check $Model#RootModel (.'$Model#RootModel')
sub json_model_42($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#RootModel'
    # .'$Model#RootModel'.'|'.0
    # .'$Model#RootModel'.'|'.1
    # .'$Model#RootModel'.'|'.2
    return json_model_34($val, undef, undef) || json_model_21($val, undef, undef) || json_model_41($val, undef, undef);
}

# check $Model#Array (.'$Model#Array')
sub json_model_21($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Array'
    my $res = jm_is_array($val);
    if ($res)
    {
        for my $arr_0_idx (0 .. $#$val)
        {
            my $arr_0_item = $$val[$arr_0_idx];
            # .'$Model#Array'.0
            $res = json_model_20($arr_0_item, undef, undef);
            last unless $res;
        }
    }
    return $res;
}

# check $Model#CModel (.'$Model#CModel')
sub json_model_20($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#CModel'
    # .'$Model#CModel'.'|'.0
    # .'$Model#CModel'.'|'.1
    return json_model_35($val, undef, undef) || json_model_7($val, undef, undef);
}

# check $Model#ArrayComment (.'$Model#ArrayComment')
sub json_model_7($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#ArrayComment'
    # "/^#/"
    return jm_is_string($val) && jm_starts_with($val, "#");
}

# check $Model#ScalarModel (.'$Model#ScalarModel')
sub json_model_34($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#ScalarModel'
    # .'$Model#ScalarModel'.'|'.0
    # .'$Model#ScalarModel'.'|'.1
    return json_model_16($val, undef, undef) || json_model_17($val, undef, undef);
}

# check $Model#ValModel (.'$Model#ValModel')
sub json_model_16($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#ValModel'
    # .'$Model#ValModel'.'|'.0
    # .'$Model#ValModel'.'|'.1
    # .'$Model#ValModel'.'|'.2
    # .'$Model#ValModel'.'|'.3
    # .'$Model#ValModel'.'|'.4
    # .'$Model#ValModel'.'|'.5
    # .'$Model#ValModel'.'|'.6
    # .'$Model#ValModel'.'|'.7
    return !defined($val) || jm_is_boolean($val) && $val == 1 || jm_is_integer($val) && $val == 0 || jm_is_integer($val) && $val == 1 || jm_is_integer($val) && $val == -1 || jm_is_numeric($val) && $val == 0.0 || jm_is_numeric($val) && $val == 1.0 || jm_is_numeric($val) && $val == -1.0;
}

# check $Model#StrModel (.'$Model#StrModel')
sub json_model_17($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#StrModel'
    # .'$Model#StrModel'.'|'.0
    # .'$Model#StrModel'.'|'.1
    # .'$Model#StrModel'.'|'.2
    # .'$Model#StrModel'.'|'.3
    # .'$Model#StrModel'.'|'.4
    # .'$Model#StrModel'.'|'.5
    return json_model_11($val, undef, undef) || json_model_10($val, undef, undef) || json_model_12($val, undef, undef) || json_model_5($val, undef, undef) || json_model_9($val, undef, undef) || jm_is_string($val) && $val eq "";
}


# check $Model#PreDef (.'$Model#PreDef')
sub json_model_5($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#PreDef'
    return jm_is_string($val) && exists $_jm_cst_0{$val};
}

sub _jm_re_0($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\$./;
    return $res;
}

# check $Model#Ref (.'$Model#Ref')
sub json_model_9($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Ref'
    # "/^\\$./"
    return jm_is_string($val) && _jm_re_0($val, undef, undef);
}

sub _jm_re_1($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^=(null|true|false|[-+]?\d+(\.\d+)?([Ee][-+]?\d+)?)$/;
    return $res;
}

# check $Model#ValConst (.'$Model#ValConst')
sub json_model_10($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#ValConst'
    # "/^=(null|true|false|[-+]?\\d+(\\.\\d+)?([Ee][-+]?\\d+)?)$/"
    return jm_is_string($val) && _jm_re_1($val, undef, undef);
}

sub _jm_re_2($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^[A-Za-z0-9_]/;
    return $res;
}

# check $Model#StrConst (.'$Model#StrConst')
sub json_model_11($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#StrConst'
    # "/^[A-Za-z0-9_]/"
    return jm_is_string($val) && _jm_re_2($val, undef, undef);
}

sub _jm_xre_0_re($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\/(?<s1>.*)\/[a-z]*$/;
    return $res;
}

sub _jm_xre_0($$$)
{
    my ($val, $path, $rep) = @_;
    my $extract;
    my $match = $val =~ /^\/(?<s1>.*)\/[a-z]*$/;
    return 0 unless $match;
    $extract = $+{"s1"};
    return 0 unless jm_is_valid_regex($extract, undef, undef);
    return 1;
}

sub _jm_xre_1_re($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\/(?<s1>.*)\/[a-z]*X[a-z]*$/;
    return $res;
}

sub _jm_xre_1($$$)
{
    my ($val, $path, $rep) = @_;
    my $extract;
    my $match = $val =~ /^\/(?<s1>.*)\/[a-z]*X[a-z]*$/;
    return 0 unless $match;
    $extract = $+{"s1"};
    return 0 unless jm_is_valid_exreg($extract, undef, undef);
    return 1;
}

# check $Model#Regex (.'$Model#Regex')
sub json_model_12($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Regex'
    # .'$Model#Regex'.'|'.0
    # "/^/($EXREG)/[a-z]*X[a-z]*$/X"
    # .'$Model#Regex'.'|'.1
    # "/^/($REGEX)/[a-z]*$/X"
    return jm_is_string($val) && (_jm_xre_1($val, undef, undef) || _jm_xre_0($val, undef, undef));
}

# check $Model#Model (.'$Model#Model')
sub json_model_35($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Model'
    # .'$Model#Model'.'|'.0
    # .'$Model#Model'.'|'.1
    # .'$Model#Model'.'|'.2
    return json_model_34($val, undef, undef) || json_model_21($val, undef, undef) || json_model_31($val, undef, undef);
}

# object .'$Model#Element'.'|'.4
sub _jm_obj_0($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Element'.'|'.4.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (json_model_28($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Element'.'|'.4.'$Prop'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Element'.'|'.4.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Element'.'|'.3
sub _jm_obj_1($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "^")
        {
            # handle must ^ property
            $must_count++;
            # .'$Model#Element'.'|'.3.'^'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_1_idx (0 .. $#$pval)
                {
                    my $arr_1_item = $$pval[$arr_1_idx];
                    # .'$Model#Element'.'|'.3.'^'.0
                    $res = json_model_20($arr_1_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Element'.'|'.3.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Element'.'|'.3.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Model#Element'.'|'.2
sub _jm_obj_2($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "&")
        {
            # handle must & property
            $must_count++;
            # .'$Model#Element'.'|'.2.'&'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_2_idx (0 .. $#$pval)
                {
                    my $arr_2_item = $$pval[$arr_2_idx];
                    # .'$Model#Element'.'|'.2.'&'.0
                    $res = json_model_20($arr_2_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Element'.'|'.2.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Element'.'|'.2.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Model#Element'.'|'.1
sub _jm_obj_3($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "|")
        {
            # handle must | property
            $must_count++;
            # .'$Model#Element'.'|'.1.'|'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_3_idx (0 .. $#$pval)
                {
                    my $arr_3_item = $$pval[$arr_3_idx];
                    # .'$Model#Element'.'|'.1.'|'.0
                    $res = json_model_20($arr_3_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Element'.'|'.1.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Element'.'|'.1.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

sub _jm_re_3($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^(<=|>=|<|>)$/;
    return $res;
}

sub _jm_re_4($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^(=|!=)$/;
    return $res;
}

# object .'$Model#Element'.'|'.0
sub _jm_obj_4($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "\@")
        {
            # handle must @ property
            $must_count++;
            # .'$Model#Element'.'|'.0.'@'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Element'.'|'.0.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "!")
        {
            # handle may ! property
            # .'$Model#Element'.'|'.0.'!'
            $res = jm_is_boolean($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 3 re props
            # .'$Model#Element'.'|'.0.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_3($prop, undef, undef))
        {
            # handle 3 re props
            # .'$Model#Element'.'|'.0.'/^(<=|>=|<|>)$/'
            $res = json_model_14($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (_jm_re_4($prop, undef, undef))
        {
            # handle 3 re props
            # .'$Model#Element'.'|'.0.'/^(=|!=)$/'
            $res = json_model_15($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# check $Model#Element (.'$Model#Element')
sub json_model_31($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Element'
    # .'$Model#Element'.'|'.0
    # .'$Model#Element'.'|'.1
    # .'$Model#Element'.'|'.2
    # .'$Model#Element'.'|'.3
    # .'$Model#Element'.'|'.4
    return jm_is_object($val) && (_jm_obj_4($val, undef, undef) || _jm_obj_3($val, undef, undef) || _jm_obj_2($val, undef, undef) || _jm_obj_1($val, undef, undef) || _jm_obj_0($val, undef, undef));
}

# check $Model#CmpValue (.'$Model#CmpValue')
sub json_model_14($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#CmpValue'
    # .'$Model#CmpValue'.'|'.0
    # .'$Model#CmpValue'.'|'.1
    # .'$Model#CmpValue'.'|'.2
    return jm_is_integer($val) || jm_is_numeric($val) || jm_is_string($val);
}

# check $Model#EqValue (.'$Model#EqValue')
sub json_model_15($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#EqValue'
    # .'$Model#EqValue'.'|'.0
    # .'$Model#EqValue'.'|'.1
    # .'$Model#EqValue'.'|'.2
    return !defined($val) || jm_is_boolean($val) || json_model_14($val, undef, undef);
}

sub _jm_re_5($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^[?!]/;
    return $res;
}

# check $Model#Prop (.'$Model#Prop')
sub json_model_28($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Prop'
    # .'$Model#Prop'.'|'.0
    # "/^[?!]/"
    # .'$Model#Prop'.'|'.1
    # .'$Model#Prop'.'|'.2
    # .'$Model#Prop'.'|'.3
    # .'$Model#Prop'.'|'.4
    # .'$Model#Prop'.'|'.5
    return jm_is_string($val) && _jm_re_5($val, undef, undef) || json_model_11($val, undef, undef) || json_model_12($val, undef, undef) || json_model_9($val, undef, undef) || json_model_13($val, undef, undef) || jm_is_string($val) && $val eq "";
}

sub _jm_re_6($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\..+$/;
    return $res;
}

# check $Model#Name (.'$Model#Name')
sub json_model_13($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Name'
    # "/^\\..+$/"
    return jm_is_string($val) && _jm_re_6($val, undef, undef);
}

# object .'$Model#Root'.'|'.4.'$'
sub _jm_obj_6($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.4.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Model#Root'.'|'.4.'$'.''
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_6($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.4.'$'.'$Identifier'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.4.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

sub _jm_re_7($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^([#|&^+\/*@~=\$%]|[<>!]=?)$/;
    return $res;
}

# object .'$Model#Root'.'|'.4.'%'
sub _jm_obj_7($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.4.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Model#Root'.'|'.4.'%'.'<'
            # .'$Model#Root'.'|'.4.'%'.'<'.'|'.0
            $res = json_model_9($pval, undef, undef);
            unless ($res)
            {
                # .'$Model#Root'.'|'.4.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_4_idx (0 .. $#$pval)
                    {
                        my $arr_4_item = $$pval[$arr_4_idx];
                        # .'$Model#Root'.'|'.4.'%'.'<'.'|'.1.0
                        $res = json_model_9($arr_4_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_13($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.4.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_7($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.4.'%'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.4
sub _jm_obj_5($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Model#Root'.'|'.4.'~'
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Model#Root'.'|'.4.'$'
            $res = _jm_obj_6($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Model#Root'.'|'.4.'%'
            $res = _jm_obj_7($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.4.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (json_model_28($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.4.'$Prop'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.4.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.3.'$'
sub _jm_obj_9($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.3.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Model#Root'.'|'.3.'$'.''
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_6($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.3.'$'.'$Identifier'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.3.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.3.'%'
sub _jm_obj_10($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.3.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Model#Root'.'|'.3.'%'.'<'
            # .'$Model#Root'.'|'.3.'%'.'<'.'|'.0
            $res = json_model_9($pval, undef, undef);
            unless ($res)
            {
                # .'$Model#Root'.'|'.3.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_6_idx (0 .. $#$pval)
                    {
                        my $arr_6_item = $$pval[$arr_6_idx];
                        # .'$Model#Root'.'|'.3.'%'.'<'.'|'.1.0
                        $res = json_model_9($arr_6_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_13($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.3.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_7($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.3.'%'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.3
sub _jm_obj_8($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "^")
        {
            # handle must ^ property
            $must_count++;
            # .'$Model#Root'.'|'.3.'^'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_5_idx (0 .. $#$pval)
                {
                    my $arr_5_item = $$pval[$arr_5_idx];
                    # .'$Model#Root'.'|'.3.'^'.0
                    $res = json_model_20($arr_5_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Model#Root'.'|'.3.'~'
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Model#Root'.'|'.3.'$'
            $res = _jm_obj_9($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Model#Root'.'|'.3.'%'
            $res = _jm_obj_10($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.3.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.3.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Model#Root'.'|'.2.'$'
sub _jm_obj_12($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.2.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Model#Root'.'|'.2.'$'.''
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_6($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.2.'$'.'$Identifier'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.2.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.2.'%'
sub _jm_obj_13($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.2.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Model#Root'.'|'.2.'%'.'<'
            # .'$Model#Root'.'|'.2.'%'.'<'.'|'.0
            $res = json_model_9($pval, undef, undef);
            unless ($res)
            {
                # .'$Model#Root'.'|'.2.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_8_idx (0 .. $#$pval)
                    {
                        my $arr_8_item = $$pval[$arr_8_idx];
                        # .'$Model#Root'.'|'.2.'%'.'<'.'|'.1.0
                        $res = json_model_9($arr_8_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_13($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.2.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_7($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.2.'%'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.2
sub _jm_obj_11($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "&")
        {
            # handle must & property
            $must_count++;
            # .'$Model#Root'.'|'.2.'&'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_7_idx (0 .. $#$pval)
                {
                    my $arr_7_item = $$pval[$arr_7_idx];
                    # .'$Model#Root'.'|'.2.'&'.0
                    $res = json_model_20($arr_7_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Model#Root'.'|'.2.'~'
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Model#Root'.'|'.2.'$'
            $res = _jm_obj_12($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Model#Root'.'|'.2.'%'
            $res = _jm_obj_13($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.2.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.2.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Model#Root'.'|'.1.'$'
sub _jm_obj_15($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.1.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Model#Root'.'|'.1.'$'.''
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_6($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.1.'$'.'$Identifier'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.1.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.1.'%'
sub _jm_obj_16($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.1.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Model#Root'.'|'.1.'%'.'<'
            # .'$Model#Root'.'|'.1.'%'.'<'.'|'.0
            $res = json_model_9($pval, undef, undef);
            unless ($res)
            {
                # .'$Model#Root'.'|'.1.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_10_idx (0 .. $#$pval)
                    {
                        my $arr_10_item = $$pval[$arr_10_idx];
                        # .'$Model#Root'.'|'.1.'%'.'<'.'|'.1.0
                        $res = json_model_9($arr_10_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_13($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.1.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_7($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.1.'%'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.1
sub _jm_obj_14($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "|")
        {
            # handle must | property
            $must_count++;
            # .'$Model#Root'.'|'.1.'|'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_9_idx (0 .. $#$pval)
                {
                    my $arr_9_item = $$pval[$arr_9_idx];
                    # .'$Model#Root'.'|'.1.'|'.0
                    $res = json_model_20($arr_9_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Model#Root'.'|'.1.'~'
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Model#Root'.'|'.1.'$'
            $res = _jm_obj_15($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Model#Root'.'|'.1.'%'
            $res = _jm_obj_16($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.1.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.1.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Model#Root'.'|'.0.'$'
sub _jm_obj_18($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.0.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Model#Root'.'|'.0.'$'.''
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_6($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.0.'$'.'$Identifier'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.0.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.0.'%'
sub _jm_obj_19($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.0.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Model#Root'.'|'.0.'%'.'<'
            # .'$Model#Root'.'|'.0.'%'.'<'.'|'.0
            $res = json_model_9($pval, undef, undef);
            unless ($res)
            {
                # .'$Model#Root'.'|'.0.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_11_idx (0 .. $#$pval)
                    {
                        my $arr_11_item = $$pval[$arr_11_idx];
                        # .'$Model#Root'.'|'.0.'%'.'<'.'|'.1.0
                        $res = json_model_9($arr_11_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_13($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Model#Root'.'|'.0.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_7($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Model#Root'.'|'.0.'%'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Model#Root'.'|'.0
sub _jm_obj_17($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "\@")
        {
            # handle must @ property
            $must_count++;
            # .'$Model#Root'.'|'.0.'@'
            $res = json_model_35($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Model#Root'.'|'.0.'~'
            $res = json_model_4($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Model#Root'.'|'.0.'$'
            $res = _jm_obj_18($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Model#Root'.'|'.0.'%'
            $res = _jm_obj_19($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Model#Root'.'|'.0.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "!")
        {
            # handle may ! property
            # .'$Model#Root'.'|'.0.'!'
            $res = jm_is_boolean($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 3 re props
            # .'$Model#Root'.'|'.0.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_3($prop, undef, undef))
        {
            # handle 3 re props
            # .'$Model#Root'.'|'.0.'/^(<=|>=|<|>)$/'
            $res = json_model_14($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (_jm_re_4($prop, undef, undef))
        {
            # handle 3 re props
            # .'$Model#Root'.'|'.0.'/^(=|!=)$/'
            $res = json_model_15($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# check $Model#Root (.'$Model#Root')
sub json_model_41($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Root'
    # .'$Model#Root'.'|'.0
    # .'$Model#Root'.'|'.1
    # .'$Model#Root'.'|'.2
    # .'$Model#Root'.'|'.3
    # .'$Model#Root'.'|'.4
    return jm_is_object($val) && (_jm_obj_17($val, undef, undef) || _jm_obj_14($val, undef, undef) || _jm_obj_11($val, undef, undef) || _jm_obj_8($val, undef, undef) || _jm_obj_5($val, undef, undef));
}

sub _jm_re_8($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^((file|https?):\/\/.+|\.\/.*|\.\.\/.*)$/;
    return $res;
}

# check $Model#Url (.'$Model#Url')
sub json_model_4($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Url'
    # "/^((file|https?)://.+|\\./.*|\\.\\./.*)$/"
    return jm_is_string($val) && _jm_re_8($val, undef, undef);
}

sub _jm_re_9($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /[^A-Z0-9]/;
    return $res;
}

sub _jm_re_10($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\w(\w|-)*$/;
    return $res;
}

# check $Model#Identifier (.'$Model#Identifier')
sub json_model_6($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model#Identifier'
    # .'$Model#Identifier'.'&'.0
    # "/^\\w(\\w|-)*$/"
    # .'$Model#Identifier'.'&'.1
    # "/[^A-Z0-9]/"
    return jm_is_string($val) && _jm_re_10($val, undef, undef) && _jm_re_9($val, undef, undef);
}


# initialization of global variables

our $initialized = 0;

sub check_model_init()
{
    if (!$initialized)
    {
        $initialized = 1;
        %_jm_cst_0 = (
            "\$DATE" => 1,
            "\$TIME" => 1,
            "\$DATETIME" => 1,
            "\$URL" => 1,
            "\$URI" => 1,
            "\$UUID" => 1,
            "\$EMAIL" => 1,
            "\$REGEX" => 1,
            "\$EXREG" => 1,
            "\$NULL" => 1,
            "\$BOOL" => 1,
            "\$FLOAT" => 1,
            "\$F16" => 1,
            "\$F32" => 1,
            "\$F64" => 1,
            "\$NUMBER" => 1,
            "\$INT" => 1,
            "\$INTEGER" => 1,
            "\$I8" => 1,
            "\$I16" => 1,
            "\$I32" => 1,
            "\$I64" => 1,
            "\$U8" => 1,
            "\$U16" => 1,
            "\$U32" => 1,
            "\$U64" => 1,
            "\$STRING" => 1,
            "\$NONE" => 1,
            "\$ANY" => 1,
        );
        %check_model_map = (
            "" => \&json_model_42,
            "Model" => \&json_model_42,
        );
    }
}

sub check_model_free()
{
    if ($initialized)
    {
        $initialized = 0;
        %_jm_cst_0 = ();
        %check_model_map = ();
    }
}

sub check_model_mapper($)
{
    my ($name) = @_;
    die "unexpected model name \"$name\"" unless exists $check_model_map{$name};
    return $check_model_map{$name};
}

sub check_model($$$)
{
    my ($json, $name, $rep) = @_;
    die "unexpected model name \"$name\"" unless exists $check_model_map{$name};
    my $path = defined $rep ? [] : undef;
    return $check_model_map{$name}($json, $path, $rep);
}

#
# SCRIPT EXECUTION
#
check_model_init();
jm_main(\&check_model, \%check_model_map, JMC_VERSION);
check_model_free();
