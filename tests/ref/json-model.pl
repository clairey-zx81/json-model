#! /usr/bin/perl
#
# Generated by JSON Model Compiler version 2.0b1
# see https://github.com/clairey-zx81/json-model
#
use strict;
use warnings;
use re::engine::RE2;
use JsonModel;
use constant JMC_VERSION => '2';


my %_jm_cst_0;
my %check_model_map;

sub _jm_re_0($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^((file|https?):\/\/.+|\.\/.*|\.\.\/.*)$/;
    return $res;
}

# check $Url (.'$Url')
sub json_model_2($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Url'
    # "/^((file|https?)://.+|\\./.*|\\.\\./.*)$/"
    $res = jm_is_string($val) && _jm_re_0($val, $path, $rep);
    return $res;
}


# check $PreDef (.'$PreDef')
sub json_model_3($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$PreDef'
    $res = jm_is_scalar($val) && jm_is_string($val) && exists $_jm_cst_0{$val};
    return $res;
}

# check $Comment (.'$Comment')
sub json_model_4($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Comment'
    # "/^#/"
    $res = jm_is_string($val) && jm_starts_with($val, '#');
    return $res;
}

sub _jm_re_1($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\$/;
    return $res;
}

# check $Ref (.'$Ref')
sub json_model_5($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Ref'
    # "/^\\$/"
    $res = jm_is_string($val) && _jm_re_1($val, $path, $rep);
    return $res;
}

sub _jm_re_2($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^=(null|true|false|-?\d+(\.\d+)?([Ee]-?\d+)?)$/;
    return $res;
}

# check $ValConst (.'$ValConst')
sub json_model_6($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$ValConst'
    # "/^=(null|true|false|-?\\d+(\\.\\d+)?([Ee]-?\\d+)?)$/"
    $res = jm_is_string($val) && _jm_re_2($val, $path, $rep);
    return $res;
}

sub _jm_re_3($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^[_a-zA-Z0-9]/;
    return $res;
}

# check $StrConst (.'$StrConst')
sub json_model_7($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$StrConst'
    # "/^[_a-zA-Z0-9]/"
    $res = jm_is_string($val) && _jm_re_3($val, $path, $rep);
    return $res;
}

sub _jm_xre_0_re($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\/(?<s1>.*)\/[a-z]*$/;
    return $res;
}

sub _jm_xre_0($$$)
{
    my ($val, $path, $rep) = @_;
    my $extract;
    my $match = $val =~ /^\/(?<s1>.*)\/[a-z]*$/;
    if (! $match)
    {
        return 0;
    }
    $extract = $+{'s1'};
    if (! (jm_is_string($extract) && jm_is_valid_regex($extract, $path, $rep)))
    {
        return 0;
    }
    return 1;
}

sub _jm_xre_1_re($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\/(?<s1>.*)\/[a-z]*X[a-z]*$/;
    return $res;
}

sub _jm_xre_1($$$)
{
    my ($val, $path, $rep) = @_;
    my $extract;
    my $match = $val =~ /^\/(?<s1>.*)\/[a-z]*X[a-z]*$/;
    if (! $match)
    {
        return 0;
    }
    $extract = $+{'s1'};
    if (! (jm_is_string($extract) && jm_is_valid_exreg($extract, $path, $rep)))
    {
        return 0;
    }
    return 1;
}

# check $Regex (.'$Regex')
sub json_model_8($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Regex'
    $res = jm_is_string($val);
    if ($res)
    {
        # .'$Regex'.'|'.0
        # "/^/($EXREG)/[a-z]*X[a-z]*$/X"
        $res = _jm_xre_1($val, $path, $rep);
        if (! $res)
        {
            # .'$Regex'.'|'.1
            # "/^/($REGEX)/[a-z]*$/X"
            $res = _jm_xre_0($val, $path, $rep);
        }
    }
    return $res;
}

sub _jm_re_4($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\..+$/;
    return $res;
}

# check $Name (.'$Name')
sub json_model_9($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Name'
    # "/^\\..+$/"
    $res = jm_is_string($val) && _jm_re_4($val, $path, $rep);
    return $res;
}

# check $CmpValue (.'$CmpValue')
sub json_model_10($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$CmpValue'
    # .'$CmpValue'.'|'.0
    $res = jm_is_integer($val);
    if (! $res)
    {
        # .'$CmpValue'.'|'.1
        $res = jm_is_numeric($val);
        if (! $res)
        {
            # .'$CmpValue'.'|'.2
            $res = jm_is_string($val);
        }
    }
    return $res;
}

# check $EqValue (.'$EqValue')
sub json_model_11($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$EqValue'
    # .'$EqValue'.'|'.0
    $res = !defined($val);
    if (! $res)
    {
        # .'$EqValue'.'|'.1
        $res = jm_is_boolean($val);
        if (! $res)
        {
            # .'$EqValue'.'|'.2
            $res = json_model_10($val, $path, $rep);
        }
    }
    return $res;
}

# check $ValModel (.'$ValModel')
sub json_model_12($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$ValModel'
    # .'$ValModel'.'|'.0
    $res = !defined($val);
    if (! $res)
    {
        # .'$ValModel'.'|'.1
        $res = jm_is_boolean($val) && $val == 1;
        if (! $res)
        {
            # .'$ValModel'.'|'.2
            $res = jm_is_integer($val) && $val == 0;
            if (! $res)
            {
                # .'$ValModel'.'|'.3
                $res = jm_is_integer($val) && $val == 1;
                if (! $res)
                {
                    # .'$ValModel'.'|'.4
                    $res = jm_is_integer($val) && $val == -1;
                    if (! $res)
                    {
                        # .'$ValModel'.'|'.5
                        $res = jm_is_numeric($val) && $val == 0.0;
                        if (! $res)
                        {
                            # .'$ValModel'.'|'.6
                            $res = jm_is_numeric($val) && $val == 1.0;
                            if (! $res)
                            {
                                # .'$ValModel'.'|'.7
                                $res = jm_is_numeric($val) && $val == -1.0;
                            }
                        }
                    }
                }
            }
        }
    }
    return $res;
}

# check $StrModel (.'$StrModel')
sub json_model_13($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$StrModel'
    # .'$StrModel'.'|'.0
    $res = json_model_7($val, $path, $rep);
    if (! $res)
    {
        # .'$StrModel'.'|'.1
        $res = json_model_6($val, $path, $rep);
        if (! $res)
        {
            # .'$StrModel'.'|'.2
            $res = json_model_8($val, $path, $rep);
            if (! $res)
            {
                # .'$StrModel'.'|'.3
                $res = json_model_3($val, $path, $rep);
                if (! $res)
                {
                    # .'$StrModel'.'|'.4
                    $res = json_model_5($val, $path, $rep);
                    if (! $res)
                    {
                        # .'$StrModel'.'|'.5
                        $res = jm_is_string($val) && $val eq '';
                    }
                }
            }
        }
    }
    return $res;
}

# check $Any (.'$Any')
sub json_model_14($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Any'
    $res = 1;
    return $res;
}

# check $None (.'$None')
sub json_model_15($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$None'
    $res = 0;
    return $res;
}

# object .'$Common'
sub _jm_obj_0($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Common'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Common (.'$Common')
sub json_model_16($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Common'
    $res = _jm_obj_0($val, $path, $rep);
    return $res;
}

# check $Array (.'$Array')
sub json_model_17($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Array'
    $res = jm_is_array($val);
    if ($res)
    {
        for my $arr_0_idx (0 .. $#$val)
        {
            my $arr_0_item = $$val[$arr_0_idx];
            # .'$Array'.0
            # .'$Array'.0.'|'.0
            $res = json_model_30($arr_0_item, undef, $rep);
            if (! $res)
            {
                # .'$Array'.0.'|'.1
                $res = json_model_4($arr_0_item, undef, $rep);
            }
            if (! $res)
            {
                last;
            }
        }
    }
    return $res;
}

sub _jm_re_5($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^(<=|>=|<|>)$/;
    return $res;
}

sub _jm_re_6($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^(=|!=)$/;
    return $res;
}

# object .'$Constraint'
sub _jm_obj_1($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '@')
        {
            # handle must @ property
            $must_count++;
            # .'$Constraint'.'@'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '!')
        {
            # handle may ! property
            # .'$Constraint'.'!'
            $res = jm_is_boolean($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_5($prop, $path, $rep))
        {
            # handle 2 re props
            # .'$Constraint'.'/^(<=|>=|<|>)$/'
            $res = json_model_10($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_6($prop, $path, $rep))
        {
            # handle 2 re props
            # .'$Constraint'.'/^(=|!=)$/'
            $res = json_model_11($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# check $Constraint (.'$Constraint')
sub json_model_18($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Constraint'
    $res = _jm_obj_1($val, $path, $rep);
    return $res;
}

# object .'$Or'
sub _jm_obj_2($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '|')
        {
            # handle must | property
            $must_count++;
            # .'$Or'.'|'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_1_idx (0 .. $#$pval)
                {
                    my $arr_1_item = $$pval[$arr_1_idx];
                    # .'$Or'.'|'.0
                    $res = json_model_30($arr_1_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# check $Or (.'$Or')
sub json_model_19($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Or'
    $res = _jm_obj_2($val, $path, $rep);
    return $res;
}

# object .'$And'
sub _jm_obj_3($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '&')
        {
            # handle must & property
            $must_count++;
            # .'$And'.'&'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_2_idx (0 .. $#$pval)
                {
                    my $arr_2_item = $$pval[$arr_2_idx];
                    # .'$And'.'&'.0
                    $res = json_model_30($arr_2_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# check $And (.'$And')
sub json_model_20($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$And'
    $res = _jm_obj_3($val, $path, $rep);
    return $res;
}

# object .'$Xor'
sub _jm_obj_4($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '^')
        {
            # handle must ^ property
            $must_count++;
            # .'$Xor'.'^'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_3_idx (0 .. $#$pval)
                {
                    my $arr_3_item = $$pval[$arr_3_idx];
                    # .'$Xor'.'^'.0
                    $res = json_model_30($arr_3_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# check $Xor (.'$Xor')
sub json_model_21($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Xor'
    $res = _jm_obj_4($val, $path, $rep);
    return $res;
}

# object .'$Add'
sub _jm_obj_5($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '+')
        {
            # handle must + property
            $must_count++;
            # .'$Add'.'+'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_4_idx (0 .. $#$pval)
                {
                    my $arr_4_item = $$pval[$arr_4_idx];
                    # .'$Add'.'+'.0
                    $res = json_model_30($arr_4_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# check $Add (.'$Add')
sub json_model_22($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Add'
    $res = _jm_obj_5($val, $path, $rep);
    return $res;
}

sub _jm_re_7($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^[?!]/;
    return $res;
}

# check $Prop (.'$Prop')
sub json_model_23($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Prop'
    # .'$Prop'.'|'.0
    # "/^[?!]/"
    $res = jm_is_string($val) && _jm_re_7($val, $path, $rep);
    if (! $res)
    {
        # .'$Prop'.'|'.1
        $res = json_model_7($val, $path, $rep);
        if (! $res)
        {
            # .'$Prop'.'|'.2
            $res = json_model_8($val, $path, $rep);
            if (! $res)
            {
                # .'$Prop'.'|'.3
                $res = json_model_5($val, $path, $rep);
                if (! $res)
                {
                    # .'$Prop'.'|'.4
                    $res = json_model_9($val, $path, $rep);
                    if (! $res)
                    {
                        # .'$Prop'.'|'.5
                        $res = jm_is_string($val) && $val eq '';
                    }
                }
            }
        }
    }
    return $res;
}

# object .'$Obj'
sub _jm_obj_6($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if (json_model_23($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Obj'.'$Prop'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Obj (.'$Obj')
sub json_model_24($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Obj'
    $res = _jm_obj_6($val, $path, $rep);
    return $res;
}

# check $Elem (.'$Elem')
sub json_model_25($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Elem'
    # .'$Elem'.'|'.0
    $res = json_model_18($val, $path, $rep);
    if (! $res)
    {
        # .'$Elem'.'|'.1
        $res = json_model_19($val, $path, $rep);
        if (! $res)
        {
            # .'$Elem'.'|'.2
            $res = json_model_20($val, $path, $rep);
            if (! $res)
            {
                # .'$Elem'.'|'.3
                $res = json_model_21($val, $path, $rep);
                if (! $res)
                {
                    # .'$Elem'.'|'.4
                    $res = json_model_22($val, $path, $rep);
                    if (! $res)
                    {
                        # .'$Elem'.'|'.5
                        $res = json_model_24($val, $path, $rep);
                    }
                }
            }
        }
    }
    return $res;
}

# object .'$Element'.'|'.5
sub _jm_obj_7($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Element'.'|'.5.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_23($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Element'.'|'.5.'$Prop'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Element'.'|'.4
sub _jm_obj_8($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '+')
        {
            # handle must + property
            $must_count++;
            # .'$Element'.'|'.4.'+'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_5_idx (0 .. $#$pval)
                {
                    my $arr_5_item = $$pval[$arr_5_idx];
                    # .'$Element'.'|'.4.'+'.0
                    $res = json_model_30($arr_5_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Element'.'|'.4.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# object .'$Element'.'|'.3
sub _jm_obj_9($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '^')
        {
            # handle must ^ property
            $must_count++;
            # .'$Element'.'|'.3.'^'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_6_idx (0 .. $#$pval)
                {
                    my $arr_6_item = $$pval[$arr_6_idx];
                    # .'$Element'.'|'.3.'^'.0
                    $res = json_model_30($arr_6_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Element'.'|'.3.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# object .'$Element'.'|'.2
sub _jm_obj_10($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '&')
        {
            # handle must & property
            $must_count++;
            # .'$Element'.'|'.2.'&'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_7_idx (0 .. $#$pval)
                {
                    my $arr_7_item = $$pval[$arr_7_idx];
                    # .'$Element'.'|'.2.'&'.0
                    $res = json_model_30($arr_7_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Element'.'|'.2.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# object .'$Element'.'|'.1
sub _jm_obj_11($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '|')
        {
            # handle must | property
            $must_count++;
            # .'$Element'.'|'.1.'|'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_8_idx (0 .. $#$pval)
                {
                    my $arr_8_item = $$pval[$arr_8_idx];
                    # .'$Element'.'|'.1.'|'.0
                    $res = json_model_30($arr_8_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Element'.'|'.1.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# object .'$Element'.'|'.0
sub _jm_obj_12($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '@')
        {
            # handle must @ property
            $must_count++;
            # .'$Element'.'|'.0.'@'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '!')
        {
            # handle may ! property
            # .'$Element'.'|'.0.'!'
            $res = jm_is_boolean($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Element'.'|'.0.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_5($prop, $path, $rep))
        {
            # handle 2 re props
            # .'$Element'.'|'.0.'/^(<=|>=|<|>)$/'
            $res = json_model_10($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_6($prop, $path, $rep))
        {
            # handle 2 re props
            # .'$Element'.'|'.0.'/^(=|!=)$/'
            $res = json_model_11($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# check $Element (.'$Element')
sub json_model_26($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Element'
    $res = jm_is_object($val);
    if ($res)
    {
        # .'$Element'.'|'.0
        $res = _jm_obj_12($val, $path, $rep);
        if (! $res)
        {
            # .'$Element'.'|'.1
            $res = _jm_obj_11($val, $path, $rep);
            if (! $res)
            {
                # .'$Element'.'|'.2
                $res = _jm_obj_10($val, $path, $rep);
                if (! $res)
                {
                    # .'$Element'.'|'.3
                    $res = _jm_obj_9($val, $path, $rep);
                    if (! $res)
                    {
                        # .'$Element'.'|'.4
                        $res = _jm_obj_8($val, $path, $rep);
                        if (! $res)
                        {
                            # .'$Element'.'|'.5
                            $res = _jm_obj_7($val, $path, $rep);
                        }
                    }
                }
            }
        }
    }
    return $res;
}

# object .'$Trafo'.'*'.'|'.1
sub _jm_obj_14($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        # handle other props
        # .'$Trafo'.'*'.'|'.1.''
        $res = json_model_30($pval, undef, $rep);
        if (! $res)
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Trafo'
sub _jm_obj_13($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '/')
        {
            # handle may / property
            # .'$Trafo'.'/'
            # .'$Trafo'.'/'.'|'.0
            $res = json_model_11($pval, undef, $rep);
            if (! $res)
            {
                # .'$Trafo'.'/'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    # accept any array
                    ;
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '*')
        {
            # handle may * property
            # .'$Trafo'.'*'
            # .'$Trafo'.'*'.'|'.0
            $res = jm_is_array($pval);
            if ($res)
            {
                # accept any array
                ;
            }
            if (! $res)
            {
                # .'$Trafo'.'*'.'|'.1
                $res = _jm_obj_14($pval, undef, $rep);
            }
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Trafo (.'$Trafo')
sub json_model_27($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # remove and/or add stuff
    # .'$Trafo'
    $res = _jm_obj_13($val, $path, $rep);
    return $res;
}

# object .'$Transformation'.'|'.1.'*'.'|'.1
sub _jm_obj_16($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        # handle other props
        # .'$Transformation'.'|'.1.'*'.'|'.1.''
        $res = json_model_30($pval, undef, $rep);
        if (! $res)
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Transformation'.'|'.1
sub _jm_obj_15($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '/')
        {
            # handle may / property
            # .'$Transformation'.'|'.1.'/'
            # .'$Transformation'.'|'.1.'/'.'|'.0
            $res = json_model_11($pval, undef, $rep);
            if (! $res)
            {
                # .'$Transformation'.'|'.1.'/'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    # accept any array
                    ;
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '*')
        {
            # handle may * property
            # .'$Transformation'.'|'.1.'*'
            # .'$Transformation'.'|'.1.'*'.'|'.0
            $res = jm_is_array($pval);
            if ($res)
            {
                # accept any array
                ;
            }
            if (! $res)
            {
                # .'$Transformation'.'|'.1.'*'.'|'.1
                $res = _jm_obj_16($pval, undef, $rep);
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Transformation'.'|'.1.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Transformation (.'$Transformation')
sub json_model_28($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Transformation'
    # .'$Transformation'.'|'.0
    $res = json_model_30($val, $path, $rep);
    if (! $res)
    {
        # .'$Transformation'.'|'.1
        $res = _jm_obj_15($val, $path, $rep);
    }
    return $res;
}

# check $ScalarModel (.'$ScalarModel')
sub json_model_29($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$ScalarModel'
    # .'$ScalarModel'.'|'.0
    $res = json_model_12($val, $path, $rep);
    if (! $res)
    {
        # .'$ScalarModel'.'|'.1
        $res = json_model_13($val, $path, $rep);
    }
    return $res;
}

# check $Model (.'$Model')
sub json_model_30($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Model'
    # .'$Model'.'|'.0
    $res = json_model_29($val, $path, $rep);
    if (! $res)
    {
        # .'$Model'.'|'.1
        $res = json_model_17($val, $path, $rep);
        if (! $res)
        {
            # .'$Model'.'|'.2
            $res = json_model_26($val, $path, $rep);
        }
    }
    return $res;
}

sub _jm_re_8($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /./;
    return $res;
}

# object .'$Defs'
sub _jm_obj_17($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '')
        {
            # handle may  property
            # .'$Defs'.''
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_8($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Defs'.'/./'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Defs (.'$Defs')
sub json_model_31($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Defs'
    $res = _jm_obj_17($val, $path, $rep);
    return $res;
}

sub _jm_re_9($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^([#|&^+\/*@~=\$%]|[<>!]=?)$/;
    return $res;
}

# object .'$Rename'
sub _jm_obj_18($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if (json_model_9($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Rename'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_9($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Rename (.'$Rename')
sub json_model_32($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Rename'
    $res = _jm_obj_18($val, $path, $rep);
    return $res;
}

sub _jm_re_10($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\$.*$/;
    return $res;
}

# object .'$Rewrite'
sub _jm_obj_19($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if (_jm_re_10($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Rewrite'.'/^\\$.*$/'
            $res = json_model_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Rewrite (.'$Rewrite')
sub json_model_33($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Rewrite'
    $res = _jm_obj_19($val, $path, $rep);
    return $res;
}

# object .'$Import'
sub _jm_obj_20($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '<')
        {
            # handle may < property
            # .'$Import'.'<'
            # .'$Import'.'<'.'|'.0
            $res = json_model_5($pval, undef, $rep);
            if (! $res)
            {
                # .'$Import'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_9_idx (0 .. $#$pval)
                    {
                        my $arr_9_item = $$pval[$arr_9_idx];
                        # .'$Import'.'<'.'|'.1.0
                        $res = json_model_5($arr_9_item, undef, $rep);
                        if (! $res)
                        {
                            last;
                        }
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Import (.'$Import')
sub json_model_34($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Import'
    $res = _jm_obj_20($val, $path, $rep);
    return $res;
}

# object .'$RootOnly'.'$'
sub _jm_obj_22($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '')
        {
            # handle may  property
            # .'$RootOnly'.'$'.''
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$RootOnly'.'$'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_8($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$RootOnly'.'$'.'/./'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$RootOnly'.'%'
sub _jm_obj_23($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '<')
        {
            # handle may < property
            # .'$RootOnly'.'%'.'<'
            # .'$RootOnly'.'%'.'<'.'|'.0
            $res = json_model_5($pval, undef, $rep);
            if (! $res)
            {
                # .'$RootOnly'.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_10_idx (0 .. $#$pval)
                    {
                        my $arr_10_item = $$pval[$arr_10_idx];
                        # .'$RootOnly'.'%'.'<'.'|'.1.0
                        $res = json_model_5($arr_10_item, undef, $rep);
                        if (! $res)
                        {
                            last;
                        }
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$RootOnly'.'%'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_9($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$RootOnly'.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_9($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_10($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$RootOnly'.'%'.'/^\\$.*$/'
            $res = json_model_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$RootOnly'
sub _jm_obj_21($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '~')
        {
            # handle may ~ property
            # .'$RootOnly'.'~'
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '$')
        {
            # handle may $ property
            # .'$RootOnly'.'$'
            $res = _jm_obj_22($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '%')
        {
            # handle may % property
            # .'$RootOnly'.'%'
            $res = _jm_obj_23($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $RootOnly (.'$RootOnly')
sub json_model_35($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # symbols that can only appear at the model's root
    # .'$RootOnly'
    $res = _jm_obj_21($val, $path, $rep);
    return $res;
}

# object .'$Root'.'|'.5.'$'
sub _jm_obj_25($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '')
        {
            # handle may  property
            # .'$Root'.'|'.5.'$'.''
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.5.'$'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_8($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.5.'$'.'/./'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.5.'%'
sub _jm_obj_26($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '<')
        {
            # handle may < property
            # .'$Root'.'|'.5.'%'.'<'
            # .'$Root'.'|'.5.'%'.'<'.'|'.0
            $res = json_model_5($pval, undef, $rep);
            if (! $res)
            {
                # .'$Root'.'|'.5.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_11_idx (0 .. $#$pval)
                    {
                        my $arr_11_item = $$pval[$arr_11_idx];
                        # .'$Root'.'|'.5.'%'.'<'.'|'.1.0
                        $res = json_model_5($arr_11_item, undef, $rep);
                        if (! $res)
                        {
                            last;
                        }
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.5.'%'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_9($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.5.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_9($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_10($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.5.'%'.'/^\\$.*$/'
            $res = json_model_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.5
sub _jm_obj_24($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '~')
        {
            # handle may ~ property
            # .'$Root'.'|'.5.'~'
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '$')
        {
            # handle may $ property
            # .'$Root'.'|'.5.'$'
            $res = _jm_obj_25($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '%')
        {
            # handle may % property
            # .'$Root'.'|'.5.'%'
            $res = _jm_obj_26($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.5.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_23($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.5.'$Prop'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.4.'$'
sub _jm_obj_28($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '')
        {
            # handle may  property
            # .'$Root'.'|'.4.'$'.''
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.4.'$'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_8($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.4.'$'.'/./'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.4.'%'
sub _jm_obj_29($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '<')
        {
            # handle may < property
            # .'$Root'.'|'.4.'%'.'<'
            # .'$Root'.'|'.4.'%'.'<'.'|'.0
            $res = json_model_5($pval, undef, $rep);
            if (! $res)
            {
                # .'$Root'.'|'.4.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_13_idx (0 .. $#$pval)
                    {
                        my $arr_13_item = $$pval[$arr_13_idx];
                        # .'$Root'.'|'.4.'%'.'<'.'|'.1.0
                        $res = json_model_5($arr_13_item, undef, $rep);
                        if (! $res)
                        {
                            last;
                        }
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.4.'%'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_9($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.4.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_9($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_10($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.4.'%'.'/^\\$.*$/'
            $res = json_model_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.4
sub _jm_obj_27($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '+')
        {
            # handle must + property
            $must_count++;
            # .'$Root'.'|'.4.'+'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_12_idx (0 .. $#$pval)
                {
                    my $arr_12_item = $$pval[$arr_12_idx];
                    # .'$Root'.'|'.4.'+'.0
                    $res = json_model_30($arr_12_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '~')
        {
            # handle may ~ property
            # .'$Root'.'|'.4.'~'
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '$')
        {
            # handle may $ property
            # .'$Root'.'|'.4.'$'
            $res = _jm_obj_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '%')
        {
            # handle may % property
            # .'$Root'.'|'.4.'%'
            $res = _jm_obj_29($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.4.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# object .'$Root'.'|'.3.'$'
sub _jm_obj_31($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '')
        {
            # handle may  property
            # .'$Root'.'|'.3.'$'.''
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.3.'$'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_8($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.3.'$'.'/./'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.3.'%'
sub _jm_obj_32($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '<')
        {
            # handle may < property
            # .'$Root'.'|'.3.'%'.'<'
            # .'$Root'.'|'.3.'%'.'<'.'|'.0
            $res = json_model_5($pval, undef, $rep);
            if (! $res)
            {
                # .'$Root'.'|'.3.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_15_idx (0 .. $#$pval)
                    {
                        my $arr_15_item = $$pval[$arr_15_idx];
                        # .'$Root'.'|'.3.'%'.'<'.'|'.1.0
                        $res = json_model_5($arr_15_item, undef, $rep);
                        if (! $res)
                        {
                            last;
                        }
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.3.'%'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_9($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.3.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_9($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_10($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.3.'%'.'/^\\$.*$/'
            $res = json_model_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.3
sub _jm_obj_30($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '^')
        {
            # handle must ^ property
            $must_count++;
            # .'$Root'.'|'.3.'^'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_14_idx (0 .. $#$pval)
                {
                    my $arr_14_item = $$pval[$arr_14_idx];
                    # .'$Root'.'|'.3.'^'.0
                    $res = json_model_30($arr_14_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '~')
        {
            # handle may ~ property
            # .'$Root'.'|'.3.'~'
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '$')
        {
            # handle may $ property
            # .'$Root'.'|'.3.'$'
            $res = _jm_obj_31($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '%')
        {
            # handle may % property
            # .'$Root'.'|'.3.'%'
            $res = _jm_obj_32($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.3.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# object .'$Root'.'|'.2.'$'
sub _jm_obj_34($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '')
        {
            # handle may  property
            # .'$Root'.'|'.2.'$'.''
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.2.'$'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_8($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.2.'$'.'/./'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.2.'%'
sub _jm_obj_35($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '<')
        {
            # handle may < property
            # .'$Root'.'|'.2.'%'.'<'
            # .'$Root'.'|'.2.'%'.'<'.'|'.0
            $res = json_model_5($pval, undef, $rep);
            if (! $res)
            {
                # .'$Root'.'|'.2.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_17_idx (0 .. $#$pval)
                    {
                        my $arr_17_item = $$pval[$arr_17_idx];
                        # .'$Root'.'|'.2.'%'.'<'.'|'.1.0
                        $res = json_model_5($arr_17_item, undef, $rep);
                        if (! $res)
                        {
                            last;
                        }
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.2.'%'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_9($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.2.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_9($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_10($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.2.'%'.'/^\\$.*$/'
            $res = json_model_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.2
sub _jm_obj_33($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '&')
        {
            # handle must & property
            $must_count++;
            # .'$Root'.'|'.2.'&'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_16_idx (0 .. $#$pval)
                {
                    my $arr_16_item = $$pval[$arr_16_idx];
                    # .'$Root'.'|'.2.'&'.0
                    $res = json_model_30($arr_16_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '~')
        {
            # handle may ~ property
            # .'$Root'.'|'.2.'~'
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '$')
        {
            # handle may $ property
            # .'$Root'.'|'.2.'$'
            $res = _jm_obj_34($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '%')
        {
            # handle may % property
            # .'$Root'.'|'.2.'%'
            $res = _jm_obj_35($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.2.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# object .'$Root'.'|'.1.'$'
sub _jm_obj_37($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '')
        {
            # handle may  property
            # .'$Root'.'|'.1.'$'.''
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.1.'$'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_8($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.1.'$'.'/./'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.1.'%'
sub _jm_obj_38($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '<')
        {
            # handle may < property
            # .'$Root'.'|'.1.'%'.'<'
            # .'$Root'.'|'.1.'%'.'<'.'|'.0
            $res = json_model_5($pval, undef, $rep);
            if (! $res)
            {
                # .'$Root'.'|'.1.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_19_idx (0 .. $#$pval)
                    {
                        my $arr_19_item = $$pval[$arr_19_idx];
                        # .'$Root'.'|'.1.'%'.'<'.'|'.1.0
                        $res = json_model_5($arr_19_item, undef, $rep);
                        if (! $res)
                        {
                            last;
                        }
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.1.'%'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_9($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.1.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_9($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_10($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.1.'%'.'/^\\$.*$/'
            $res = json_model_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.1
sub _jm_obj_36($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '|')
        {
            # handle must | property
            $must_count++;
            # .'$Root'.'|'.1.'|'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_18_idx (0 .. $#$pval)
                {
                    my $arr_18_item = $$pval[$arr_18_idx];
                    # .'$Root'.'|'.1.'|'.0
                    $res = json_model_30($arr_18_item, undef, $rep);
                    if (! $res)
                    {
                        last;
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '~')
        {
            # handle may ~ property
            # .'$Root'.'|'.1.'~'
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '$')
        {
            # handle may $ property
            # .'$Root'.'|'.1.'$'
            $res = _jm_obj_37($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '%')
        {
            # handle may % property
            # .'$Root'.'|'.1.'%'
            $res = _jm_obj_38($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.1.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# object .'$Root'.'|'.0.'$'
sub _jm_obj_40($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '')
        {
            # handle may  property
            # .'$Root'.'|'.0.'$'.''
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.0.'$'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_8($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.0.'$'.'/./'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.0.'%'
sub _jm_obj_41($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '<')
        {
            # handle may < property
            # .'$Root'.'|'.0.'%'.'<'
            # .'$Root'.'|'.0.'%'.'<'.'|'.0
            $res = json_model_5($pval, undef, $rep);
            if (! $res)
            {
                # .'$Root'.'|'.0.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_20_idx (0 .. $#$pval)
                    {
                        my $arr_20_item = $$pval[$arr_20_idx];
                        # .'$Root'.'|'.0.'%'.'<'.'|'.1.0
                        $res = json_model_5($arr_20_item, undef, $rep);
                        if (! $res)
                        {
                            last;
                        }
                    }
                }
            }
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.0.'%'.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_9($prop, undef, $rep))
        {
            # handle 2 key props
            # .'$Root'.'|'.0.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_9($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_10($prop, $path, $rep))
        {
            # handle 1 re props
            # .'$Root'.'|'.0.'%'.'/^\\$.*$/'
            $res = json_model_28($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.0
sub _jm_obj_39($$$)
{
    my ($val, $path, $rep) = @_;
    if (! jm_is_object($val))
    {
        return 0;
    }
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq '@')
        {
            # handle must @ property
            $must_count++;
            # .'$Root'.'|'.0.'@'
            $res = json_model_30($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '~')
        {
            # handle may ~ property
            # .'$Root'.'|'.0.'~'
            $res = json_model_2($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '$')
        {
            # handle may $ property
            # .'$Root'.'|'.0.'$'
            $res = _jm_obj_40($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '%')
        {
            # handle may % property
            # .'$Root'.'|'.0.'%'
            $res = _jm_obj_41($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif ($prop eq '!')
        {
            # handle may ! property
            # .'$Root'.'|'.0.'!'
            $res = jm_is_boolean($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (json_model_4($prop, undef, $rep))
        {
            # handle 1 key props
            # .'$Root'.'|'.0.'$Comment'
            $res = jm_is_string($pval);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_5($prop, $path, $rep))
        {
            # handle 2 re props
            # .'$Root'.'|'.0.'/^(<=|>=|<|>)$/'
            $res = json_model_10($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        elsif (_jm_re_6($prop, $path, $rep))
        {
            # handle 2 re props
            # .'$Root'.'|'.0.'/^(=|!=)$/'
            $res = json_model_11($pval, undef, $rep);
            if (! $res)
            {
                return 0;
            }
        }
        else
        {
            return 0;
        }
    }
    if ($must_count != 1)
    {
        return 0;
    }
    return 1;
}

# check $Root (.'$Root')
sub json_model_36($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$Root'
    $res = jm_is_object($val);
    if ($res)
    {
        # .'$Root'.'|'.0
        $res = _jm_obj_39($val, $path, $rep);
        if (! $res)
        {
            # .'$Root'.'|'.1
            $res = _jm_obj_36($val, $path, $rep);
            if (! $res)
            {
                # .'$Root'.'|'.2
                $res = _jm_obj_33($val, $path, $rep);
                if (! $res)
                {
                    # .'$Root'.'|'.3
                    $res = _jm_obj_30($val, $path, $rep);
                    if (! $res)
                    {
                        # .'$Root'.'|'.4
                        $res = _jm_obj_27($val, $path, $rep);
                        if (! $res)
                        {
                            # .'$Root'.'|'.5
                            $res = _jm_obj_24($val, $path, $rep);
                        }
                    }
                }
            }
        }
    }
    return $res;
}

# check $RootModel (.'$RootModel')
sub json_model_37($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .'$RootModel'
    # .'$RootModel'.'|'.0
    $res = json_model_29($val, $path, $rep);
    if (! $res)
    {
        # .'$RootModel'.'|'.1
        $res = json_model_17($val, $path, $rep);
        if (! $res)
        {
            # .'$RootModel'.'|'.2
            $res = json_model_36($val, $path, $rep);
        }
    }
    return $res;
}

# check $ (.)
sub json_model_1($$$)
{
    my ($val, $path, $rep) = @_;
    my $res;
    # .
    $res = json_model_37($val, $path, $rep);
    return $res;
}


# initialization of global variables

our $initialized = 0;

sub check_model_init()
{
    if (!$initialized)
    {
        $initialized = 1;
        %_jm_cst_0 = (
            '$DATE' => 1,
            '$TIME' => 1,
            '$DATETIME' => 1,
            '$URL' => 1,
            '$URI' => 1,
            '$UUID' => 1,
            '$EMAIL' => 1,
            '$REGEX' => 1,
            '$EXREG' => 1,
            '$NULL' => 1,
            '$BOOL' => 1,
            '$FLOAT' => 1,
            '$F32' => 1,
            '$F64' => 1,
            '$NUMBER' => 1,
            '$INT' => 1,
            '$INTEGER' => 1,
            '$I32' => 1,
            '$I64' => 1,
            '$U32' => 1,
            '$U64' => 1,
            '$STRING' => 1,
            '$NONE' => 1,
            '$ANY' => 1,
        );
        %check_model_map = (
            '' => \&json_model_1,
            'Url' => \&json_model_2,
            'PreDef' => \&json_model_3,
            'Comment' => \&json_model_4,
            'Ref' => \&json_model_5,
            'ValConst' => \&json_model_6,
            'StrConst' => \&json_model_7,
            'Regex' => \&json_model_8,
            'Name' => \&json_model_9,
            'CmpValue' => \&json_model_10,
            'EqValue' => \&json_model_11,
            'ValModel' => \&json_model_12,
            'StrModel' => \&json_model_13,
            'Any' => \&json_model_14,
            'None' => \&json_model_15,
            'Common' => \&json_model_16,
            'Array' => \&json_model_17,
            'Constraint' => \&json_model_18,
            'Or' => \&json_model_19,
            'And' => \&json_model_20,
            'Xor' => \&json_model_21,
            'Add' => \&json_model_22,
            'Prop' => \&json_model_23,
            'Obj' => \&json_model_24,
            'Elem' => \&json_model_25,
            'Element' => \&json_model_26,
            'Trafo' => \&json_model_27,
            'Transformation' => \&json_model_28,
            'ScalarModel' => \&json_model_29,
            'Model' => \&json_model_30,
            'Defs' => \&json_model_31,
            'Rename' => \&json_model_32,
            'Rewrite' => \&json_model_33,
            'Import' => \&json_model_34,
            'RootOnly' => \&json_model_35,
            'Root' => \&json_model_36,
            'RootModel' => \&json_model_37,
        );
    }
}

sub check_model_free()
{
    if ($initialized)
    {
        $initialized = 0;
        %_jm_cst_0 = ();
        %check_model_map = ();
    }
}

sub check_model($$$)
{
    my ($json, $name, $rep) = @_;
    die "unexpected model name \"$name\"" unless exists $check_model_map{$name};
    my $path = defined $rep ? [] : undef;
    return $check_model_map{$name}($json, $path, $rep);
}

#
# SCRIPT EXECUTION
#
check_model_init();
jm_main(\&check_model, \%check_model_map, JMC_VERSION);
check_model_free();
