#! /usr/bin/perl
#
# Generated by JSON Model Compiler version 2
# see https://github.com/clairey-zx81/json-model
#
use utf8;
use strict;
use warnings;
no warnings "uninitialized";
use re::engine::RE2;
use JSON::JsonModel;
use constant JMC_VERSION => "2";


sub json_model_2($$$);
my %_jm_cst_0;
sub json_model_3($$$);
sub json_model_4($$$);
sub json_model_5($$$);
sub json_model_6($$$);
sub json_model_7($$$);
sub json_model_8($$$);
sub json_model_9($$$);
sub _jm_xre_0($$$);
sub _jm_xre_1($$$);
sub json_model_10($$$);
sub json_model_11($$$);
sub json_model_12($$$);
sub json_model_13($$$);
sub json_model_14($$$);
sub json_model_15($$$);
sub json_model_16($$$);
sub json_model_17($$$);
sub json_model_18($$$);
sub json_model_19($$$);
sub json_model_20($$$);
sub json_model_21($$$);
sub json_model_22($$$);
sub json_model_23($$$);
sub json_model_24($$$);
sub json_model_25($$$);
sub json_model_26($$$);
sub json_model_27($$$);
sub json_model_28($$$);
sub _jm_obj_0($$$);
sub _jm_obj_1($$$);
sub _jm_obj_2($$$);
sub _jm_obj_3($$$);
sub _jm_obj_4($$$);
sub _jm_obj_5($$$);
sub json_model_29($$$);
sub _jm_obj_6($$$);
sub json_model_30($$$);
sub _jm_obj_8($$$);
sub _jm_obj_7($$$);
sub json_model_31($$$);
sub json_model_32($$$);
sub json_model_33($$$);
sub json_model_34($$$);
sub json_model_35($$$);
sub json_model_36($$$);
sub json_model_37($$$);
sub _jm_obj_9($$$);
sub _jm_obj_10($$$);
sub json_model_38($$$);
sub _jm_obj_12($$$);
sub _jm_obj_13($$$);
sub _jm_obj_11($$$);
sub _jm_obj_15($$$);
sub _jm_obj_16($$$);
sub _jm_obj_14($$$);
sub _jm_obj_18($$$);
sub _jm_obj_19($$$);
sub _jm_obj_17($$$);
sub _jm_obj_21($$$);
sub _jm_obj_22($$$);
sub _jm_obj_20($$$);
sub _jm_obj_24($$$);
sub _jm_obj_25($$$);
sub _jm_obj_23($$$);
sub _jm_obj_27($$$);
sub _jm_obj_28($$$);
sub _jm_obj_26($$$);
sub json_model_39($$$);
sub json_model_40($$$);
sub json_model_1($$$);
my %check_model_map;

sub _jm_re_0($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^((file|https?):\/\/.+|\.\/.*|\.\.\/.*)$/;
    return $res;
}

# check $Url (.'$Url')
sub json_model_2($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Url'
    # "/^((file|https?)://.+|\\./.*|\\.\\./.*)$/"
    return jm_is_string($val) && _jm_re_0($val, undef, undef);
}


# check $PreDef (.'$PreDef')
sub json_model_3($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$PreDef'
    return jm_is_string($val) && exists $_jm_cst_0{$val};
}

sub _jm_re_1($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /[^A-Z0-9]/;
    return $res;
}

sub _jm_re_2($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\w(\w|-)*$/;
    return $res;
}

# check $Identifier (.'$Identifier')
sub json_model_4($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Identifier'
    # .'$Identifier'.'&'.0
    # "/^\\w(\\w|-)*$/"
    # .'$Identifier'.'&'.1
    # "/[^A-Z0-9]/"
    return jm_is_string($val) && _jm_re_2($val, undef, undef) && _jm_re_1($val, undef, undef);
}

# check $ArrayComment (.'$ArrayComment')
sub json_model_5($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$ArrayComment'
    # "/^#/"
    return jm_is_string($val) && jm_starts_with($val, "#");
}

# check $ObjectComment (.'$ObjectComment')
sub json_model_6($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$ObjectComment'
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$ObjectComment'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$ObjectComment'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

sub _jm_re_3($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\$./;
    return $res;
}

# check $Ref (.'$Ref')
sub json_model_7($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Ref'
    # "/^\\$./"
    return jm_is_string($val) && _jm_re_3($val, undef, undef);
}

sub _jm_re_4($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^=(null|true|false|[-+]?\d+(\.\d+)?([Ee][-+]?\d+)?)$/;
    return $res;
}

# check $ValConst (.'$ValConst')
sub json_model_8($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$ValConst'
    # "/^=(null|true|false|[-+]?\\d+(\\.\\d+)?([Ee][-+]?\\d+)?)$/"
    return jm_is_string($val) && _jm_re_4($val, undef, undef);
}

sub _jm_re_5($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^[A-Za-z0-9_]/;
    return $res;
}

# check $StrConst (.'$StrConst')
sub json_model_9($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$StrConst'
    # "/^[A-Za-z0-9_]/"
    return jm_is_string($val) && _jm_re_5($val, undef, undef);
}

sub _jm_xre_0_re($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\/(?<s1>.*)\/[a-z]*$/;
    return $res;
}

sub _jm_xre_0($$$)
{
    my ($val, $path, $rep) = @_;
    my $extract;
    my $match = $val =~ /^\/(?<s1>.*)\/[a-z]*$/;
    return 0 unless $match;
    $extract = $+{"s1"};
    return 0 unless jm_is_valid_regex($extract, undef, undef);
    return 1;
}

sub _jm_xre_1_re($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\/(?<s1>.*)\/[a-z]*X[a-z]*$/;
    return $res;
}

sub _jm_xre_1($$$)
{
    my ($val, $path, $rep) = @_;
    my $extract;
    my $match = $val =~ /^\/(?<s1>.*)\/[a-z]*X[a-z]*$/;
    return 0 unless $match;
    $extract = $+{"s1"};
    return 0 unless jm_is_valid_exreg($extract, undef, undef);
    return 1;
}

# check $Regex (.'$Regex')
sub json_model_10($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Regex'
    # .'$Regex'.'|'.0
    # "/^/($EXREG)/[a-z]*X[a-z]*$/X"
    # .'$Regex'.'|'.1
    # "/^/($REGEX)/[a-z]*$/X"
    return jm_is_string($val) && (_jm_xre_1($val, undef, undef) || _jm_xre_0($val, undef, undef));
}

sub _jm_re_6($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\..+$/;
    return $res;
}

# check $Name (.'$Name')
sub json_model_11($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Name'
    # "/^\\..+$/"
    return jm_is_string($val) && _jm_re_6($val, undef, undef);
}

# check $CmpValue (.'$CmpValue')
sub json_model_12($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$CmpValue'
    # .'$CmpValue'.'|'.0
    # .'$CmpValue'.'|'.1
    # .'$CmpValue'.'|'.2
    return jm_is_integer($val) || jm_is_numeric($val) || jm_is_string($val);
}

# check $EqValue (.'$EqValue')
sub json_model_13($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$EqValue'
    # .'$EqValue'.'|'.0
    # .'$EqValue'.'|'.1
    # .'$EqValue'.'|'.2
    return !defined($val) || jm_is_boolean($val) || json_model_12($val, undef, undef);
}

# check $ValModel (.'$ValModel')
sub json_model_14($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$ValModel'
    # .'$ValModel'.'|'.0
    # .'$ValModel'.'|'.1
    # .'$ValModel'.'|'.2
    # .'$ValModel'.'|'.3
    # .'$ValModel'.'|'.4
    # .'$ValModel'.'|'.5
    # .'$ValModel'.'|'.6
    # .'$ValModel'.'|'.7
    return !defined($val) || jm_is_boolean($val) && $val == 1 || jm_is_integer($val) && $val == 0 || jm_is_integer($val) && $val == 1 || jm_is_integer($val) && $val == -1 || jm_is_numeric($val) && $val == 0.0 || jm_is_numeric($val) && $val == 1.0 || jm_is_numeric($val) && $val == -1.0;
}

# check $StrModel (.'$StrModel')
sub json_model_15($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$StrModel'
    # .'$StrModel'.'|'.0
    # .'$StrModel'.'|'.1
    # .'$StrModel'.'|'.2
    # .'$StrModel'.'|'.3
    # .'$StrModel'.'|'.4
    # .'$StrModel'.'|'.5
    return json_model_9($val, undef, undef) || json_model_8($val, undef, undef) || json_model_10($val, undef, undef) || json_model_3($val, undef, undef) || json_model_7($val, undef, undef) || jm_is_string($val) && $val eq "";
}

# check $Any (.'$Any')
sub json_model_16($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Any'
    return 1;
}

# check $None (.'$None')
sub json_model_17($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$None'
    return 0;
}

# check $CModel (.'$CModel')
sub json_model_18($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$CModel'
    # .'$CModel'.'|'.0
    # .'$CModel'.'|'.1
    return json_model_33($val, undef, undef) || json_model_5($val, undef, undef);
}

# check $Array (.'$Array')
sub json_model_19($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Array'
    my $res = jm_is_array($val);
    if ($res)
    {
        for my $arr_0_idx (0 .. $#$val)
        {
            my $arr_0_item = $$val[$arr_0_idx];
            # .'$Array'.0
            $res = json_model_18($arr_0_item, undef, undef);
            last unless $res;
        }
    }
    return $res;
}

# check $Common (.'$Common')
sub json_model_20($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Common'
    return json_model_6($val, undef, undef);
}

sub _jm_re_7($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^(<=|>=|<|>)$/;
    return $res;
}

sub _jm_re_8($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^(=|!=)$/;
    return $res;
}

# check $Constraint (.'$Constraint')
sub json_model_21($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Constraint'
    return 0 unless jm_is_object($val);
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "\@")
        {
            # handle must @ property
            $must_count++;
            # .'$Constraint'.'@'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if ($prop eq "!")
        {
            # handle may ! property
            # .'$Constraint'.'!'
            $res = jm_is_boolean($pval);
            return 0 unless $res;
            next;
        }
        if (_jm_re_7($prop, undef, undef))
        {
            # handle 2 re props
            # .'$Constraint'.'/^(<=|>=|<|>)$/'
            $res = json_model_12($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (_jm_re_8($prop, undef, undef))
        {
            # handle 2 re props
            # .'$Constraint'.'/^(=|!=)$/'
            $res = json_model_13($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# check $Or (.'$Or')
sub json_model_22($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Or'
    # check close must only props
    return 0 unless jm_is_object($val);
    return 0 if jm_obj_size($val) != 1;
    my $pval;
    return 0 unless exists $$val{"|"};
    $pval = $$val{"|"};
    # .'$Or'.'|'
    my $res = jm_is_array($pval);
    if ($res)
    {
        for my $arr_1_idx (0 .. $#$pval)
        {
            my $arr_1_item = $$pval[$arr_1_idx];
            # .'$Or'.'|'.0
            $res = json_model_18($arr_1_item, undef, undef);
            last unless $res;
        }
    }
    return $res;
}

# check $And (.'$And')
sub json_model_23($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$And'
    # check close must only props
    return 0 unless jm_is_object($val);
    return 0 if jm_obj_size($val) != 1;
    my $pval;
    return 0 unless exists $$val{"&"};
    $pval = $$val{"&"};
    # .'$And'.'&'
    my $res = jm_is_array($pval);
    if ($res)
    {
        for my $arr_2_idx (0 .. $#$pval)
        {
            my $arr_2_item = $$pval[$arr_2_idx];
            # .'$And'.'&'.0
            $res = json_model_18($arr_2_item, undef, undef);
            last unless $res;
        }
    }
    return $res;
}

# check $Xor (.'$Xor')
sub json_model_24($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Xor'
    # check close must only props
    return 0 unless jm_is_object($val);
    return 0 if jm_obj_size($val) != 1;
    my $pval;
    return 0 unless exists $$val{"^"};
    $pval = $$val{"^"};
    # .'$Xor'.'^'
    my $res = jm_is_array($pval);
    if ($res)
    {
        for my $arr_3_idx (0 .. $#$pval)
        {
            my $arr_3_item = $$pval[$arr_3_idx];
            # .'$Xor'.'^'.0
            $res = json_model_18($arr_3_item, undef, undef);
            last unless $res;
        }
    }
    return $res;
}

# check $Add (.'$Add')
sub json_model_25($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Add'
    # check close must only props
    return 0 unless jm_is_object($val);
    return 0 if jm_obj_size($val) != 1;
    my $pval;
    return 0 unless exists $$val{"+"};
    $pval = $$val{"+"};
    # .'$Add'.'+'
    my $res = jm_is_array($pval);
    if ($res)
    {
        for my $arr_4_idx (0 .. $#$pval)
        {
            my $arr_4_item = $$pval[$arr_4_idx];
            # .'$Add'.'+'.0
            $res = json_model_18($arr_4_item, undef, undef);
            last unless $res;
        }
    }
    return $res;
}

sub _jm_re_9($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^[?!]/;
    return $res;
}

# check $Prop (.'$Prop')
sub json_model_26($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Prop'
    # .'$Prop'.'|'.0
    # "/^[?!]/"
    # .'$Prop'.'|'.1
    # .'$Prop'.'|'.2
    # .'$Prop'.'|'.3
    # .'$Prop'.'|'.4
    # .'$Prop'.'|'.5
    return jm_is_string($val) && _jm_re_9($val, undef, undef) || json_model_9($val, undef, undef) || json_model_10($val, undef, undef) || json_model_7($val, undef, undef) || json_model_11($val, undef, undef) || jm_is_string($val) && $val eq "";
}

# check $Obj (.'$Obj')
sub json_model_27($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Obj'
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if (json_model_26($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Obj'.'$Prop'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Elem (.'$Elem')
sub json_model_28($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Elem'
    # .'$Elem'.'|'.0
    # .'$Elem'.'|'.1
    # .'$Elem'.'|'.2
    # .'$Elem'.'|'.3
    # .'$Elem'.'|'.4
    # .'$Elem'.'|'.5
    return json_model_21($val, undef, undef) || json_model_22($val, undef, undef) || json_model_23($val, undef, undef) || json_model_24($val, undef, undef) || json_model_25($val, undef, undef) || json_model_27($val, undef, undef);
}

# object .'$Element'.'|'.5
sub _jm_obj_0($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Element'.'|'.5.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (json_model_26($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Element'.'|'.5.'$Prop'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Element'.'|'.5.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Element'.'|'.4
sub _jm_obj_1($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "+")
        {
            # handle must + property
            $must_count++;
            # .'$Element'.'|'.4.'+'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_5_idx (0 .. $#$pval)
                {
                    my $arr_5_item = $$pval[$arr_5_idx];
                    # .'$Element'.'|'.4.'+'.0
                    $res = json_model_18($arr_5_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Element'.'|'.4.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Element'.'|'.4.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Element'.'|'.3
sub _jm_obj_2($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "^")
        {
            # handle must ^ property
            $must_count++;
            # .'$Element'.'|'.3.'^'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_6_idx (0 .. $#$pval)
                {
                    my $arr_6_item = $$pval[$arr_6_idx];
                    # .'$Element'.'|'.3.'^'.0
                    $res = json_model_18($arr_6_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Element'.'|'.3.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Element'.'|'.3.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Element'.'|'.2
sub _jm_obj_3($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "&")
        {
            # handle must & property
            $must_count++;
            # .'$Element'.'|'.2.'&'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_7_idx (0 .. $#$pval)
                {
                    my $arr_7_item = $$pval[$arr_7_idx];
                    # .'$Element'.'|'.2.'&'.0
                    $res = json_model_18($arr_7_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Element'.'|'.2.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Element'.'|'.2.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Element'.'|'.1
sub _jm_obj_4($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "|")
        {
            # handle must | property
            $must_count++;
            # .'$Element'.'|'.1.'|'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_8_idx (0 .. $#$pval)
                {
                    my $arr_8_item = $$pval[$arr_8_idx];
                    # .'$Element'.'|'.1.'|'.0
                    $res = json_model_18($arr_8_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Element'.'|'.1.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Element'.'|'.1.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Element'.'|'.0
sub _jm_obj_5($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "\@")
        {
            # handle must @ property
            $must_count++;
            # .'$Element'.'|'.0.'@'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Element'.'|'.0.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "!")
        {
            # handle may ! property
            # .'$Element'.'|'.0.'!'
            $res = jm_is_boolean($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 3 re props
            # .'$Element'.'|'.0.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_7($prop, undef, undef))
        {
            # handle 3 re props
            # .'$Element'.'|'.0.'/^(<=|>=|<|>)$/'
            $res = json_model_12($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (_jm_re_8($prop, undef, undef))
        {
            # handle 3 re props
            # .'$Element'.'|'.0.'/^(=|!=)$/'
            $res = json_model_13($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# check $Element (.'$Element')
sub json_model_29($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Element'
    # .'$Element'.'|'.0
    # .'$Element'.'|'.1
    # .'$Element'.'|'.2
    # .'$Element'.'|'.3
    # .'$Element'.'|'.4
    # .'$Element'.'|'.5
    return jm_is_object($val) && (_jm_obj_5($val, undef, undef) || _jm_obj_4($val, undef, undef) || _jm_obj_3($val, undef, undef) || _jm_obj_2($val, undef, undef) || _jm_obj_1($val, undef, undef) || _jm_obj_0($val, undef, undef));
}

# object .'$Trafo'.'~'
sub _jm_obj_6($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        # handle other props
        # .'$Trafo'.'~'.''
        $res = jm_is_string($pval);
        return 0 unless $res;
    }
    return 1;
}

# check $Trafo (.'$Trafo')
sub json_model_30($$$)
{
    my ($val, $path, $rep) = @_;
    # remove, rename or add stuff
    # .'$Trafo'
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "/")
        {
            # handle may / property
            # .'$Trafo'.'/'
            # .'$Trafo'.'/'.'|'.0
            # .'$Trafo'.'/'.'|'.1
            $res = json_model_13($pval, undef, undef) || jm_is_array($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "~")
        {
            # handle may ~ property
            # .'$Trafo'.'~'
            $res = _jm_obj_6($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "*")
        {
            # handle may * property
            # .'$Trafo'.'*'
            $res = 1;
            next;
        }
        return 0;
    }
    return 1;
}

# object .'$Transformation'.'|'.1.'~'
sub _jm_obj_8($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        # handle other props
        # .'$Transformation'.'|'.1.'~'.''
        $res = jm_is_string($pval);
        return 0 unless $res;
    }
    return 1;
}

# object .'$Transformation'.'|'.1
sub _jm_obj_7($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Transformation'.'|'.1.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "/")
        {
            # handle may / property
            # .'$Transformation'.'|'.1.'/'
            # .'$Transformation'.'|'.1.'/'.'|'.0
            # .'$Transformation'.'|'.1.'/'.'|'.1
            $res = json_model_13($pval, undef, undef) || jm_is_array($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "~")
        {
            # handle may ~ property
            # .'$Transformation'.'|'.1.'~'
            $res = _jm_obj_8($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "*")
        {
            # handle may * property
            # .'$Transformation'.'|'.1.'*'
            $res = 1;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Transformation'.'|'.1.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Transformation (.'$Transformation')
sub json_model_31($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Transformation'
    # .'$Transformation'.'|'.0
    # .'$Transformation'.'|'.1
    return json_model_33($val, undef, undef) || _jm_obj_7($val, undef, undef);
}

# check $ScalarModel (.'$ScalarModel')
sub json_model_32($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$ScalarModel'
    # .'$ScalarModel'.'|'.0
    # .'$ScalarModel'.'|'.1
    return json_model_14($val, undef, undef) || json_model_15($val, undef, undef);
}

# check $Model (.'$Model')
sub json_model_33($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Model'
    # .'$Model'.'|'.0
    # .'$Model'.'|'.1
    # .'$Model'.'|'.2
    return json_model_32($val, undef, undef) || json_model_19($val, undef, undef) || json_model_29($val, undef, undef);
}

# check $Defs (.'$Defs')
sub json_model_34($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Defs'
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "")
        {
            # handle may  property
            # .'$Defs'.''
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_4($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Defs'.'$Identifier'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

sub _jm_re_10($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^([#|&^+\/*@~=\$%]|[<>!]=?)$/;
    return $res;
}

# check $Rename (.'$Rename')
sub json_model_35($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Rename'
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if (json_model_11($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Rename'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_10($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

sub _jm_re_11($$$)
{
    my ($val, $path, $rep) = @_;
    my $res = $val =~ /^\$.*$/;
    return $res;
}

# check $Rewrite (.'$Rewrite')
sub json_model_36($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Rewrite'
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if (_jm_re_11($prop, undef, undef))
        {
            # handle 1 re props
            # .'$Rewrite'.'/^\\$.*$/'
            $res = json_model_31($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $Import (.'$Import')
sub json_model_37($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Import'
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "<")
        {
            # handle may < property
            # .'$Import'.'<'
            # .'$Import'.'<'.'|'.0
            $res = json_model_7($pval, undef, undef);
            unless ($res)
            {
                # .'$Import'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_9_idx (0 .. $#$pval)
                    {
                        my $arr_9_item = $$pval[$arr_9_idx];
                        # .'$Import'.'<'.'|'.1.0
                        $res = json_model_7($arr_9_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        return 0;
    }
    return 1;
}

# object .'$RootOnly'.'$'
sub _jm_obj_9($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$RootOnly'.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$RootOnly'.'$'.''
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_4($prop, undef, undef))
        {
            # handle 1 key props
            # .'$RootOnly'.'$'.'$Identifier'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$RootOnly'.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$RootOnly'.'%'
sub _jm_obj_10($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$RootOnly'.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$RootOnly'.'%'.'<'
            # .'$RootOnly'.'%'.'<'.'|'.0
            $res = json_model_7($pval, undef, undef);
            unless ($res)
            {
                # .'$RootOnly'.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_10_idx (0 .. $#$pval)
                    {
                        my $arr_10_item = $$pval[$arr_10_idx];
                        # .'$RootOnly'.'%'.'<'.'|'.1.0
                        $res = json_model_7($arr_10_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_11($prop, undef, undef))
        {
            # handle 1 key props
            # .'$RootOnly'.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_10($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 2 re props
            # .'$RootOnly'.'%'.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_11($prop, undef, undef))
        {
            # handle 2 re props
            # .'$RootOnly'.'%'.'/^\\$.*$/'
            $res = json_model_31($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# check $RootOnly (.'$RootOnly')
sub json_model_38($$$)
{
    my ($val, $path, $rep) = @_;
    # symbols that can only appear at the model's root
    # .'$RootOnly'
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$RootOnly'.'~'
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$RootOnly'.'$'
            $res = _jm_obj_9($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$RootOnly'.'%'
            $res = _jm_obj_10($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        return 0;
    }
    return 1;
}

# object .'$Root'.'|'.5.'$'
sub _jm_obj_12($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.5.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Root'.'|'.5.'$'.''
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_4($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.5.'$'.'$Identifier'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.5.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.5.'%'
sub _jm_obj_13($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.5.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Root'.'|'.5.'%'.'<'
            # .'$Root'.'|'.5.'%'.'<'.'|'.0
            $res = json_model_7($pval, undef, undef);
            unless ($res)
            {
                # .'$Root'.'|'.5.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_11_idx (0 .. $#$pval)
                    {
                        my $arr_11_item = $$pval[$arr_11_idx];
                        # .'$Root'.'|'.5.'%'.'<'.'|'.1.0
                        $res = json_model_7($arr_11_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_11($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.5.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_10($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 2 re props
            # .'$Root'.'|'.5.'%'.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_11($prop, undef, undef))
        {
            # handle 2 re props
            # .'$Root'.'|'.5.'%'.'/^\\$.*$/'
            $res = json_model_31($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.5
sub _jm_obj_11($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Root'.'|'.5.'~'
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Root'.'|'.5.'$'
            $res = _jm_obj_12($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Root'.'|'.5.'%'
            $res = _jm_obj_13($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.5.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (json_model_26($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.5.'$Prop'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.5.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.4.'$'
sub _jm_obj_15($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.4.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Root'.'|'.4.'$'.''
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_4($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.4.'$'.'$Identifier'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.4.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.4.'%'
sub _jm_obj_16($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.4.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Root'.'|'.4.'%'.'<'
            # .'$Root'.'|'.4.'%'.'<'.'|'.0
            $res = json_model_7($pval, undef, undef);
            unless ($res)
            {
                # .'$Root'.'|'.4.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_13_idx (0 .. $#$pval)
                    {
                        my $arr_13_item = $$pval[$arr_13_idx];
                        # .'$Root'.'|'.4.'%'.'<'.'|'.1.0
                        $res = json_model_7($arr_13_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_11($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.4.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_10($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 2 re props
            # .'$Root'.'|'.4.'%'.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_11($prop, undef, undef))
        {
            # handle 2 re props
            # .'$Root'.'|'.4.'%'.'/^\\$.*$/'
            $res = json_model_31($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.4
sub _jm_obj_14($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "+")
        {
            # handle must + property
            $must_count++;
            # .'$Root'.'|'.4.'+'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_12_idx (0 .. $#$pval)
                {
                    my $arr_12_item = $$pval[$arr_12_idx];
                    # .'$Root'.'|'.4.'+'.0
                    $res = json_model_18($arr_12_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Root'.'|'.4.'~'
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Root'.'|'.4.'$'
            $res = _jm_obj_15($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Root'.'|'.4.'%'
            $res = _jm_obj_16($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.4.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.4.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Root'.'|'.3.'$'
sub _jm_obj_18($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.3.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Root'.'|'.3.'$'.''
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_4($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.3.'$'.'$Identifier'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.3.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.3.'%'
sub _jm_obj_19($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.3.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Root'.'|'.3.'%'.'<'
            # .'$Root'.'|'.3.'%'.'<'.'|'.0
            $res = json_model_7($pval, undef, undef);
            unless ($res)
            {
                # .'$Root'.'|'.3.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_15_idx (0 .. $#$pval)
                    {
                        my $arr_15_item = $$pval[$arr_15_idx];
                        # .'$Root'.'|'.3.'%'.'<'.'|'.1.0
                        $res = json_model_7($arr_15_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_11($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.3.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_10($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 2 re props
            # .'$Root'.'|'.3.'%'.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_11($prop, undef, undef))
        {
            # handle 2 re props
            # .'$Root'.'|'.3.'%'.'/^\\$.*$/'
            $res = json_model_31($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.3
sub _jm_obj_17($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "^")
        {
            # handle must ^ property
            $must_count++;
            # .'$Root'.'|'.3.'^'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_14_idx (0 .. $#$pval)
                {
                    my $arr_14_item = $$pval[$arr_14_idx];
                    # .'$Root'.'|'.3.'^'.0
                    $res = json_model_18($arr_14_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Root'.'|'.3.'~'
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Root'.'|'.3.'$'
            $res = _jm_obj_18($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Root'.'|'.3.'%'
            $res = _jm_obj_19($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.3.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.3.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Root'.'|'.2.'$'
sub _jm_obj_21($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.2.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Root'.'|'.2.'$'.''
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_4($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.2.'$'.'$Identifier'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.2.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.2.'%'
sub _jm_obj_22($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.2.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Root'.'|'.2.'%'.'<'
            # .'$Root'.'|'.2.'%'.'<'.'|'.0
            $res = json_model_7($pval, undef, undef);
            unless ($res)
            {
                # .'$Root'.'|'.2.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_17_idx (0 .. $#$pval)
                    {
                        my $arr_17_item = $$pval[$arr_17_idx];
                        # .'$Root'.'|'.2.'%'.'<'.'|'.1.0
                        $res = json_model_7($arr_17_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_11($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.2.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_10($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 2 re props
            # .'$Root'.'|'.2.'%'.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_11($prop, undef, undef))
        {
            # handle 2 re props
            # .'$Root'.'|'.2.'%'.'/^\\$.*$/'
            $res = json_model_31($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.2
sub _jm_obj_20($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "&")
        {
            # handle must & property
            $must_count++;
            # .'$Root'.'|'.2.'&'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_16_idx (0 .. $#$pval)
                {
                    my $arr_16_item = $$pval[$arr_16_idx];
                    # .'$Root'.'|'.2.'&'.0
                    $res = json_model_18($arr_16_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Root'.'|'.2.'~'
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Root'.'|'.2.'$'
            $res = _jm_obj_21($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Root'.'|'.2.'%'
            $res = _jm_obj_22($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.2.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.2.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Root'.'|'.1.'$'
sub _jm_obj_24($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.1.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Root'.'|'.1.'$'.''
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_4($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.1.'$'.'$Identifier'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.1.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.1.'%'
sub _jm_obj_25($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.1.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Root'.'|'.1.'%'.'<'
            # .'$Root'.'|'.1.'%'.'<'.'|'.0
            $res = json_model_7($pval, undef, undef);
            unless ($res)
            {
                # .'$Root'.'|'.1.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_19_idx (0 .. $#$pval)
                    {
                        my $arr_19_item = $$pval[$arr_19_idx];
                        # .'$Root'.'|'.1.'%'.'<'.'|'.1.0
                        $res = json_model_7($arr_19_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_11($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.1.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_10($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 2 re props
            # .'$Root'.'|'.1.'%'.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_11($prop, undef, undef))
        {
            # handle 2 re props
            # .'$Root'.'|'.1.'%'.'/^\\$.*$/'
            $res = json_model_31($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.1
sub _jm_obj_23($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "|")
        {
            # handle must | property
            $must_count++;
            # .'$Root'.'|'.1.'|'
            $res = jm_is_array($pval);
            if ($res)
            {
                for my $arr_18_idx (0 .. $#$pval)
                {
                    my $arr_18_item = $$pval[$arr_18_idx];
                    # .'$Root'.'|'.1.'|'.0
                    $res = json_model_18($arr_18_item, undef, undef);
                    last unless $res;
                }
            }
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Root'.'|'.1.'~'
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Root'.'|'.1.'$'
            $res = _jm_obj_24($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Root'.'|'.1.'%'
            $res = _jm_obj_25($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.1.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.1.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# object .'$Root'.'|'.0.'$'
sub _jm_obj_27($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.0.'$'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "")
        {
            # handle may  property
            # .'$Root'.'|'.0.'$'.''
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if (json_model_4($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.0.'$'.'$Identifier'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 1 re props
            # .'$Root'.'|'.0.'$'.'/^#/'
            $res = 1;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.0.'%'
sub _jm_obj_28($$$)
{
    my ($val, $path, $rep) = @_;
    return 0 unless jm_is_object($val);
    my $res;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.0.'%'.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "<")
        {
            # handle may < property
            # .'$Root'.'|'.0.'%'.'<'
            # .'$Root'.'|'.0.'%'.'<'.'|'.0
            $res = json_model_7($pval, undef, undef);
            unless ($res)
            {
                # .'$Root'.'|'.0.'%'.'<'.'|'.1
                $res = jm_is_array($pval);
                if ($res)
                {
                    for my $arr_20_idx (0 .. $#$pval)
                    {
                        my $arr_20_item = $$pval[$arr_20_idx];
                        # .'$Root'.'|'.0.'%'.'<'.'|'.1.0
                        $res = json_model_7($arr_20_item, undef, undef);
                        last unless $res;
                    }
                }
            }
            return 0 unless $res;
            next;
        }
        if (json_model_11($prop, undef, undef))
        {
            # handle 1 key props
            # .'$Root'.'|'.0.'%'.'$Name'
            # "/^([#|&^+/*@~=$%]|[<>!]=?)$/"
            $res = jm_is_string($pval) && _jm_re_10($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (jm_starts_with($prop, "#"))
        {
            # handle 2 re props
            # .'$Root'.'|'.0.'%'.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_11($prop, undef, undef))
        {
            # handle 2 re props
            # .'$Root'.'|'.0.'%'.'/^\\$.*$/'
            $res = json_model_31($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return 1;
}

# object .'$Root'.'|'.0
sub _jm_obj_26($$$)
{
    my ($val, $path, $rep) = @_;
    # value known to be an object
    my $res;
    my $must_count = 0;
    scalar keys %$val;
    while (my ($prop, $pval) = each %$val)
    {
        if ($prop eq "\@")
        {
            # handle must @ property
            $must_count++;
            # .'$Root'.'|'.0.'@'
            $res = json_model_33($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        if ($prop eq "~")
        {
            # handle may ~ property
            # .'$Root'.'|'.0.'~'
            $res = json_model_2($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "\$")
        {
            # handle may $ property
            # .'$Root'.'|'.0.'$'
            $res = _jm_obj_27($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "%")
        {
            # handle may % property
            # .'$Root'.'|'.0.'%'
            $res = _jm_obj_28($pval, undef, undef);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "#")
        {
            # handle may # property
            # .'$Root'.'|'.0.'#'
            $res = jm_is_string($pval);
            return 0 unless $res;
            next;
        }
        elsif ($prop eq "!")
        {
            # handle may ! property
            # .'$Root'.'|'.0.'!'
            $res = jm_is_boolean($pval);
            return 0 unless $res;
            next;
        }
        if (jm_starts_with($prop, "#"))
        {
            # handle 3 re props
            # .'$Root'.'|'.0.'/^#/'
            $res = 1;
        }
        elsif (_jm_re_7($prop, undef, undef))
        {
            # handle 3 re props
            # .'$Root'.'|'.0.'/^(<=|>=|<|>)$/'
            $res = json_model_12($pval, undef, undef);
            return 0 unless $res;
        }
        elsif (_jm_re_8($prop, undef, undef))
        {
            # handle 3 re props
            # .'$Root'.'|'.0.'/^(=|!=)$/'
            $res = json_model_13($pval, undef, undef);
            return 0 unless $res;
        }
        else
        {
            return 0;
        }
    }
    return $must_count == 1;
}

# check $Root (.'$Root')
sub json_model_39($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$Root'
    # .'$Root'.'|'.0
    # .'$Root'.'|'.1
    # .'$Root'.'|'.2
    # .'$Root'.'|'.3
    # .'$Root'.'|'.4
    # .'$Root'.'|'.5
    return jm_is_object($val) && (_jm_obj_26($val, undef, undef) || _jm_obj_23($val, undef, undef) || _jm_obj_20($val, undef, undef) || _jm_obj_17($val, undef, undef) || _jm_obj_14($val, undef, undef) || _jm_obj_11($val, undef, undef));
}

# check $RootModel (.'$RootModel')
sub json_model_40($$$)
{
    my ($val, $path, $rep) = @_;
    # .'$RootModel'
    # .'$RootModel'.'|'.0
    # .'$RootModel'.'|'.1
    # .'$RootModel'.'|'.2
    return json_model_32($val, undef, undef) || json_model_19($val, undef, undef) || json_model_39($val, undef, undef);
}

# check $ (.)
sub json_model_1($$$)
{
    my ($val, $path, $rep) = @_;
    # .
    return json_model_40($val, undef, undef);
}


# initialization of global variables

our $initialized = 0;

sub check_model_init()
{
    if (!$initialized)
    {
        $initialized = 1;
        %_jm_cst_0 = (
            "\$DATE" => 1,
            "\$TIME" => 1,
            "\$DATETIME" => 1,
            "\$URL" => 1,
            "\$URI" => 1,
            "\$UUID" => 1,
            "\$EMAIL" => 1,
            "\$REGEX" => 1,
            "\$EXREG" => 1,
            "\$NULL" => 1,
            "\$BOOL" => 1,
            "\$FLOAT" => 1,
            "\$F16" => 1,
            "\$F32" => 1,
            "\$F64" => 1,
            "\$NUMBER" => 1,
            "\$INT" => 1,
            "\$INTEGER" => 1,
            "\$I8" => 1,
            "\$I16" => 1,
            "\$I32" => 1,
            "\$I64" => 1,
            "\$U8" => 1,
            "\$U16" => 1,
            "\$U32" => 1,
            "\$U64" => 1,
            "\$STRING" => 1,
            "\$NONE" => 1,
            "\$ANY" => 1,
        );
        %check_model_map = (
            "" => \&json_model_40,
            "Url" => \&json_model_2,
            "PreDef" => \&json_model_3,
            "Identifier" => \&json_model_4,
            "ArrayComment" => \&json_model_5,
            "ObjectComment" => \&json_model_6,
            "Ref" => \&json_model_7,
            "ValConst" => \&json_model_8,
            "StrConst" => \&json_model_9,
            "Regex" => \&json_model_10,
            "Name" => \&json_model_11,
            "CmpValue" => \&json_model_12,
            "EqValue" => \&json_model_13,
            "ValModel" => \&json_model_14,
            "StrModel" => \&json_model_15,
            "Any" => \&json_model_16,
            "None" => \&json_model_17,
            "CModel" => \&json_model_18,
            "Array" => \&json_model_19,
            "Common" => \&json_model_6,
            "Constraint" => \&json_model_21,
            "Or" => \&json_model_22,
            "And" => \&json_model_23,
            "Xor" => \&json_model_24,
            "Add" => \&json_model_25,
            "Prop" => \&json_model_26,
            "Obj" => \&json_model_27,
            "Elem" => \&json_model_28,
            "Element" => \&json_model_29,
            "Trafo" => \&json_model_30,
            "Transformation" => \&json_model_31,
            "ScalarModel" => \&json_model_32,
            "Model" => \&json_model_33,
            "Defs" => \&json_model_34,
            "Rename" => \&json_model_35,
            "Rewrite" => \&json_model_36,
            "Import" => \&json_model_37,
            "RootOnly" => \&json_model_38,
            "Root" => \&json_model_39,
            "RootModel" => \&json_model_40,
        );
    }
}

sub check_model_free()
{
    if ($initialized)
    {
        $initialized = 0;
        %_jm_cst_0 = ();
        %check_model_map = ();
    }
}

sub check_model_mapper($)
{
    my ($name) = @_;
    die "unexpected model name \"$name\"" unless exists $check_model_map{$name};
    return $check_model_map{$name};
}

sub check_model($$$)
{
    my ($json, $name, $rep) = @_;
    die "unexpected model name \"$name\"" unless exists $check_model_map{$name};
    my $path = defined $rep ? [] : undef;
    return $check_model_map{$name}($json, $path, $rep);
}

#
# SCRIPT EXECUTION
#
check_model_init();
jm_main(\&check_model, \%check_model_map, JMC_VERSION);
check_model_free();
