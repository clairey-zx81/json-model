...
static bool _jm_f_0(const json_t* val, Path* path, Report* rep)
{
    ...
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a 1 strict int [$.'$A'.'|'.1.b]", path);
    }
    return res;
}
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if ((pfun = _jm_obj_2_must(prop)))
        {
            ...
            if (pfun != NULL)
            {
                ...
                {
                    if (rep) jm_report_add_entry(rep, "invalid must property value [$.'$B'.'|'.0]", (path ? &lpath_2 : NULL));
                    return false;
                }
            }
        }
