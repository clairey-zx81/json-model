...
sub json_model_1($$$)
{
    ...
    if ($res)
    {
        ...
        {
            $res = exists $$val{'c'} && _jm_obj_3($val, undef, undef) || exists $$val{'d'} && _jm_obj_2($val, undef, undef) || exists $$val{'e'} && _jm_obj_1($val, undef, undef) || exists $$val{'c'} && _jm_obj_0($val, undef, undef);
        }
    }
    return $res;
}
