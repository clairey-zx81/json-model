//
// Generated by JSON Model Compiler version 2
// see https://github.com/clairey-zx81/json-model
//

// regular expression engine
#include <stddef.h>
#include <cre2.h>

#include <json-model.h>
#define JSON_MODEL_VERSION "2"

static bool json_model_2(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_0(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_1(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_2(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_3(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_4(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_5(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_6(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_7(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_8(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_propmap_t _jm_obj_0_map_tab[9];
static bool json_model_3(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_9(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_10(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_11(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_12(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_13(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_14(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_15(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_propmap_t _jm_obj_3_map_tab[7];
static bool json_model_4(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool json_model_5(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool json_model_6(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool json_model_7(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_constant_t _jm_cst_0[7];
static bool json_model_8(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool json_model_9(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool json_model_10(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool json_model_11(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_16(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_17(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_18(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_19(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_20(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_21(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_22(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_23(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_24(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_25(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_26(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_27(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_28(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_29(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_30(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_31(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_32(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_33(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_constant_t _jm_cst_1[7];
static bool _jm_f_34(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_35(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_propmap_t _jm_obj_7_map_tab[20];
static bool json_model_12(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool json_model_13(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_36(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_37(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_38(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_39(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_40(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_41(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_42(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_43(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_44(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_45(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_46(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_47(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_48(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_49(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_50(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_51(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_propmap_t _jm_obj_9_map_tab[16];
static bool json_model_14(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_52(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_53(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_54(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_55(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_56(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_57(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_58(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_59(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_60(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_61(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_62(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_63(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_64(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_65(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_66(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_67(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_68(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_69(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_70(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_71(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_72(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_73(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_74(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_75(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_76(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_77(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_78(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_79(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_80(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_81(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_82(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_83(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_84(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_85(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_86(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_87(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_88(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_89(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_90(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_91(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_92(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_93(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_94(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_95(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_96(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_97(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_98(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_99(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_100(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_101(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_102(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_103(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_104(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_constant_t _jm_cst_2[7];
static bool _jm_f_105(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_106(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_107(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_108(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_109(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_propmap_t _jm_obj_15_map_tab[58];
static bool json_model_15(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_110(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_111(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_112(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_113(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_114(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_115(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_116(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_117(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_118(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_119(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_120(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_121(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_122(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_123(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_124(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_125(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_126(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_127(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_128(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_129(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_130(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_131(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_132(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_133(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_134(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_135(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_136(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_137(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_138(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_139(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_140(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_141(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_142(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_143(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_144(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_145(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_146(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_147(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_148(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_149(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_150(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_151(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_152(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_153(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_154(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_155(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_156(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_157(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_158(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_159(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_160(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_161(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_162(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_constant_t _jm_cst_3[7];
static bool _jm_f_163(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_164(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_165(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_166(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool _jm_f_167(const json_t *val, jm_path_t *path, jm_report_t *rep);
static jm_propmap_t _jm_obj_24_map_tab[58];
static bool json_model_16(const json_t *val, jm_path_t *path, jm_report_t *rep);
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep);
jm_propmap_t check_model_map_tab[16];
const size_t check_model_map_size = 16;

// check $URI-REFERENCE (.'$URI-REFERENCE')
static bool json_model_2(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$URI-REFERENCE'
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$URI-REFERENCE']", path);
    }
    return res;
}

// check _jm_obj_0_map_$anchor (.'$core'.'$anchor')
static bool _jm_f_0(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$anchor'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$core'.'$anchor']", path);
    }
    return res;
}

// check _jm_obj_0_map_$comment (.'$core'.'$comment')
static bool _jm_f_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$comment'
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$core'.'$comment']", path);
    }
    return res;
}

// object .'$core'.'$defs'
static INLINE bool _jm_obj_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$core'.'$defs']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_1 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$core'.'$defs'.''
        res = json_model_16(pval, (path ? &lpath_1 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$core'.'$defs'.'']", (path ? &lpath_1 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_0_map_$defs (.'$core'.'$defs')
static bool _jm_f_2(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$defs'
    res = _jm_obj_1(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$core'.'$defs']", path);
    }
    return res;
}

// check _jm_obj_0_map_$dynamicAnchor (.'$core'.'$dynamicAnchor')
static bool _jm_f_3(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$dynamicAnchor'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$core'.'$dynamicAnchor']", path);
    }
    return res;
}

// check _jm_obj_0_map_$dynamicRef (.'$core'.'$dynamicRef')
static bool _jm_f_4(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$dynamicRef'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$core'.'$dynamicRef']", path);
    }
    return res;
}

// check _jm_obj_0_map_$id (.'$core'.'$id')
static bool _jm_f_5(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$id'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$core'.'$id']", path);
    }
    return res;
}

// check _jm_obj_0_map_$ref (.'$core'.'$ref')
static bool _jm_f_6(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$ref'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$core'.'$ref']", path);
    }
    return res;
}

// check _jm_obj_0_map_$schema (.'$core'.'$schema')
static bool _jm_f_7(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$schema'
    res = jm_is_valid_url(json_string_value(val), path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI [.'$core'.'$schema']", path);
    }
    return res;
}

// object .'$core'.'$vocabulary'
static INLINE bool _jm_obj_2(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$core'.'$vocabulary']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_2 = (jm_path_t) { prop, 0, path, NULL };
        if (jm_is_valid_url(prop, (path ? &lpath_2 : NULL), rep))
        {
            // handle 1 key props
            // .'$core'.'$vocabulary'.'$URI'
            res = json_is_boolean(pval);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "not a bool [.'$core'.'$vocabulary'.'$URI']", (path ? &lpath_2 : NULL));
            }
            if (! res)
            {
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$core'.'$vocabulary']", (path ? &lpath_2 : NULL));
            return false;
        }
    }
    return true;
}

// check _jm_obj_0_map_$vocabulary (.'$core'.'$vocabulary')
static bool _jm_f_8(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'.'$vocabulary'
    res = _jm_obj_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$core'.'$vocabulary']", path);
    }
    return res;
}

static INLINE jm_check_fun_t _jm_obj_0_map(const char *pname)
{
    return jm_search_propmap(pname, _jm_obj_0_map_tab, 9);
}

// object .'$core'
static INLINE bool _jm_obj_0(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$core']", path);
        return false;
    }
    jm_check_fun_t pfun;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_0 = (jm_path_t) { prop, 0, path, NULL };
        if ((pfun = _jm_obj_0_map(prop)))
        {
            // handle 9 may props
            if (pfun != NULL && ! pfun(pval, (path ? &lpath_0 : NULL), rep))
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$core']", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$core']", (path ? &lpath_0 : NULL));
            return false;
        }
    }
    return true;
}

// check $core (.'$core')
static bool json_model_3(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$core'
    res = _jm_obj_0(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$core']", path);
    }
    return res;
}

// check _jm_obj_3_map_default (.'$meta'.default)
static bool _jm_f_9(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$meta'.default
    res = true;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $ANY [.'$meta'.default]", path);
    }
    return res;
}

// check _jm_obj_3_map_deprecated (.'$meta'.deprecated)
static bool _jm_f_10(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$meta'.deprecated
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$meta'.deprecated]", path);
    }
    return res;
}

// check _jm_obj_3_map_description (.'$meta'.description)
static bool _jm_f_11(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$meta'.description
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$meta'.description]", path);
    }
    return res;
}

// check _jm_obj_3_map_examples (.'$meta'.examples)
static bool _jm_f_12(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$meta'.examples
    res = json_is_array(val);
    if (res)
    {
        // accept any array
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$meta'.examples]", path);
    }
    return res;
}

// check _jm_obj_3_map_readOnly (.'$meta'.readOnly)
static bool _jm_f_13(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$meta'.readOnly
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$meta'.readOnly]", path);
    }
    return res;
}

// check _jm_obj_3_map_title (.'$meta'.title)
static bool _jm_f_14(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$meta'.title
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$meta'.title]", path);
    }
    return res;
}

// check _jm_obj_3_map_writeOnly (.'$meta'.writeOnly)
static bool _jm_f_15(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$meta'.writeOnly
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$meta'.writeOnly]", path);
    }
    return res;
}

static INLINE jm_check_fun_t _jm_obj_3_map(const char *pname)
{
    return jm_search_propmap(pname, _jm_obj_3_map_tab, 7);
}

// object .'$meta'
static INLINE bool _jm_obj_3(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$meta']", path);
        return false;
    }
    jm_check_fun_t pfun;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_3 = (jm_path_t) { prop, 0, path, NULL };
        if ((pfun = _jm_obj_3_map(prop)))
        {
            // handle 7 may props
            if (pfun != NULL && ! pfun(pval, (path ? &lpath_3 : NULL), rep))
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$meta']", (path ? &lpath_3 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$meta']", (path ? &lpath_3 : NULL));
            return false;
        }
    }
    return true;
}

// check $meta (.'$meta')
static bool json_model_4(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$meta'
    res = _jm_obj_3(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$meta']", path);
    }
    return res;
}

// object .'$content'
static INLINE bool _jm_obj_4(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$content']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_4 = (jm_path_t) { prop, 0, path, NULL };
        if (strcmp(prop, "contentEncoding") == 0)
        {
            // handle may contentEncoding property
            // .'$content'.contentEncoding
            res = json_is_string(pval);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected string [.'$content'.contentEncoding]", (path ? &lpath_4 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$content'.contentEncoding]", (path ? &lpath_4 : NULL));
                return false;
            }
        }
        else if (strcmp(prop, "contentMediaType") == 0)
        {
            // handle may contentMediaType property
            // .'$content'.contentMediaType
            res = json_is_string(pval);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected string [.'$content'.contentMediaType]", (path ? &lpath_4 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$content'.contentMediaType]", (path ? &lpath_4 : NULL));
                return false;
            }
        }
        else if (strcmp(prop, "contentSchema") == 0)
        {
            // handle may contentSchema property
            // .'$content'.contentSchema
            res = json_model_16(pval, (path ? &lpath_4 : NULL), rep);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$content'.contentSchema]", (path ? &lpath_4 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$content'.contentSchema]", (path ? &lpath_4 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$content']", (path ? &lpath_4 : NULL));
            return false;
        }
    }
    return true;
}

// check $content (.'$content')
static bool json_model_5(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$content'
    res = _jm_obj_4(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$content']", path);
    }
    return res;
}

// object .'$unevaluated'
static INLINE bool _jm_obj_5(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$unevaluated']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_5 = (jm_path_t) { prop, 0, path, NULL };
        if (strcmp(prop, "unevaluatedItems") == 0)
        {
            // handle may unevaluatedItems property
            // .'$unevaluated'.unevaluatedItems
            res = json_model_16(pval, (path ? &lpath_5 : NULL), rep);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$unevaluated'.unevaluatedItems]", (path ? &lpath_5 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$unevaluated'.unevaluatedItems]", (path ? &lpath_5 : NULL));
                return false;
            }
        }
        else if (strcmp(prop, "unevaluatedProperties") == 0)
        {
            // handle may unevaluatedProperties property
            // .'$unevaluated'.unevaluatedProperties
            res = json_model_16(pval, (path ? &lpath_5 : NULL), rep);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$unevaluated'.unevaluatedProperties]", (path ? &lpath_5 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$unevaluated'.unevaluatedProperties]", (path ? &lpath_5 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$unevaluated']", (path ? &lpath_5 : NULL));
            return false;
        }
    }
    return true;
}

// check $unevaluated (.'$unevaluated')
static bool json_model_6(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$unevaluated'
    res = _jm_obj_5(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$unevaluated']", path);
    }
    return res;
}

// object .'$format'
static INLINE bool _jm_obj_6(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$format']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_6 = (jm_path_t) { prop, 0, path, NULL };
        if (strcmp(prop, "format") == 0)
        {
            // handle may format property
            // .'$format'.format
            res = json_is_string(pval);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected string [.'$format'.format]", (path ? &lpath_6 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$format'.format]", (path ? &lpath_6 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$format']", (path ? &lpath_6 : NULL));
            return false;
        }
    }
    return true;
}

// check $format (.'$format')
static bool json_model_7(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$format'
    res = _jm_obj_6(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$format']", path);
    }
    return res;
}


// check $simpleTypes (.'$simpleTypes')
static bool json_model_8(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$simpleTypes'
    res = jm_json_is_scalar(val) && json_is_string(val) && jm_search_cst(&(jm_constant_t) { cst_is_string, { .s = json_string_value(val) } }, _jm_cst_0, 7);;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "value not in enum [.'$simpleTypes'.'|']", path);
    }
    return res;
}

// check $simpleTypesArray (.'$simpleTypesArray')
static bool json_model_9(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$simpleTypesArray'
    // .'$simpleTypesArray'.'@'
    res = json_is_array(val);
    if (res)
    {
        size_t arr_0_idx;
        json_t *arr_0_item;
        json_array_foreach(val, arr_0_idx, arr_0_item)
        {
            jm_path_t arr_0_lpath = (jm_path_t) { NULL, arr_0_idx, path, NULL };
            // .'$simpleTypesArray'.'@'.0
            res = json_model_8(arr_0_item, (path ? &arr_0_lpath : NULL), rep);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected $simpleTypes [.'$simpleTypesArray'.'@'.0]", (path ? &arr_0_lpath : NULL));
            }
            if (! res)
            {
                break;
            }
        }
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$simpleTypesArray'.'@']", path);
    }
    if (res)
    {
        int64_t ival_0 = json_array_size(val);
        res = jm_array_is_unique(val, path, rep) && ival_0 >= 1;
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "constraints failed [.'$simpleTypesArray']", path);
        }
    }
    return res;
}

// check $stringArray (.'$stringArray')
static bool json_model_10(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$stringArray'
    // .'$stringArray'.'@'
    res = json_is_array(val);
    if (res)
    {
        size_t arr_1_idx;
        json_t *arr_1_item;
        json_array_foreach(val, arr_1_idx, arr_1_item)
        {
            jm_path_t arr_1_lpath = (jm_path_t) { NULL, arr_1_idx, path, NULL };
            // .'$stringArray'.'@'.0
            res = json_is_string(arr_1_item);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected string [.'$stringArray'.'@'.0]", (path ? &arr_1_lpath : NULL));
            }
            if (! res)
            {
                break;
            }
        }
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$stringArray'.'@']", path);
    }
    if (res)
    {
        res = jm_array_is_unique(val, path, rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "constraints failed [.'$stringArray']", path);
        }
    }
    return res;
}

// check $nonNegativeInteger (.'$nonNegativeInteger')
static bool json_model_11(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$nonNegativeInteger'
    res = ((json_is_integer(val) || (json_is_real(val) && json_real_value(val) == ((int64_t) json_real_value(val))))) && json_number_value(val) >= 0;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a 0 loose int [.'$nonNegativeInteger']", path);
    }
    return res;
}

// check _jm_obj_7_map_const (.'$validation'.const)
static bool _jm_f_16(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.const
    res = true;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $ANY [.'$validation'.const]", path);
    }
    return res;
}

// object .'$validation'.dependentRequired
static INLINE bool _jm_obj_8(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$validation'.dependentRequired]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_8 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$validation'.dependentRequired.''
        res = json_model_10(pval, (path ? &lpath_8 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $stringArray [.'$validation'.dependentRequired.'']", (path ? &lpath_8 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_7_map_dependentRequired (.'$validation'.dependentRequired)
static bool _jm_f_17(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.dependentRequired
    res = _jm_obj_8(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$validation'.dependentRequired]", path);
    }
    return res;
}

// check _jm_obj_7_map_enum (.'$validation'.enum)
static bool _jm_f_18(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.enum
    res = json_is_array(val);
    if (res)
    {
        // accept any array
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$validation'.enum]", path);
    }
    return res;
}

// check _jm_obj_7_map_exclusiveMaximum (.'$validation'.exclusiveMaximum)
static bool _jm_f_19(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.exclusiveMaximum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$validation'.exclusiveMaximum]", path);
    }
    return res;
}

// check _jm_obj_7_map_exclusiveMinimum (.'$validation'.exclusiveMinimum)
static bool _jm_f_20(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.exclusiveMinimum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$validation'.exclusiveMinimum]", path);
    }
    return res;
}

// check _jm_obj_7_map_maxContains (.'$validation'.maxContains)
static bool _jm_f_21(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.maxContains
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$validation'.maxContains]", path);
    }
    return res;
}

// check _jm_obj_7_map_maxItems (.'$validation'.maxItems)
static bool _jm_f_22(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.maxItems
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$validation'.maxItems]", path);
    }
    return res;
}

// check _jm_obj_7_map_maxLength (.'$validation'.maxLength)
static bool _jm_f_23(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.maxLength
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$validation'.maxLength]", path);
    }
    return res;
}

// check _jm_obj_7_map_maxProperties (.'$validation'.maxProperties)
static bool _jm_f_24(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.maxProperties
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$validation'.maxProperties]", path);
    }
    return res;
}

// check _jm_obj_7_map_maximum (.'$validation'.maximum)
static bool _jm_f_25(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.maximum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$validation'.maximum]", path);
    }
    return res;
}

// check _jm_obj_7_map_minContains (.'$validation'.minContains)
static bool _jm_f_26(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.minContains
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$validation'.minContains]", path);
    }
    return res;
}

// check _jm_obj_7_map_minItems (.'$validation'.minItems)
static bool _jm_f_27(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.minItems
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$validation'.minItems]", path);
    }
    return res;
}

// check _jm_obj_7_map_minLength (.'$validation'.minLength)
static bool _jm_f_28(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.minLength
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$validation'.minLength]", path);
    }
    return res;
}

// check _jm_obj_7_map_minProperties (.'$validation'.minProperties)
static bool _jm_f_29(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.minProperties
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$validation'.minProperties]", path);
    }
    return res;
}

// check _jm_obj_7_map_minimum (.'$validation'.minimum)
static bool _jm_f_30(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.minimum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$validation'.minimum]", path);
    }
    return res;
}

// check _jm_obj_7_map_multipleOf (.'$validation'.multipleOf)
static bool _jm_f_31(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.multipleOf
    res = json_is_number(val) && json_number_value(val) > 0.0;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a 1.0 loose float [.'$validation'.multipleOf]", path);
    }
    return res;
}

// check _jm_obj_7_map_pattern (.'$validation'.pattern)
static bool _jm_f_32(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.pattern
    res = jm_is_valid_regex(json_string_value(val), false, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $REGEX [.'$validation'.pattern]", path);
    }
    return res;
}

// check _jm_obj_7_map_required (.'$validation'.required)
static bool _jm_f_33(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.required
    res = json_model_10(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $stringArray [.'$validation'.required]", path);
    }
    return res;
}


// check _jm_obj_7_map_type (.'$validation'.type)
static bool _jm_f_34(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.type
    res = jm_json_is_scalar(val) && json_is_string(val) && jm_search_cst(&(jm_constant_t) { cst_is_string, { .s = json_string_value(val) } }, _jm_cst_1, 7);;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "value not in enum [.'$validation'.type.'|']", path);
    }
    if (! res)
    {
        // .'$validation'.type.'|'.0
        res = json_model_9(val, path, rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $simpleTypesArray [.'$validation'.type.'|'.0]", path);
        }
        if (res)
        {
            if (rep) jm_report_free_entries(rep);
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "no model matched [.'$validation'.type.'|']", path);
        }
    }
    return res;
}

// check _jm_obj_7_map_uniqueItems (.'$validation'.uniqueItems)
static bool _jm_f_35(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'.uniqueItems
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$validation'.uniqueItems]", path);
    }
    return res;
}

static INLINE jm_check_fun_t _jm_obj_7_map(const char *pname)
{
    return jm_search_propmap(pname, _jm_obj_7_map_tab, 20);
}

// object .'$validation'
static INLINE bool _jm_obj_7(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$validation']", path);
        return false;
    }
    jm_check_fun_t pfun;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_7 = (jm_path_t) { prop, 0, path, NULL };
        if ((pfun = _jm_obj_7_map(prop)))
        {
            // handle 20 may props
            if (pfun != NULL && ! pfun(pval, (path ? &lpath_7 : NULL), rep))
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$validation']", (path ? &lpath_7 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$validation']", (path ? &lpath_7 : NULL));
            return false;
        }
    }
    return true;
}

// check $validation (.'$validation')
static bool json_model_12(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$validation'
    res = _jm_obj_7(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$validation']", path);
    }
    return res;
}

// check $schemaArray (.'$schemaArray')
static bool json_model_13(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$schemaArray'
    // .'$schemaArray'.'@'
    res = json_is_array(val);
    if (res)
    {
        size_t arr_2_idx;
        json_t *arr_2_item;
        json_array_foreach(val, arr_2_idx, arr_2_item)
        {
            jm_path_t arr_2_lpath = (jm_path_t) { NULL, arr_2_idx, path, NULL };
            // .'$schemaArray'.'@'.0
            res = json_model_16(arr_2_item, (path ? &arr_2_lpath : NULL), rep);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$schemaArray'.'@'.0]", (path ? &arr_2_lpath : NULL));
            }
            if (! res)
            {
                break;
            }
        }
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$schemaArray'.'@']", path);
    }
    if (res)
    {
        int64_t ival_1 = json_array_size(val);
        res = ival_1 >= 1;
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "constraints failed [.'$schemaArray']", path);
        }
    }
    return res;
}

// check _jm_obj_9_map_additionalProperties (.'$applicator'.additionalProperties)
static bool _jm_f_36(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.additionalProperties
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.additionalProperties]", path);
    }
    return res;
}

// check _jm_obj_9_map_allOf (.'$applicator'.allOf)
static bool _jm_f_37(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.allOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$applicator'.allOf]", path);
    }
    return res;
}

// check _jm_obj_9_map_anyOf (.'$applicator'.anyOf)
static bool _jm_f_38(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.anyOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$applicator'.anyOf]", path);
    }
    return res;
}

// check _jm_obj_9_map_contains (.'$applicator'.contains)
static bool _jm_f_39(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.contains
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.contains]", path);
    }
    return res;
}

// object .'$applicator'.dependentSchemas
static INLINE bool _jm_obj_10(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$applicator'.dependentSchemas]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_10 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$applicator'.dependentSchemas.''
        res = json_model_16(pval, (path ? &lpath_10 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.dependentSchemas.'']", (path ? &lpath_10 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_9_map_dependentSchemas (.'$applicator'.dependentSchemas)
static bool _jm_f_40(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.dependentSchemas
    res = _jm_obj_10(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$applicator'.dependentSchemas]", path);
    }
    return res;
}

// check _jm_obj_9_map_else (.'$applicator'.else)
static bool _jm_f_41(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.else
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.else]", path);
    }
    return res;
}

// check _jm_obj_9_map_if (.'$applicator'.if)
static bool _jm_f_42(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.if
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.if]", path);
    }
    return res;
}

// check _jm_obj_9_map_items (.'$applicator'.items)
static bool _jm_f_43(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.items
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.items]", path);
    }
    return res;
}

// check _jm_obj_9_map_not (.'$applicator'.not)
static bool _jm_f_44(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.not
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.not]", path);
    }
    return res;
}

// check _jm_obj_9_map_oneOf (.'$applicator'.oneOf)
static bool _jm_f_45(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.oneOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$applicator'.oneOf]", path);
    }
    return res;
}

// object .'$applicator'.patternProperties
static INLINE bool _jm_obj_11(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$applicator'.patternProperties]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_11 = (jm_path_t) { prop, 0, path, NULL };
        if (jm_is_valid_regex(prop, false, (path ? &lpath_11 : NULL), rep))
        {
            // handle 1 key props
            // .'$applicator'.patternProperties.'$REGEX'
            res = json_model_16(pval, (path ? &lpath_11 : NULL), rep);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.patternProperties.'$REGEX']", (path ? &lpath_11 : NULL));
            }
            if (! res)
            {
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$applicator'.patternProperties]", (path ? &lpath_11 : NULL));
            return false;
        }
    }
    return true;
}

// check _jm_obj_9_map_patternProperties (.'$applicator'.patternProperties)
static bool _jm_f_46(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.patternProperties
    res = _jm_obj_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$applicator'.patternProperties]", path);
    }
    return res;
}

// check _jm_obj_9_map_prefixItems (.'$applicator'.prefixItems)
static bool _jm_f_47(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.prefixItems
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$applicator'.prefixItems]", path);
    }
    return res;
}

// object .'$applicator'.properties
static INLINE bool _jm_obj_12(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$applicator'.properties]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_12 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$applicator'.properties.''
        res = json_model_16(pval, (path ? &lpath_12 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.properties.'']", (path ? &lpath_12 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_9_map_properties (.'$applicator'.properties)
static bool _jm_f_48(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.properties
    res = _jm_obj_12(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$applicator'.properties]", path);
    }
    return res;
}

// object .'$applicator'.propertyDependencies.''
static INLINE bool _jm_obj_14(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$applicator'.propertyDependencies.'']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_14 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$applicator'.propertyDependencies.''.''
        res = json_model_16(pval, (path ? &lpath_14 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.propertyDependencies.''.'']", (path ? &lpath_14 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// object .'$applicator'.propertyDependencies
static INLINE bool _jm_obj_13(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$applicator'.propertyDependencies]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_13 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$applicator'.propertyDependencies.''
        res = _jm_obj_14(pval, (path ? &lpath_13 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected element [.'$applicator'.propertyDependencies.'']", (path ? &lpath_13 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_9_map_propertyDependencies (.'$applicator'.propertyDependencies)
static bool _jm_f_49(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.propertyDependencies
    res = _jm_obj_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$applicator'.propertyDependencies]", path);
    }
    return res;
}

// check _jm_obj_9_map_propertyNames (.'$applicator'.propertyNames)
static bool _jm_f_50(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.propertyNames
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.propertyNames]", path);
    }
    return res;
}

// check _jm_obj_9_map_then (.'$applicator'.then)
static bool _jm_f_51(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'.then
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$applicator'.then]", path);
    }
    return res;
}

static INLINE jm_check_fun_t _jm_obj_9_map(const char *pname)
{
    return jm_search_propmap(pname, _jm_obj_9_map_tab, 16);
}

// object .'$applicator'
static INLINE bool _jm_obj_9(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$applicator']", path);
        return false;
    }
    jm_check_fun_t pfun;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_9 = (jm_path_t) { prop, 0, path, NULL };
        if ((pfun = _jm_obj_9_map(prop)))
        {
            // handle 16 may props
            if (pfun != NULL && ! pfun(pval, (path ? &lpath_9 : NULL), rep))
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$applicator']", (path ? &lpath_9 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$applicator']", (path ? &lpath_9 : NULL));
            return false;
        }
    }
    return true;
}

// check $applicator (.'$applicator')
static bool json_model_14(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$applicator'
    res = _jm_obj_9(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$applicator']", path);
    }
    return res;
}

// check _jm_obj_15_map_$anchor (.'$ObjectSchema'.'$anchor')
static bool _jm_f_52(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$anchor'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$ObjectSchema'.'$anchor']", path);
    }
    return res;
}

// check _jm_obj_15_map_$comment (.'$ObjectSchema'.'$comment')
static bool _jm_f_53(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$comment'
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$ObjectSchema'.'$comment']", path);
    }
    return res;
}

// object .'$ObjectSchema'.'$defs'
static INLINE bool _jm_obj_16(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema'.'$defs']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_16 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$ObjectSchema'.'$defs'.''
        res = json_model_16(pval, (path ? &lpath_16 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.'$defs'.'']", (path ? &lpath_16 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_15_map_$defs (.'$ObjectSchema'.'$defs')
static bool _jm_f_54(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$defs'
    res = _jm_obj_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema'.'$defs']", path);
    }
    return res;
}

// check _jm_obj_15_map_$dynamicAnchor (.'$ObjectSchema'.'$dynamicAnchor')
static bool _jm_f_55(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$dynamicAnchor'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$ObjectSchema'.'$dynamicAnchor']", path);
    }
    return res;
}

// check _jm_obj_15_map_$dynamicRef (.'$ObjectSchema'.'$dynamicRef')
static bool _jm_f_56(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$dynamicRef'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$ObjectSchema'.'$dynamicRef']", path);
    }
    return res;
}

// check _jm_obj_15_map_$id (.'$ObjectSchema'.'$id')
static bool _jm_f_57(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$id'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$ObjectSchema'.'$id']", path);
    }
    return res;
}

// check _jm_obj_15_map_$ref (.'$ObjectSchema'.'$ref')
static bool _jm_f_58(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$ref'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$ObjectSchema'.'$ref']", path);
    }
    return res;
}

// check _jm_obj_15_map_$schema (.'$ObjectSchema'.'$schema')
static bool _jm_f_59(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$schema'
    res = jm_is_valid_url(json_string_value(val), path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI [.'$ObjectSchema'.'$schema']", path);
    }
    return res;
}

// object .'$ObjectSchema'.'$vocabulary'
static INLINE bool _jm_obj_17(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema'.'$vocabulary']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_17 = (jm_path_t) { prop, 0, path, NULL };
        if (jm_is_valid_url(prop, (path ? &lpath_17 : NULL), rep))
        {
            // handle 1 key props
            // .'$ObjectSchema'.'$vocabulary'.'$URI'
            res = json_is_boolean(pval);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "not a bool [.'$ObjectSchema'.'$vocabulary'.'$URI']", (path ? &lpath_17 : NULL));
            }
            if (! res)
            {
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$ObjectSchema'.'$vocabulary']", (path ? &lpath_17 : NULL));
            return false;
        }
    }
    return true;
}

// check _jm_obj_15_map_$vocabulary (.'$ObjectSchema'.'$vocabulary')
static bool _jm_f_60(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.'$vocabulary'
    res = _jm_obj_17(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema'.'$vocabulary']", path);
    }
    return res;
}

// check _jm_obj_15_map_additionalProperties (.'$ObjectSchema'.additionalProperties)
static bool _jm_f_61(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.additionalProperties
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.additionalProperties]", path);
    }
    return res;
}

// check _jm_obj_15_map_allOf (.'$ObjectSchema'.allOf)
static bool _jm_f_62(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.allOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$ObjectSchema'.allOf]", path);
    }
    return res;
}

// check _jm_obj_15_map_anyOf (.'$ObjectSchema'.anyOf)
static bool _jm_f_63(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.anyOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$ObjectSchema'.anyOf]", path);
    }
    return res;
}

// check _jm_obj_15_map_const (.'$ObjectSchema'.const)
static bool _jm_f_64(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.const
    res = true;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $ANY [.'$ObjectSchema'.const]", path);
    }
    return res;
}

// check _jm_obj_15_map_contains (.'$ObjectSchema'.contains)
static bool _jm_f_65(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.contains
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.contains]", path);
    }
    return res;
}

// check _jm_obj_15_map_contentEncoding (.'$ObjectSchema'.contentEncoding)
static bool _jm_f_66(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.contentEncoding
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$ObjectSchema'.contentEncoding]", path);
    }
    return res;
}

// check _jm_obj_15_map_contentMediaType (.'$ObjectSchema'.contentMediaType)
static bool _jm_f_67(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.contentMediaType
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$ObjectSchema'.contentMediaType]", path);
    }
    return res;
}

// check _jm_obj_15_map_contentSchema (.'$ObjectSchema'.contentSchema)
static bool _jm_f_68(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.contentSchema
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.contentSchema]", path);
    }
    return res;
}

// check _jm_obj_15_map_default (.'$ObjectSchema'.default)
static bool _jm_f_69(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.default
    res = true;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $ANY [.'$ObjectSchema'.default]", path);
    }
    return res;
}

// object .'$ObjectSchema'.dependentRequired
static INLINE bool _jm_obj_18(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema'.dependentRequired]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_18 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$ObjectSchema'.dependentRequired.''
        res = json_model_10(pval, (path ? &lpath_18 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $stringArray [.'$ObjectSchema'.dependentRequired.'']", (path ? &lpath_18 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_15_map_dependentRequired (.'$ObjectSchema'.dependentRequired)
static bool _jm_f_70(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.dependentRequired
    res = _jm_obj_18(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema'.dependentRequired]", path);
    }
    return res;
}

// object .'$ObjectSchema'.dependentSchemas
static INLINE bool _jm_obj_19(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema'.dependentSchemas]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_19 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$ObjectSchema'.dependentSchemas.''
        res = json_model_16(pval, (path ? &lpath_19 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.dependentSchemas.'']", (path ? &lpath_19 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_15_map_dependentSchemas (.'$ObjectSchema'.dependentSchemas)
static bool _jm_f_71(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.dependentSchemas
    res = _jm_obj_19(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema'.dependentSchemas]", path);
    }
    return res;
}

// check _jm_obj_15_map_deprecated (.'$ObjectSchema'.deprecated)
static bool _jm_f_72(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.deprecated
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$ObjectSchema'.deprecated]", path);
    }
    return res;
}

// check _jm_obj_15_map_description (.'$ObjectSchema'.description)
static bool _jm_f_73(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.description
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$ObjectSchema'.description]", path);
    }
    return res;
}

// check _jm_obj_15_map_else (.'$ObjectSchema'.else)
static bool _jm_f_74(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.else
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.else]", path);
    }
    return res;
}

// check _jm_obj_15_map_enum (.'$ObjectSchema'.enum)
static bool _jm_f_75(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.enum
    res = json_is_array(val);
    if (res)
    {
        // accept any array
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$ObjectSchema'.enum]", path);
    }
    return res;
}

// check _jm_obj_15_map_examples (.'$ObjectSchema'.examples)
static bool _jm_f_76(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.examples
    res = json_is_array(val);
    if (res)
    {
        // accept any array
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$ObjectSchema'.examples]", path);
    }
    return res;
}

// check _jm_obj_15_map_exclusiveMaximum (.'$ObjectSchema'.exclusiveMaximum)
static bool _jm_f_77(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.exclusiveMaximum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$ObjectSchema'.exclusiveMaximum]", path);
    }
    return res;
}

// check _jm_obj_15_map_exclusiveMinimum (.'$ObjectSchema'.exclusiveMinimum)
static bool _jm_f_78(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.exclusiveMinimum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$ObjectSchema'.exclusiveMinimum]", path);
    }
    return res;
}

// check _jm_obj_15_map_format (.'$ObjectSchema'.format)
static bool _jm_f_79(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.format
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$ObjectSchema'.format]", path);
    }
    return res;
}

// check _jm_obj_15_map_if (.'$ObjectSchema'.if)
static bool _jm_f_80(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.if
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.if]", path);
    }
    return res;
}

// check _jm_obj_15_map_items (.'$ObjectSchema'.items)
static bool _jm_f_81(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.items
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.items]", path);
    }
    return res;
}

// check _jm_obj_15_map_maxContains (.'$ObjectSchema'.maxContains)
static bool _jm_f_82(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.maxContains
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$ObjectSchema'.maxContains]", path);
    }
    return res;
}

// check _jm_obj_15_map_maxItems (.'$ObjectSchema'.maxItems)
static bool _jm_f_83(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.maxItems
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$ObjectSchema'.maxItems]", path);
    }
    return res;
}

// check _jm_obj_15_map_maxLength (.'$ObjectSchema'.maxLength)
static bool _jm_f_84(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.maxLength
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$ObjectSchema'.maxLength]", path);
    }
    return res;
}

// check _jm_obj_15_map_maxProperties (.'$ObjectSchema'.maxProperties)
static bool _jm_f_85(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.maxProperties
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$ObjectSchema'.maxProperties]", path);
    }
    return res;
}

// check _jm_obj_15_map_maximum (.'$ObjectSchema'.maximum)
static bool _jm_f_86(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.maximum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$ObjectSchema'.maximum]", path);
    }
    return res;
}

// check _jm_obj_15_map_minContains (.'$ObjectSchema'.minContains)
static bool _jm_f_87(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.minContains
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$ObjectSchema'.minContains]", path);
    }
    return res;
}

// check _jm_obj_15_map_minItems (.'$ObjectSchema'.minItems)
static bool _jm_f_88(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.minItems
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$ObjectSchema'.minItems]", path);
    }
    return res;
}

// check _jm_obj_15_map_minLength (.'$ObjectSchema'.minLength)
static bool _jm_f_89(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.minLength
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$ObjectSchema'.minLength]", path);
    }
    return res;
}

// check _jm_obj_15_map_minProperties (.'$ObjectSchema'.minProperties)
static bool _jm_f_90(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.minProperties
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$ObjectSchema'.minProperties]", path);
    }
    return res;
}

// check _jm_obj_15_map_minimum (.'$ObjectSchema'.minimum)
static bool _jm_f_91(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.minimum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$ObjectSchema'.minimum]", path);
    }
    return res;
}

// check _jm_obj_15_map_multipleOf (.'$ObjectSchema'.multipleOf)
static bool _jm_f_92(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.multipleOf
    res = json_is_number(val) && json_number_value(val) > 0.0;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a 1.0 loose float [.'$ObjectSchema'.multipleOf]", path);
    }
    return res;
}

// check _jm_obj_15_map_not (.'$ObjectSchema'.not)
static bool _jm_f_93(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.not
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.not]", path);
    }
    return res;
}

// check _jm_obj_15_map_oneOf (.'$ObjectSchema'.oneOf)
static bool _jm_f_94(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.oneOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$ObjectSchema'.oneOf]", path);
    }
    return res;
}

// check _jm_obj_15_map_pattern (.'$ObjectSchema'.pattern)
static bool _jm_f_95(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.pattern
    res = jm_is_valid_regex(json_string_value(val), false, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $REGEX [.'$ObjectSchema'.pattern]", path);
    }
    return res;
}

// object .'$ObjectSchema'.patternProperties
static INLINE bool _jm_obj_20(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema'.patternProperties]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_20 = (jm_path_t) { prop, 0, path, NULL };
        if (jm_is_valid_regex(prop, false, (path ? &lpath_20 : NULL), rep))
        {
            // handle 1 key props
            // .'$ObjectSchema'.patternProperties.'$REGEX'
            res = json_model_16(pval, (path ? &lpath_20 : NULL), rep);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.patternProperties.'$REGEX']", (path ? &lpath_20 : NULL));
            }
            if (! res)
            {
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$ObjectSchema'.patternProperties]", (path ? &lpath_20 : NULL));
            return false;
        }
    }
    return true;
}

// check _jm_obj_15_map_patternProperties (.'$ObjectSchema'.patternProperties)
static bool _jm_f_96(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.patternProperties
    res = _jm_obj_20(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema'.patternProperties]", path);
    }
    return res;
}

// check _jm_obj_15_map_prefixItems (.'$ObjectSchema'.prefixItems)
static bool _jm_f_97(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.prefixItems
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$ObjectSchema'.prefixItems]", path);
    }
    return res;
}

// object .'$ObjectSchema'.properties
static INLINE bool _jm_obj_21(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema'.properties]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_21 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$ObjectSchema'.properties.''
        res = json_model_16(pval, (path ? &lpath_21 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.properties.'']", (path ? &lpath_21 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_15_map_properties (.'$ObjectSchema'.properties)
static bool _jm_f_98(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.properties
    res = _jm_obj_21(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema'.properties]", path);
    }
    return res;
}

// object .'$ObjectSchema'.propertyDependencies.''
static INLINE bool _jm_obj_23(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema'.propertyDependencies.'']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_23 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$ObjectSchema'.propertyDependencies.''.''
        res = json_model_16(pval, (path ? &lpath_23 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.propertyDependencies.''.'']", (path ? &lpath_23 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// object .'$ObjectSchema'.propertyDependencies
static INLINE bool _jm_obj_22(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema'.propertyDependencies]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_22 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$ObjectSchema'.propertyDependencies.''
        res = _jm_obj_23(pval, (path ? &lpath_22 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema'.propertyDependencies.'']", (path ? &lpath_22 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_15_map_propertyDependencies (.'$ObjectSchema'.propertyDependencies)
static bool _jm_f_99(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.propertyDependencies
    res = _jm_obj_22(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema'.propertyDependencies]", path);
    }
    return res;
}

// check _jm_obj_15_map_propertyNames (.'$ObjectSchema'.propertyNames)
static bool _jm_f_100(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.propertyNames
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.propertyNames]", path);
    }
    return res;
}

// check _jm_obj_15_map_readOnly (.'$ObjectSchema'.readOnly)
static bool _jm_f_101(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.readOnly
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$ObjectSchema'.readOnly]", path);
    }
    return res;
}

// check _jm_obj_15_map_required (.'$ObjectSchema'.required)
static bool _jm_f_102(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.required
    res = json_model_10(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $stringArray [.'$ObjectSchema'.required]", path);
    }
    return res;
}

// check _jm_obj_15_map_then (.'$ObjectSchema'.then)
static bool _jm_f_103(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.then
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.then]", path);
    }
    return res;
}

// check _jm_obj_15_map_title (.'$ObjectSchema'.title)
static bool _jm_f_104(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.title
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$ObjectSchema'.title]", path);
    }
    return res;
}


// check _jm_obj_15_map_type (.'$ObjectSchema'.type)
static bool _jm_f_105(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.type
    res = jm_json_is_scalar(val) && json_is_string(val) && jm_search_cst(&(jm_constant_t) { cst_is_string, { .s = json_string_value(val) } }, _jm_cst_2, 7);;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "value not in enum [.'$ObjectSchema'.type.'|']", path);
    }
    if (! res)
    {
        // .'$ObjectSchema'.type.'|'.0
        res = json_model_9(val, path, rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $simpleTypesArray [.'$ObjectSchema'.type.'|'.0]", path);
        }
        if (res)
        {
            if (rep) jm_report_free_entries(rep);
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "no model matched [.'$ObjectSchema'.type.'|']", path);
        }
    }
    return res;
}

// check _jm_obj_15_map_unevaluatedItems (.'$ObjectSchema'.unevaluatedItems)
static bool _jm_f_106(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.unevaluatedItems
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.unevaluatedItems]", path);
    }
    return res;
}

// check _jm_obj_15_map_unevaluatedProperties (.'$ObjectSchema'.unevaluatedProperties)
static bool _jm_f_107(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.unevaluatedProperties
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$ObjectSchema'.unevaluatedProperties]", path);
    }
    return res;
}

// check _jm_obj_15_map_uniqueItems (.'$ObjectSchema'.uniqueItems)
static bool _jm_f_108(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.uniqueItems
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$ObjectSchema'.uniqueItems]", path);
    }
    return res;
}

// check _jm_obj_15_map_writeOnly (.'$ObjectSchema'.writeOnly)
static bool _jm_f_109(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'.writeOnly
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$ObjectSchema'.writeOnly]", path);
    }
    return res;
}

static INLINE jm_check_fun_t _jm_obj_15_map(const char *pname)
{
    return jm_search_propmap(pname, _jm_obj_15_map_tab, 58);
}

// object .'$ObjectSchema'
static INLINE bool _jm_obj_15(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$ObjectSchema']", path);
        return false;
    }
    jm_check_fun_t pfun;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_15 = (jm_path_t) { prop, 0, path, NULL };
        if ((pfun = _jm_obj_15_map(prop)))
        {
            // handle 58 may props
            if (pfun != NULL && ! pfun(pval, (path ? &lpath_15 : NULL), rep))
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$ObjectSchema']", (path ? &lpath_15 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$ObjectSchema']", (path ? &lpath_15 : NULL));
            return false;
        }
    }
    return true;
}

// check $ObjectSchema (.'$ObjectSchema')
static bool json_model_15(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$ObjectSchema'
    res = _jm_obj_15(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$ObjectSchema']", path);
    }
    return res;
}

// check _jm_obj_24_map_$anchor (.'$Schema'.'|'.1.'$anchor')
static bool _jm_f_110(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$anchor'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$Schema'.'|'.1.'$anchor']", path);
    }
    return res;
}

// check _jm_obj_24_map_$comment (.'$Schema'.'|'.1.'$comment')
static bool _jm_f_111(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$comment'
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$Schema'.'|'.1.'$comment']", path);
    }
    return res;
}

// object .'$Schema'.'|'.1.'$defs'
static INLINE bool _jm_obj_25(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1.'$defs']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_25 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$Schema'.'|'.1.'$defs'.''
        res = json_model_16(pval, (path ? &lpath_25 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.'$defs'.'']", (path ? &lpath_25 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_24_map_$defs (.'$Schema'.'|'.1.'$defs')
static bool _jm_f_112(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$defs'
    res = _jm_obj_25(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1.'$defs']", path);
    }
    return res;
}

// check _jm_obj_24_map_$dynamicAnchor (.'$Schema'.'|'.1.'$dynamicAnchor')
static bool _jm_f_113(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$dynamicAnchor'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$Schema'.'|'.1.'$dynamicAnchor']", path);
    }
    return res;
}

// check _jm_obj_24_map_$dynamicRef (.'$Schema'.'|'.1.'$dynamicRef')
static bool _jm_f_114(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$dynamicRef'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$Schema'.'|'.1.'$dynamicRef']", path);
    }
    return res;
}

// check _jm_obj_24_map_$id (.'$Schema'.'|'.1.'$id')
static bool _jm_f_115(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$id'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$Schema'.'|'.1.'$id']", path);
    }
    return res;
}

// check _jm_obj_24_map_$ref (.'$Schema'.'|'.1.'$ref')
static bool _jm_f_116(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$ref'
    res = json_model_2(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI-REFERENCE [.'$Schema'.'|'.1.'$ref']", path);
    }
    return res;
}

// check _jm_obj_24_map_$schema (.'$Schema'.'|'.1.'$schema')
static bool _jm_f_117(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$schema'
    res = jm_is_valid_url(json_string_value(val), path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $URI [.'$Schema'.'|'.1.'$schema']", path);
    }
    return res;
}

// object .'$Schema'.'|'.1.'$vocabulary'
static INLINE bool _jm_obj_26(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1.'$vocabulary']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_26 = (jm_path_t) { prop, 0, path, NULL };
        if (jm_is_valid_url(prop, (path ? &lpath_26 : NULL), rep))
        {
            // handle 1 key props
            // .'$Schema'.'|'.1.'$vocabulary'.'$URI'
            res = json_is_boolean(pval);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "not a bool [.'$Schema'.'|'.1.'$vocabulary'.'$URI']", (path ? &lpath_26 : NULL));
            }
            if (! res)
            {
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$Schema'.'|'.1.'$vocabulary']", (path ? &lpath_26 : NULL));
            return false;
        }
    }
    return true;
}

// check _jm_obj_24_map_$vocabulary (.'$Schema'.'|'.1.'$vocabulary')
static bool _jm_f_118(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.'$vocabulary'
    res = _jm_obj_26(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1.'$vocabulary']", path);
    }
    return res;
}

// check _jm_obj_24_map_additionalProperties (.'$Schema'.'|'.1.additionalProperties)
static bool _jm_f_119(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.additionalProperties
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.additionalProperties]", path);
    }
    return res;
}

// check _jm_obj_24_map_allOf (.'$Schema'.'|'.1.allOf)
static bool _jm_f_120(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.allOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$Schema'.'|'.1.allOf]", path);
    }
    return res;
}

// check _jm_obj_24_map_anyOf (.'$Schema'.'|'.1.anyOf)
static bool _jm_f_121(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.anyOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$Schema'.'|'.1.anyOf]", path);
    }
    return res;
}

// check _jm_obj_24_map_const (.'$Schema'.'|'.1.const)
static bool _jm_f_122(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.const
    res = true;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $ANY [.'$Schema'.'|'.1.const]", path);
    }
    return res;
}

// check _jm_obj_24_map_contains (.'$Schema'.'|'.1.contains)
static bool _jm_f_123(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.contains
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.contains]", path);
    }
    return res;
}

// check _jm_obj_24_map_contentEncoding (.'$Schema'.'|'.1.contentEncoding)
static bool _jm_f_124(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.contentEncoding
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$Schema'.'|'.1.contentEncoding]", path);
    }
    return res;
}

// check _jm_obj_24_map_contentMediaType (.'$Schema'.'|'.1.contentMediaType)
static bool _jm_f_125(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.contentMediaType
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$Schema'.'|'.1.contentMediaType]", path);
    }
    return res;
}

// check _jm_obj_24_map_contentSchema (.'$Schema'.'|'.1.contentSchema)
static bool _jm_f_126(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.contentSchema
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.contentSchema]", path);
    }
    return res;
}

// check _jm_obj_24_map_default (.'$Schema'.'|'.1.default)
static bool _jm_f_127(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.default
    res = true;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $ANY [.'$Schema'.'|'.1.default]", path);
    }
    return res;
}

// object .'$Schema'.'|'.1.dependentRequired
static INLINE bool _jm_obj_27(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1.dependentRequired]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_27 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$Schema'.'|'.1.dependentRequired.''
        res = json_model_10(pval, (path ? &lpath_27 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $stringArray [.'$Schema'.'|'.1.dependentRequired.'']", (path ? &lpath_27 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_24_map_dependentRequired (.'$Schema'.'|'.1.dependentRequired)
static bool _jm_f_128(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.dependentRequired
    res = _jm_obj_27(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1.dependentRequired]", path);
    }
    return res;
}

// object .'$Schema'.'|'.1.dependentSchemas
static INLINE bool _jm_obj_28(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1.dependentSchemas]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_28 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$Schema'.'|'.1.dependentSchemas.''
        res = json_model_16(pval, (path ? &lpath_28 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.dependentSchemas.'']", (path ? &lpath_28 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_24_map_dependentSchemas (.'$Schema'.'|'.1.dependentSchemas)
static bool _jm_f_129(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.dependentSchemas
    res = _jm_obj_28(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1.dependentSchemas]", path);
    }
    return res;
}

// check _jm_obj_24_map_deprecated (.'$Schema'.'|'.1.deprecated)
static bool _jm_f_130(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.deprecated
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$Schema'.'|'.1.deprecated]", path);
    }
    return res;
}

// check _jm_obj_24_map_description (.'$Schema'.'|'.1.description)
static bool _jm_f_131(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.description
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$Schema'.'|'.1.description]", path);
    }
    return res;
}

// check _jm_obj_24_map_else (.'$Schema'.'|'.1.else)
static bool _jm_f_132(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.else
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.else]", path);
    }
    return res;
}

// check _jm_obj_24_map_enum (.'$Schema'.'|'.1.enum)
static bool _jm_f_133(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.enum
    res = json_is_array(val);
    if (res)
    {
        // accept any array
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$Schema'.'|'.1.enum]", path);
    }
    return res;
}

// check _jm_obj_24_map_examples (.'$Schema'.'|'.1.examples)
static bool _jm_f_134(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.examples
    res = json_is_array(val);
    if (res)
    {
        // accept any array
    }
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not array or unexpected array [.'$Schema'.'|'.1.examples]", path);
    }
    return res;
}

// check _jm_obj_24_map_exclusiveMaximum (.'$Schema'.'|'.1.exclusiveMaximum)
static bool _jm_f_135(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.exclusiveMaximum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$Schema'.'|'.1.exclusiveMaximum]", path);
    }
    return res;
}

// check _jm_obj_24_map_exclusiveMinimum (.'$Schema'.'|'.1.exclusiveMinimum)
static bool _jm_f_136(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.exclusiveMinimum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$Schema'.'|'.1.exclusiveMinimum]", path);
    }
    return res;
}

// check _jm_obj_24_map_format (.'$Schema'.'|'.1.format)
static bool _jm_f_137(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.format
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$Schema'.'|'.1.format]", path);
    }
    return res;
}

// check _jm_obj_24_map_if (.'$Schema'.'|'.1.if)
static bool _jm_f_138(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.if
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.if]", path);
    }
    return res;
}

// check _jm_obj_24_map_items (.'$Schema'.'|'.1.items)
static bool _jm_f_139(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.items
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.items]", path);
    }
    return res;
}

// check _jm_obj_24_map_maxContains (.'$Schema'.'|'.1.maxContains)
static bool _jm_f_140(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.maxContains
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$Schema'.'|'.1.maxContains]", path);
    }
    return res;
}

// check _jm_obj_24_map_maxItems (.'$Schema'.'|'.1.maxItems)
static bool _jm_f_141(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.maxItems
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$Schema'.'|'.1.maxItems]", path);
    }
    return res;
}

// check _jm_obj_24_map_maxLength (.'$Schema'.'|'.1.maxLength)
static bool _jm_f_142(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.maxLength
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$Schema'.'|'.1.maxLength]", path);
    }
    return res;
}

// check _jm_obj_24_map_maxProperties (.'$Schema'.'|'.1.maxProperties)
static bool _jm_f_143(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.maxProperties
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$Schema'.'|'.1.maxProperties]", path);
    }
    return res;
}

// check _jm_obj_24_map_maximum (.'$Schema'.'|'.1.maximum)
static bool _jm_f_144(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.maximum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$Schema'.'|'.1.maximum]", path);
    }
    return res;
}

// check _jm_obj_24_map_minContains (.'$Schema'.'|'.1.minContains)
static bool _jm_f_145(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.minContains
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$Schema'.'|'.1.minContains]", path);
    }
    return res;
}

// check _jm_obj_24_map_minItems (.'$Schema'.'|'.1.minItems)
static bool _jm_f_146(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.minItems
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$Schema'.'|'.1.minItems]", path);
    }
    return res;
}

// check _jm_obj_24_map_minLength (.'$Schema'.'|'.1.minLength)
static bool _jm_f_147(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.minLength
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$Schema'.'|'.1.minLength]", path);
    }
    return res;
}

// check _jm_obj_24_map_minProperties (.'$Schema'.'|'.1.minProperties)
static bool _jm_f_148(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.minProperties
    res = json_model_11(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $nonNegativeInteger [.'$Schema'.'|'.1.minProperties]", path);
    }
    return res;
}

// check _jm_obj_24_map_minimum (.'$Schema'.'|'.1.minimum)
static bool _jm_f_149(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.minimum
    res = json_is_number(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a -1.0 loose float [.'$Schema'.'|'.1.minimum]", path);
    }
    return res;
}

// check _jm_obj_24_map_multipleOf (.'$Schema'.'|'.1.multipleOf)
static bool _jm_f_150(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.multipleOf
    res = json_is_number(val) && json_number_value(val) > 0.0;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a 1.0 loose float [.'$Schema'.'|'.1.multipleOf]", path);
    }
    return res;
}

// check _jm_obj_24_map_not (.'$Schema'.'|'.1.not)
static bool _jm_f_151(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.not
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.not]", path);
    }
    return res;
}

// check _jm_obj_24_map_oneOf (.'$Schema'.'|'.1.oneOf)
static bool _jm_f_152(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.oneOf
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$Schema'.'|'.1.oneOf]", path);
    }
    return res;
}

// check _jm_obj_24_map_pattern (.'$Schema'.'|'.1.pattern)
static bool _jm_f_153(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.pattern
    res = jm_is_valid_regex(json_string_value(val), false, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $REGEX [.'$Schema'.'|'.1.pattern]", path);
    }
    return res;
}

// object .'$Schema'.'|'.1.patternProperties
static INLINE bool _jm_obj_29(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1.patternProperties]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_29 = (jm_path_t) { prop, 0, path, NULL };
        if (jm_is_valid_regex(prop, false, (path ? &lpath_29 : NULL), rep))
        {
            // handle 1 key props
            // .'$Schema'.'|'.1.patternProperties.'$REGEX'
            res = json_model_16(pval, (path ? &lpath_29 : NULL), rep);
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.patternProperties.'$REGEX']", (path ? &lpath_29 : NULL));
            }
            if (! res)
            {
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$Schema'.'|'.1.patternProperties]", (path ? &lpath_29 : NULL));
            return false;
        }
    }
    return true;
}

// check _jm_obj_24_map_patternProperties (.'$Schema'.'|'.1.patternProperties)
static bool _jm_f_154(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.patternProperties
    res = _jm_obj_29(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1.patternProperties]", path);
    }
    return res;
}

// check _jm_obj_24_map_prefixItems (.'$Schema'.'|'.1.prefixItems)
static bool _jm_f_155(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.prefixItems
    res = json_model_13(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $schemaArray [.'$Schema'.'|'.1.prefixItems]", path);
    }
    return res;
}

// object .'$Schema'.'|'.1.properties
static INLINE bool _jm_obj_30(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1.properties]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_30 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$Schema'.'|'.1.properties.''
        res = json_model_16(pval, (path ? &lpath_30 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.properties.'']", (path ? &lpath_30 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_24_map_properties (.'$Schema'.'|'.1.properties)
static bool _jm_f_156(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.properties
    res = _jm_obj_30(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1.properties]", path);
    }
    return res;
}

// object .'$Schema'.'|'.1.propertyDependencies.''
static INLINE bool _jm_obj_32(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1.propertyDependencies.'']", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_32 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$Schema'.'|'.1.propertyDependencies.''.''
        res = json_model_16(pval, (path ? &lpath_32 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.propertyDependencies.''.'']", (path ? &lpath_32 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// object .'$Schema'.'|'.1.propertyDependencies
static INLINE bool _jm_obj_31(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1.propertyDependencies]", path);
        return false;
    }
    bool res;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_31 = (jm_path_t) { prop, 0, path, NULL };
        // handle other props
        // .'$Schema'.'|'.1.propertyDependencies.''
        res = _jm_obj_32(pval, (path ? &lpath_31 : NULL), rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1.propertyDependencies.'']", (path ? &lpath_31 : NULL));
        }
        if (! res)
        {
            return false;
        }
    }
    return true;
}

// check _jm_obj_24_map_propertyDependencies (.'$Schema'.'|'.1.propertyDependencies)
static bool _jm_f_157(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.propertyDependencies
    res = _jm_obj_31(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1.propertyDependencies]", path);
    }
    return res;
}

// check _jm_obj_24_map_propertyNames (.'$Schema'.'|'.1.propertyNames)
static bool _jm_f_158(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.propertyNames
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.propertyNames]", path);
    }
    return res;
}

// check _jm_obj_24_map_readOnly (.'$Schema'.'|'.1.readOnly)
static bool _jm_f_159(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.readOnly
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$Schema'.'|'.1.readOnly]", path);
    }
    return res;
}

// check _jm_obj_24_map_required (.'$Schema'.'|'.1.required)
static bool _jm_f_160(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.required
    res = json_model_10(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $stringArray [.'$Schema'.'|'.1.required]", path);
    }
    return res;
}

// check _jm_obj_24_map_then (.'$Schema'.'|'.1.then)
static bool _jm_f_161(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.then
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.then]", path);
    }
    return res;
}

// check _jm_obj_24_map_title (.'$Schema'.'|'.1.title)
static bool _jm_f_162(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.title
    res = json_is_string(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected string [.'$Schema'.'|'.1.title]", path);
    }
    return res;
}


// check _jm_obj_24_map_type (.'$Schema'.'|'.1.type)
static bool _jm_f_163(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.type
    res = jm_json_is_scalar(val) && json_is_string(val) && jm_search_cst(&(jm_constant_t) { cst_is_string, { .s = json_string_value(val) } }, _jm_cst_3, 7);;
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "value not in enum [.'$Schema'.'|'.1.type.'|']", path);
    }
    if (! res)
    {
        // .'$Schema'.'|'.1.type.'|'.0
        res = json_model_9(val, path, rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected $simpleTypesArray [.'$Schema'.'|'.1.type.'|'.0]", path);
        }
        if (res)
        {
            if (rep) jm_report_free_entries(rep);
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "no model matched [.'$Schema'.'|'.1.type.'|']", path);
        }
    }
    return res;
}

// check _jm_obj_24_map_unevaluatedItems (.'$Schema'.'|'.1.unevaluatedItems)
static bool _jm_f_164(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.unevaluatedItems
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.unevaluatedItems]", path);
    }
    return res;
}

// check _jm_obj_24_map_unevaluatedProperties (.'$Schema'.'|'.1.unevaluatedProperties)
static bool _jm_f_165(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.unevaluatedProperties
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'$Schema'.'|'.1.unevaluatedProperties]", path);
    }
    return res;
}

// check _jm_obj_24_map_uniqueItems (.'$Schema'.'|'.1.uniqueItems)
static bool _jm_f_166(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.uniqueItems
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$Schema'.'|'.1.uniqueItems]", path);
    }
    return res;
}

// check _jm_obj_24_map_writeOnly (.'$Schema'.'|'.1.writeOnly)
static bool _jm_f_167(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'.'|'.1.writeOnly
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$Schema'.'|'.1.writeOnly]", path);
    }
    return res;
}

static INLINE jm_check_fun_t _jm_obj_24_map(const char *pname)
{
    return jm_search_propmap(pname, _jm_obj_24_map_tab, 58);
}

// object .'$Schema'.'|'.1
static INLINE bool _jm_obj_24(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    if (! json_is_object(val))
    {
        if (rep) jm_report_add_entry(rep, "not an object [.'$Schema'.'|'.1]", path);
        return false;
    }
    jm_check_fun_t pfun;
    const char *prop;
    json_t *pval;
    json_object_foreach((json_t *) val, prop, pval)
    {
        jm_path_t lpath_24 = (jm_path_t) { prop, 0, path, NULL };
        if ((pfun = _jm_obj_24_map(prop)))
        {
            // handle 58 may props
            if (pfun != NULL && ! pfun(pval, (path ? &lpath_24 : NULL), rep))
            {
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$Schema'.'|'.1]", (path ? &lpath_24 : NULL));
                return false;
            }
        }
        else
        {
            if (rep) jm_report_add_entry(rep, "unexpected prop [.'$Schema'.'|'.1]", (path ? &lpath_24 : NULL));
            return false;
        }
    }
    return true;
}

// check $Schema (.'$Schema')
static bool json_model_16(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // .'$Schema'
    // .'$Schema'.'|'.0
    res = json_is_boolean(val);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "not a bool [.'$Schema'.'|'.0]", path);
    }
    if (! res)
    {
        // .'$Schema'.'|'.1
        res = _jm_obj_24(val, path, rep);
        if (! res)
        {
            if (rep) jm_report_add_entry(rep, "unexpected element [.'$Schema'.'|'.1]", path);
        }
    }
    if (res)
    {
        if (rep) jm_report_free_entries(rep);
    }
    else
    {
        if (rep) jm_report_add_entry(rep, "no model matched [.'$Schema'.'|']", path);
    }
    return res;
}

// check $ (.)
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    bool res;
    // JSON Schema Draft Next as of 2023 [JSON_MODEL_LOOSE_INT, JSON_MODEL_LOOSE_FLOAT]
    // .
    // .'@'
    res = json_model_16(val, path, rep);
    if (! res)
    {
        if (rep) jm_report_add_entry(rep, "unexpected $Schema [.'@']", path);
    }
    return res;
}

jm_check_fun_t check_model_map(const char *pname)
{
    return jm_search_propmap(pname, check_model_map_tab, 16);
}

static bool initialized = false;

const char *check_model_init(void)
{
    if (!initialized)
    {
        initialized = true;
        jm_version_string = JSON_MODEL_VERSION;
        _jm_obj_0_map_tab[0] = (jm_propmap_t) { "$anchor", _jm_f_0 };
        _jm_obj_0_map_tab[1] = (jm_propmap_t) { "$comment", _jm_f_1 };
        _jm_obj_0_map_tab[2] = (jm_propmap_t) { "$defs", _jm_f_2 };
        _jm_obj_0_map_tab[3] = (jm_propmap_t) { "$dynamicAnchor", _jm_f_3 };
        _jm_obj_0_map_tab[4] = (jm_propmap_t) { "$dynamicRef", _jm_f_4 };
        _jm_obj_0_map_tab[5] = (jm_propmap_t) { "$id", _jm_f_5 };
        _jm_obj_0_map_tab[6] = (jm_propmap_t) { "$ref", _jm_f_6 };
        _jm_obj_0_map_tab[7] = (jm_propmap_t) { "$schema", _jm_f_7 };
        _jm_obj_0_map_tab[8] = (jm_propmap_t) { "$vocabulary", _jm_f_8 };
        jm_sort_propmap(_jm_obj_0_map_tab, 9);
        _jm_obj_3_map_tab[0] = (jm_propmap_t) { "default", _jm_f_9 };
        _jm_obj_3_map_tab[1] = (jm_propmap_t) { "deprecated", _jm_f_10 };
        _jm_obj_3_map_tab[2] = (jm_propmap_t) { "description", _jm_f_11 };
        _jm_obj_3_map_tab[3] = (jm_propmap_t) { "examples", _jm_f_12 };
        _jm_obj_3_map_tab[4] = (jm_propmap_t) { "readOnly", _jm_f_13 };
        _jm_obj_3_map_tab[5] = (jm_propmap_t) { "title", _jm_f_14 };
        _jm_obj_3_map_tab[6] = (jm_propmap_t) { "writeOnly", _jm_f_15 };
        jm_sort_propmap(_jm_obj_3_map_tab, 7);
        // initialize sorted set _jm_cst_0
        _jm_cst_0[0] = (jm_constant_t) { cst_is_string, { .s = "array" } };
        _jm_cst_0[1] = (jm_constant_t) { cst_is_string, { .s = "boolean" } };
        _jm_cst_0[2] = (jm_constant_t) { cst_is_string, { .s = "integer" } };
        _jm_cst_0[3] = (jm_constant_t) { cst_is_string, { .s = "null" } };
        _jm_cst_0[4] = (jm_constant_t) { cst_is_string, { .s = "number" } };
        _jm_cst_0[5] = (jm_constant_t) { cst_is_string, { .s = "object" } };
        _jm_cst_0[6] = (jm_constant_t) { cst_is_string, { .s = "string" } };
        jm_sort_cst(_jm_cst_0, 7);
        // initialize sorted set _jm_cst_1
        _jm_cst_1[0] = (jm_constant_t) { cst_is_string, { .s = "array" } };
        _jm_cst_1[1] = (jm_constant_t) { cst_is_string, { .s = "boolean" } };
        _jm_cst_1[2] = (jm_constant_t) { cst_is_string, { .s = "integer" } };
        _jm_cst_1[3] = (jm_constant_t) { cst_is_string, { .s = "null" } };
        _jm_cst_1[4] = (jm_constant_t) { cst_is_string, { .s = "number" } };
        _jm_cst_1[5] = (jm_constant_t) { cst_is_string, { .s = "object" } };
        _jm_cst_1[6] = (jm_constant_t) { cst_is_string, { .s = "string" } };
        jm_sort_cst(_jm_cst_1, 7);
        _jm_obj_7_map_tab[0] = (jm_propmap_t) { "const", _jm_f_16 };
        _jm_obj_7_map_tab[1] = (jm_propmap_t) { "dependentRequired", _jm_f_17 };
        _jm_obj_7_map_tab[2] = (jm_propmap_t) { "enum", _jm_f_18 };
        _jm_obj_7_map_tab[3] = (jm_propmap_t) { "exclusiveMaximum", _jm_f_19 };
        _jm_obj_7_map_tab[4] = (jm_propmap_t) { "exclusiveMinimum", _jm_f_20 };
        _jm_obj_7_map_tab[5] = (jm_propmap_t) { "maxContains", _jm_f_21 };
        _jm_obj_7_map_tab[6] = (jm_propmap_t) { "maxItems", _jm_f_22 };
        _jm_obj_7_map_tab[7] = (jm_propmap_t) { "maxLength", _jm_f_23 };
        _jm_obj_7_map_tab[8] = (jm_propmap_t) { "maxProperties", _jm_f_24 };
        _jm_obj_7_map_tab[9] = (jm_propmap_t) { "maximum", _jm_f_25 };
        _jm_obj_7_map_tab[10] = (jm_propmap_t) { "minContains", _jm_f_26 };
        _jm_obj_7_map_tab[11] = (jm_propmap_t) { "minItems", _jm_f_27 };
        _jm_obj_7_map_tab[12] = (jm_propmap_t) { "minLength", _jm_f_28 };
        _jm_obj_7_map_tab[13] = (jm_propmap_t) { "minProperties", _jm_f_29 };
        _jm_obj_7_map_tab[14] = (jm_propmap_t) { "minimum", _jm_f_30 };
        _jm_obj_7_map_tab[15] = (jm_propmap_t) { "multipleOf", _jm_f_31 };
        _jm_obj_7_map_tab[16] = (jm_propmap_t) { "pattern", _jm_f_32 };
        _jm_obj_7_map_tab[17] = (jm_propmap_t) { "required", _jm_f_33 };
        _jm_obj_7_map_tab[18] = (jm_propmap_t) { "type", _jm_f_34 };
        _jm_obj_7_map_tab[19] = (jm_propmap_t) { "uniqueItems", _jm_f_35 };
        jm_sort_propmap(_jm_obj_7_map_tab, 20);
        _jm_obj_9_map_tab[0] = (jm_propmap_t) { "additionalProperties", _jm_f_36 };
        _jm_obj_9_map_tab[1] = (jm_propmap_t) { "allOf", _jm_f_37 };
        _jm_obj_9_map_tab[2] = (jm_propmap_t) { "anyOf", _jm_f_38 };
        _jm_obj_9_map_tab[3] = (jm_propmap_t) { "contains", _jm_f_39 };
        _jm_obj_9_map_tab[4] = (jm_propmap_t) { "dependentSchemas", _jm_f_40 };
        _jm_obj_9_map_tab[5] = (jm_propmap_t) { "else", _jm_f_41 };
        _jm_obj_9_map_tab[6] = (jm_propmap_t) { "if", _jm_f_42 };
        _jm_obj_9_map_tab[7] = (jm_propmap_t) { "items", _jm_f_43 };
        _jm_obj_9_map_tab[8] = (jm_propmap_t) { "not", _jm_f_44 };
        _jm_obj_9_map_tab[9] = (jm_propmap_t) { "oneOf", _jm_f_45 };
        _jm_obj_9_map_tab[10] = (jm_propmap_t) { "patternProperties", _jm_f_46 };
        _jm_obj_9_map_tab[11] = (jm_propmap_t) { "prefixItems", _jm_f_47 };
        _jm_obj_9_map_tab[12] = (jm_propmap_t) { "properties", _jm_f_48 };
        _jm_obj_9_map_tab[13] = (jm_propmap_t) { "propertyDependencies", _jm_f_49 };
        _jm_obj_9_map_tab[14] = (jm_propmap_t) { "propertyNames", _jm_f_50 };
        _jm_obj_9_map_tab[15] = (jm_propmap_t) { "then", _jm_f_51 };
        jm_sort_propmap(_jm_obj_9_map_tab, 16);
        // initialize sorted set _jm_cst_2
        _jm_cst_2[0] = (jm_constant_t) { cst_is_string, { .s = "array" } };
        _jm_cst_2[1] = (jm_constant_t) { cst_is_string, { .s = "boolean" } };
        _jm_cst_2[2] = (jm_constant_t) { cst_is_string, { .s = "integer" } };
        _jm_cst_2[3] = (jm_constant_t) { cst_is_string, { .s = "null" } };
        _jm_cst_2[4] = (jm_constant_t) { cst_is_string, { .s = "number" } };
        _jm_cst_2[5] = (jm_constant_t) { cst_is_string, { .s = "object" } };
        _jm_cst_2[6] = (jm_constant_t) { cst_is_string, { .s = "string" } };
        jm_sort_cst(_jm_cst_2, 7);
        _jm_obj_15_map_tab[0] = (jm_propmap_t) { "$anchor", _jm_f_52 };
        _jm_obj_15_map_tab[1] = (jm_propmap_t) { "$comment", _jm_f_53 };
        _jm_obj_15_map_tab[2] = (jm_propmap_t) { "$defs", _jm_f_54 };
        _jm_obj_15_map_tab[3] = (jm_propmap_t) { "$dynamicAnchor", _jm_f_55 };
        _jm_obj_15_map_tab[4] = (jm_propmap_t) { "$dynamicRef", _jm_f_56 };
        _jm_obj_15_map_tab[5] = (jm_propmap_t) { "$id", _jm_f_57 };
        _jm_obj_15_map_tab[6] = (jm_propmap_t) { "$ref", _jm_f_58 };
        _jm_obj_15_map_tab[7] = (jm_propmap_t) { "$schema", _jm_f_59 };
        _jm_obj_15_map_tab[8] = (jm_propmap_t) { "$vocabulary", _jm_f_60 };
        _jm_obj_15_map_tab[9] = (jm_propmap_t) { "additionalProperties", _jm_f_61 };
        _jm_obj_15_map_tab[10] = (jm_propmap_t) { "allOf", _jm_f_62 };
        _jm_obj_15_map_tab[11] = (jm_propmap_t) { "anyOf", _jm_f_63 };
        _jm_obj_15_map_tab[12] = (jm_propmap_t) { "const", _jm_f_64 };
        _jm_obj_15_map_tab[13] = (jm_propmap_t) { "contains", _jm_f_65 };
        _jm_obj_15_map_tab[14] = (jm_propmap_t) { "contentEncoding", _jm_f_66 };
        _jm_obj_15_map_tab[15] = (jm_propmap_t) { "contentMediaType", _jm_f_67 };
        _jm_obj_15_map_tab[16] = (jm_propmap_t) { "contentSchema", _jm_f_68 };
        _jm_obj_15_map_tab[17] = (jm_propmap_t) { "default", _jm_f_69 };
        _jm_obj_15_map_tab[18] = (jm_propmap_t) { "dependentRequired", _jm_f_70 };
        _jm_obj_15_map_tab[19] = (jm_propmap_t) { "dependentSchemas", _jm_f_71 };
        _jm_obj_15_map_tab[20] = (jm_propmap_t) { "deprecated", _jm_f_72 };
        _jm_obj_15_map_tab[21] = (jm_propmap_t) { "description", _jm_f_73 };
        _jm_obj_15_map_tab[22] = (jm_propmap_t) { "else", _jm_f_74 };
        _jm_obj_15_map_tab[23] = (jm_propmap_t) { "enum", _jm_f_75 };
        _jm_obj_15_map_tab[24] = (jm_propmap_t) { "examples", _jm_f_76 };
        _jm_obj_15_map_tab[25] = (jm_propmap_t) { "exclusiveMaximum", _jm_f_77 };
        _jm_obj_15_map_tab[26] = (jm_propmap_t) { "exclusiveMinimum", _jm_f_78 };
        _jm_obj_15_map_tab[27] = (jm_propmap_t) { "format", _jm_f_79 };
        _jm_obj_15_map_tab[28] = (jm_propmap_t) { "if", _jm_f_80 };
        _jm_obj_15_map_tab[29] = (jm_propmap_t) { "items", _jm_f_81 };
        _jm_obj_15_map_tab[30] = (jm_propmap_t) { "maxContains", _jm_f_82 };
        _jm_obj_15_map_tab[31] = (jm_propmap_t) { "maxItems", _jm_f_83 };
        _jm_obj_15_map_tab[32] = (jm_propmap_t) { "maxLength", _jm_f_84 };
        _jm_obj_15_map_tab[33] = (jm_propmap_t) { "maxProperties", _jm_f_85 };
        _jm_obj_15_map_tab[34] = (jm_propmap_t) { "maximum", _jm_f_86 };
        _jm_obj_15_map_tab[35] = (jm_propmap_t) { "minContains", _jm_f_87 };
        _jm_obj_15_map_tab[36] = (jm_propmap_t) { "minItems", _jm_f_88 };
        _jm_obj_15_map_tab[37] = (jm_propmap_t) { "minLength", _jm_f_89 };
        _jm_obj_15_map_tab[38] = (jm_propmap_t) { "minProperties", _jm_f_90 };
        _jm_obj_15_map_tab[39] = (jm_propmap_t) { "minimum", _jm_f_91 };
        _jm_obj_15_map_tab[40] = (jm_propmap_t) { "multipleOf", _jm_f_92 };
        _jm_obj_15_map_tab[41] = (jm_propmap_t) { "not", _jm_f_93 };
        _jm_obj_15_map_tab[42] = (jm_propmap_t) { "oneOf", _jm_f_94 };
        _jm_obj_15_map_tab[43] = (jm_propmap_t) { "pattern", _jm_f_95 };
        _jm_obj_15_map_tab[44] = (jm_propmap_t) { "patternProperties", _jm_f_96 };
        _jm_obj_15_map_tab[45] = (jm_propmap_t) { "prefixItems", _jm_f_97 };
        _jm_obj_15_map_tab[46] = (jm_propmap_t) { "properties", _jm_f_98 };
        _jm_obj_15_map_tab[47] = (jm_propmap_t) { "propertyDependencies", _jm_f_99 };
        _jm_obj_15_map_tab[48] = (jm_propmap_t) { "propertyNames", _jm_f_100 };
        _jm_obj_15_map_tab[49] = (jm_propmap_t) { "readOnly", _jm_f_101 };
        _jm_obj_15_map_tab[50] = (jm_propmap_t) { "required", _jm_f_102 };
        _jm_obj_15_map_tab[51] = (jm_propmap_t) { "then", _jm_f_103 };
        _jm_obj_15_map_tab[52] = (jm_propmap_t) { "title", _jm_f_104 };
        _jm_obj_15_map_tab[53] = (jm_propmap_t) { "type", _jm_f_105 };
        _jm_obj_15_map_tab[54] = (jm_propmap_t) { "unevaluatedItems", _jm_f_106 };
        _jm_obj_15_map_tab[55] = (jm_propmap_t) { "unevaluatedProperties", _jm_f_107 };
        _jm_obj_15_map_tab[56] = (jm_propmap_t) { "uniqueItems", _jm_f_108 };
        _jm_obj_15_map_tab[57] = (jm_propmap_t) { "writeOnly", _jm_f_109 };
        jm_sort_propmap(_jm_obj_15_map_tab, 58);
        // initialize sorted set _jm_cst_3
        _jm_cst_3[0] = (jm_constant_t) { cst_is_string, { .s = "array" } };
        _jm_cst_3[1] = (jm_constant_t) { cst_is_string, { .s = "boolean" } };
        _jm_cst_3[2] = (jm_constant_t) { cst_is_string, { .s = "integer" } };
        _jm_cst_3[3] = (jm_constant_t) { cst_is_string, { .s = "null" } };
        _jm_cst_3[4] = (jm_constant_t) { cst_is_string, { .s = "number" } };
        _jm_cst_3[5] = (jm_constant_t) { cst_is_string, { .s = "object" } };
        _jm_cst_3[6] = (jm_constant_t) { cst_is_string, { .s = "string" } };
        jm_sort_cst(_jm_cst_3, 7);
        _jm_obj_24_map_tab[0] = (jm_propmap_t) { "$anchor", _jm_f_110 };
        _jm_obj_24_map_tab[1] = (jm_propmap_t) { "$comment", _jm_f_111 };
        _jm_obj_24_map_tab[2] = (jm_propmap_t) { "$defs", _jm_f_112 };
        _jm_obj_24_map_tab[3] = (jm_propmap_t) { "$dynamicAnchor", _jm_f_113 };
        _jm_obj_24_map_tab[4] = (jm_propmap_t) { "$dynamicRef", _jm_f_114 };
        _jm_obj_24_map_tab[5] = (jm_propmap_t) { "$id", _jm_f_115 };
        _jm_obj_24_map_tab[6] = (jm_propmap_t) { "$ref", _jm_f_116 };
        _jm_obj_24_map_tab[7] = (jm_propmap_t) { "$schema", _jm_f_117 };
        _jm_obj_24_map_tab[8] = (jm_propmap_t) { "$vocabulary", _jm_f_118 };
        _jm_obj_24_map_tab[9] = (jm_propmap_t) { "additionalProperties", _jm_f_119 };
        _jm_obj_24_map_tab[10] = (jm_propmap_t) { "allOf", _jm_f_120 };
        _jm_obj_24_map_tab[11] = (jm_propmap_t) { "anyOf", _jm_f_121 };
        _jm_obj_24_map_tab[12] = (jm_propmap_t) { "const", _jm_f_122 };
        _jm_obj_24_map_tab[13] = (jm_propmap_t) { "contains", _jm_f_123 };
        _jm_obj_24_map_tab[14] = (jm_propmap_t) { "contentEncoding", _jm_f_124 };
        _jm_obj_24_map_tab[15] = (jm_propmap_t) { "contentMediaType", _jm_f_125 };
        _jm_obj_24_map_tab[16] = (jm_propmap_t) { "contentSchema", _jm_f_126 };
        _jm_obj_24_map_tab[17] = (jm_propmap_t) { "default", _jm_f_127 };
        _jm_obj_24_map_tab[18] = (jm_propmap_t) { "dependentRequired", _jm_f_128 };
        _jm_obj_24_map_tab[19] = (jm_propmap_t) { "dependentSchemas", _jm_f_129 };
        _jm_obj_24_map_tab[20] = (jm_propmap_t) { "deprecated", _jm_f_130 };
        _jm_obj_24_map_tab[21] = (jm_propmap_t) { "description", _jm_f_131 };
        _jm_obj_24_map_tab[22] = (jm_propmap_t) { "else", _jm_f_132 };
        _jm_obj_24_map_tab[23] = (jm_propmap_t) { "enum", _jm_f_133 };
        _jm_obj_24_map_tab[24] = (jm_propmap_t) { "examples", _jm_f_134 };
        _jm_obj_24_map_tab[25] = (jm_propmap_t) { "exclusiveMaximum", _jm_f_135 };
        _jm_obj_24_map_tab[26] = (jm_propmap_t) { "exclusiveMinimum", _jm_f_136 };
        _jm_obj_24_map_tab[27] = (jm_propmap_t) { "format", _jm_f_137 };
        _jm_obj_24_map_tab[28] = (jm_propmap_t) { "if", _jm_f_138 };
        _jm_obj_24_map_tab[29] = (jm_propmap_t) { "items", _jm_f_139 };
        _jm_obj_24_map_tab[30] = (jm_propmap_t) { "maxContains", _jm_f_140 };
        _jm_obj_24_map_tab[31] = (jm_propmap_t) { "maxItems", _jm_f_141 };
        _jm_obj_24_map_tab[32] = (jm_propmap_t) { "maxLength", _jm_f_142 };
        _jm_obj_24_map_tab[33] = (jm_propmap_t) { "maxProperties", _jm_f_143 };
        _jm_obj_24_map_tab[34] = (jm_propmap_t) { "maximum", _jm_f_144 };
        _jm_obj_24_map_tab[35] = (jm_propmap_t) { "minContains", _jm_f_145 };
        _jm_obj_24_map_tab[36] = (jm_propmap_t) { "minItems", _jm_f_146 };
        _jm_obj_24_map_tab[37] = (jm_propmap_t) { "minLength", _jm_f_147 };
        _jm_obj_24_map_tab[38] = (jm_propmap_t) { "minProperties", _jm_f_148 };
        _jm_obj_24_map_tab[39] = (jm_propmap_t) { "minimum", _jm_f_149 };
        _jm_obj_24_map_tab[40] = (jm_propmap_t) { "multipleOf", _jm_f_150 };
        _jm_obj_24_map_tab[41] = (jm_propmap_t) { "not", _jm_f_151 };
        _jm_obj_24_map_tab[42] = (jm_propmap_t) { "oneOf", _jm_f_152 };
        _jm_obj_24_map_tab[43] = (jm_propmap_t) { "pattern", _jm_f_153 };
        _jm_obj_24_map_tab[44] = (jm_propmap_t) { "patternProperties", _jm_f_154 };
        _jm_obj_24_map_tab[45] = (jm_propmap_t) { "prefixItems", _jm_f_155 };
        _jm_obj_24_map_tab[46] = (jm_propmap_t) { "properties", _jm_f_156 };
        _jm_obj_24_map_tab[47] = (jm_propmap_t) { "propertyDependencies", _jm_f_157 };
        _jm_obj_24_map_tab[48] = (jm_propmap_t) { "propertyNames", _jm_f_158 };
        _jm_obj_24_map_tab[49] = (jm_propmap_t) { "readOnly", _jm_f_159 };
        _jm_obj_24_map_tab[50] = (jm_propmap_t) { "required", _jm_f_160 };
        _jm_obj_24_map_tab[51] = (jm_propmap_t) { "then", _jm_f_161 };
        _jm_obj_24_map_tab[52] = (jm_propmap_t) { "title", _jm_f_162 };
        _jm_obj_24_map_tab[53] = (jm_propmap_t) { "type", _jm_f_163 };
        _jm_obj_24_map_tab[54] = (jm_propmap_t) { "unevaluatedItems", _jm_f_164 };
        _jm_obj_24_map_tab[55] = (jm_propmap_t) { "unevaluatedProperties", _jm_f_165 };
        _jm_obj_24_map_tab[56] = (jm_propmap_t) { "uniqueItems", _jm_f_166 };
        _jm_obj_24_map_tab[57] = (jm_propmap_t) { "writeOnly", _jm_f_167 };
        jm_sort_propmap(_jm_obj_24_map_tab, 58);
        check_model_map_tab[0] = (jm_propmap_t) { "", json_model_1 };
        check_model_map_tab[1] = (jm_propmap_t) { "URI-REFERENCE", json_model_2 };
        check_model_map_tab[2] = (jm_propmap_t) { "core", json_model_3 };
        check_model_map_tab[3] = (jm_propmap_t) { "meta", json_model_4 };
        check_model_map_tab[4] = (jm_propmap_t) { "content", json_model_5 };
        check_model_map_tab[5] = (jm_propmap_t) { "unevaluated", json_model_6 };
        check_model_map_tab[6] = (jm_propmap_t) { "format", json_model_7 };
        check_model_map_tab[7] = (jm_propmap_t) { "simpleTypes", json_model_8 };
        check_model_map_tab[8] = (jm_propmap_t) { "simpleTypesArray", json_model_9 };
        check_model_map_tab[9] = (jm_propmap_t) { "stringArray", json_model_10 };
        check_model_map_tab[10] = (jm_propmap_t) { "nonNegativeInteger", json_model_11 };
        check_model_map_tab[11] = (jm_propmap_t) { "validation", json_model_12 };
        check_model_map_tab[12] = (jm_propmap_t) { "schemaArray", json_model_13 };
        check_model_map_tab[13] = (jm_propmap_t) { "applicator", json_model_14 };
        check_model_map_tab[14] = (jm_propmap_t) { "ObjectSchema", json_model_15 };
        check_model_map_tab[15] = (jm_propmap_t) { "Schema", json_model_16 };
        jm_sort_propmap(check_model_map_tab, 16);
    }
    return NULL;
}

void check_model_free(void)
{
    if (initialized)
    {
        initialized = false;

        // cleanup code
    }
}

/*
 * # API
 *
 * ```c
 * bool error;
 * char *message;
 *
 * bool valid = check(json_value, "model-name", &error, &message);
 *
 * if (message)
 *    free(message), message = NULL;
 * ```
 *
 * - `error` is set of there was an error during initialization or if the model does not exists.
 * - `message` is provided if a non NULL pointer is passed, and must be freed by the caller.
 */
bool
check_model(const json_t *val, const char *name, bool *error, char **reasons)
{
    return jm_generic_entry(check_model_init, check_model_map, val, name, error, reasons);
}
