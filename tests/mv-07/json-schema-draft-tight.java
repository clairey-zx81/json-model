//
// Generated by JSON Model Compiler version 2
// see https://github.com/clairey-zx81/json-model
//

import json_model.*;
import java.util.Map;
import java.util.Set;
import java.util.HashMap;
import java.util.HashSet;
import java.util.Iterator;
import java.util.regex.Pattern;
import java.util.regex.Matcher;

@SuppressWarnings("unchecked")
public class json_schema_draft_tight extends ModelChecker
{
    static public final String VERSION = "2";

    Set<Object> _jm_cst_0_set;
    Map<String, Checker> json_model_9_map_pmap;
    Set<Object> _jm_cst_1_set;
    Map<String, Checker> json_model_10_map_pmap;
    Map<String, Checker> json_model_11_map_pmap;
    Map<String, Checker> json_model_12_map_pmap;
    Map<String, Checker> json_model_13_map_pmap;
    Map<String, Checker> json_model_14_map_pmap;
    Map<String, Checker> json_model_15_map_pmap;
    Map<String, Checker> json_model_16_map_pmap;
    Map<String, Checker> json_model_17_map_pmap;
    Map<String, Checker> json_model_18_map_pmap;
    Map<String, Checker> json_model_19_map_pmap;
    Map<String, Checker> json_model_20_map_pmap;
    Map<String, Checker> json_model_21_map_pmap;
    Map<String, Checker> json_model_22_map_pmap;
    Map<String, Checker> json_model_23_map_pmap;
    Map<Object, Checker> _jm_map_0_cmap;
    public Map<String, Checker> json_schema_draft_tight_map_pmap;

    // check $const (.'$const')
    public boolean json_model_2(Object val, Path path, Report rep)
    {
        // .'$const'
        // .'$const'.'|'.0
        boolean res = json.isNull(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not null [.'$const'.'|'.0]", path);
        }
        if (! res)
        {
            // .'$const'.'|'.1
            res = json.isBoolean(val);
            if (! res)
            {
                if (rep != null) rep.addEntry("not a bool [.'$const'.'|'.1]", path);
            }
            if (! res)
            {
                // .'$const'.'|'.2
                res = json.isInteger(val);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a -1 strict int [.'$const'.'|'.2]", path);
                }
                if (! res)
                {
                    // .'$const'.'|'.3
                    res = json.isDouble(val);
                    if (! res)
                    {
                        if (rep != null) rep.addEntry("not a -1.0 strict float [.'$const'.'|'.3]", path);
                    }
                    if (! res)
                    {
                        // .'$const'.'|'.4
                        res = json.isString(val);
                        if (! res)
                        {
                            if (rep != null) rep.addEntry("unexpected string [.'$const'.'|'.4]", path);
                        }
                    }
                }
            }
        }
        if (res)
        {
            if (rep != null) rep.clearEntries();
        }
        else
        {
            if (rep != null) rep.addEntry("no model matched [.'$const'.'|']", path);
        }
        return res;
    }

    // check $enum (.'$enum')
    public boolean json_model_3(Object val, Path path, Report rep)
    {
        // .'$enum'
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected type [.'$enum'.'|']", path);
        }
        if (res)
        {
            // .'$enum'.'|'.0
            int arr_2_idx = -1;
            Iterator<Object> arr_2_item_loop = json.arrayIterator(val);
            while (arr_2_item_loop.hasNext())
            {
                arr_2_idx++;
                Object arr_2_item = arr_2_item_loop.next();
                Path arr_2_lpath = new Path(arr_2_idx, path);
                // .'$enum'.'|'.0.0
                res = json.isString(arr_2_item);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected string [.'$enum'.'|'.0.0]", (path != null ? arr_2_lpath : null));
                    break;
                }
            }
            if (! res)
            {
                if (rep != null) rep.addEntry("not array or unexpected array [.'$enum'.'|'.0]", path);
            }
            if (! res)
            {
                // .'$enum'.'|'.1
                int arr_1_idx = -1;
                Iterator<Object> arr_1_item_loop = json.arrayIterator(val);
                while (arr_1_item_loop.hasNext())
                {
                    arr_1_idx++;
                    Object arr_1_item = arr_1_item_loop.next();
                    Path arr_1_lpath = new Path(arr_1_idx, path);
                    // .'$enum'.'|'.1.0
                    res = json.isInteger(arr_1_item);
                    if (! res)
                    {
                        if (rep != null) rep.addEntry("not a -1 strict int [.'$enum'.'|'.1.0]", (path != null ? arr_1_lpath : null));
                        break;
                    }
                }
                if (! res)
                {
                    if (rep != null) rep.addEntry("not array or unexpected array [.'$enum'.'|'.1]", path);
                }
                if (! res)
                {
                    // .'$enum'.'|'.2
                    int arr_0_idx = -1;
                    Iterator<Object> arr_0_item_loop = json.arrayIterator(val);
                    while (arr_0_item_loop.hasNext())
                    {
                        arr_0_idx++;
                        Object arr_0_item = arr_0_item_loop.next();
                        Path arr_0_lpath = new Path(arr_0_idx, path);
                        // .'$enum'.'|'.2.0
                        res = json.isBoolean(arr_0_item);
                        if (! res)
                        {
                            if (rep != null) rep.addEntry("not a bool [.'$enum'.'|'.2.0]", (path != null ? arr_0_lpath : null));
                            break;
                        }
                    }
                    if (! res)
                    {
                        if (rep != null) rep.addEntry("not array or unexpected array [.'$enum'.'|'.2]", path);
                    }
                }
            }
            if (res)
            {
                if (rep != null) rep.clearEntries();
            }
            else
            {
                if (rep != null) rep.addEntry("no model matched [.'$enum'.'|']", path);
            }
        }
        return res;
    }

    // check $schemaArray (.'$schemaArray')
    public boolean json_model_4(Object val, Path path, Report rep)
    {
        // .'$schemaArray'
        boolean res = json.isArray(val);
        if (res)
        {
            int arr_3_idx = -1;
            Iterator<Object> arr_3_item_loop = json.arrayIterator(val);
            while (arr_3_item_loop.hasNext())
            {
                arr_3_idx++;
                Object arr_3_item = arr_3_item_loop.next();
                Path arr_3_lpath = new Path(arr_3_idx, path);
                // .'$schemaArray'.0
                res = json_model_25(arr_3_item, (path != null ? arr_3_lpath : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $Schema [.'$schemaArray'.0]", (path != null ? arr_3_lpath : null));
                    break;
                }
            }
        }
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$schemaArray']", path);
        }
        return res;
    }


    // check $stringKeywords (.'$stringKeywords')
    public boolean json_model_5(Object val, Path path, Report rep)
    {
        // .'$stringKeywords'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$stringKeywords']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_0 = new Path(prop, path);
            if (prop.compareTo("format") == 0)
            {
                // handle may format property
                // .'$stringKeywords'.format
                res = json.isScalar(pval) && _jm_cst_0_set.contains(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("value not in enum [.'$stringKeywords'.format.'|']", (path != null ? lpath_0 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$stringKeywords'.format]", (path != null ? lpath_0 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("pattern") == 0)
            {
                // handle may pattern property
                // .'$stringKeywords'.pattern
                res = json.isString(pval) && rt.is_valid_regex(json.asString(pval));
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $REGEX [.'$stringKeywords'.pattern]", (path != null ? lpath_0 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$stringKeywords'.pattern]", (path != null ? lpath_0 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("maxLength") == 0)
            {
                // handle may maxLength property
                // .'$stringKeywords'.maxLength
                res = json.isInteger(pval) && json.asLong(pval) >= 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a 0 strict int [.'$stringKeywords'.maxLength]", (path != null ? lpath_0 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$stringKeywords'.maxLength]", (path != null ? lpath_0 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("minLength") == 0)
            {
                // handle may minLength property
                // .'$stringKeywords'.minLength
                res = json.isInteger(pval) && json.asLong(pval) >= 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a 0 strict int [.'$stringKeywords'.minLength]", (path != null ? lpath_0 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$stringKeywords'.minLength]", (path != null ? lpath_0 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$stringKeywords']", (path != null ? lpath_0 : null));
            return false;
        }
        return true;
    }

    // check $arrayKeywords (.'$arrayKeywords')
    public boolean json_model_6(Object val, Path path, Report rep)
    {
        // not included: contains…
        // .'$arrayKeywords'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$arrayKeywords']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_1 = new Path(prop, path);
            if (prop.compareTo("items") == 0)
            {
                // handle may items property
                // .'$arrayKeywords'.items
                // .'$arrayKeywords'.items.'|'.0
                res = json_model_25(pval, (path != null ? lpath_1 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $Schema [.'$arrayKeywords'.items.'|'.0]", (path != null ? lpath_1 : null));
                }
                if (! res)
                {
                    // .'$arrayKeywords'.items.'|'.1
                    res = json_model_4(pval, (path != null ? lpath_1 : null), rep);
                    if (! res)
                    {
                        if (rep != null) rep.addEntry("unexpected $schemaArray [.'$arrayKeywords'.items.'|'.1]", (path != null ? lpath_1 : null));
                    }
                }
                if (res)
                {
                    if (rep != null) rep.clearEntries();
                }
                else
                {
                    if (rep != null) rep.addEntry("no model matched [.'$arrayKeywords'.items.'|']", (path != null ? lpath_1 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$arrayKeywords'.items]", (path != null ? lpath_1 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("maxItems") == 0)
            {
                // handle may maxItems property
                // .'$arrayKeywords'.maxItems
                res = json.isInteger(pval) && json.asLong(pval) >= 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a 0 strict int [.'$arrayKeywords'.maxItems]", (path != null ? lpath_1 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$arrayKeywords'.maxItems]", (path != null ? lpath_1 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("minItems") == 0)
            {
                // handle may minItems property
                // .'$arrayKeywords'.minItems
                res = json.isInteger(pval) && json.asLong(pval) >= 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a 0 strict int [.'$arrayKeywords'.minItems]", (path != null ? lpath_1 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$arrayKeywords'.minItems]", (path != null ? lpath_1 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("prefixItems") == 0)
            {
                // handle may prefixItems property
                // .'$arrayKeywords'.prefixItems
                res = json_model_4(pval, (path != null ? lpath_1 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $schemaArray [.'$arrayKeywords'.prefixItems]", (path != null ? lpath_1 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$arrayKeywords'.prefixItems]", (path != null ? lpath_1 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("uniqueItems") == 0)
            {
                // handle may uniqueItems property
                // .'$arrayKeywords'.uniqueItems
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$arrayKeywords'.uniqueItems]", (path != null ? lpath_1 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$arrayKeywords'.uniqueItems]", (path != null ? lpath_1 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("additionalItems") == 0)
            {
                // handle may additionalItems property
                // .'$arrayKeywords'.additionalItems
                res = json_model_25(pval, (path != null ? lpath_1 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $Schema [.'$arrayKeywords'.additionalItems]", (path != null ? lpath_1 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$arrayKeywords'.additionalItems]", (path != null ? lpath_1 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("unevaluatedItems") == 0)
            {
                // handle may unevaluatedItems property
                // .'$arrayKeywords'.unevaluatedItems
                res = json_model_25(pval, (path != null ? lpath_1 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $Schema [.'$arrayKeywords'.unevaluatedItems]", (path != null ? lpath_1 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$arrayKeywords'.unevaluatedItems]", (path != null ? lpath_1 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$arrayKeywords']", (path != null ? lpath_1 : null));
            return false;
        }
        return true;
    }

    // object .'$objectKeywords'.properties
    public boolean _jm_obj_0(Object val, Path path, Report rep)
    {
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$objectKeywords'.properties]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_3 = new Path(prop, path);
            // handle other props
            // .'$objectKeywords'.properties.''
            res = json_model_25(pval, (path != null ? lpath_3 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$objectKeywords'.properties.'']", (path != null ? lpath_3 : null));
                return false;
            }
        }
        return true;
    }

    // object .'$objectKeywords'.patternProperties
    public boolean _jm_obj_1(Object val, Path path, Report rep)
    {
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$objectKeywords'.patternProperties]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_4 = new Path(prop, path);
            if (rt.is_valid_regex(prop))
            {
                // handle 1 key props
                // .'$objectKeywords'.patternProperties.'$REGEX'
                res = json_model_25(pval, (path != null ? lpath_4 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $Schema [.'$objectKeywords'.patternProperties.'$REGEX']", (path != null ? lpath_4 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$objectKeywords'.patternProperties]", (path != null ? lpath_4 : null));
                return false;
            }
        }
        return true;
    }

    // check $objectKeywords (.'$objectKeywords')
    public boolean json_model_7(Object val, Path path, Report rep)
    {
        // not included: dependentRequired…
        // .'$objectKeywords'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$objectKeywords']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_2 = new Path(prop, path);
            if (prop.compareTo("required") == 0)
            {
                // handle may required property
                // .'$objectKeywords'.required
                res = json.isArray(pval);
                if (res)
                {
                    int arr_4_idx = -1;
                    Iterator<Object> arr_4_item_loop = json.arrayIterator(pval);
                    while (arr_4_item_loop.hasNext())
                    {
                        arr_4_idx++;
                        Object arr_4_item = arr_4_item_loop.next();
                        Path arr_4_lpath = new Path(arr_4_idx, (path != null ? lpath_2 : null));
                        // .'$objectKeywords'.required.0
                        res = json.isString(arr_4_item);
                        if (! res)
                        {
                            if (rep != null) rep.addEntry("unexpected string [.'$objectKeywords'.required.0]", ((path != null ? lpath_2 : null) != null ? arr_4_lpath : null));
                            break;
                        }
                    }
                }
                if (! res)
                {
                    if (rep != null) rep.addEntry("not array or unexpected array [.'$objectKeywords'.required]", (path != null ? lpath_2 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$objectKeywords'.required]", (path != null ? lpath_2 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("properties") == 0)
            {
                // handle may properties property
                // .'$objectKeywords'.properties
                res = _jm_obj_0(pval, (path != null ? lpath_2 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected element [.'$objectKeywords'.properties]", (path != null ? lpath_2 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$objectKeywords'.properties]", (path != null ? lpath_2 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("maxProperties") == 0)
            {
                // handle may maxProperties property
                // .'$objectKeywords'.maxProperties
                res = json.isInteger(pval) && json.asLong(pval) >= 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a 0 strict int [.'$objectKeywords'.maxProperties]", (path != null ? lpath_2 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$objectKeywords'.maxProperties]", (path != null ? lpath_2 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("minProperties") == 0)
            {
                // handle may minProperties property
                // .'$objectKeywords'.minProperties
                res = json.isInteger(pval) && json.asLong(pval) >= 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a 0 strict int [.'$objectKeywords'.minProperties]", (path != null ? lpath_2 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$objectKeywords'.minProperties]", (path != null ? lpath_2 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("propertyNames") == 0)
            {
                // handle may propertyNames property
                // .'$objectKeywords'.propertyNames
                res = json_model_24(pval, (path != null ? lpath_2 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $ObjectSchema [.'$objectKeywords'.propertyNames]", (path != null ? lpath_2 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$objectKeywords'.propertyNames]", (path != null ? lpath_2 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("patternProperties") == 0)
            {
                // handle may patternProperties property
                // .'$objectKeywords'.patternProperties
                res = _jm_obj_1(pval, (path != null ? lpath_2 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected element [.'$objectKeywords'.patternProperties]", (path != null ? lpath_2 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$objectKeywords'.patternProperties]", (path != null ? lpath_2 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("additionalProperties") == 0)
            {
                // handle may additionalProperties property
                // .'$objectKeywords'.additionalProperties
                res = json_model_25(pval, (path != null ? lpath_2 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $Schema [.'$objectKeywords'.additionalProperties]", (path != null ? lpath_2 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$objectKeywords'.additionalProperties]", (path != null ? lpath_2 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("unevaluatedProperties") == 0)
            {
                // handle may unevaluatedProperties property
                // .'$objectKeywords'.unevaluatedProperties
                res = json_model_25(pval, (path != null ? lpath_2 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $Schema [.'$objectKeywords'.unevaluatedProperties]", (path != null ? lpath_2 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$objectKeywords'.unevaluatedProperties]", (path != null ? lpath_2 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$objectKeywords']", (path != null ? lpath_2 : null));
            return false;
        }
        return true;
    }

    // check $numberKeywords (.'$numberKeywords')
    public boolean json_model_8(Object val, Path path, Report rep)
    {
        // not included: exclusive*, multipleOf…
        // .'$numberKeywords'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$numberKeywords']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_5 = new Path(prop, path);
            if (prop.compareTo("maximum") == 0)
            {
                // handle may maximum property
                // .'$numberKeywords'.maximum
                res = json.isDouble(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a -1.0 strict float [.'$numberKeywords'.maximum]", (path != null ? lpath_5 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$numberKeywords'.maximum]", (path != null ? lpath_5 : null));
                    return false;
                }
                continue;
            }
            else if (prop.compareTo("minimum") == 0)
            {
                // handle may minimum property
                // .'$numberKeywords'.minimum
                res = json.isDouble(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a -1.0 strict float [.'$numberKeywords'.minimum]", (path != null ? lpath_5 : null));
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$numberKeywords'.minimum]", (path != null ? lpath_5 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$numberKeywords']", (path != null ? lpath_5 : null));
            return false;
        }
        return true;
    }

    // check json_model_9_map_$anchor (.'$metas'.'$anchor')
    public boolean _jm_f_0(Object val, Path path, Report rep)
    {
        // .'$metas'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$metas'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_9_map_$comment (.'$metas'.'$comment')
    public boolean _jm_f_1(Object val, Path path, Report rep)
    {
        // .'$metas'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$metas'.'$comment']", path);
        }
        return res;
    }

    // check json_model_9_map_$defs (.'$metas'.'$defs')
    public boolean _jm_f_2(Object val, Path path, Report rep)
    {
        // .'$metas'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$metas'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_7 = new Path(prop, path);
            // handle other props
            // .'$metas'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_7 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$metas'.'$defs'.'']", (path != null ? lpath_7 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_9_map_$dynamicAnchor (.'$metas'.'$dynamicAnchor')
    public boolean _jm_f_3(Object val, Path path, Report rep)
    {
        // .'$metas'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$metas'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_9_map_$id (.'$metas'.'$id')
    public boolean _jm_f_4(Object val, Path path, Report rep)
    {
        // .'$metas'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$metas'.'$id']", path);
        }
        return res;
    }

    // check json_model_9_map_$schema (.'$metas'.'$schema')
    public boolean _jm_f_5(Object val, Path path, Report rep)
    {
        // .'$metas'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$metas'.'$schema']", path);
        }
        return res;
    }

    // check json_model_9_map_$vocabulary (.'$metas'.'$vocabulary')
    public boolean _jm_f_6(Object val, Path path, Report rep)
    {
        // .'$metas'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$metas'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_8 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$metas'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$metas'.'$vocabulary'.'$URI']", (path != null ? lpath_8 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$metas'.'$vocabulary']", (path != null ? lpath_8 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_9_map_default (.'$metas'.default)
    public boolean _jm_f_7(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$metas'.default
        res = true;
        return res;
    }

    // check json_model_9_map_definitions (.'$metas'.definitions)
    public boolean _jm_f_8(Object val, Path path, Report rep)
    {
        // .'$metas'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$metas'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_9 = new Path(prop, path);
            // handle other props
            // .'$metas'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_9 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$metas'.definitions.'']", (path != null ? lpath_9 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_9_map_deprecated (.'$metas'.deprecated)
    public boolean _jm_f_9(Object val, Path path, Report rep)
    {
        // .'$metas'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$metas'.deprecated]", path);
        }
        return res;
    }

    // check json_model_9_map_description (.'$metas'.description)
    public boolean _jm_f_10(Object val, Path path, Report rep)
    {
        // .'$metas'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$metas'.description]", path);
        }
        return res;
    }

    // check json_model_9_map_examples (.'$metas'.examples)
    public boolean _jm_f_11(Object val, Path path, Report rep)
    {
        // .'$metas'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$metas'.examples]", path);
        }
        return res;
    }

    // check json_model_9_map_id (.'$metas'.id)
    public boolean _jm_f_12(Object val, Path path, Report rep)
    {
        // .'$metas'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$metas'.id]", path);
        }
        return res;
    }

    // check json_model_9_map_readOnly (.'$metas'.readOnly)
    public boolean _jm_f_13(Object val, Path path, Report rep)
    {
        // .'$metas'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$metas'.readOnly]", path);
        }
        return res;
    }

    // check json_model_9_map_title (.'$metas'.title)
    public boolean _jm_f_14(Object val, Path path, Report rep)
    {
        // .'$metas'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$metas'.title]", path);
        }
        return res;
    }

    // check json_model_9_map_writeOnly (.'$metas'.writeOnly)
    public boolean _jm_f_15(Object val, Path path, Report rep)
    {
        // .'$metas'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$metas'.writeOnly]", path);
        }
        return res;
    }


    // check $metas (.'$metas')
    public boolean json_model_9(Object val, Path path, Report rep)
    {
        // there is a trick with $schema… in RootSchema below
        // .'$metas'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$metas']", path);
            return false;
        }
        Checker pfun;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_6 = new Path(prop, path);
            if ((pfun = json_model_9_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_6 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$metas']", (path != null ? lpath_6 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$metas']", (path != null ? lpath_6 : null));
            return false;
        }
        return true;
    }

    // check json_model_10_map_$anchor (.'$String'.'$anchor')
    public boolean _jm_f_16(Object val, Path path, Report rep)
    {
        // .'$String'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$String'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_10_map_$comment (.'$String'.'$comment')
    public boolean _jm_f_17(Object val, Path path, Report rep)
    {
        // .'$String'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$String'.'$comment']", path);
        }
        return res;
    }

    // check json_model_10_map_$defs (.'$String'.'$defs')
    public boolean _jm_f_18(Object val, Path path, Report rep)
    {
        // .'$String'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$String'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_11 = new Path(prop, path);
            // handle other props
            // .'$String'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_11 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$String'.'$defs'.'']", (path != null ? lpath_11 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_10_map_$dynamicAnchor (.'$String'.'$dynamicAnchor')
    public boolean _jm_f_19(Object val, Path path, Report rep)
    {
        // .'$String'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$String'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_10_map_$id (.'$String'.'$id')
    public boolean _jm_f_20(Object val, Path path, Report rep)
    {
        // .'$String'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$String'.'$id']", path);
        }
        return res;
    }

    // check json_model_10_map_$schema (.'$String'.'$schema')
    public boolean _jm_f_21(Object val, Path path, Report rep)
    {
        // .'$String'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$String'.'$schema']", path);
        }
        return res;
    }

    // check json_model_10_map_$vocabulary (.'$String'.'$vocabulary')
    public boolean _jm_f_22(Object val, Path path, Report rep)
    {
        // .'$String'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$String'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_12 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$String'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$String'.'$vocabulary'.'$URI']", (path != null ? lpath_12 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$String'.'$vocabulary']", (path != null ? lpath_12 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_10_map_default (.'$String'.default)
    public boolean _jm_f_23(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$String'.default
        res = true;
        return res;
    }

    // check json_model_10_map_definitions (.'$String'.definitions)
    public boolean _jm_f_24(Object val, Path path, Report rep)
    {
        // .'$String'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$String'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_13 = new Path(prop, path);
            // handle other props
            // .'$String'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_13 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$String'.definitions.'']", (path != null ? lpath_13 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_10_map_deprecated (.'$String'.deprecated)
    public boolean _jm_f_25(Object val, Path path, Report rep)
    {
        // .'$String'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$String'.deprecated]", path);
        }
        return res;
    }

    // check json_model_10_map_description (.'$String'.description)
    public boolean _jm_f_26(Object val, Path path, Report rep)
    {
        // .'$String'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$String'.description]", path);
        }
        return res;
    }

    // check json_model_10_map_examples (.'$String'.examples)
    public boolean _jm_f_27(Object val, Path path, Report rep)
    {
        // .'$String'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$String'.examples]", path);
        }
        return res;
    }


    // check json_model_10_map_format (.'$String'.format)
    public boolean _jm_f_28(Object val, Path path, Report rep)
    {
        // .'$String'.format
        boolean res = json.isScalar(val) && _jm_cst_1_set.contains(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("value not in enum [.'$String'.format.'|']", path);
        }
        return res;
    }

    // check json_model_10_map_id (.'$String'.id)
    public boolean _jm_f_29(Object val, Path path, Report rep)
    {
        // .'$String'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$String'.id]", path);
        }
        return res;
    }

    // check json_model_10_map_maxLength (.'$String'.maxLength)
    public boolean _jm_f_30(Object val, Path path, Report rep)
    {
        // .'$String'.maxLength
        boolean res = json.isInteger(val) && json.asLong(val) >= 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("not a 0 strict int [.'$String'.maxLength]", path);
        }
        return res;
    }

    // check json_model_10_map_minLength (.'$String'.minLength)
    public boolean _jm_f_31(Object val, Path path, Report rep)
    {
        // .'$String'.minLength
        boolean res = json.isInteger(val) && json.asLong(val) >= 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("not a 0 strict int [.'$String'.minLength]", path);
        }
        return res;
    }

    // check json_model_10_map_pattern (.'$String'.pattern)
    public boolean _jm_f_32(Object val, Path path, Report rep)
    {
        // .'$String'.pattern
        boolean res = json.isString(val) && rt.is_valid_regex(json.asString(val));
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected $REGEX [.'$String'.pattern]", path);
        }
        return res;
    }

    // check json_model_10_map_readOnly (.'$String'.readOnly)
    public boolean _jm_f_33(Object val, Path path, Report rep)
    {
        // .'$String'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$String'.readOnly]", path);
        }
        return res;
    }

    // check json_model_10_map_title (.'$String'.title)
    public boolean _jm_f_34(Object val, Path path, Report rep)
    {
        // .'$String'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$String'.title]", path);
        }
        return res;
    }

    // check json_model_10_map_writeOnly (.'$String'.writeOnly)
    public boolean _jm_f_35(Object val, Path path, Report rep)
    {
        // .'$String'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$String'.writeOnly]", path);
        }
        return res;
    }


    // check $String (.'$String')
    public boolean json_model_10(Object val, Path path, Report rep)
    {
        // .'$String'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$String']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_10 = new Path(prop, path);
            if (prop.compareTo("type") == 0)
            {
                // handle must type property
                must_count += 1;
                // .'$String'.type
                res = json.isString(pval) && json.asString(pval).compareTo("string") == 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected string [.'$String'.type]", (path != null ? lpath_10 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$String'.type]", (path != null ? lpath_10 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_10_map_pmap.get(prop)) != null)
            {
                // handle 20 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_10 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$String']", (path != null ? lpath_10 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$String']", (path != null ? lpath_10 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "type"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <type> [.'$String']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_11_map_$anchor (.'$Array'.'$anchor')
    public boolean _jm_f_36(Object val, Path path, Report rep)
    {
        // .'$Array'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Array'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_11_map_$comment (.'$Array'.'$comment')
    public boolean _jm_f_37(Object val, Path path, Report rep)
    {
        // .'$Array'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Array'.'$comment']", path);
        }
        return res;
    }

    // check json_model_11_map_$defs (.'$Array'.'$defs')
    public boolean _jm_f_38(Object val, Path path, Report rep)
    {
        // .'$Array'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Array'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_15 = new Path(prop, path);
            // handle other props
            // .'$Array'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_15 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Array'.'$defs'.'']", (path != null ? lpath_15 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_11_map_$dynamicAnchor (.'$Array'.'$dynamicAnchor')
    public boolean _jm_f_39(Object val, Path path, Report rep)
    {
        // .'$Array'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Array'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_11_map_$id (.'$Array'.'$id')
    public boolean _jm_f_40(Object val, Path path, Report rep)
    {
        // .'$Array'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Array'.'$id']", path);
        }
        return res;
    }

    // check json_model_11_map_$schema (.'$Array'.'$schema')
    public boolean _jm_f_41(Object val, Path path, Report rep)
    {
        // .'$Array'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Array'.'$schema']", path);
        }
        return res;
    }

    // check json_model_11_map_$vocabulary (.'$Array'.'$vocabulary')
    public boolean _jm_f_42(Object val, Path path, Report rep)
    {
        // .'$Array'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Array'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_16 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Array'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Array'.'$vocabulary'.'$URI']", (path != null ? lpath_16 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Array'.'$vocabulary']", (path != null ? lpath_16 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_11_map_default (.'$Array'.default)
    public boolean _jm_f_43(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Array'.default
        res = true;
        return res;
    }

    // check json_model_11_map_definitions (.'$Array'.definitions)
    public boolean _jm_f_44(Object val, Path path, Report rep)
    {
        // .'$Array'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Array'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_17 = new Path(prop, path);
            // handle other props
            // .'$Array'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_17 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Array'.definitions.'']", (path != null ? lpath_17 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_11_map_deprecated (.'$Array'.deprecated)
    public boolean _jm_f_45(Object val, Path path, Report rep)
    {
        // .'$Array'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Array'.deprecated]", path);
        }
        return res;
    }

    // check json_model_11_map_description (.'$Array'.description)
    public boolean _jm_f_46(Object val, Path path, Report rep)
    {
        // .'$Array'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Array'.description]", path);
        }
        return res;
    }

    // check json_model_11_map_examples (.'$Array'.examples)
    public boolean _jm_f_47(Object val, Path path, Report rep)
    {
        // .'$Array'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Array'.examples]", path);
        }
        return res;
    }

    // check json_model_11_map_id (.'$Array'.id)
    public boolean _jm_f_48(Object val, Path path, Report rep)
    {
        // .'$Array'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Array'.id]", path);
        }
        return res;
    }

    // check json_model_11_map_items (.'$Array'.items)
    public boolean _jm_f_49(Object val, Path path, Report rep)
    {
        // .'$Array'.items
        // .'$Array'.items.'|'.0
        boolean res = json_model_25(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected $Schema [.'$Array'.items.'|'.0]", path);
        }
        if (! res)
        {
            // .'$Array'.items.'|'.1
            res = json_model_4(val, path, rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $schemaArray [.'$Array'.items.'|'.1]", path);
            }
        }
        if (res)
        {
            if (rep != null) rep.clearEntries();
        }
        else
        {
            if (rep != null) rep.addEntry("no model matched [.'$Array'.items.'|']", path);
        }
        return res;
    }

    // check json_model_11_map_maxItems (.'$Array'.maxItems)
    public boolean _jm_f_50(Object val, Path path, Report rep)
    {
        // .'$Array'.maxItems
        boolean res = json.isInteger(val) && json.asLong(val) >= 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("not a 0 strict int [.'$Array'.maxItems]", path);
        }
        return res;
    }

    // check json_model_11_map_minItems (.'$Array'.minItems)
    public boolean _jm_f_51(Object val, Path path, Report rep)
    {
        // .'$Array'.minItems
        boolean res = json.isInteger(val) && json.asLong(val) >= 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("not a 0 strict int [.'$Array'.minItems]", path);
        }
        return res;
    }

    // check json_model_11_map_readOnly (.'$Array'.readOnly)
    public boolean _jm_f_52(Object val, Path path, Report rep)
    {
        // .'$Array'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Array'.readOnly]", path);
        }
        return res;
    }

    // check json_model_11_map_title (.'$Array'.title)
    public boolean _jm_f_53(Object val, Path path, Report rep)
    {
        // .'$Array'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Array'.title]", path);
        }
        return res;
    }

    // check json_model_11_map_uniqueItems (.'$Array'.uniqueItems)
    public boolean _jm_f_54(Object val, Path path, Report rep)
    {
        // .'$Array'.uniqueItems
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Array'.uniqueItems]", path);
        }
        return res;
    }

    // check json_model_11_map_writeOnly (.'$Array'.writeOnly)
    public boolean _jm_f_55(Object val, Path path, Report rep)
    {
        // .'$Array'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Array'.writeOnly]", path);
        }
        return res;
    }


    // check $Array (.'$Array')
    public boolean json_model_11(Object val, Path path, Report rep)
    {
        // .'$Array'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Array']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_14 = new Path(prop, path);
            if (prop.compareTo("type") == 0)
            {
                // handle must type property
                must_count += 1;
                // .'$Array'.type
                res = json.isString(pval) && json.asString(pval).compareTo("array") == 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected array [.'$Array'.type]", (path != null ? lpath_14 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Array'.type]", (path != null ? lpath_14 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_11_map_pmap.get(prop)) != null)
            {
                // handle 23 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_14 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Array']", (path != null ? lpath_14 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Array']", (path != null ? lpath_14 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "type"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <type> [.'$Array']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_12_map_$anchor (.'$Object'.'$anchor')
    public boolean _jm_f_56(Object val, Path path, Report rep)
    {
        // .'$Object'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Object'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_12_map_$comment (.'$Object'.'$comment')
    public boolean _jm_f_57(Object val, Path path, Report rep)
    {
        // .'$Object'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Object'.'$comment']", path);
        }
        return res;
    }

    // check json_model_12_map_$defs (.'$Object'.'$defs')
    public boolean _jm_f_58(Object val, Path path, Report rep)
    {
        // .'$Object'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Object'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_19 = new Path(prop, path);
            // handle other props
            // .'$Object'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_19 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Object'.'$defs'.'']", (path != null ? lpath_19 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_12_map_$dynamicAnchor (.'$Object'.'$dynamicAnchor')
    public boolean _jm_f_59(Object val, Path path, Report rep)
    {
        // .'$Object'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Object'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_12_map_$id (.'$Object'.'$id')
    public boolean _jm_f_60(Object val, Path path, Report rep)
    {
        // .'$Object'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Object'.'$id']", path);
        }
        return res;
    }

    // check json_model_12_map_$schema (.'$Object'.'$schema')
    public boolean _jm_f_61(Object val, Path path, Report rep)
    {
        // .'$Object'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Object'.'$schema']", path);
        }
        return res;
    }

    // check json_model_12_map_$vocabulary (.'$Object'.'$vocabulary')
    public boolean _jm_f_62(Object val, Path path, Report rep)
    {
        // .'$Object'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Object'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_20 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Object'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Object'.'$vocabulary'.'$URI']", (path != null ? lpath_20 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Object'.'$vocabulary']", (path != null ? lpath_20 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_12_map_default (.'$Object'.default)
    public boolean _jm_f_63(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Object'.default
        res = true;
        return res;
    }

    // check json_model_12_map_definitions (.'$Object'.definitions)
    public boolean _jm_f_64(Object val, Path path, Report rep)
    {
        // .'$Object'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Object'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_21 = new Path(prop, path);
            // handle other props
            // .'$Object'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_21 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Object'.definitions.'']", (path != null ? lpath_21 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_12_map_deprecated (.'$Object'.deprecated)
    public boolean _jm_f_65(Object val, Path path, Report rep)
    {
        // .'$Object'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Object'.deprecated]", path);
        }
        return res;
    }

    // check json_model_12_map_description (.'$Object'.description)
    public boolean _jm_f_66(Object val, Path path, Report rep)
    {
        // .'$Object'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Object'.description]", path);
        }
        return res;
    }

    // check json_model_12_map_examples (.'$Object'.examples)
    public boolean _jm_f_67(Object val, Path path, Report rep)
    {
        // .'$Object'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Object'.examples]", path);
        }
        return res;
    }

    // check json_model_12_map_id (.'$Object'.id)
    public boolean _jm_f_68(Object val, Path path, Report rep)
    {
        // .'$Object'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Object'.id]", path);
        }
        return res;
    }

    // check json_model_12_map_maxProperties (.'$Object'.maxProperties)
    public boolean _jm_f_69(Object val, Path path, Report rep)
    {
        // .'$Object'.maxProperties
        boolean res = json.isInteger(val) && json.asLong(val) >= 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("not a 0 strict int [.'$Object'.maxProperties]", path);
        }
        return res;
    }

    // check json_model_12_map_minProperties (.'$Object'.minProperties)
    public boolean _jm_f_70(Object val, Path path, Report rep)
    {
        // .'$Object'.minProperties
        boolean res = json.isInteger(val) && json.asLong(val) >= 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("not a 0 strict int [.'$Object'.minProperties]", path);
        }
        return res;
    }

    // check json_model_12_map_patternProperties (.'$Object'.patternProperties)
    public boolean _jm_f_71(Object val, Path path, Report rep)
    {
        // .'$Object'.patternProperties
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Object'.patternProperties]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_22 = new Path(prop, path);
            if (rt.is_valid_regex(prop))
            {
                // handle 1 key props
                // .'$Object'.patternProperties.'$REGEX'
                res = json_model_25(pval, (path != null ? lpath_22 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $Schema [.'$Object'.patternProperties.'$REGEX']", (path != null ? lpath_22 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Object'.patternProperties]", (path != null ? lpath_22 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_12_map_properties (.'$Object'.properties)
    public boolean _jm_f_72(Object val, Path path, Report rep)
    {
        // .'$Object'.properties
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Object'.properties]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_23 = new Path(prop, path);
            // handle other props
            // .'$Object'.properties.''
            res = json_model_25(pval, (path != null ? lpath_23 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Object'.properties.'']", (path != null ? lpath_23 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_12_map_readOnly (.'$Object'.readOnly)
    public boolean _jm_f_73(Object val, Path path, Report rep)
    {
        // .'$Object'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Object'.readOnly]", path);
        }
        return res;
    }

    // check json_model_12_map_required (.'$Object'.required)
    public boolean _jm_f_74(Object val, Path path, Report rep)
    {
        // .'$Object'.required
        boolean res = json.isArray(val);
        if (res)
        {
            int arr_5_idx = -1;
            Iterator<Object> arr_5_item_loop = json.arrayIterator(val);
            while (arr_5_item_loop.hasNext())
            {
                arr_5_idx++;
                Object arr_5_item = arr_5_item_loop.next();
                Path arr_5_lpath = new Path(arr_5_idx, path);
                // .'$Object'.required.0
                res = json.isString(arr_5_item);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected string [.'$Object'.required.0]", (path != null ? arr_5_lpath : null));
                    break;
                }
            }
        }
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Object'.required]", path);
        }
        return res;
    }

    // check json_model_12_map_title (.'$Object'.title)
    public boolean _jm_f_75(Object val, Path path, Report rep)
    {
        // .'$Object'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Object'.title]", path);
        }
        return res;
    }

    // check json_model_12_map_writeOnly (.'$Object'.writeOnly)
    public boolean _jm_f_76(Object val, Path path, Report rep)
    {
        // .'$Object'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Object'.writeOnly]", path);
        }
        return res;
    }


    // check $Object (.'$Object')
    public boolean json_model_12(Object val, Path path, Report rep)
    {
        // .'$Object'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Object']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_18 = new Path(prop, path);
            if (prop.compareTo("type") == 0)
            {
                // handle must type property
                must_count += 1;
                // .'$Object'.type
                res = json.isString(pval) && json.asString(pval).compareTo("object") == 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected object [.'$Object'.type]", (path != null ? lpath_18 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Object'.type]", (path != null ? lpath_18 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_12_map_pmap.get(prop)) != null)
            {
                // handle 24 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_18 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Object']", (path != null ? lpath_18 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Object']", (path != null ? lpath_18 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "type"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <type> [.'$Object']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_13_map_$anchor (.'$Integer'.'$anchor')
    public boolean _jm_f_77(Object val, Path path, Report rep)
    {
        // .'$Integer'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Integer'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_13_map_$comment (.'$Integer'.'$comment')
    public boolean _jm_f_78(Object val, Path path, Report rep)
    {
        // .'$Integer'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Integer'.'$comment']", path);
        }
        return res;
    }

    // check json_model_13_map_$defs (.'$Integer'.'$defs')
    public boolean _jm_f_79(Object val, Path path, Report rep)
    {
        // .'$Integer'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Integer'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_25 = new Path(prop, path);
            // handle other props
            // .'$Integer'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_25 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Integer'.'$defs'.'']", (path != null ? lpath_25 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_13_map_$dynamicAnchor (.'$Integer'.'$dynamicAnchor')
    public boolean _jm_f_80(Object val, Path path, Report rep)
    {
        // .'$Integer'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Integer'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_13_map_$id (.'$Integer'.'$id')
    public boolean _jm_f_81(Object val, Path path, Report rep)
    {
        // .'$Integer'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Integer'.'$id']", path);
        }
        return res;
    }

    // check json_model_13_map_$schema (.'$Integer'.'$schema')
    public boolean _jm_f_82(Object val, Path path, Report rep)
    {
        // .'$Integer'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Integer'.'$schema']", path);
        }
        return res;
    }

    // check json_model_13_map_$vocabulary (.'$Integer'.'$vocabulary')
    public boolean _jm_f_83(Object val, Path path, Report rep)
    {
        // .'$Integer'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Integer'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_26 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Integer'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Integer'.'$vocabulary'.'$URI']", (path != null ? lpath_26 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Integer'.'$vocabulary']", (path != null ? lpath_26 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_13_map_default (.'$Integer'.default)
    public boolean _jm_f_84(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Integer'.default
        res = true;
        return res;
    }

    // check json_model_13_map_definitions (.'$Integer'.definitions)
    public boolean _jm_f_85(Object val, Path path, Report rep)
    {
        // .'$Integer'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Integer'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_27 = new Path(prop, path);
            // handle other props
            // .'$Integer'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_27 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Integer'.definitions.'']", (path != null ? lpath_27 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_13_map_deprecated (.'$Integer'.deprecated)
    public boolean _jm_f_86(Object val, Path path, Report rep)
    {
        // .'$Integer'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Integer'.deprecated]", path);
        }
        return res;
    }

    // check json_model_13_map_description (.'$Integer'.description)
    public boolean _jm_f_87(Object val, Path path, Report rep)
    {
        // .'$Integer'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Integer'.description]", path);
        }
        return res;
    }

    // check json_model_13_map_examples (.'$Integer'.examples)
    public boolean _jm_f_88(Object val, Path path, Report rep)
    {
        // .'$Integer'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Integer'.examples]", path);
        }
        return res;
    }

    // check json_model_13_map_id (.'$Integer'.id)
    public boolean _jm_f_89(Object val, Path path, Report rep)
    {
        // .'$Integer'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Integer'.id]", path);
        }
        return res;
    }

    // check json_model_13_map_maximum (.'$Integer'.maximum)
    public boolean _jm_f_90(Object val, Path path, Report rep)
    {
        // .'$Integer'.maximum
        boolean res = json.isDouble(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a -1.0 strict float [.'$Integer'.maximum]", path);
        }
        return res;
    }

    // check json_model_13_map_minimum (.'$Integer'.minimum)
    public boolean _jm_f_91(Object val, Path path, Report rep)
    {
        // .'$Integer'.minimum
        boolean res = json.isDouble(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a -1.0 strict float [.'$Integer'.minimum]", path);
        }
        return res;
    }

    // check json_model_13_map_readOnly (.'$Integer'.readOnly)
    public boolean _jm_f_92(Object val, Path path, Report rep)
    {
        // .'$Integer'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Integer'.readOnly]", path);
        }
        return res;
    }

    // check json_model_13_map_title (.'$Integer'.title)
    public boolean _jm_f_93(Object val, Path path, Report rep)
    {
        // .'$Integer'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Integer'.title]", path);
        }
        return res;
    }

    // check json_model_13_map_writeOnly (.'$Integer'.writeOnly)
    public boolean _jm_f_94(Object val, Path path, Report rep)
    {
        // .'$Integer'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Integer'.writeOnly]", path);
        }
        return res;
    }


    // check $Integer (.'$Integer')
    public boolean json_model_13(Object val, Path path, Report rep)
    {
        // .'$Integer'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Integer']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_24 = new Path(prop, path);
            if (prop.compareTo("type") == 0)
            {
                // handle must type property
                must_count += 1;
                // .'$Integer'.type
                res = json.isString(pval) && json.asString(pval).compareTo("integer") == 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected integer [.'$Integer'.type]", (path != null ? lpath_24 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Integer'.type]", (path != null ? lpath_24 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_13_map_pmap.get(prop)) != null)
            {
                // handle 18 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_24 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Integer']", (path != null ? lpath_24 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Integer']", (path != null ? lpath_24 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "type"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <type> [.'$Integer']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_14_map_$anchor (.'$Number'.'$anchor')
    public boolean _jm_f_95(Object val, Path path, Report rep)
    {
        // .'$Number'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Number'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_14_map_$comment (.'$Number'.'$comment')
    public boolean _jm_f_96(Object val, Path path, Report rep)
    {
        // .'$Number'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Number'.'$comment']", path);
        }
        return res;
    }

    // check json_model_14_map_$defs (.'$Number'.'$defs')
    public boolean _jm_f_97(Object val, Path path, Report rep)
    {
        // .'$Number'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Number'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_29 = new Path(prop, path);
            // handle other props
            // .'$Number'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_29 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Number'.'$defs'.'']", (path != null ? lpath_29 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_14_map_$dynamicAnchor (.'$Number'.'$dynamicAnchor')
    public boolean _jm_f_98(Object val, Path path, Report rep)
    {
        // .'$Number'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Number'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_14_map_$id (.'$Number'.'$id')
    public boolean _jm_f_99(Object val, Path path, Report rep)
    {
        // .'$Number'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Number'.'$id']", path);
        }
        return res;
    }

    // check json_model_14_map_$schema (.'$Number'.'$schema')
    public boolean _jm_f_100(Object val, Path path, Report rep)
    {
        // .'$Number'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Number'.'$schema']", path);
        }
        return res;
    }

    // check json_model_14_map_$vocabulary (.'$Number'.'$vocabulary')
    public boolean _jm_f_101(Object val, Path path, Report rep)
    {
        // .'$Number'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Number'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_30 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Number'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Number'.'$vocabulary'.'$URI']", (path != null ? lpath_30 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Number'.'$vocabulary']", (path != null ? lpath_30 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_14_map_default (.'$Number'.default)
    public boolean _jm_f_102(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Number'.default
        res = true;
        return res;
    }

    // check json_model_14_map_definitions (.'$Number'.definitions)
    public boolean _jm_f_103(Object val, Path path, Report rep)
    {
        // .'$Number'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Number'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_31 = new Path(prop, path);
            // handle other props
            // .'$Number'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_31 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Number'.definitions.'']", (path != null ? lpath_31 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_14_map_deprecated (.'$Number'.deprecated)
    public boolean _jm_f_104(Object val, Path path, Report rep)
    {
        // .'$Number'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Number'.deprecated]", path);
        }
        return res;
    }

    // check json_model_14_map_description (.'$Number'.description)
    public boolean _jm_f_105(Object val, Path path, Report rep)
    {
        // .'$Number'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Number'.description]", path);
        }
        return res;
    }

    // check json_model_14_map_examples (.'$Number'.examples)
    public boolean _jm_f_106(Object val, Path path, Report rep)
    {
        // .'$Number'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Number'.examples]", path);
        }
        return res;
    }

    // check json_model_14_map_id (.'$Number'.id)
    public boolean _jm_f_107(Object val, Path path, Report rep)
    {
        // .'$Number'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Number'.id]", path);
        }
        return res;
    }

    // check json_model_14_map_maximum (.'$Number'.maximum)
    public boolean _jm_f_108(Object val, Path path, Report rep)
    {
        // .'$Number'.maximum
        boolean res = json.isDouble(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a -1.0 strict float [.'$Number'.maximum]", path);
        }
        return res;
    }

    // check json_model_14_map_minimum (.'$Number'.minimum)
    public boolean _jm_f_109(Object val, Path path, Report rep)
    {
        // .'$Number'.minimum
        boolean res = json.isDouble(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a -1.0 strict float [.'$Number'.minimum]", path);
        }
        return res;
    }

    // check json_model_14_map_readOnly (.'$Number'.readOnly)
    public boolean _jm_f_110(Object val, Path path, Report rep)
    {
        // .'$Number'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Number'.readOnly]", path);
        }
        return res;
    }

    // check json_model_14_map_title (.'$Number'.title)
    public boolean _jm_f_111(Object val, Path path, Report rep)
    {
        // .'$Number'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Number'.title]", path);
        }
        return res;
    }

    // check json_model_14_map_writeOnly (.'$Number'.writeOnly)
    public boolean _jm_f_112(Object val, Path path, Report rep)
    {
        // .'$Number'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Number'.writeOnly]", path);
        }
        return res;
    }


    // check $Number (.'$Number')
    public boolean json_model_14(Object val, Path path, Report rep)
    {
        // .'$Number'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Number']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_28 = new Path(prop, path);
            if (prop.compareTo("type") == 0)
            {
                // handle must type property
                must_count += 1;
                // .'$Number'.type
                res = json.isString(pval) && json.asString(pval).compareTo("number") == 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected number [.'$Number'.type]", (path != null ? lpath_28 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Number'.type]", (path != null ? lpath_28 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_14_map_pmap.get(prop)) != null)
            {
                // handle 18 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_28 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Number']", (path != null ? lpath_28 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Number']", (path != null ? lpath_28 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "type"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <type> [.'$Number']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_15_map_$anchor (.'$Bool'.'$anchor')
    public boolean _jm_f_113(Object val, Path path, Report rep)
    {
        // .'$Bool'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Bool'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_15_map_$comment (.'$Bool'.'$comment')
    public boolean _jm_f_114(Object val, Path path, Report rep)
    {
        // .'$Bool'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Bool'.'$comment']", path);
        }
        return res;
    }

    // check json_model_15_map_$defs (.'$Bool'.'$defs')
    public boolean _jm_f_115(Object val, Path path, Report rep)
    {
        // .'$Bool'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Bool'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_33 = new Path(prop, path);
            // handle other props
            // .'$Bool'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_33 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Bool'.'$defs'.'']", (path != null ? lpath_33 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_15_map_$dynamicAnchor (.'$Bool'.'$dynamicAnchor')
    public boolean _jm_f_116(Object val, Path path, Report rep)
    {
        // .'$Bool'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Bool'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_15_map_$id (.'$Bool'.'$id')
    public boolean _jm_f_117(Object val, Path path, Report rep)
    {
        // .'$Bool'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Bool'.'$id']", path);
        }
        return res;
    }

    // check json_model_15_map_$schema (.'$Bool'.'$schema')
    public boolean _jm_f_118(Object val, Path path, Report rep)
    {
        // .'$Bool'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Bool'.'$schema']", path);
        }
        return res;
    }

    // check json_model_15_map_$vocabulary (.'$Bool'.'$vocabulary')
    public boolean _jm_f_119(Object val, Path path, Report rep)
    {
        // .'$Bool'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Bool'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_34 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Bool'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Bool'.'$vocabulary'.'$URI']", (path != null ? lpath_34 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Bool'.'$vocabulary']", (path != null ? lpath_34 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_15_map_default (.'$Bool'.default)
    public boolean _jm_f_120(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Bool'.default
        res = true;
        return res;
    }

    // check json_model_15_map_definitions (.'$Bool'.definitions)
    public boolean _jm_f_121(Object val, Path path, Report rep)
    {
        // .'$Bool'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Bool'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_35 = new Path(prop, path);
            // handle other props
            // .'$Bool'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_35 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Bool'.definitions.'']", (path != null ? lpath_35 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_15_map_deprecated (.'$Bool'.deprecated)
    public boolean _jm_f_122(Object val, Path path, Report rep)
    {
        // .'$Bool'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Bool'.deprecated]", path);
        }
        return res;
    }

    // check json_model_15_map_description (.'$Bool'.description)
    public boolean _jm_f_123(Object val, Path path, Report rep)
    {
        // .'$Bool'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Bool'.description]", path);
        }
        return res;
    }

    // check json_model_15_map_examples (.'$Bool'.examples)
    public boolean _jm_f_124(Object val, Path path, Report rep)
    {
        // .'$Bool'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Bool'.examples]", path);
        }
        return res;
    }

    // check json_model_15_map_id (.'$Bool'.id)
    public boolean _jm_f_125(Object val, Path path, Report rep)
    {
        // .'$Bool'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Bool'.id]", path);
        }
        return res;
    }

    // check json_model_15_map_readOnly (.'$Bool'.readOnly)
    public boolean _jm_f_126(Object val, Path path, Report rep)
    {
        // .'$Bool'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Bool'.readOnly]", path);
        }
        return res;
    }

    // check json_model_15_map_title (.'$Bool'.title)
    public boolean _jm_f_127(Object val, Path path, Report rep)
    {
        // .'$Bool'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Bool'.title]", path);
        }
        return res;
    }

    // check json_model_15_map_writeOnly (.'$Bool'.writeOnly)
    public boolean _jm_f_128(Object val, Path path, Report rep)
    {
        // .'$Bool'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Bool'.writeOnly]", path);
        }
        return res;
    }


    // check $Bool (.'$Bool')
    public boolean json_model_15(Object val, Path path, Report rep)
    {
        // .'$Bool'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Bool']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_32 = new Path(prop, path);
            if (prop.compareTo("type") == 0)
            {
                // handle must type property
                must_count += 1;
                // .'$Bool'.type
                res = json.isString(pval) && json.asString(pval).compareTo("boolean") == 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected boolean [.'$Bool'.type]", (path != null ? lpath_32 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Bool'.type]", (path != null ? lpath_32 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_15_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_32 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Bool']", (path != null ? lpath_32 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Bool']", (path != null ? lpath_32 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "type"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <type> [.'$Bool']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_16_map_$anchor (.'$Null'.'$anchor')
    public boolean _jm_f_129(Object val, Path path, Report rep)
    {
        // .'$Null'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Null'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_16_map_$comment (.'$Null'.'$comment')
    public boolean _jm_f_130(Object val, Path path, Report rep)
    {
        // .'$Null'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Null'.'$comment']", path);
        }
        return res;
    }

    // check json_model_16_map_$defs (.'$Null'.'$defs')
    public boolean _jm_f_131(Object val, Path path, Report rep)
    {
        // .'$Null'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Null'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_37 = new Path(prop, path);
            // handle other props
            // .'$Null'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_37 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Null'.'$defs'.'']", (path != null ? lpath_37 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_16_map_$dynamicAnchor (.'$Null'.'$dynamicAnchor')
    public boolean _jm_f_132(Object val, Path path, Report rep)
    {
        // .'$Null'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Null'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_16_map_$id (.'$Null'.'$id')
    public boolean _jm_f_133(Object val, Path path, Report rep)
    {
        // .'$Null'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Null'.'$id']", path);
        }
        return res;
    }

    // check json_model_16_map_$schema (.'$Null'.'$schema')
    public boolean _jm_f_134(Object val, Path path, Report rep)
    {
        // .'$Null'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Null'.'$schema']", path);
        }
        return res;
    }

    // check json_model_16_map_$vocabulary (.'$Null'.'$vocabulary')
    public boolean _jm_f_135(Object val, Path path, Report rep)
    {
        // .'$Null'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Null'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_38 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Null'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Null'.'$vocabulary'.'$URI']", (path != null ? lpath_38 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Null'.'$vocabulary']", (path != null ? lpath_38 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_16_map_default (.'$Null'.default)
    public boolean _jm_f_136(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Null'.default
        res = true;
        return res;
    }

    // check json_model_16_map_definitions (.'$Null'.definitions)
    public boolean _jm_f_137(Object val, Path path, Report rep)
    {
        // .'$Null'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Null'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_39 = new Path(prop, path);
            // handle other props
            // .'$Null'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_39 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Null'.definitions.'']", (path != null ? lpath_39 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_16_map_deprecated (.'$Null'.deprecated)
    public boolean _jm_f_138(Object val, Path path, Report rep)
    {
        // .'$Null'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Null'.deprecated]", path);
        }
        return res;
    }

    // check json_model_16_map_description (.'$Null'.description)
    public boolean _jm_f_139(Object val, Path path, Report rep)
    {
        // .'$Null'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Null'.description]", path);
        }
        return res;
    }

    // check json_model_16_map_examples (.'$Null'.examples)
    public boolean _jm_f_140(Object val, Path path, Report rep)
    {
        // .'$Null'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Null'.examples]", path);
        }
        return res;
    }

    // check json_model_16_map_id (.'$Null'.id)
    public boolean _jm_f_141(Object val, Path path, Report rep)
    {
        // .'$Null'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Null'.id]", path);
        }
        return res;
    }

    // check json_model_16_map_readOnly (.'$Null'.readOnly)
    public boolean _jm_f_142(Object val, Path path, Report rep)
    {
        // .'$Null'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Null'.readOnly]", path);
        }
        return res;
    }

    // check json_model_16_map_title (.'$Null'.title)
    public boolean _jm_f_143(Object val, Path path, Report rep)
    {
        // .'$Null'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Null'.title]", path);
        }
        return res;
    }

    // check json_model_16_map_writeOnly (.'$Null'.writeOnly)
    public boolean _jm_f_144(Object val, Path path, Report rep)
    {
        // .'$Null'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Null'.writeOnly]", path);
        }
        return res;
    }


    // check $Null (.'$Null')
    public boolean json_model_16(Object val, Path path, Report rep)
    {
        // .'$Null'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Null']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_36 = new Path(prop, path);
            if (prop.compareTo("type") == 0)
            {
                // handle must type property
                must_count += 1;
                // .'$Null'.type
                res = json.isString(pval) && json.asString(pval).compareTo("null") == 0;
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected null [.'$Null'.type]", (path != null ? lpath_36 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Null'.type]", (path != null ? lpath_36 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_16_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_36 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Null']", (path != null ? lpath_36 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Null']", (path != null ? lpath_36 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "type"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <type> [.'$Null']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_17_map_$anchor (.'$AllOf'.'$anchor')
    public boolean _jm_f_145(Object val, Path path, Report rep)
    {
        // .'$AllOf'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AllOf'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_17_map_$comment (.'$AllOf'.'$comment')
    public boolean _jm_f_146(Object val, Path path, Report rep)
    {
        // .'$AllOf'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AllOf'.'$comment']", path);
        }
        return res;
    }

    // check json_model_17_map_$defs (.'$AllOf'.'$defs')
    public boolean _jm_f_147(Object val, Path path, Report rep)
    {
        // .'$AllOf'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$AllOf'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_41 = new Path(prop, path);
            // handle other props
            // .'$AllOf'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_41 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$AllOf'.'$defs'.'']", (path != null ? lpath_41 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_17_map_$dynamicAnchor (.'$AllOf'.'$dynamicAnchor')
    public boolean _jm_f_148(Object val, Path path, Report rep)
    {
        // .'$AllOf'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AllOf'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_17_map_$id (.'$AllOf'.'$id')
    public boolean _jm_f_149(Object val, Path path, Report rep)
    {
        // .'$AllOf'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AllOf'.'$id']", path);
        }
        return res;
    }

    // check json_model_17_map_$schema (.'$AllOf'.'$schema')
    public boolean _jm_f_150(Object val, Path path, Report rep)
    {
        // .'$AllOf'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AllOf'.'$schema']", path);
        }
        return res;
    }

    // check json_model_17_map_$vocabulary (.'$AllOf'.'$vocabulary')
    public boolean _jm_f_151(Object val, Path path, Report rep)
    {
        // .'$AllOf'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$AllOf'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_42 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$AllOf'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$AllOf'.'$vocabulary'.'$URI']", (path != null ? lpath_42 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$AllOf'.'$vocabulary']", (path != null ? lpath_42 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_17_map_default (.'$AllOf'.default)
    public boolean _jm_f_152(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$AllOf'.default
        res = true;
        return res;
    }

    // check json_model_17_map_definitions (.'$AllOf'.definitions)
    public boolean _jm_f_153(Object val, Path path, Report rep)
    {
        // .'$AllOf'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$AllOf'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_43 = new Path(prop, path);
            // handle other props
            // .'$AllOf'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_43 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$AllOf'.definitions.'']", (path != null ? lpath_43 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_17_map_deprecated (.'$AllOf'.deprecated)
    public boolean _jm_f_154(Object val, Path path, Report rep)
    {
        // .'$AllOf'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$AllOf'.deprecated]", path);
        }
        return res;
    }

    // check json_model_17_map_description (.'$AllOf'.description)
    public boolean _jm_f_155(Object val, Path path, Report rep)
    {
        // .'$AllOf'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AllOf'.description]", path);
        }
        return res;
    }

    // check json_model_17_map_examples (.'$AllOf'.examples)
    public boolean _jm_f_156(Object val, Path path, Report rep)
    {
        // .'$AllOf'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$AllOf'.examples]", path);
        }
        return res;
    }

    // check json_model_17_map_id (.'$AllOf'.id)
    public boolean _jm_f_157(Object val, Path path, Report rep)
    {
        // .'$AllOf'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AllOf'.id]", path);
        }
        return res;
    }

    // check json_model_17_map_readOnly (.'$AllOf'.readOnly)
    public boolean _jm_f_158(Object val, Path path, Report rep)
    {
        // .'$AllOf'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$AllOf'.readOnly]", path);
        }
        return res;
    }

    // check json_model_17_map_title (.'$AllOf'.title)
    public boolean _jm_f_159(Object val, Path path, Report rep)
    {
        // .'$AllOf'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AllOf'.title]", path);
        }
        return res;
    }

    // check json_model_17_map_writeOnly (.'$AllOf'.writeOnly)
    public boolean _jm_f_160(Object val, Path path, Report rep)
    {
        // .'$AllOf'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$AllOf'.writeOnly]", path);
        }
        return res;
    }


    // check $AllOf (.'$AllOf')
    public boolean json_model_17(Object val, Path path, Report rep)
    {
        // .'$AllOf'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$AllOf']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_40 = new Path(prop, path);
            if (prop.compareTo("allOf") == 0)
            {
                // handle must allOf property
                must_count += 1;
                // .'$AllOf'.allOf
                res = json_model_4(pval, (path != null ? lpath_40 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $schemaArray [.'$AllOf'.allOf]", (path != null ? lpath_40 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$AllOf'.allOf]", (path != null ? lpath_40 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_17_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_40 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$AllOf']", (path != null ? lpath_40 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$AllOf']", (path != null ? lpath_40 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "allOf"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <allOf> [.'$AllOf']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_18_map_$anchor (.'$AnyOf'.'$anchor')
    public boolean _jm_f_161(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AnyOf'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_18_map_$comment (.'$AnyOf'.'$comment')
    public boolean _jm_f_162(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AnyOf'.'$comment']", path);
        }
        return res;
    }

    // check json_model_18_map_$defs (.'$AnyOf'.'$defs')
    public boolean _jm_f_163(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$AnyOf'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_45 = new Path(prop, path);
            // handle other props
            // .'$AnyOf'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_45 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$AnyOf'.'$defs'.'']", (path != null ? lpath_45 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_18_map_$dynamicAnchor (.'$AnyOf'.'$dynamicAnchor')
    public boolean _jm_f_164(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AnyOf'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_18_map_$id (.'$AnyOf'.'$id')
    public boolean _jm_f_165(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AnyOf'.'$id']", path);
        }
        return res;
    }

    // check json_model_18_map_$schema (.'$AnyOf'.'$schema')
    public boolean _jm_f_166(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AnyOf'.'$schema']", path);
        }
        return res;
    }

    // check json_model_18_map_$vocabulary (.'$AnyOf'.'$vocabulary')
    public boolean _jm_f_167(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$AnyOf'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_46 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$AnyOf'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$AnyOf'.'$vocabulary'.'$URI']", (path != null ? lpath_46 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$AnyOf'.'$vocabulary']", (path != null ? lpath_46 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_18_map_default (.'$AnyOf'.default)
    public boolean _jm_f_168(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$AnyOf'.default
        res = true;
        return res;
    }

    // check json_model_18_map_definitions (.'$AnyOf'.definitions)
    public boolean _jm_f_169(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$AnyOf'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_47 = new Path(prop, path);
            // handle other props
            // .'$AnyOf'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_47 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$AnyOf'.definitions.'']", (path != null ? lpath_47 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_18_map_deprecated (.'$AnyOf'.deprecated)
    public boolean _jm_f_170(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$AnyOf'.deprecated]", path);
        }
        return res;
    }

    // check json_model_18_map_description (.'$AnyOf'.description)
    public boolean _jm_f_171(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AnyOf'.description]", path);
        }
        return res;
    }

    // check json_model_18_map_examples (.'$AnyOf'.examples)
    public boolean _jm_f_172(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$AnyOf'.examples]", path);
        }
        return res;
    }

    // check json_model_18_map_id (.'$AnyOf'.id)
    public boolean _jm_f_173(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AnyOf'.id]", path);
        }
        return res;
    }

    // check json_model_18_map_readOnly (.'$AnyOf'.readOnly)
    public boolean _jm_f_174(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$AnyOf'.readOnly]", path);
        }
        return res;
    }

    // check json_model_18_map_title (.'$AnyOf'.title)
    public boolean _jm_f_175(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$AnyOf'.title]", path);
        }
        return res;
    }

    // check json_model_18_map_writeOnly (.'$AnyOf'.writeOnly)
    public boolean _jm_f_176(Object val, Path path, Report rep)
    {
        // .'$AnyOf'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$AnyOf'.writeOnly]", path);
        }
        return res;
    }


    // check $AnyOf (.'$AnyOf')
    public boolean json_model_18(Object val, Path path, Report rep)
    {
        // .'$AnyOf'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$AnyOf']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_44 = new Path(prop, path);
            if (prop.compareTo("anyOf") == 0)
            {
                // handle must anyOf property
                must_count += 1;
                // .'$AnyOf'.anyOf
                res = json_model_4(pval, (path != null ? lpath_44 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $schemaArray [.'$AnyOf'.anyOf]", (path != null ? lpath_44 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$AnyOf'.anyOf]", (path != null ? lpath_44 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_18_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_44 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$AnyOf']", (path != null ? lpath_44 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$AnyOf']", (path != null ? lpath_44 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "anyOf"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <anyOf> [.'$AnyOf']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_19_map_$anchor (.'$OneOf'.'$anchor')
    public boolean _jm_f_177(Object val, Path path, Report rep)
    {
        // .'$OneOf'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$OneOf'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_19_map_$comment (.'$OneOf'.'$comment')
    public boolean _jm_f_178(Object val, Path path, Report rep)
    {
        // .'$OneOf'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$OneOf'.'$comment']", path);
        }
        return res;
    }

    // check json_model_19_map_$defs (.'$OneOf'.'$defs')
    public boolean _jm_f_179(Object val, Path path, Report rep)
    {
        // .'$OneOf'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$OneOf'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_49 = new Path(prop, path);
            // handle other props
            // .'$OneOf'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_49 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$OneOf'.'$defs'.'']", (path != null ? lpath_49 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_19_map_$dynamicAnchor (.'$OneOf'.'$dynamicAnchor')
    public boolean _jm_f_180(Object val, Path path, Report rep)
    {
        // .'$OneOf'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$OneOf'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_19_map_$id (.'$OneOf'.'$id')
    public boolean _jm_f_181(Object val, Path path, Report rep)
    {
        // .'$OneOf'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$OneOf'.'$id']", path);
        }
        return res;
    }

    // check json_model_19_map_$schema (.'$OneOf'.'$schema')
    public boolean _jm_f_182(Object val, Path path, Report rep)
    {
        // .'$OneOf'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$OneOf'.'$schema']", path);
        }
        return res;
    }

    // check json_model_19_map_$vocabulary (.'$OneOf'.'$vocabulary')
    public boolean _jm_f_183(Object val, Path path, Report rep)
    {
        // .'$OneOf'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$OneOf'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_50 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$OneOf'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$OneOf'.'$vocabulary'.'$URI']", (path != null ? lpath_50 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$OneOf'.'$vocabulary']", (path != null ? lpath_50 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_19_map_default (.'$OneOf'.default)
    public boolean _jm_f_184(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$OneOf'.default
        res = true;
        return res;
    }

    // check json_model_19_map_definitions (.'$OneOf'.definitions)
    public boolean _jm_f_185(Object val, Path path, Report rep)
    {
        // .'$OneOf'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$OneOf'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_51 = new Path(prop, path);
            // handle other props
            // .'$OneOf'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_51 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$OneOf'.definitions.'']", (path != null ? lpath_51 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_19_map_deprecated (.'$OneOf'.deprecated)
    public boolean _jm_f_186(Object val, Path path, Report rep)
    {
        // .'$OneOf'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$OneOf'.deprecated]", path);
        }
        return res;
    }

    // check json_model_19_map_description (.'$OneOf'.description)
    public boolean _jm_f_187(Object val, Path path, Report rep)
    {
        // .'$OneOf'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$OneOf'.description]", path);
        }
        return res;
    }

    // check json_model_19_map_examples (.'$OneOf'.examples)
    public boolean _jm_f_188(Object val, Path path, Report rep)
    {
        // .'$OneOf'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$OneOf'.examples]", path);
        }
        return res;
    }

    // check json_model_19_map_id (.'$OneOf'.id)
    public boolean _jm_f_189(Object val, Path path, Report rep)
    {
        // .'$OneOf'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$OneOf'.id]", path);
        }
        return res;
    }

    // check json_model_19_map_readOnly (.'$OneOf'.readOnly)
    public boolean _jm_f_190(Object val, Path path, Report rep)
    {
        // .'$OneOf'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$OneOf'.readOnly]", path);
        }
        return res;
    }

    // check json_model_19_map_title (.'$OneOf'.title)
    public boolean _jm_f_191(Object val, Path path, Report rep)
    {
        // .'$OneOf'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$OneOf'.title]", path);
        }
        return res;
    }

    // check json_model_19_map_writeOnly (.'$OneOf'.writeOnly)
    public boolean _jm_f_192(Object val, Path path, Report rep)
    {
        // .'$OneOf'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$OneOf'.writeOnly]", path);
        }
        return res;
    }


    // check $OneOf (.'$OneOf')
    public boolean json_model_19(Object val, Path path, Report rep)
    {
        // .'$OneOf'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$OneOf']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_48 = new Path(prop, path);
            if (prop.compareTo("oneOf") == 0)
            {
                // handle must oneOf property
                must_count += 1;
                // .'$OneOf'.oneOf
                res = json_model_4(pval, (path != null ? lpath_48 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $schemaArray [.'$OneOf'.oneOf]", (path != null ? lpath_48 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$OneOf'.oneOf]", (path != null ? lpath_48 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_19_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_48 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$OneOf']", (path != null ? lpath_48 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$OneOf']", (path != null ? lpath_48 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "oneOf"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <oneOf> [.'$OneOf']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_20_map_$anchor (.'$Enum'.'$anchor')
    public boolean _jm_f_193(Object val, Path path, Report rep)
    {
        // .'$Enum'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Enum'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_20_map_$comment (.'$Enum'.'$comment')
    public boolean _jm_f_194(Object val, Path path, Report rep)
    {
        // .'$Enum'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Enum'.'$comment']", path);
        }
        return res;
    }

    // check json_model_20_map_$defs (.'$Enum'.'$defs')
    public boolean _jm_f_195(Object val, Path path, Report rep)
    {
        // .'$Enum'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Enum'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_53 = new Path(prop, path);
            // handle other props
            // .'$Enum'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_53 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Enum'.'$defs'.'']", (path != null ? lpath_53 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_20_map_$dynamicAnchor (.'$Enum'.'$dynamicAnchor')
    public boolean _jm_f_196(Object val, Path path, Report rep)
    {
        // .'$Enum'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Enum'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_20_map_$id (.'$Enum'.'$id')
    public boolean _jm_f_197(Object val, Path path, Report rep)
    {
        // .'$Enum'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Enum'.'$id']", path);
        }
        return res;
    }

    // check json_model_20_map_$schema (.'$Enum'.'$schema')
    public boolean _jm_f_198(Object val, Path path, Report rep)
    {
        // .'$Enum'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Enum'.'$schema']", path);
        }
        return res;
    }

    // check json_model_20_map_$vocabulary (.'$Enum'.'$vocabulary')
    public boolean _jm_f_199(Object val, Path path, Report rep)
    {
        // .'$Enum'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Enum'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_54 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Enum'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Enum'.'$vocabulary'.'$URI']", (path != null ? lpath_54 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Enum'.'$vocabulary']", (path != null ? lpath_54 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_20_map_default (.'$Enum'.default)
    public boolean _jm_f_200(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Enum'.default
        res = true;
        return res;
    }

    // check json_model_20_map_definitions (.'$Enum'.definitions)
    public boolean _jm_f_201(Object val, Path path, Report rep)
    {
        // .'$Enum'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Enum'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_55 = new Path(prop, path);
            // handle other props
            // .'$Enum'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_55 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Enum'.definitions.'']", (path != null ? lpath_55 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_20_map_deprecated (.'$Enum'.deprecated)
    public boolean _jm_f_202(Object val, Path path, Report rep)
    {
        // .'$Enum'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Enum'.deprecated]", path);
        }
        return res;
    }

    // check json_model_20_map_description (.'$Enum'.description)
    public boolean _jm_f_203(Object val, Path path, Report rep)
    {
        // .'$Enum'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Enum'.description]", path);
        }
        return res;
    }

    // check json_model_20_map_examples (.'$Enum'.examples)
    public boolean _jm_f_204(Object val, Path path, Report rep)
    {
        // .'$Enum'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Enum'.examples]", path);
        }
        return res;
    }

    // check json_model_20_map_id (.'$Enum'.id)
    public boolean _jm_f_205(Object val, Path path, Report rep)
    {
        // .'$Enum'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Enum'.id]", path);
        }
        return res;
    }

    // check json_model_20_map_readOnly (.'$Enum'.readOnly)
    public boolean _jm_f_206(Object val, Path path, Report rep)
    {
        // .'$Enum'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Enum'.readOnly]", path);
        }
        return res;
    }

    // check json_model_20_map_title (.'$Enum'.title)
    public boolean _jm_f_207(Object val, Path path, Report rep)
    {
        // .'$Enum'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Enum'.title]", path);
        }
        return res;
    }

    // check json_model_20_map_writeOnly (.'$Enum'.writeOnly)
    public boolean _jm_f_208(Object val, Path path, Report rep)
    {
        // .'$Enum'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Enum'.writeOnly]", path);
        }
        return res;
    }


    // check $Enum (.'$Enum')
    public boolean json_model_20(Object val, Path path, Report rep)
    {
        // .'$Enum'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Enum']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_52 = new Path(prop, path);
            if (prop.compareTo("enum") == 0)
            {
                // handle must enum property
                must_count += 1;
                // .'$Enum'.enum
                res = json_model_3(pval, (path != null ? lpath_52 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $enum [.'$Enum'.enum]", (path != null ? lpath_52 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Enum'.enum]", (path != null ? lpath_52 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_20_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_52 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Enum']", (path != null ? lpath_52 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Enum']", (path != null ? lpath_52 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "enum"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <enum> [.'$Enum']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_21_map_$anchor (.'$Const'.'$anchor')
    public boolean _jm_f_209(Object val, Path path, Report rep)
    {
        // .'$Const'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Const'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_21_map_$comment (.'$Const'.'$comment')
    public boolean _jm_f_210(Object val, Path path, Report rep)
    {
        // .'$Const'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Const'.'$comment']", path);
        }
        return res;
    }

    // check json_model_21_map_$defs (.'$Const'.'$defs')
    public boolean _jm_f_211(Object val, Path path, Report rep)
    {
        // .'$Const'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Const'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_57 = new Path(prop, path);
            // handle other props
            // .'$Const'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_57 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Const'.'$defs'.'']", (path != null ? lpath_57 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_21_map_$dynamicAnchor (.'$Const'.'$dynamicAnchor')
    public boolean _jm_f_212(Object val, Path path, Report rep)
    {
        // .'$Const'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Const'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_21_map_$id (.'$Const'.'$id')
    public boolean _jm_f_213(Object val, Path path, Report rep)
    {
        // .'$Const'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Const'.'$id']", path);
        }
        return res;
    }

    // check json_model_21_map_$schema (.'$Const'.'$schema')
    public boolean _jm_f_214(Object val, Path path, Report rep)
    {
        // .'$Const'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Const'.'$schema']", path);
        }
        return res;
    }

    // check json_model_21_map_$vocabulary (.'$Const'.'$vocabulary')
    public boolean _jm_f_215(Object val, Path path, Report rep)
    {
        // .'$Const'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Const'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_58 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Const'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Const'.'$vocabulary'.'$URI']", (path != null ? lpath_58 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Const'.'$vocabulary']", (path != null ? lpath_58 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_21_map_default (.'$Const'.default)
    public boolean _jm_f_216(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Const'.default
        res = true;
        return res;
    }

    // check json_model_21_map_definitions (.'$Const'.definitions)
    public boolean _jm_f_217(Object val, Path path, Report rep)
    {
        // .'$Const'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Const'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_59 = new Path(prop, path);
            // handle other props
            // .'$Const'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_59 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Const'.definitions.'']", (path != null ? lpath_59 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_21_map_deprecated (.'$Const'.deprecated)
    public boolean _jm_f_218(Object val, Path path, Report rep)
    {
        // .'$Const'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Const'.deprecated]", path);
        }
        return res;
    }

    // check json_model_21_map_description (.'$Const'.description)
    public boolean _jm_f_219(Object val, Path path, Report rep)
    {
        // .'$Const'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Const'.description]", path);
        }
        return res;
    }

    // check json_model_21_map_examples (.'$Const'.examples)
    public boolean _jm_f_220(Object val, Path path, Report rep)
    {
        // .'$Const'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Const'.examples]", path);
        }
        return res;
    }

    // check json_model_21_map_id (.'$Const'.id)
    public boolean _jm_f_221(Object val, Path path, Report rep)
    {
        // .'$Const'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Const'.id]", path);
        }
        return res;
    }

    // check json_model_21_map_readOnly (.'$Const'.readOnly)
    public boolean _jm_f_222(Object val, Path path, Report rep)
    {
        // .'$Const'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Const'.readOnly]", path);
        }
        return res;
    }

    // check json_model_21_map_title (.'$Const'.title)
    public boolean _jm_f_223(Object val, Path path, Report rep)
    {
        // .'$Const'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Const'.title]", path);
        }
        return res;
    }

    // check json_model_21_map_writeOnly (.'$Const'.writeOnly)
    public boolean _jm_f_224(Object val, Path path, Report rep)
    {
        // .'$Const'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Const'.writeOnly]", path);
        }
        return res;
    }


    // check $Const (.'$Const')
    public boolean json_model_21(Object val, Path path, Report rep)
    {
        // .'$Const'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Const']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_56 = new Path(prop, path);
            if (prop.compareTo("const") == 0)
            {
                // handle must const property
                must_count += 1;
                // .'$Const'.const
                res = json_model_2(pval, (path != null ? lpath_56 : null), rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $const [.'$Const'.const]", (path != null ? lpath_56 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Const'.const]", (path != null ? lpath_56 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_21_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_56 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Const']", (path != null ? lpath_56 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Const']", (path != null ? lpath_56 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "const"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <const> [.'$Const']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_22_map_$anchor (.'$Ref'.'$anchor')
    public boolean _jm_f_225(Object val, Path path, Report rep)
    {
        // .'$Ref'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Ref'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_22_map_$comment (.'$Ref'.'$comment')
    public boolean _jm_f_226(Object val, Path path, Report rep)
    {
        // .'$Ref'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Ref'.'$comment']", path);
        }
        return res;
    }

    // check json_model_22_map_$defs (.'$Ref'.'$defs')
    public boolean _jm_f_227(Object val, Path path, Report rep)
    {
        // .'$Ref'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Ref'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_61 = new Path(prop, path);
            // handle other props
            // .'$Ref'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_61 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Ref'.'$defs'.'']", (path != null ? lpath_61 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_22_map_$dynamicAnchor (.'$Ref'.'$dynamicAnchor')
    public boolean _jm_f_228(Object val, Path path, Report rep)
    {
        // .'$Ref'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Ref'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_22_map_$id (.'$Ref'.'$id')
    public boolean _jm_f_229(Object val, Path path, Report rep)
    {
        // .'$Ref'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Ref'.'$id']", path);
        }
        return res;
    }

    // check json_model_22_map_$schema (.'$Ref'.'$schema')
    public boolean _jm_f_230(Object val, Path path, Report rep)
    {
        // .'$Ref'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Ref'.'$schema']", path);
        }
        return res;
    }

    // check json_model_22_map_$vocabulary (.'$Ref'.'$vocabulary')
    public boolean _jm_f_231(Object val, Path path, Report rep)
    {
        // .'$Ref'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Ref'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_62 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$Ref'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$Ref'.'$vocabulary'.'$URI']", (path != null ? lpath_62 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$Ref'.'$vocabulary']", (path != null ? lpath_62 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_22_map_default (.'$Ref'.default)
    public boolean _jm_f_232(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Ref'.default
        res = true;
        return res;
    }

    // check json_model_22_map_definitions (.'$Ref'.definitions)
    public boolean _jm_f_233(Object val, Path path, Report rep)
    {
        // .'$Ref'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Ref'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_63 = new Path(prop, path);
            // handle other props
            // .'$Ref'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_63 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$Ref'.definitions.'']", (path != null ? lpath_63 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_22_map_deprecated (.'$Ref'.deprecated)
    public boolean _jm_f_234(Object val, Path path, Report rep)
    {
        // .'$Ref'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Ref'.deprecated]", path);
        }
        return res;
    }

    // check json_model_22_map_description (.'$Ref'.description)
    public boolean _jm_f_235(Object val, Path path, Report rep)
    {
        // .'$Ref'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Ref'.description]", path);
        }
        return res;
    }

    // check json_model_22_map_examples (.'$Ref'.examples)
    public boolean _jm_f_236(Object val, Path path, Report rep)
    {
        // .'$Ref'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$Ref'.examples]", path);
        }
        return res;
    }

    // check json_model_22_map_id (.'$Ref'.id)
    public boolean _jm_f_237(Object val, Path path, Report rep)
    {
        // .'$Ref'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Ref'.id]", path);
        }
        return res;
    }

    // check json_model_22_map_readOnly (.'$Ref'.readOnly)
    public boolean _jm_f_238(Object val, Path path, Report rep)
    {
        // .'$Ref'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Ref'.readOnly]", path);
        }
        return res;
    }

    // check json_model_22_map_title (.'$Ref'.title)
    public boolean _jm_f_239(Object val, Path path, Report rep)
    {
        // .'$Ref'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Ref'.title]", path);
        }
        return res;
    }

    // check json_model_22_map_writeOnly (.'$Ref'.writeOnly)
    public boolean _jm_f_240(Object val, Path path, Report rep)
    {
        // .'$Ref'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Ref'.writeOnly]", path);
        }
        return res;
    }


    // check $Ref (.'$Ref')
    public boolean json_model_22(Object val, Path path, Report rep)
    {
        // .'$Ref'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Ref']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_60 = new Path(prop, path);
            if (prop.compareTo("$ref") == 0)
            {
                // handle must $ref property
                must_count += 1;
                // .'$Ref'.'$ref'
                res = json.isString(pval) && rt.is_valid_url(json.asString(pval));
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $URI [.'$Ref'.'$ref']", (path != null ? lpath_60 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$Ref'.'$ref']", (path != null ? lpath_60 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_22_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_60 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$Ref']", (path != null ? lpath_60 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$Ref']", (path != null ? lpath_60 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "$ref"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <$ref> [.'$Ref']", path);
                }
            }
            return false;
        }
        return true;
    }

    // check json_model_23_map_$anchor (.'$DynRef'.'$anchor')
    public boolean _jm_f_241(Object val, Path path, Report rep)
    {
        // .'$DynRef'.'$anchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$DynRef'.'$anchor']", path);
        }
        return res;
    }

    // check json_model_23_map_$comment (.'$DynRef'.'$comment')
    public boolean _jm_f_242(Object val, Path path, Report rep)
    {
        // .'$DynRef'.'$comment'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$DynRef'.'$comment']", path);
        }
        return res;
    }

    // check json_model_23_map_$defs (.'$DynRef'.'$defs')
    public boolean _jm_f_243(Object val, Path path, Report rep)
    {
        // .'$DynRef'.'$defs'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$DynRef'.'$defs']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_65 = new Path(prop, path);
            // handle other props
            // .'$DynRef'.'$defs'.''
            res = json_model_25(pval, (path != null ? lpath_65 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$DynRef'.'$defs'.'']", (path != null ? lpath_65 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_23_map_$dynamicAnchor (.'$DynRef'.'$dynamicAnchor')
    public boolean _jm_f_244(Object val, Path path, Report rep)
    {
        // .'$DynRef'.'$dynamicAnchor'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$DynRef'.'$dynamicAnchor']", path);
        }
        return res;
    }

    // check json_model_23_map_$id (.'$DynRef'.'$id')
    public boolean _jm_f_245(Object val, Path path, Report rep)
    {
        // .'$DynRef'.'$id'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$DynRef'.'$id']", path);
        }
        return res;
    }

    // check json_model_23_map_$schema (.'$DynRef'.'$schema')
    public boolean _jm_f_246(Object val, Path path, Report rep)
    {
        // .'$DynRef'.'$schema'
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$DynRef'.'$schema']", path);
        }
        return res;
    }

    // check json_model_23_map_$vocabulary (.'$DynRef'.'$vocabulary')
    public boolean _jm_f_247(Object val, Path path, Report rep)
    {
        // .'$DynRef'.'$vocabulary'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$DynRef'.'$vocabulary']", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_66 = new Path(prop, path);
            if (rt.is_valid_url(prop))
            {
                // handle 1 key props
                // .'$DynRef'.'$vocabulary'.'$URI'
                res = json.isBoolean(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("not a bool [.'$DynRef'.'$vocabulary'.'$URI']", (path != null ? lpath_66 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$DynRef'.'$vocabulary']", (path != null ? lpath_66 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_23_map_default (.'$DynRef'.default)
    public boolean _jm_f_248(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$DynRef'.default
        res = true;
        return res;
    }

    // check json_model_23_map_definitions (.'$DynRef'.definitions)
    public boolean _jm_f_249(Object val, Path path, Report rep)
    {
        // .'$DynRef'.definitions
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$DynRef'.definitions]", path);
            return false;
        }
        boolean res;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_67 = new Path(prop, path);
            // handle other props
            // .'$DynRef'.definitions.''
            res = json_model_25(pval, (path != null ? lpath_67 : null), rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $Schema [.'$DynRef'.definitions.'']", (path != null ? lpath_67 : null));
                return false;
            }
        }
        return true;
    }

    // check json_model_23_map_deprecated (.'$DynRef'.deprecated)
    public boolean _jm_f_250(Object val, Path path, Report rep)
    {
        // .'$DynRef'.deprecated
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$DynRef'.deprecated]", path);
        }
        return res;
    }

    // check json_model_23_map_description (.'$DynRef'.description)
    public boolean _jm_f_251(Object val, Path path, Report rep)
    {
        // .'$DynRef'.description
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$DynRef'.description]", path);
        }
        return res;
    }

    // check json_model_23_map_examples (.'$DynRef'.examples)
    public boolean _jm_f_252(Object val, Path path, Report rep)
    {
        // .'$DynRef'.examples
        boolean res = json.isArray(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not array or unexpected array [.'$DynRef'.examples]", path);
        }
        return res;
    }

    // check json_model_23_map_id (.'$DynRef'.id)
    public boolean _jm_f_253(Object val, Path path, Report rep)
    {
        // .'$DynRef'.id
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$DynRef'.id]", path);
        }
        return res;
    }

    // check json_model_23_map_readOnly (.'$DynRef'.readOnly)
    public boolean _jm_f_254(Object val, Path path, Report rep)
    {
        // .'$DynRef'.readOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$DynRef'.readOnly]", path);
        }
        return res;
    }

    // check json_model_23_map_title (.'$DynRef'.title)
    public boolean _jm_f_255(Object val, Path path, Report rep)
    {
        // .'$DynRef'.title
        boolean res = json.isString(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$DynRef'.title]", path);
        }
        return res;
    }

    // check json_model_23_map_writeOnly (.'$DynRef'.writeOnly)
    public boolean _jm_f_256(Object val, Path path, Report rep)
    {
        // .'$DynRef'.writeOnly
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$DynRef'.writeOnly]", path);
        }
        return res;
    }


    // check $DynRef (.'$DynRef')
    public boolean json_model_23(Object val, Path path, Report rep)
    {
        // .'$DynRef'
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$DynRef']", path);
            return false;
        }
        boolean res;
        Checker pfun;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_64 = new Path(prop, path);
            if (prop.compareTo("$dynamicRef") == 0)
            {
                // handle must $dynamicRef property
                must_count += 1;
                // .'$DynRef'.'$dynamicRef'
                res = json.isString(pval) && rt.is_valid_url(json.asString(pval));
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $URI [.'$DynRef'.'$dynamicRef']", (path != null ? lpath_64 : null));
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$DynRef'.'$dynamicRef']", (path != null ? lpath_64 : null));
                    return false;
                }
                continue;
            }
            if ((pfun = json_model_23_map_pmap.get(prop)) != null)
            {
                // handle 16 may props
                if (pfun != null && ! (pfun.call(pval, (path != null ? lpath_64 : null), rep)))
                {
                    if (rep != null) rep.addEntry("invalid optional prop value [.'$DynRef']", (path != null ? lpath_64 : null));
                    return false;
                }
                continue;
            }
            if (rep != null) rep.addEntry("unexpected prop [.'$DynRef']", (path != null ? lpath_64 : null));
            return false;
        }
        if (must_count != 1)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "$dynamicRef"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <$dynamicRef> [.'$DynRef']", path);
                }
            }
            return false;
        }
        return true;
    }


    // check $ObjectSchema (.'$ObjectSchema')
    public boolean json_model_24(Object val, Path path, Report rep)
    {
        boolean res;
        // we could use ^ instead of | below
        // .'$ObjectSchema'
        boolean iso_0 = json.isObject(val);
        res = iso_0;
        if (res)
        {
            if (json.objectHasProp(val, "type"))
            {
                Object tag_0 = json.objectValue(val, "type");
                Checker fun_0 = _jm_map_0_cmap.get(tag_0);
                if (fun_0 != null)
                {
                    res = fun_0.call(val, path, rep);
                }
                else
                {
                    res = false;
                    if (rep != null) rep.addEntry("tag <type> value not found [.'$ObjectSchema'.'|']", path);
                }
            }
            else
            {
                res = false;
                if (rep != null) rep.addEntry("tag prop <type> is missing [.'$ObjectSchema'.'|']", path);
            }
        }
        else
        {
            if (rep != null) rep.addEntry("value is not an object [.'$ObjectSchema'.'|']", path);
        }
        return res || json_model_9(val, path, rep) || json_model_21(val, path, rep) || json_model_20(val, path, rep) || json_model_17(val, path, rep) || json_model_18(val, path, rep) || json_model_19(val, path, rep) || json_model_22(val, path, rep) || json_model_23(val, path, rep);
    }

    // check $Schema (.'$Schema')
    public boolean json_model_25(Object val, Path path, Report rep)
    {
        // .'$Schema'
        // .'$Schema'.'|'.0
        boolean res = json.isBoolean(val);
        if (! res)
        {
            if (rep != null) rep.addEntry("not a bool [.'$Schema'.'|'.0]", path);
        }
        if (! res)
        {
            // .'$Schema'.'|'.1
            res = json_model_24(val, path, rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected $ObjectSchema [.'$Schema'.'|'.1]", path);
            }
        }
        if (res)
        {
            if (rep != null) rep.clearEntries();
        }
        else
        {
            if (rep != null) rep.addEntry("no model matched [.'$Schema'.'|']", path);
        }
        return res;
    }

    // object .'$RootSchema'.'&'.0
    public boolean _jm_obj_2(Object val, Path path, Report rep)
    {
        // check open must/may only props
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$RootSchema'.'&'.0]", path);
            return false;
        }
        Path lpath;
        Object pval;
        boolean res;
        if (! ((pval = json.objectValue(val, "$schema")) != null))
        {
            if (rep != null) rep.addEntry("missing mandatory prop <$schema> [.'$RootSchema'.'&'.0]", path);
            return false;
        }
        lpath = new Path("$schema", path);
        // .'$RootSchema'.'&'.0.'$schema'
        res = json.isString(pval);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$RootSchema'.'&'.0.'$schema']", (path != null ? lpath : null));
            if (rep != null) rep.addEntry("unexpected value for mandatory prop <$schema> [.'$RootSchema'.'&'.0]", (path != null ? lpath : null));
            return false;
        }
        return true;
    }

    // check $RootSchema (.'$RootSchema')
    public boolean json_model_26(Object val, Path path, Report rep)
    {
        // keyword $schema is mandatory at the root, and optional elsewhere
        // .'$RootSchema'
        boolean res = true;
        if (res)
        {
            // .'$RootSchema'.'&'.0
            res = _jm_obj_2(val, path, rep);
            if (! res)
            {
                if (rep != null) rep.addEntry("unexpected element [.'$RootSchema'.'&'.0]", path);
            }
            if (res)
            {
                // .'$RootSchema'.'&'.1
                res = json_model_24(val, path, rep);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected $ObjectSchema [.'$RootSchema'.'&'.1]", path);
                }
            }
        }
        if (res)
        {
            if (rep != null) rep.clearEntries();
        }
        else
        {
            if (rep != null) rep.addEntry("not all model match [.'$RootSchema'.'&']", path);
        }
        return res;
    }

    // check $ (.)
    public boolean json_model_1(Object val, Path path, Report rep)
    {
        // .
        boolean res = json_model_26(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected $RootSchema [.]", path);
        }
        return res;
    }


    public void init(JSON json)
    {
        if (!initialized)
        {
            try {
            _jm_cst_0_set = new HashSet<Object>();
            _jm_cst_0_set.add(json.safeJSON("\"uri\""));
            _jm_cst_0_set.add(json.safeJSON("\"uri-reference\""));
            _jm_cst_0_set.add(json.safeJSON("\"iri\""));
            _jm_cst_0_set.add(json.safeJSON("\"iri-reference\""));
            _jm_cst_0_set.add(json.safeJSON("\"date\""));
            _jm_cst_0_set.add(json.safeJSON("\"time\""));
            _jm_cst_0_set.add(json.safeJSON("\"date-time\""));
            _jm_cst_0_set.add(json.safeJSON("\"ipv4\""));
            _jm_cst_0_set.add(json.safeJSON("\"ipv6\""));
            _jm_cst_0_set.add(json.safeJSON("\"regex\""));
            _jm_cst_0_set.add(json.safeJSON("\"uuid\""));
            _jm_cst_0_set.add(json.safeJSON("\"hostname\""));
            _jm_cst_0_set.add(json.safeJSON("\"email\""));
            _jm_cst_0_set.add(json.safeJSON("\"idn-hostname\""));
            _jm_cst_0_set.add(json.safeJSON("\"idn-email\""));
            _jm_cst_0_set.add(json.safeJSON("\"duration\""));
            _jm_cst_0_set.add(json.safeJSON("\"int32\""));
            _jm_cst_0_set.add(json.safeJSON("\"int64\""));
            _jm_cst_0_set.add(json.safeJSON("\"uint\""));
            _jm_cst_0_set.add(json.safeJSON("\"uint32\""));
            _jm_cst_0_set.add(json.safeJSON("\"uint64\""));
            _jm_cst_0_set.add(json.safeJSON("\"json-pointer\""));
            _jm_cst_0_set.add(json.safeJSON("\"relative-json-pointer\""));
            _jm_cst_0_set.add(json.safeJSON("\"byte\""));
            _jm_cst_0_set.add(json.safeJSON("\"double\""));
            _jm_cst_0_set.add(json.safeJSON("\"int\""));
            _jm_cst_0_set.add(json.safeJSON("\"mime-type\""));
            json_model_9_map_pmap = new HashMap<String, Checker>();
            json_model_9_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_0(o, p, r);} });
            json_model_9_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_1(o, p, r);} });
            json_model_9_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_2(o, p, r);} });
            json_model_9_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_3(o, p, r);} });
            json_model_9_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_4(o, p, r);} });
            json_model_9_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_5(o, p, r);} });
            json_model_9_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_6(o, p, r);} });
            json_model_9_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_7(o, p, r);} });
            json_model_9_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_8(o, p, r);} });
            json_model_9_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_9(o, p, r);} });
            json_model_9_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_10(o, p, r);} });
            json_model_9_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_11(o, p, r);} });
            json_model_9_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_12(o, p, r);} });
            json_model_9_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_13(o, p, r);} });
            json_model_9_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_14(o, p, r);} });
            json_model_9_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_15(o, p, r);} });
            _jm_cst_1_set = new HashSet<Object>();
            _jm_cst_1_set.add(json.safeJSON("\"uri\""));
            _jm_cst_1_set.add(json.safeJSON("\"uri-reference\""));
            _jm_cst_1_set.add(json.safeJSON("\"iri\""));
            _jm_cst_1_set.add(json.safeJSON("\"iri-reference\""));
            _jm_cst_1_set.add(json.safeJSON("\"date\""));
            _jm_cst_1_set.add(json.safeJSON("\"time\""));
            _jm_cst_1_set.add(json.safeJSON("\"date-time\""));
            _jm_cst_1_set.add(json.safeJSON("\"ipv4\""));
            _jm_cst_1_set.add(json.safeJSON("\"ipv6\""));
            _jm_cst_1_set.add(json.safeJSON("\"regex\""));
            _jm_cst_1_set.add(json.safeJSON("\"uuid\""));
            _jm_cst_1_set.add(json.safeJSON("\"hostname\""));
            _jm_cst_1_set.add(json.safeJSON("\"email\""));
            _jm_cst_1_set.add(json.safeJSON("\"idn-hostname\""));
            _jm_cst_1_set.add(json.safeJSON("\"idn-email\""));
            _jm_cst_1_set.add(json.safeJSON("\"duration\""));
            _jm_cst_1_set.add(json.safeJSON("\"int32\""));
            _jm_cst_1_set.add(json.safeJSON("\"int64\""));
            _jm_cst_1_set.add(json.safeJSON("\"uint\""));
            _jm_cst_1_set.add(json.safeJSON("\"uint32\""));
            _jm_cst_1_set.add(json.safeJSON("\"uint64\""));
            _jm_cst_1_set.add(json.safeJSON("\"json-pointer\""));
            _jm_cst_1_set.add(json.safeJSON("\"relative-json-pointer\""));
            _jm_cst_1_set.add(json.safeJSON("\"byte\""));
            _jm_cst_1_set.add(json.safeJSON("\"double\""));
            _jm_cst_1_set.add(json.safeJSON("\"int\""));
            _jm_cst_1_set.add(json.safeJSON("\"mime-type\""));
            json_model_10_map_pmap = new HashMap<String, Checker>();
            json_model_10_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_16(o, p, r);} });
            json_model_10_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_17(o, p, r);} });
            json_model_10_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_18(o, p, r);} });
            json_model_10_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_19(o, p, r);} });
            json_model_10_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_20(o, p, r);} });
            json_model_10_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_21(o, p, r);} });
            json_model_10_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_22(o, p, r);} });
            json_model_10_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_23(o, p, r);} });
            json_model_10_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_24(o, p, r);} });
            json_model_10_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_25(o, p, r);} });
            json_model_10_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_26(o, p, r);} });
            json_model_10_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_27(o, p, r);} });
            json_model_10_map_pmap.put("format", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_28(o, p, r);} });
            json_model_10_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_29(o, p, r);} });
            json_model_10_map_pmap.put("maxLength", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_30(o, p, r);} });
            json_model_10_map_pmap.put("minLength", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_31(o, p, r);} });
            json_model_10_map_pmap.put("pattern", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_32(o, p, r);} });
            json_model_10_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_33(o, p, r);} });
            json_model_10_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_34(o, p, r);} });
            json_model_10_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_35(o, p, r);} });
            json_model_11_map_pmap = new HashMap<String, Checker>();
            json_model_11_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_36(o, p, r);} });
            json_model_11_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_37(o, p, r);} });
            json_model_11_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_38(o, p, r);} });
            json_model_11_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_39(o, p, r);} });
            json_model_11_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_40(o, p, r);} });
            json_model_11_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_41(o, p, r);} });
            json_model_11_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_42(o, p, r);} });
            json_model_11_map_pmap.put("additionalItems", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_25(o, p, r);} });
            json_model_11_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_43(o, p, r);} });
            json_model_11_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_44(o, p, r);} });
            json_model_11_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_45(o, p, r);} });
            json_model_11_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_46(o, p, r);} });
            json_model_11_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_47(o, p, r);} });
            json_model_11_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_48(o, p, r);} });
            json_model_11_map_pmap.put("items", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_49(o, p, r);} });
            json_model_11_map_pmap.put("maxItems", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_50(o, p, r);} });
            json_model_11_map_pmap.put("minItems", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_51(o, p, r);} });
            json_model_11_map_pmap.put("prefixItems", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_4(o, p, r);} });
            json_model_11_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_52(o, p, r);} });
            json_model_11_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_53(o, p, r);} });
            json_model_11_map_pmap.put("unevaluatedItems", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_25(o, p, r);} });
            json_model_11_map_pmap.put("uniqueItems", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_54(o, p, r);} });
            json_model_11_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_55(o, p, r);} });
            json_model_12_map_pmap = new HashMap<String, Checker>();
            json_model_12_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_56(o, p, r);} });
            json_model_12_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_57(o, p, r);} });
            json_model_12_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_58(o, p, r);} });
            json_model_12_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_59(o, p, r);} });
            json_model_12_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_60(o, p, r);} });
            json_model_12_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_61(o, p, r);} });
            json_model_12_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_62(o, p, r);} });
            json_model_12_map_pmap.put("additionalProperties", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_25(o, p, r);} });
            json_model_12_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_63(o, p, r);} });
            json_model_12_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_64(o, p, r);} });
            json_model_12_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_65(o, p, r);} });
            json_model_12_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_66(o, p, r);} });
            json_model_12_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_67(o, p, r);} });
            json_model_12_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_68(o, p, r);} });
            json_model_12_map_pmap.put("maxProperties", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_69(o, p, r);} });
            json_model_12_map_pmap.put("minProperties", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_70(o, p, r);} });
            json_model_12_map_pmap.put("patternProperties", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_71(o, p, r);} });
            json_model_12_map_pmap.put("properties", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_72(o, p, r);} });
            json_model_12_map_pmap.put("propertyNames", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_24(o, p, r);} });
            json_model_12_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_73(o, p, r);} });
            json_model_12_map_pmap.put("required", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_74(o, p, r);} });
            json_model_12_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_75(o, p, r);} });
            json_model_12_map_pmap.put("unevaluatedProperties", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_25(o, p, r);} });
            json_model_12_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_76(o, p, r);} });
            json_model_13_map_pmap = new HashMap<String, Checker>();
            json_model_13_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_77(o, p, r);} });
            json_model_13_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_78(o, p, r);} });
            json_model_13_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_79(o, p, r);} });
            json_model_13_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_80(o, p, r);} });
            json_model_13_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_81(o, p, r);} });
            json_model_13_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_82(o, p, r);} });
            json_model_13_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_83(o, p, r);} });
            json_model_13_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_84(o, p, r);} });
            json_model_13_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_85(o, p, r);} });
            json_model_13_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_86(o, p, r);} });
            json_model_13_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_87(o, p, r);} });
            json_model_13_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_88(o, p, r);} });
            json_model_13_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_89(o, p, r);} });
            json_model_13_map_pmap.put("maximum", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_90(o, p, r);} });
            json_model_13_map_pmap.put("minimum", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_91(o, p, r);} });
            json_model_13_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_92(o, p, r);} });
            json_model_13_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_93(o, p, r);} });
            json_model_13_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_94(o, p, r);} });
            json_model_14_map_pmap = new HashMap<String, Checker>();
            json_model_14_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_95(o, p, r);} });
            json_model_14_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_96(o, p, r);} });
            json_model_14_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_97(o, p, r);} });
            json_model_14_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_98(o, p, r);} });
            json_model_14_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_99(o, p, r);} });
            json_model_14_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_100(o, p, r);} });
            json_model_14_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_101(o, p, r);} });
            json_model_14_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_102(o, p, r);} });
            json_model_14_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_103(o, p, r);} });
            json_model_14_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_104(o, p, r);} });
            json_model_14_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_105(o, p, r);} });
            json_model_14_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_106(o, p, r);} });
            json_model_14_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_107(o, p, r);} });
            json_model_14_map_pmap.put("maximum", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_108(o, p, r);} });
            json_model_14_map_pmap.put("minimum", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_109(o, p, r);} });
            json_model_14_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_110(o, p, r);} });
            json_model_14_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_111(o, p, r);} });
            json_model_14_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_112(o, p, r);} });
            json_model_15_map_pmap = new HashMap<String, Checker>();
            json_model_15_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_113(o, p, r);} });
            json_model_15_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_114(o, p, r);} });
            json_model_15_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_115(o, p, r);} });
            json_model_15_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_116(o, p, r);} });
            json_model_15_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_117(o, p, r);} });
            json_model_15_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_118(o, p, r);} });
            json_model_15_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_119(o, p, r);} });
            json_model_15_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_120(o, p, r);} });
            json_model_15_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_121(o, p, r);} });
            json_model_15_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_122(o, p, r);} });
            json_model_15_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_123(o, p, r);} });
            json_model_15_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_124(o, p, r);} });
            json_model_15_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_125(o, p, r);} });
            json_model_15_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_126(o, p, r);} });
            json_model_15_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_127(o, p, r);} });
            json_model_15_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_128(o, p, r);} });
            json_model_16_map_pmap = new HashMap<String, Checker>();
            json_model_16_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_129(o, p, r);} });
            json_model_16_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_130(o, p, r);} });
            json_model_16_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_131(o, p, r);} });
            json_model_16_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_132(o, p, r);} });
            json_model_16_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_133(o, p, r);} });
            json_model_16_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_134(o, p, r);} });
            json_model_16_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_135(o, p, r);} });
            json_model_16_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_136(o, p, r);} });
            json_model_16_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_137(o, p, r);} });
            json_model_16_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_138(o, p, r);} });
            json_model_16_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_139(o, p, r);} });
            json_model_16_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_140(o, p, r);} });
            json_model_16_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_141(o, p, r);} });
            json_model_16_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_142(o, p, r);} });
            json_model_16_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_143(o, p, r);} });
            json_model_16_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_144(o, p, r);} });
            json_model_17_map_pmap = new HashMap<String, Checker>();
            json_model_17_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_145(o, p, r);} });
            json_model_17_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_146(o, p, r);} });
            json_model_17_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_147(o, p, r);} });
            json_model_17_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_148(o, p, r);} });
            json_model_17_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_149(o, p, r);} });
            json_model_17_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_150(o, p, r);} });
            json_model_17_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_151(o, p, r);} });
            json_model_17_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_152(o, p, r);} });
            json_model_17_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_153(o, p, r);} });
            json_model_17_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_154(o, p, r);} });
            json_model_17_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_155(o, p, r);} });
            json_model_17_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_156(o, p, r);} });
            json_model_17_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_157(o, p, r);} });
            json_model_17_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_158(o, p, r);} });
            json_model_17_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_159(o, p, r);} });
            json_model_17_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_160(o, p, r);} });
            json_model_18_map_pmap = new HashMap<String, Checker>();
            json_model_18_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_161(o, p, r);} });
            json_model_18_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_162(o, p, r);} });
            json_model_18_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_163(o, p, r);} });
            json_model_18_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_164(o, p, r);} });
            json_model_18_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_165(o, p, r);} });
            json_model_18_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_166(o, p, r);} });
            json_model_18_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_167(o, p, r);} });
            json_model_18_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_168(o, p, r);} });
            json_model_18_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_169(o, p, r);} });
            json_model_18_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_170(o, p, r);} });
            json_model_18_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_171(o, p, r);} });
            json_model_18_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_172(o, p, r);} });
            json_model_18_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_173(o, p, r);} });
            json_model_18_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_174(o, p, r);} });
            json_model_18_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_175(o, p, r);} });
            json_model_18_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_176(o, p, r);} });
            json_model_19_map_pmap = new HashMap<String, Checker>();
            json_model_19_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_177(o, p, r);} });
            json_model_19_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_178(o, p, r);} });
            json_model_19_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_179(o, p, r);} });
            json_model_19_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_180(o, p, r);} });
            json_model_19_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_181(o, p, r);} });
            json_model_19_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_182(o, p, r);} });
            json_model_19_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_183(o, p, r);} });
            json_model_19_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_184(o, p, r);} });
            json_model_19_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_185(o, p, r);} });
            json_model_19_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_186(o, p, r);} });
            json_model_19_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_187(o, p, r);} });
            json_model_19_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_188(o, p, r);} });
            json_model_19_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_189(o, p, r);} });
            json_model_19_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_190(o, p, r);} });
            json_model_19_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_191(o, p, r);} });
            json_model_19_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_192(o, p, r);} });
            json_model_20_map_pmap = new HashMap<String, Checker>();
            json_model_20_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_193(o, p, r);} });
            json_model_20_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_194(o, p, r);} });
            json_model_20_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_195(o, p, r);} });
            json_model_20_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_196(o, p, r);} });
            json_model_20_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_197(o, p, r);} });
            json_model_20_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_198(o, p, r);} });
            json_model_20_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_199(o, p, r);} });
            json_model_20_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_200(o, p, r);} });
            json_model_20_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_201(o, p, r);} });
            json_model_20_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_202(o, p, r);} });
            json_model_20_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_203(o, p, r);} });
            json_model_20_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_204(o, p, r);} });
            json_model_20_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_205(o, p, r);} });
            json_model_20_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_206(o, p, r);} });
            json_model_20_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_207(o, p, r);} });
            json_model_20_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_208(o, p, r);} });
            json_model_21_map_pmap = new HashMap<String, Checker>();
            json_model_21_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_209(o, p, r);} });
            json_model_21_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_210(o, p, r);} });
            json_model_21_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_211(o, p, r);} });
            json_model_21_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_212(o, p, r);} });
            json_model_21_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_213(o, p, r);} });
            json_model_21_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_214(o, p, r);} });
            json_model_21_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_215(o, p, r);} });
            json_model_21_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_216(o, p, r);} });
            json_model_21_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_217(o, p, r);} });
            json_model_21_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_218(o, p, r);} });
            json_model_21_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_219(o, p, r);} });
            json_model_21_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_220(o, p, r);} });
            json_model_21_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_221(o, p, r);} });
            json_model_21_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_222(o, p, r);} });
            json_model_21_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_223(o, p, r);} });
            json_model_21_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_224(o, p, r);} });
            json_model_22_map_pmap = new HashMap<String, Checker>();
            json_model_22_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_225(o, p, r);} });
            json_model_22_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_226(o, p, r);} });
            json_model_22_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_227(o, p, r);} });
            json_model_22_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_228(o, p, r);} });
            json_model_22_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_229(o, p, r);} });
            json_model_22_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_230(o, p, r);} });
            json_model_22_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_231(o, p, r);} });
            json_model_22_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_232(o, p, r);} });
            json_model_22_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_233(o, p, r);} });
            json_model_22_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_234(o, p, r);} });
            json_model_22_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_235(o, p, r);} });
            json_model_22_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_236(o, p, r);} });
            json_model_22_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_237(o, p, r);} });
            json_model_22_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_238(o, p, r);} });
            json_model_22_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_239(o, p, r);} });
            json_model_22_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_240(o, p, r);} });
            json_model_23_map_pmap = new HashMap<String, Checker>();
            json_model_23_map_pmap.put("$anchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_241(o, p, r);} });
            json_model_23_map_pmap.put("$comment", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_242(o, p, r);} });
            json_model_23_map_pmap.put("$defs", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_243(o, p, r);} });
            json_model_23_map_pmap.put("$dynamicAnchor", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_244(o, p, r);} });
            json_model_23_map_pmap.put("$id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_245(o, p, r);} });
            json_model_23_map_pmap.put("$schema", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_246(o, p, r);} });
            json_model_23_map_pmap.put("$vocabulary", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_247(o, p, r);} });
            json_model_23_map_pmap.put("default", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_248(o, p, r);} });
            json_model_23_map_pmap.put("definitions", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_249(o, p, r);} });
            json_model_23_map_pmap.put("deprecated", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_250(o, p, r);} });
            json_model_23_map_pmap.put("description", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_251(o, p, r);} });
            json_model_23_map_pmap.put("examples", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_252(o, p, r);} });
            json_model_23_map_pmap.put("id", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_253(o, p, r);} });
            json_model_23_map_pmap.put("readOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_254(o, p, r);} });
            json_model_23_map_pmap.put("title", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_255(o, p, r);} });
            json_model_23_map_pmap.put("writeOnly", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_f_256(o, p, r);} });
            _jm_map_0_cmap = new HashMap<Object, Checker>();
            _jm_map_0_cmap.put(json.safeJSON("\"null\""), new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_16(o, p, r);} });
            _jm_map_0_cmap.put(json.safeJSON("\"boolean\""), new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_15(o, p, r);} });
            _jm_map_0_cmap.put(json.safeJSON("\"integer\""), new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_13(o, p, r);} });
            _jm_map_0_cmap.put(json.safeJSON("\"number\""), new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_14(o, p, r);} });
            _jm_map_0_cmap.put(json.safeJSON("\"string\""), new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_10(o, p, r);} });
            _jm_map_0_cmap.put(json.safeJSON("\"array\""), new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_11(o, p, r);} });
            _jm_map_0_cmap.put(json.safeJSON("\"object\""), new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_12(o, p, r);} });
            json_schema_draft_tight_map_pmap = new HashMap<String, Checker>();
            json_schema_draft_tight_map_pmap.put("", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_26(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("const", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_2(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("enum", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_3(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("schemaArray", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_4(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("stringKeywords", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_5(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("arrayKeywords", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_6(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("objectKeywords", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_7(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("numberKeywords", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_8(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("metas", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_9(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("String", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_10(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Array", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_11(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Object", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_12(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Integer", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_13(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Number", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_14(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Bool", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_15(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Null", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_16(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("AllOf", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_17(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("AnyOf", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_18(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("OneOf", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_19(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Enum", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_20(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Const", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_21(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Ref", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_22(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("DynRef", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_23(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("ObjectSchema", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_24(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("Schema", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_25(o, p, r);} });
            json_schema_draft_tight_map_pmap.put("RootSchema", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_26(o, p, r);} });
                super.init(json);
            }
            catch (Exception e) {
                throw new Error("cannot initialized model checker: " + e);
            }
        }
    }

    public void free()
    {
        if (initialized)
        {
            super.free();
            _jm_cst_0_set = null;
            json_model_9_map_pmap = null;
            _jm_cst_1_set = null;
            json_model_10_map_pmap = null;
            json_model_11_map_pmap = null;
            json_model_12_map_pmap = null;
            json_model_13_map_pmap = null;
            json_model_14_map_pmap = null;
            json_model_15_map_pmap = null;
            json_model_16_map_pmap = null;
            json_model_17_map_pmap = null;
            json_model_18_map_pmap = null;
            json_model_19_map_pmap = null;
            json_model_20_map_pmap = null;
            json_model_21_map_pmap = null;
            json_model_22_map_pmap = null;
            json_model_23_map_pmap = null;
            _jm_map_0_cmap = null;
            json_schema_draft_tight_map_pmap = null;
        }
    }

    public Checker get(String name)
    {
        return json_schema_draft_tight_map_pmap.get(name);
    }

    public Set<String> models()
    {
        return json_schema_draft_tight_map_pmap.keySet();
    }

    static public void main(String[] args) throws Exception
    {
        ModelChecker checker = new json_schema_draft_tight();
        Main.main("json_schema_draft_tight", checker, VERSION, args);
    }
}
