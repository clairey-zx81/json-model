#! /bin/env python
#
# Generated by JSON Model Compiler version 2
# see https://github.com/clairey-zx81/json-model
#
import math
from typing import Callable
import re2 as re
from json_model.runtime import *
__version__ = "2"

def check_model_fun(name: str) -> CheckFun:
    """Return check function for JSON model name."""
    return check_model_map[name]

# entry point for generated checkers
def check_model(val: Jsonable, name: str = "", rep: Report = None) -> bool:
    """Check val validity against JSON Model name."""
    if name not in check_model_map:
        raise Exception(f"unexpected model name: {name}")
    checker = check_model_map[name]
    return checker(val, [], rep)

_jm_cst_0: set[str]
json_model_9_map: PropMap
_jm_cst_1: set[str]
json_model_10_map: PropMap
json_model_11_map: PropMap
json_model_12_map: PropMap
json_model_13_map: PropMap
json_model_14_map: PropMap
json_model_15_map: PropMap
json_model_16_map: PropMap
json_model_17_map: PropMap
json_model_18_map: PropMap
json_model_19_map: PropMap
json_model_20_map: PropMap
json_model_21_map: PropMap
json_model_22_map: PropMap
json_model_23_map: PropMap
_jm_map_0: dict[str, str]
check_model_map: PropMap

# check $const (.'$const')
def json_model_2(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$const'
    res: bool = not isinstance(val, dict) and not isinstance(val, list)
    if not res:
        rep is None or rep.append(("unexpected type [.'$const'.'|']", path))
    return res

# check $enum (.'$enum')
def json_model_3(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$enum'
    res: bool = isinstance(val, list)
    if res:
        # .'$enum'.'|'.0
        for arr_3_idx, arr_3_item in enumerate(val):
            arr_3_lpath: Path = (path + [ arr_3_idx ]) if path is not None else None
            # .'$enum'.'|'.0.0
            res = isinstance(arr_3_item, str)
            if not res:
                rep is None or rep.append(("unexpected value for model \"\" [.'$enum'.'|'.0.0]", arr_3_lpath if path is not None else None))
                break
        if not res:
            rep is None or rep.append(("not array or unexpected array [.'$enum'.'|'.0]", path))
            # .'$enum'.'|'.1
            for arr_2_idx, arr_2_item in enumerate(val):
                arr_2_lpath: Path = (path + [ arr_2_idx ]) if path is not None else None
                # .'$enum'.'|'.1.0
                res = isinstance(arr_2_item, int) and not isinstance(arr_2_item, bool)
                if not res:
                    rep is None or rep.append(("not a -1 strict int [.'$enum'.'|'.1.0]", arr_2_lpath if path is not None else None))
                    break
            if not res:
                rep is None or rep.append(("not array or unexpected array [.'$enum'.'|'.1]", path))
                # .'$enum'.'|'.2
                for arr_1_idx, arr_1_item in enumerate(val):
                    arr_1_lpath: Path = (path + [ arr_1_idx ]) if path is not None else None
                    # .'$enum'.'|'.2.0
                    res = isinstance(arr_1_item, bool)
                    if not res:
                        rep is None or rep.append(("not a bool [.'$enum'.'|'.2.0]", arr_1_lpath if path is not None else None))
                        break
                if not res:
                    rep is None or rep.append(("not array or unexpected array [.'$enum'.'|'.2]", path))
                    # .'$enum'.'|'.3
                    for arr_0_idx, arr_0_item in enumerate(val):
                        arr_0_lpath: Path = (path + [ arr_0_idx ]) if path is not None else None
                        # .'$enum'.'|'.3.0
                        res = isinstance(arr_0_item, float)
                        if not res:
                            rep is None or rep.append(("not a -1.0 strict float [.'$enum'.'|'.3.0]", arr_0_lpath if path is not None else None))
                            break
                    if not res:
                        rep is None or rep.append(("not array or unexpected array [.'$enum'.'|'.3]", path))
        if res:
            rep is None or rep.clear()
        else:
            rep is None or rep.append(("no model matched [.'$enum'.'|']", path))
    else:
        rep is None or rep.append(("unexpected type [.'$enum'.'|']", path))
    return res

# check $schemaArray (.'$schemaArray')
def json_model_4(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$schemaArray'
    res: bool = isinstance(val, list)
    if res:
        for arr_4_idx, arr_4_item in enumerate(val):
            arr_4_lpath: Path = (path + [ arr_4_idx ]) if path is not None else None
            # .'$schemaArray'.0
            res = json_model_25(arr_4_item, arr_4_lpath if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$schemaArray'.0]", arr_4_lpath if path is not None else None))
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$schemaArray']", path))
    return res


# check $stringKeywords (.'$stringKeywords')
def json_model_5(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$stringKeywords'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$stringKeywords']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_0: Path = (path + [ prop ]) if path is not None else None
        if prop == "pattern":
            # handle may pattern property
            # .'$stringKeywords'.pattern
            res = is_valid_regex(pval, lpath_0 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$REGEX\" [.'$stringKeywords'.pattern]", lpath_0 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$stringKeywords'.pattern]", lpath_0 if path is not None else None))
                return False
            continue
        elif prop == "minLength":
            # handle may minLength property
            # .'$stringKeywords'.minLength
            res = isinstance(pval, int) and not isinstance(pval, bool) and pval >= 0
            if not res:
                rep is None or rep.append(("not a 0 strict int [.'$stringKeywords'.minLength]", lpath_0 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$stringKeywords'.minLength]", lpath_0 if path is not None else None))
                return False
            continue
        elif prop == "maxLength":
            # handle may maxLength property
            # .'$stringKeywords'.maxLength
            res = isinstance(pval, int) and not isinstance(pval, bool) and pval >= 0
            if not res:
                rep is None or rep.append(("not a 0 strict int [.'$stringKeywords'.maxLength]", lpath_0 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$stringKeywords'.maxLength]", lpath_0 if path is not None else None))
                return False
            continue
        elif prop == "format":
            # handle may format property
            # .'$stringKeywords'.format
            res = ((pval is None or isinstance(pval, (bool, int, float, str)))) and pval in _jm_cst_0
            if not res:
                rep is None or rep.append(("value not in enum [.'$stringKeywords'.format.'|']", lpath_0 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$stringKeywords'.format]", lpath_0 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$stringKeywords']", lpath_0 if path is not None else None))
        return False
    return True

# check $arrayKeywords (.'$arrayKeywords')
def json_model_6(val: Jsonable, path: Path, rep: Report) -> bool:
    # not included: contains…
    # .'$arrayKeywords'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$arrayKeywords']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_1: Path = (path + [ prop ]) if path is not None else None
        if prop == "prefixItems":
            # handle may prefixItems property
            # .'$arrayKeywords'.prefixItems
            res = json_model_4(pval, lpath_1 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$schemaArray\" [.'$arrayKeywords'.prefixItems]", lpath_1 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$arrayKeywords'.prefixItems]", lpath_1 if path is not None else None))
                return False
            continue
        elif prop == "items":
            # handle may items property
            # .'$arrayKeywords'.items
            # .'$arrayKeywords'.items.'|'.0
            res = json_model_25(pval, lpath_1 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$arrayKeywords'.items.'|'.0]", lpath_1 if path is not None else None))
                # .'$arrayKeywords'.items.'|'.1
                res = json_model_4(pval, lpath_1 if path is not None else None, rep)
                if not res:
                    rep is None or rep.append(("unexpected value for model \"$schemaArray\" [.'$arrayKeywords'.items.'|'.1]", lpath_1 if path is not None else None))
            if res:
                rep is None or rep.clear()
            else:
                rep is None or rep.append(("no model matched [.'$arrayKeywords'.items.'|']", lpath_1 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$arrayKeywords'.items]", lpath_1 if path is not None else None))
                return False
            continue
        elif prop == "additionalItems":
            # handle may additionalItems property
            # .'$arrayKeywords'.additionalItems
            res = json_model_25(pval, lpath_1 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$arrayKeywords'.additionalItems]", lpath_1 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$arrayKeywords'.additionalItems]", lpath_1 if path is not None else None))
                return False
            continue
        elif prop == "unevaluatedItems":
            # handle may unevaluatedItems property
            # .'$arrayKeywords'.unevaluatedItems
            res = json_model_25(pval, lpath_1 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$arrayKeywords'.unevaluatedItems]", lpath_1 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$arrayKeywords'.unevaluatedItems]", lpath_1 if path is not None else None))
                return False
            continue
        elif prop == "minItems":
            # handle may minItems property
            # .'$arrayKeywords'.minItems
            res = isinstance(pval, int) and not isinstance(pval, bool) and pval >= 0
            if not res:
                rep is None or rep.append(("not a 0 strict int [.'$arrayKeywords'.minItems]", lpath_1 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$arrayKeywords'.minItems]", lpath_1 if path is not None else None))
                return False
            continue
        elif prop == "maxItems":
            # handle may maxItems property
            # .'$arrayKeywords'.maxItems
            res = isinstance(pval, int) and not isinstance(pval, bool) and pval >= 0
            if not res:
                rep is None or rep.append(("not a 0 strict int [.'$arrayKeywords'.maxItems]", lpath_1 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$arrayKeywords'.maxItems]", lpath_1 if path is not None else None))
                return False
            continue
        elif prop == "uniqueItems":
            # handle may uniqueItems property
            # .'$arrayKeywords'.uniqueItems
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$arrayKeywords'.uniqueItems]", lpath_1 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$arrayKeywords'.uniqueItems]", lpath_1 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$arrayKeywords']", lpath_1 if path is not None else None))
        return False
    return True

# object .'$objectKeywords'.properties
def _jm_obj_0(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$objectKeywords'.properties]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_3: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$objectKeywords'.properties.''
        res = json_model_25(pval, lpath_3 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$objectKeywords'.properties.'']", lpath_3 if path is not None else None))
            return False
    return True

# object .'$objectKeywords'.patternProperties
def _jm_obj_1(val: Jsonable, path: Path, rep: Report) -> bool:
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$objectKeywords'.patternProperties]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_4: Path = (path + [ prop ]) if path is not None else None
        if is_valid_regex(prop, lpath_4 if path is not None else None, rep):
            # handle 1 key props
            # .'$objectKeywords'.patternProperties.'$REGEX'
            res = json_model_25(pval, lpath_4 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$objectKeywords'.patternProperties.'$REGEX']", lpath_4 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$objectKeywords'.patternProperties]", lpath_4 if path is not None else None))
            return False
    return True

# check $objectKeywords (.'$objectKeywords')
def json_model_7(val: Jsonable, path: Path, rep: Report) -> bool:
    # not included: dependentRequired…
    # .'$objectKeywords'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$objectKeywords']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_2: Path = (path + [ prop ]) if path is not None else None
        if prop == "properties":
            # handle may properties property
            # .'$objectKeywords'.properties
            res = _jm_obj_0(pval, lpath_2 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected element [.'$objectKeywords'.properties]", lpath_2 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$objectKeywords'.properties]", lpath_2 if path is not None else None))
                return False
            continue
        elif prop == "required":
            # handle may required property
            # .'$objectKeywords'.required
            res = isinstance(pval, list)
            if res:
                for arr_5_idx, arr_5_item in enumerate(pval):
                    arr_5_lpath: Path = ((lpath_2 if path is not None else None) + [ arr_5_idx ]) if (lpath_2 if path is not None else None) is not None else None
                    # .'$objectKeywords'.required.0
                    res = isinstance(arr_5_item, str)
                    if not res:
                        rep is None or rep.append(("unexpected value for model \"\" [.'$objectKeywords'.required.0]", arr_5_lpath if (lpath_2 if path is not None else None) is not None else None))
                        break
            if not res:
                rep is None or rep.append(("not array or unexpected array [.'$objectKeywords'.required]", lpath_2 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$objectKeywords'.required]", lpath_2 if path is not None else None))
                return False
            continue
        elif prop == "additionalProperties":
            # handle may additionalProperties property
            # .'$objectKeywords'.additionalProperties
            res = json_model_25(pval, lpath_2 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$objectKeywords'.additionalProperties]", lpath_2 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$objectKeywords'.additionalProperties]", lpath_2 if path is not None else None))
                return False
            continue
        elif prop == "unevaluatedProperties":
            # handle may unevaluatedProperties property
            # .'$objectKeywords'.unevaluatedProperties
            res = json_model_25(pval, lpath_2 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$objectKeywords'.unevaluatedProperties]", lpath_2 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$objectKeywords'.unevaluatedProperties]", lpath_2 if path is not None else None))
                return False
            continue
        elif prop == "minProperties":
            # handle may minProperties property
            # .'$objectKeywords'.minProperties
            res = isinstance(pval, int) and not isinstance(pval, bool) and pval >= 0
            if not res:
                rep is None or rep.append(("not a 0 strict int [.'$objectKeywords'.minProperties]", lpath_2 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$objectKeywords'.minProperties]", lpath_2 if path is not None else None))
                return False
            continue
        elif prop == "maxProperties":
            # handle may maxProperties property
            # .'$objectKeywords'.maxProperties
            res = isinstance(pval, int) and not isinstance(pval, bool) and pval >= 0
            if not res:
                rep is None or rep.append(("not a 0 strict int [.'$objectKeywords'.maxProperties]", lpath_2 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$objectKeywords'.maxProperties]", lpath_2 if path is not None else None))
                return False
            continue
        elif prop == "patternProperties":
            # handle may patternProperties property
            # .'$objectKeywords'.patternProperties
            res = _jm_obj_1(pval, lpath_2 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected element [.'$objectKeywords'.patternProperties]", lpath_2 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$objectKeywords'.patternProperties]", lpath_2 if path is not None else None))
                return False
            continue
        elif prop == "propertyNames":
            # handle may propertyNames property
            # .'$objectKeywords'.propertyNames
            res = json_model_24(pval, lpath_2 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$ObjectSchema\" [.'$objectKeywords'.propertyNames]", lpath_2 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$objectKeywords'.propertyNames]", lpath_2 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$objectKeywords']", lpath_2 if path is not None else None))
        return False
    return True

# check $numberKeywords (.'$numberKeywords')
def json_model_8(val: Jsonable, path: Path, rep: Report) -> bool:
    # not included: exclusive*, multipleOf…
    # .'$numberKeywords'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$numberKeywords']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_5: Path = (path + [ prop ]) if path is not None else None
        if prop == "minimum":
            # handle may minimum property
            # .'$numberKeywords'.minimum
            res = isinstance(pval, float)
            if not res:
                rep is None or rep.append(("not a -1.0 strict float [.'$numberKeywords'.minimum]", lpath_5 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$numberKeywords'.minimum]", lpath_5 if path is not None else None))
                return False
            continue
        elif prop == "maximum":
            # handle may maximum property
            # .'$numberKeywords'.maximum
            res = isinstance(pval, float)
            if not res:
                rep is None or rep.append(("not a -1.0 strict float [.'$numberKeywords'.maximum]", lpath_5 if path is not None else None))
                rep is None or rep.append(("invalid optional prop value [.'$numberKeywords'.maximum]", lpath_5 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$numberKeywords']", lpath_5 if path is not None else None))
        return False
    return True

# check json_model_9_map_$anchor (.'$metas'.'$anchor')
def _jm_f_0(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$metas'.'$anchor']", path))
    return res

# check json_model_9_map_$comment (.'$metas'.'$comment')
def _jm_f_1(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$metas'.'$comment']", path))
    return res

# check json_model_9_map_$defs (.'$metas'.'$defs')
def _jm_f_2(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$metas'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_7: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$metas'.'$defs'.''
        res = json_model_25(pval, lpath_7 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$metas'.'$defs'.'']", lpath_7 if path is not None else None))
            return False
    return True

# check json_model_9_map_$dynamicAnchor (.'$metas'.'$dynamicAnchor')
def _jm_f_3(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$metas'.'$dynamicAnchor']", path))
    return res

# check json_model_9_map_$id (.'$metas'.'$id')
def _jm_f_4(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$metas'.'$id']", path))
    return res

# check json_model_9_map_$schema (.'$metas'.'$schema')
def _jm_f_5(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$metas'.'$schema']", path))
    return res

# check json_model_9_map_$vocabulary (.'$metas'.'$vocabulary')
def _jm_f_6(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$metas'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_8: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_8 if path is not None else None, rep):
            # handle 1 key props
            # .'$metas'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$metas'.'$vocabulary'.'$URI']", lpath_8 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$metas'.'$vocabulary']", lpath_8 if path is not None else None))
            return False
    return True

# check json_model_9_map_default (.'$metas'.default)
def _jm_f_7(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.default
    return True

# check json_model_9_map_definitions (.'$metas'.definitions)
def _jm_f_8(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$metas'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_9: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$metas'.definitions.''
        res = json_model_25(pval, lpath_9 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$metas'.definitions.'']", lpath_9 if path is not None else None))
            return False
    return True

# check json_model_9_map_deprecated (.'$metas'.deprecated)
def _jm_f_9(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$metas'.deprecated]", path))
    return res

# check json_model_9_map_description (.'$metas'.description)
def _jm_f_10(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$metas'.description]", path))
    return res

# check json_model_9_map_examples (.'$metas'.examples)
def _jm_f_11(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$metas'.examples]", path))
    return res

# check json_model_9_map_id (.'$metas'.id)
def _jm_f_12(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$metas'.id]", path))
    return res

# check json_model_9_map_readOnly (.'$metas'.readOnly)
def _jm_f_13(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$metas'.readOnly]", path))
    return res

# check json_model_9_map_title (.'$metas'.title)
def _jm_f_14(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$metas'.title]", path))
    return res

# check json_model_9_map_writeOnly (.'$metas'.writeOnly)
def _jm_f_15(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$metas'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$metas'.writeOnly]", path))
    return res


# check $metas (.'$metas')
def json_model_9(val: Jsonable, path: Path, rep: Report) -> bool:
    # there is a trick with $schema… in RootSchema below
    # .'$metas'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$metas']", path))
        return False
    pfun: CheckFun
    for prop, pval in val.items():
        lpath_6: Path = (path + [ prop ]) if path is not None else None
        if pfun := json_model_9_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_6 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$metas']", lpath_6 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$metas']", lpath_6 if path is not None else None))
        return False
    return True

# check json_model_10_map_$anchor (.'$String'.'$anchor')
def _jm_f_16(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$String'.'$anchor']", path))
    return res

# check json_model_10_map_$comment (.'$String'.'$comment')
def _jm_f_17(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$String'.'$comment']", path))
    return res

# check json_model_10_map_$defs (.'$String'.'$defs')
def _jm_f_18(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$String'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_11: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$String'.'$defs'.''
        res = json_model_25(pval, lpath_11 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$String'.'$defs'.'']", lpath_11 if path is not None else None))
            return False
    return True

# check json_model_10_map_$dynamicAnchor (.'$String'.'$dynamicAnchor')
def _jm_f_19(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$String'.'$dynamicAnchor']", path))
    return res

# check json_model_10_map_$id (.'$String'.'$id')
def _jm_f_20(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$String'.'$id']", path))
    return res

# check json_model_10_map_$schema (.'$String'.'$schema')
def _jm_f_21(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$String'.'$schema']", path))
    return res

# check json_model_10_map_$vocabulary (.'$String'.'$vocabulary')
def _jm_f_22(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$String'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_12: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_12 if path is not None else None, rep):
            # handle 1 key props
            # .'$String'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$String'.'$vocabulary'.'$URI']", lpath_12 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$String'.'$vocabulary']", lpath_12 if path is not None else None))
            return False
    return True

# check json_model_10_map_default (.'$String'.default)
def _jm_f_23(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.default
    return True

# check json_model_10_map_definitions (.'$String'.definitions)
def _jm_f_24(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$String'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_13: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$String'.definitions.''
        res = json_model_25(pval, lpath_13 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$String'.definitions.'']", lpath_13 if path is not None else None))
            return False
    return True

# check json_model_10_map_deprecated (.'$String'.deprecated)
def _jm_f_25(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$String'.deprecated]", path))
    return res

# check json_model_10_map_description (.'$String'.description)
def _jm_f_26(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$String'.description]", path))
    return res

# check json_model_10_map_examples (.'$String'.examples)
def _jm_f_27(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$String'.examples]", path))
    return res


# check json_model_10_map_format (.'$String'.format)
def _jm_f_28(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.format
    res: bool = ((val is None or isinstance(val, (bool, int, float, str)))) and val in _jm_cst_1
    if not res:
        rep is None or rep.append(("value not in enum [.'$String'.format.'|']", path))
    return res

# check json_model_10_map_id (.'$String'.id)
def _jm_f_29(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$String'.id]", path))
    return res

# check json_model_10_map_maxLength (.'$String'.maxLength)
def _jm_f_30(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.maxLength
    res: bool = isinstance(val, int) and not isinstance(val, bool) and val >= 0
    if not res:
        rep is None or rep.append(("not a 0 strict int [.'$String'.maxLength]", path))
    return res

# check json_model_10_map_minLength (.'$String'.minLength)
def _jm_f_31(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.minLength
    res: bool = isinstance(val, int) and not isinstance(val, bool) and val >= 0
    if not res:
        rep is None or rep.append(("not a 0 strict int [.'$String'.minLength]", path))
    return res

# check json_model_10_map_pattern (.'$String'.pattern)
def _jm_f_32(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.pattern
    res: bool = is_valid_regex(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected value for model \"$REGEX\" [.'$String'.pattern]", path))
    return res

# check json_model_10_map_readOnly (.'$String'.readOnly)
def _jm_f_33(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$String'.readOnly]", path))
    return res

# check json_model_10_map_title (.'$String'.title)
def _jm_f_34(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$String'.title]", path))
    return res

# check json_model_10_map_writeOnly (.'$String'.writeOnly)
def _jm_f_35(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$String'.writeOnly]", path))
    return res


# check $String (.'$String')
def json_model_10(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$String'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$String']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_10: Path = (path + [ prop ]) if path is not None else None
        if prop == "type":
            # handle must type property
            must_count += 1
            # .'$String'.type
            res = isinstance(pval, str) and pval == "string"
            if not res:
                rep is None or rep.append(("unexpected value for model \"string\" [.'$String'.type]", lpath_10 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$String'.type]", lpath_10 if path is not None else None))
                return False
            continue
        if pfun := json_model_10_map.get(prop):
            # handle 20 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_10 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$String']", lpath_10 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$String']", lpath_10 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "type" in val:
                rep is None or rep.append(("missing mandatory prop <type> [.'$String']", path))
        return False
    return True

# check json_model_11_map_$anchor (.'$Array'.'$anchor')
def _jm_f_36(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Array'.'$anchor']", path))
    return res

# check json_model_11_map_$comment (.'$Array'.'$comment')
def _jm_f_37(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Array'.'$comment']", path))
    return res

# check json_model_11_map_$defs (.'$Array'.'$defs')
def _jm_f_38(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Array'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_15: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Array'.'$defs'.''
        res = json_model_25(pval, lpath_15 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Array'.'$defs'.'']", lpath_15 if path is not None else None))
            return False
    return True

# check json_model_11_map_$dynamicAnchor (.'$Array'.'$dynamicAnchor')
def _jm_f_39(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Array'.'$dynamicAnchor']", path))
    return res

# check json_model_11_map_$id (.'$Array'.'$id')
def _jm_f_40(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Array'.'$id']", path))
    return res

# check json_model_11_map_$schema (.'$Array'.'$schema')
def _jm_f_41(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Array'.'$schema']", path))
    return res

# check json_model_11_map_$vocabulary (.'$Array'.'$vocabulary')
def _jm_f_42(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Array'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_16: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_16 if path is not None else None, rep):
            # handle 1 key props
            # .'$Array'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Array'.'$vocabulary'.'$URI']", lpath_16 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Array'.'$vocabulary']", lpath_16 if path is not None else None))
            return False
    return True

# check json_model_11_map_default (.'$Array'.default)
def _jm_f_43(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.default
    return True

# check json_model_11_map_definitions (.'$Array'.definitions)
def _jm_f_44(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Array'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_17: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Array'.definitions.''
        res = json_model_25(pval, lpath_17 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Array'.definitions.'']", lpath_17 if path is not None else None))
            return False
    return True

# check json_model_11_map_deprecated (.'$Array'.deprecated)
def _jm_f_45(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Array'.deprecated]", path))
    return res

# check json_model_11_map_description (.'$Array'.description)
def _jm_f_46(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Array'.description]", path))
    return res

# check json_model_11_map_examples (.'$Array'.examples)
def _jm_f_47(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Array'.examples]", path))
    return res

# check json_model_11_map_id (.'$Array'.id)
def _jm_f_48(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Array'.id]", path))
    return res

# check json_model_11_map_items (.'$Array'.items)
def _jm_f_49(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.items
    # .'$Array'.items.'|'.0
    res: bool = json_model_25(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Array'.items.'|'.0]", path))
        # .'$Array'.items.'|'.1
        res = json_model_4(val, path, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$schemaArray\" [.'$Array'.items.'|'.1]", path))
    if res:
        rep is None or rep.clear()
    else:
        rep is None or rep.append(("no model matched [.'$Array'.items.'|']", path))
    return res

# check json_model_11_map_maxItems (.'$Array'.maxItems)
def _jm_f_50(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.maxItems
    res: bool = isinstance(val, int) and not isinstance(val, bool) and val >= 0
    if not res:
        rep is None or rep.append(("not a 0 strict int [.'$Array'.maxItems]", path))
    return res

# check json_model_11_map_minItems (.'$Array'.minItems)
def _jm_f_51(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.minItems
    res: bool = isinstance(val, int) and not isinstance(val, bool) and val >= 0
    if not res:
        rep is None or rep.append(("not a 0 strict int [.'$Array'.minItems]", path))
    return res

# check json_model_11_map_readOnly (.'$Array'.readOnly)
def _jm_f_52(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Array'.readOnly]", path))
    return res

# check json_model_11_map_title (.'$Array'.title)
def _jm_f_53(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Array'.title]", path))
    return res

# check json_model_11_map_uniqueItems (.'$Array'.uniqueItems)
def _jm_f_54(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.uniqueItems
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Array'.uniqueItems]", path))
    return res

# check json_model_11_map_writeOnly (.'$Array'.writeOnly)
def _jm_f_55(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Array'.writeOnly]", path))
    return res


# check $Array (.'$Array')
def json_model_11(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Array'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Array']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_14: Path = (path + [ prop ]) if path is not None else None
        if prop == "type":
            # handle must type property
            must_count += 1
            # .'$Array'.type
            res = isinstance(pval, str) and pval == "array"
            if not res:
                rep is None or rep.append(("unexpected value for model \"array\" [.'$Array'.type]", lpath_14 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Array'.type]", lpath_14 if path is not None else None))
                return False
            continue
        if pfun := json_model_11_map.get(prop):
            # handle 23 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_14 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Array']", lpath_14 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Array']", lpath_14 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "type" in val:
                rep is None or rep.append(("missing mandatory prop <type> [.'$Array']", path))
        return False
    return True

# check json_model_12_map_$anchor (.'$Object'.'$anchor')
def _jm_f_56(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.'$anchor']", path))
    return res

# check json_model_12_map_$comment (.'$Object'.'$comment')
def _jm_f_57(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.'$comment']", path))
    return res

# check json_model_12_map_$defs (.'$Object'.'$defs')
def _jm_f_58(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Object'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_19: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Object'.'$defs'.''
        res = json_model_25(pval, lpath_19 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Object'.'$defs'.'']", lpath_19 if path is not None else None))
            return False
    return True

# check json_model_12_map_$dynamicAnchor (.'$Object'.'$dynamicAnchor')
def _jm_f_59(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.'$dynamicAnchor']", path))
    return res

# check json_model_12_map_$id (.'$Object'.'$id')
def _jm_f_60(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.'$id']", path))
    return res

# check json_model_12_map_$schema (.'$Object'.'$schema')
def _jm_f_61(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.'$schema']", path))
    return res

# check json_model_12_map_$vocabulary (.'$Object'.'$vocabulary')
def _jm_f_62(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Object'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_20: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_20 if path is not None else None, rep):
            # handle 1 key props
            # .'$Object'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Object'.'$vocabulary'.'$URI']", lpath_20 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Object'.'$vocabulary']", lpath_20 if path is not None else None))
            return False
    return True

# check json_model_12_map_default (.'$Object'.default)
def _jm_f_63(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.default
    return True

# check json_model_12_map_definitions (.'$Object'.definitions)
def _jm_f_64(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Object'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_21: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Object'.definitions.''
        res = json_model_25(pval, lpath_21 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Object'.definitions.'']", lpath_21 if path is not None else None))
            return False
    return True

# check json_model_12_map_deprecated (.'$Object'.deprecated)
def _jm_f_65(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Object'.deprecated]", path))
    return res

# check json_model_12_map_description (.'$Object'.description)
def _jm_f_66(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.description]", path))
    return res

# check json_model_12_map_examples (.'$Object'.examples)
def _jm_f_67(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Object'.examples]", path))
    return res

# check json_model_12_map_id (.'$Object'.id)
def _jm_f_68(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.id]", path))
    return res

# check json_model_12_map_maxProperties (.'$Object'.maxProperties)
def _jm_f_69(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.maxProperties
    res: bool = isinstance(val, int) and not isinstance(val, bool) and val >= 0
    if not res:
        rep is None or rep.append(("not a 0 strict int [.'$Object'.maxProperties]", path))
    return res

# check json_model_12_map_minProperties (.'$Object'.minProperties)
def _jm_f_70(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.minProperties
    res: bool = isinstance(val, int) and not isinstance(val, bool) and val >= 0
    if not res:
        rep is None or rep.append(("not a 0 strict int [.'$Object'.minProperties]", path))
    return res

# check json_model_12_map_patternProperties (.'$Object'.patternProperties)
def _jm_f_71(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.patternProperties
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Object'.patternProperties]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_22: Path = (path + [ prop ]) if path is not None else None
        if is_valid_regex(prop, lpath_22 if path is not None else None, rep):
            # handle 1 key props
            # .'$Object'.patternProperties.'$REGEX'
            res = json_model_25(pval, lpath_22 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Object'.patternProperties.'$REGEX']", lpath_22 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Object'.patternProperties]", lpath_22 if path is not None else None))
            return False
    return True

# check json_model_12_map_properties (.'$Object'.properties)
def _jm_f_72(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.properties
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Object'.properties]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_23: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Object'.properties.''
        res = json_model_25(pval, lpath_23 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Object'.properties.'']", lpath_23 if path is not None else None))
            return False
    return True

# check json_model_12_map_readOnly (.'$Object'.readOnly)
def _jm_f_73(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Object'.readOnly]", path))
    return res

# check json_model_12_map_required (.'$Object'.required)
def _jm_f_74(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.required
    res: bool = isinstance(val, list)
    if res:
        for arr_6_idx, arr_6_item in enumerate(val):
            arr_6_lpath: Path = (path + [ arr_6_idx ]) if path is not None else None
            # .'$Object'.required.0
            res = isinstance(arr_6_item, str)
            if not res:
                rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.required.0]", arr_6_lpath if path is not None else None))
                break
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Object'.required]", path))
    return res

# check json_model_12_map_title (.'$Object'.title)
def _jm_f_75(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Object'.title]", path))
    return res

# check json_model_12_map_writeOnly (.'$Object'.writeOnly)
def _jm_f_76(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Object'.writeOnly]", path))
    return res


# check $Object (.'$Object')
def json_model_12(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Object'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Object']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_18: Path = (path + [ prop ]) if path is not None else None
        if prop == "type":
            # handle must type property
            must_count += 1
            # .'$Object'.type
            res = isinstance(pval, str) and pval == "object"
            if not res:
                rep is None or rep.append(("unexpected value for model \"object\" [.'$Object'.type]", lpath_18 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Object'.type]", lpath_18 if path is not None else None))
                return False
            continue
        if pfun := json_model_12_map.get(prop):
            # handle 24 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_18 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Object']", lpath_18 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Object']", lpath_18 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "type" in val:
                rep is None or rep.append(("missing mandatory prop <type> [.'$Object']", path))
        return False
    return True

# check json_model_13_map_$anchor (.'$Integer'.'$anchor')
def _jm_f_77(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Integer'.'$anchor']", path))
    return res

# check json_model_13_map_$comment (.'$Integer'.'$comment')
def _jm_f_78(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Integer'.'$comment']", path))
    return res

# check json_model_13_map_$defs (.'$Integer'.'$defs')
def _jm_f_79(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Integer'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_25: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Integer'.'$defs'.''
        res = json_model_25(pval, lpath_25 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Integer'.'$defs'.'']", lpath_25 if path is not None else None))
            return False
    return True

# check json_model_13_map_$dynamicAnchor (.'$Integer'.'$dynamicAnchor')
def _jm_f_80(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Integer'.'$dynamicAnchor']", path))
    return res

# check json_model_13_map_$id (.'$Integer'.'$id')
def _jm_f_81(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Integer'.'$id']", path))
    return res

# check json_model_13_map_$schema (.'$Integer'.'$schema')
def _jm_f_82(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Integer'.'$schema']", path))
    return res

# check json_model_13_map_$vocabulary (.'$Integer'.'$vocabulary')
def _jm_f_83(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Integer'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_26: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_26 if path is not None else None, rep):
            # handle 1 key props
            # .'$Integer'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Integer'.'$vocabulary'.'$URI']", lpath_26 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Integer'.'$vocabulary']", lpath_26 if path is not None else None))
            return False
    return True

# check json_model_13_map_default (.'$Integer'.default)
def _jm_f_84(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.default
    return True

# check json_model_13_map_definitions (.'$Integer'.definitions)
def _jm_f_85(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Integer'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_27: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Integer'.definitions.''
        res = json_model_25(pval, lpath_27 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Integer'.definitions.'']", lpath_27 if path is not None else None))
            return False
    return True

# check json_model_13_map_deprecated (.'$Integer'.deprecated)
def _jm_f_86(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Integer'.deprecated]", path))
    return res

# check json_model_13_map_description (.'$Integer'.description)
def _jm_f_87(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Integer'.description]", path))
    return res

# check json_model_13_map_examples (.'$Integer'.examples)
def _jm_f_88(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Integer'.examples]", path))
    return res

# check json_model_13_map_id (.'$Integer'.id)
def _jm_f_89(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Integer'.id]", path))
    return res

# check json_model_13_map_maximum (.'$Integer'.maximum)
def _jm_f_90(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.maximum
    res: bool = isinstance(val, float)
    if not res:
        rep is None or rep.append(("not a -1.0 strict float [.'$Integer'.maximum]", path))
    return res

# check json_model_13_map_minimum (.'$Integer'.minimum)
def _jm_f_91(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.minimum
    res: bool = isinstance(val, float)
    if not res:
        rep is None or rep.append(("not a -1.0 strict float [.'$Integer'.minimum]", path))
    return res

# check json_model_13_map_readOnly (.'$Integer'.readOnly)
def _jm_f_92(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Integer'.readOnly]", path))
    return res

# check json_model_13_map_title (.'$Integer'.title)
def _jm_f_93(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Integer'.title]", path))
    return res

# check json_model_13_map_writeOnly (.'$Integer'.writeOnly)
def _jm_f_94(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Integer'.writeOnly]", path))
    return res


# check $Integer (.'$Integer')
def json_model_13(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Integer'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Integer']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_24: Path = (path + [ prop ]) if path is not None else None
        if prop == "type":
            # handle must type property
            must_count += 1
            # .'$Integer'.type
            res = isinstance(pval, str) and pval == "integer"
            if not res:
                rep is None or rep.append(("unexpected value for model \"integer\" [.'$Integer'.type]", lpath_24 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Integer'.type]", lpath_24 if path is not None else None))
                return False
            continue
        if pfun := json_model_13_map.get(prop):
            # handle 18 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_24 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Integer']", lpath_24 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Integer']", lpath_24 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "type" in val:
                rep is None or rep.append(("missing mandatory prop <type> [.'$Integer']", path))
        return False
    return True

# check json_model_14_map_$anchor (.'$Number'.'$anchor')
def _jm_f_95(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Number'.'$anchor']", path))
    return res

# check json_model_14_map_$comment (.'$Number'.'$comment')
def _jm_f_96(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Number'.'$comment']", path))
    return res

# check json_model_14_map_$defs (.'$Number'.'$defs')
def _jm_f_97(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Number'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_29: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Number'.'$defs'.''
        res = json_model_25(pval, lpath_29 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Number'.'$defs'.'']", lpath_29 if path is not None else None))
            return False
    return True

# check json_model_14_map_$dynamicAnchor (.'$Number'.'$dynamicAnchor')
def _jm_f_98(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Number'.'$dynamicAnchor']", path))
    return res

# check json_model_14_map_$id (.'$Number'.'$id')
def _jm_f_99(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Number'.'$id']", path))
    return res

# check json_model_14_map_$schema (.'$Number'.'$schema')
def _jm_f_100(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Number'.'$schema']", path))
    return res

# check json_model_14_map_$vocabulary (.'$Number'.'$vocabulary')
def _jm_f_101(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Number'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_30: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_30 if path is not None else None, rep):
            # handle 1 key props
            # .'$Number'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Number'.'$vocabulary'.'$URI']", lpath_30 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Number'.'$vocabulary']", lpath_30 if path is not None else None))
            return False
    return True

# check json_model_14_map_default (.'$Number'.default)
def _jm_f_102(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.default
    return True

# check json_model_14_map_definitions (.'$Number'.definitions)
def _jm_f_103(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Number'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_31: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Number'.definitions.''
        res = json_model_25(pval, lpath_31 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Number'.definitions.'']", lpath_31 if path is not None else None))
            return False
    return True

# check json_model_14_map_deprecated (.'$Number'.deprecated)
def _jm_f_104(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Number'.deprecated]", path))
    return res

# check json_model_14_map_description (.'$Number'.description)
def _jm_f_105(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Number'.description]", path))
    return res

# check json_model_14_map_examples (.'$Number'.examples)
def _jm_f_106(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Number'.examples]", path))
    return res

# check json_model_14_map_id (.'$Number'.id)
def _jm_f_107(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Number'.id]", path))
    return res

# check json_model_14_map_maximum (.'$Number'.maximum)
def _jm_f_108(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.maximum
    res: bool = isinstance(val, float)
    if not res:
        rep is None or rep.append(("not a -1.0 strict float [.'$Number'.maximum]", path))
    return res

# check json_model_14_map_minimum (.'$Number'.minimum)
def _jm_f_109(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.minimum
    res: bool = isinstance(val, float)
    if not res:
        rep is None or rep.append(("not a -1.0 strict float [.'$Number'.minimum]", path))
    return res

# check json_model_14_map_readOnly (.'$Number'.readOnly)
def _jm_f_110(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Number'.readOnly]", path))
    return res

# check json_model_14_map_title (.'$Number'.title)
def _jm_f_111(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Number'.title]", path))
    return res

# check json_model_14_map_writeOnly (.'$Number'.writeOnly)
def _jm_f_112(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Number'.writeOnly]", path))
    return res


# check $Number (.'$Number')
def json_model_14(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Number'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Number']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_28: Path = (path + [ prop ]) if path is not None else None
        if prop == "type":
            # handle must type property
            must_count += 1
            # .'$Number'.type
            res = isinstance(pval, str) and pval == "number"
            if not res:
                rep is None or rep.append(("unexpected value for model \"number\" [.'$Number'.type]", lpath_28 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Number'.type]", lpath_28 if path is not None else None))
                return False
            continue
        if pfun := json_model_14_map.get(prop):
            # handle 18 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_28 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Number']", lpath_28 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Number']", lpath_28 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "type" in val:
                rep is None or rep.append(("missing mandatory prop <type> [.'$Number']", path))
        return False
    return True

# check json_model_15_map_$anchor (.'$Bool'.'$anchor')
def _jm_f_113(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Bool'.'$anchor']", path))
    return res

# check json_model_15_map_$comment (.'$Bool'.'$comment')
def _jm_f_114(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Bool'.'$comment']", path))
    return res

# check json_model_15_map_$defs (.'$Bool'.'$defs')
def _jm_f_115(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Bool'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_33: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Bool'.'$defs'.''
        res = json_model_25(pval, lpath_33 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Bool'.'$defs'.'']", lpath_33 if path is not None else None))
            return False
    return True

# check json_model_15_map_$dynamicAnchor (.'$Bool'.'$dynamicAnchor')
def _jm_f_116(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Bool'.'$dynamicAnchor']", path))
    return res

# check json_model_15_map_$id (.'$Bool'.'$id')
def _jm_f_117(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Bool'.'$id']", path))
    return res

# check json_model_15_map_$schema (.'$Bool'.'$schema')
def _jm_f_118(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Bool'.'$schema']", path))
    return res

# check json_model_15_map_$vocabulary (.'$Bool'.'$vocabulary')
def _jm_f_119(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Bool'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_34: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_34 if path is not None else None, rep):
            # handle 1 key props
            # .'$Bool'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Bool'.'$vocabulary'.'$URI']", lpath_34 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Bool'.'$vocabulary']", lpath_34 if path is not None else None))
            return False
    return True

# check json_model_15_map_default (.'$Bool'.default)
def _jm_f_120(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.default
    return True

# check json_model_15_map_definitions (.'$Bool'.definitions)
def _jm_f_121(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Bool'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_35: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Bool'.definitions.''
        res = json_model_25(pval, lpath_35 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Bool'.definitions.'']", lpath_35 if path is not None else None))
            return False
    return True

# check json_model_15_map_deprecated (.'$Bool'.deprecated)
def _jm_f_122(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Bool'.deprecated]", path))
    return res

# check json_model_15_map_description (.'$Bool'.description)
def _jm_f_123(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Bool'.description]", path))
    return res

# check json_model_15_map_examples (.'$Bool'.examples)
def _jm_f_124(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Bool'.examples]", path))
    return res

# check json_model_15_map_id (.'$Bool'.id)
def _jm_f_125(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Bool'.id]", path))
    return res

# check json_model_15_map_readOnly (.'$Bool'.readOnly)
def _jm_f_126(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Bool'.readOnly]", path))
    return res

# check json_model_15_map_title (.'$Bool'.title)
def _jm_f_127(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Bool'.title]", path))
    return res

# check json_model_15_map_writeOnly (.'$Bool'.writeOnly)
def _jm_f_128(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Bool'.writeOnly]", path))
    return res


# check $Bool (.'$Bool')
def json_model_15(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Bool'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Bool']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_32: Path = (path + [ prop ]) if path is not None else None
        if prop == "type":
            # handle must type property
            must_count += 1
            # .'$Bool'.type
            res = isinstance(pval, str) and pval == "boolean"
            if not res:
                rep is None or rep.append(("unexpected value for model \"boolean\" [.'$Bool'.type]", lpath_32 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Bool'.type]", lpath_32 if path is not None else None))
                return False
            continue
        if pfun := json_model_15_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_32 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Bool']", lpath_32 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Bool']", lpath_32 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "type" in val:
                rep is None or rep.append(("missing mandatory prop <type> [.'$Bool']", path))
        return False
    return True

# check json_model_16_map_$anchor (.'$Null'.'$anchor')
def _jm_f_129(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Null'.'$anchor']", path))
    return res

# check json_model_16_map_$comment (.'$Null'.'$comment')
def _jm_f_130(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Null'.'$comment']", path))
    return res

# check json_model_16_map_$defs (.'$Null'.'$defs')
def _jm_f_131(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Null'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_37: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Null'.'$defs'.''
        res = json_model_25(pval, lpath_37 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Null'.'$defs'.'']", lpath_37 if path is not None else None))
            return False
    return True

# check json_model_16_map_$dynamicAnchor (.'$Null'.'$dynamicAnchor')
def _jm_f_132(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Null'.'$dynamicAnchor']", path))
    return res

# check json_model_16_map_$id (.'$Null'.'$id')
def _jm_f_133(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Null'.'$id']", path))
    return res

# check json_model_16_map_$schema (.'$Null'.'$schema')
def _jm_f_134(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Null'.'$schema']", path))
    return res

# check json_model_16_map_$vocabulary (.'$Null'.'$vocabulary')
def _jm_f_135(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Null'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_38: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_38 if path is not None else None, rep):
            # handle 1 key props
            # .'$Null'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Null'.'$vocabulary'.'$URI']", lpath_38 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Null'.'$vocabulary']", lpath_38 if path is not None else None))
            return False
    return True

# check json_model_16_map_default (.'$Null'.default)
def _jm_f_136(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.default
    return True

# check json_model_16_map_definitions (.'$Null'.definitions)
def _jm_f_137(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Null'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_39: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Null'.definitions.''
        res = json_model_25(pval, lpath_39 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Null'.definitions.'']", lpath_39 if path is not None else None))
            return False
    return True

# check json_model_16_map_deprecated (.'$Null'.deprecated)
def _jm_f_138(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Null'.deprecated]", path))
    return res

# check json_model_16_map_description (.'$Null'.description)
def _jm_f_139(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Null'.description]", path))
    return res

# check json_model_16_map_examples (.'$Null'.examples)
def _jm_f_140(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Null'.examples]", path))
    return res

# check json_model_16_map_id (.'$Null'.id)
def _jm_f_141(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Null'.id]", path))
    return res

# check json_model_16_map_readOnly (.'$Null'.readOnly)
def _jm_f_142(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Null'.readOnly]", path))
    return res

# check json_model_16_map_title (.'$Null'.title)
def _jm_f_143(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Null'.title]", path))
    return res

# check json_model_16_map_writeOnly (.'$Null'.writeOnly)
def _jm_f_144(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Null'.writeOnly]", path))
    return res


# check $Null (.'$Null')
def json_model_16(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Null'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Null']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_36: Path = (path + [ prop ]) if path is not None else None
        if prop == "type":
            # handle must type property
            must_count += 1
            # .'$Null'.type
            res = isinstance(pval, str) and pval == "null"
            if not res:
                rep is None or rep.append(("unexpected value for model \"null\" [.'$Null'.type]", lpath_36 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Null'.type]", lpath_36 if path is not None else None))
                return False
            continue
        if pfun := json_model_16_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_36 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Null']", lpath_36 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Null']", lpath_36 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "type" in val:
                rep is None or rep.append(("missing mandatory prop <type> [.'$Null']", path))
        return False
    return True

# check json_model_17_map_$anchor (.'$AllOf'.'$anchor')
def _jm_f_145(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AllOf'.'$anchor']", path))
    return res

# check json_model_17_map_$comment (.'$AllOf'.'$comment')
def _jm_f_146(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AllOf'.'$comment']", path))
    return res

# check json_model_17_map_$defs (.'$AllOf'.'$defs')
def _jm_f_147(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$AllOf'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_41: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$AllOf'.'$defs'.''
        res = json_model_25(pval, lpath_41 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$AllOf'.'$defs'.'']", lpath_41 if path is not None else None))
            return False
    return True

# check json_model_17_map_$dynamicAnchor (.'$AllOf'.'$dynamicAnchor')
def _jm_f_148(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AllOf'.'$dynamicAnchor']", path))
    return res

# check json_model_17_map_$id (.'$AllOf'.'$id')
def _jm_f_149(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AllOf'.'$id']", path))
    return res

# check json_model_17_map_$schema (.'$AllOf'.'$schema')
def _jm_f_150(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AllOf'.'$schema']", path))
    return res

# check json_model_17_map_$vocabulary (.'$AllOf'.'$vocabulary')
def _jm_f_151(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$AllOf'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_42: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_42 if path is not None else None, rep):
            # handle 1 key props
            # .'$AllOf'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$AllOf'.'$vocabulary'.'$URI']", lpath_42 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$AllOf'.'$vocabulary']", lpath_42 if path is not None else None))
            return False
    return True

# check json_model_17_map_default (.'$AllOf'.default)
def _jm_f_152(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.default
    return True

# check json_model_17_map_definitions (.'$AllOf'.definitions)
def _jm_f_153(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$AllOf'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_43: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$AllOf'.definitions.''
        res = json_model_25(pval, lpath_43 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$AllOf'.definitions.'']", lpath_43 if path is not None else None))
            return False
    return True

# check json_model_17_map_deprecated (.'$AllOf'.deprecated)
def _jm_f_154(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$AllOf'.deprecated]", path))
    return res

# check json_model_17_map_description (.'$AllOf'.description)
def _jm_f_155(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AllOf'.description]", path))
    return res

# check json_model_17_map_examples (.'$AllOf'.examples)
def _jm_f_156(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$AllOf'.examples]", path))
    return res

# check json_model_17_map_id (.'$AllOf'.id)
def _jm_f_157(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AllOf'.id]", path))
    return res

# check json_model_17_map_readOnly (.'$AllOf'.readOnly)
def _jm_f_158(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$AllOf'.readOnly]", path))
    return res

# check json_model_17_map_title (.'$AllOf'.title)
def _jm_f_159(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AllOf'.title]", path))
    return res

# check json_model_17_map_writeOnly (.'$AllOf'.writeOnly)
def _jm_f_160(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$AllOf'.writeOnly]", path))
    return res


# check $AllOf (.'$AllOf')
def json_model_17(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AllOf'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$AllOf']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_40: Path = (path + [ prop ]) if path is not None else None
        if prop == "allOf":
            # handle must allOf property
            must_count += 1
            # .'$AllOf'.allOf
            res = json_model_4(pval, lpath_40 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$schemaArray\" [.'$AllOf'.allOf]", lpath_40 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$AllOf'.allOf]", lpath_40 if path is not None else None))
                return False
            continue
        if pfun := json_model_17_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_40 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$AllOf']", lpath_40 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$AllOf']", lpath_40 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "allOf" in val:
                rep is None or rep.append(("missing mandatory prop <allOf> [.'$AllOf']", path))
        return False
    return True

# check json_model_18_map_$anchor (.'$AnyOf'.'$anchor')
def _jm_f_161(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AnyOf'.'$anchor']", path))
    return res

# check json_model_18_map_$comment (.'$AnyOf'.'$comment')
def _jm_f_162(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AnyOf'.'$comment']", path))
    return res

# check json_model_18_map_$defs (.'$AnyOf'.'$defs')
def _jm_f_163(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$AnyOf'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_45: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$AnyOf'.'$defs'.''
        res = json_model_25(pval, lpath_45 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$AnyOf'.'$defs'.'']", lpath_45 if path is not None else None))
            return False
    return True

# check json_model_18_map_$dynamicAnchor (.'$AnyOf'.'$dynamicAnchor')
def _jm_f_164(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AnyOf'.'$dynamicAnchor']", path))
    return res

# check json_model_18_map_$id (.'$AnyOf'.'$id')
def _jm_f_165(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AnyOf'.'$id']", path))
    return res

# check json_model_18_map_$schema (.'$AnyOf'.'$schema')
def _jm_f_166(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AnyOf'.'$schema']", path))
    return res

# check json_model_18_map_$vocabulary (.'$AnyOf'.'$vocabulary')
def _jm_f_167(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$AnyOf'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_46: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_46 if path is not None else None, rep):
            # handle 1 key props
            # .'$AnyOf'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$AnyOf'.'$vocabulary'.'$URI']", lpath_46 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$AnyOf'.'$vocabulary']", lpath_46 if path is not None else None))
            return False
    return True

# check json_model_18_map_default (.'$AnyOf'.default)
def _jm_f_168(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.default
    return True

# check json_model_18_map_definitions (.'$AnyOf'.definitions)
def _jm_f_169(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$AnyOf'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_47: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$AnyOf'.definitions.''
        res = json_model_25(pval, lpath_47 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$AnyOf'.definitions.'']", lpath_47 if path is not None else None))
            return False
    return True

# check json_model_18_map_deprecated (.'$AnyOf'.deprecated)
def _jm_f_170(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$AnyOf'.deprecated]", path))
    return res

# check json_model_18_map_description (.'$AnyOf'.description)
def _jm_f_171(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AnyOf'.description]", path))
    return res

# check json_model_18_map_examples (.'$AnyOf'.examples)
def _jm_f_172(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$AnyOf'.examples]", path))
    return res

# check json_model_18_map_id (.'$AnyOf'.id)
def _jm_f_173(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AnyOf'.id]", path))
    return res

# check json_model_18_map_readOnly (.'$AnyOf'.readOnly)
def _jm_f_174(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$AnyOf'.readOnly]", path))
    return res

# check json_model_18_map_title (.'$AnyOf'.title)
def _jm_f_175(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$AnyOf'.title]", path))
    return res

# check json_model_18_map_writeOnly (.'$AnyOf'.writeOnly)
def _jm_f_176(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$AnyOf'.writeOnly]", path))
    return res


# check $AnyOf (.'$AnyOf')
def json_model_18(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$AnyOf'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$AnyOf']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_44: Path = (path + [ prop ]) if path is not None else None
        if prop == "anyOf":
            # handle must anyOf property
            must_count += 1
            # .'$AnyOf'.anyOf
            res = json_model_4(pval, lpath_44 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$schemaArray\" [.'$AnyOf'.anyOf]", lpath_44 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$AnyOf'.anyOf]", lpath_44 if path is not None else None))
                return False
            continue
        if pfun := json_model_18_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_44 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$AnyOf']", lpath_44 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$AnyOf']", lpath_44 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "anyOf" in val:
                rep is None or rep.append(("missing mandatory prop <anyOf> [.'$AnyOf']", path))
        return False
    return True

# check json_model_19_map_$anchor (.'$OneOf'.'$anchor')
def _jm_f_177(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$OneOf'.'$anchor']", path))
    return res

# check json_model_19_map_$comment (.'$OneOf'.'$comment')
def _jm_f_178(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$OneOf'.'$comment']", path))
    return res

# check json_model_19_map_$defs (.'$OneOf'.'$defs')
def _jm_f_179(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$OneOf'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_49: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$OneOf'.'$defs'.''
        res = json_model_25(pval, lpath_49 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$OneOf'.'$defs'.'']", lpath_49 if path is not None else None))
            return False
    return True

# check json_model_19_map_$dynamicAnchor (.'$OneOf'.'$dynamicAnchor')
def _jm_f_180(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$OneOf'.'$dynamicAnchor']", path))
    return res

# check json_model_19_map_$id (.'$OneOf'.'$id')
def _jm_f_181(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$OneOf'.'$id']", path))
    return res

# check json_model_19_map_$schema (.'$OneOf'.'$schema')
def _jm_f_182(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$OneOf'.'$schema']", path))
    return res

# check json_model_19_map_$vocabulary (.'$OneOf'.'$vocabulary')
def _jm_f_183(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$OneOf'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_50: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_50 if path is not None else None, rep):
            # handle 1 key props
            # .'$OneOf'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$OneOf'.'$vocabulary'.'$URI']", lpath_50 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$OneOf'.'$vocabulary']", lpath_50 if path is not None else None))
            return False
    return True

# check json_model_19_map_default (.'$OneOf'.default)
def _jm_f_184(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.default
    return True

# check json_model_19_map_definitions (.'$OneOf'.definitions)
def _jm_f_185(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$OneOf'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_51: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$OneOf'.definitions.''
        res = json_model_25(pval, lpath_51 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$OneOf'.definitions.'']", lpath_51 if path is not None else None))
            return False
    return True

# check json_model_19_map_deprecated (.'$OneOf'.deprecated)
def _jm_f_186(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$OneOf'.deprecated]", path))
    return res

# check json_model_19_map_description (.'$OneOf'.description)
def _jm_f_187(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$OneOf'.description]", path))
    return res

# check json_model_19_map_examples (.'$OneOf'.examples)
def _jm_f_188(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$OneOf'.examples]", path))
    return res

# check json_model_19_map_id (.'$OneOf'.id)
def _jm_f_189(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$OneOf'.id]", path))
    return res

# check json_model_19_map_readOnly (.'$OneOf'.readOnly)
def _jm_f_190(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$OneOf'.readOnly]", path))
    return res

# check json_model_19_map_title (.'$OneOf'.title)
def _jm_f_191(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$OneOf'.title]", path))
    return res

# check json_model_19_map_writeOnly (.'$OneOf'.writeOnly)
def _jm_f_192(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$OneOf'.writeOnly]", path))
    return res


# check $OneOf (.'$OneOf')
def json_model_19(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$OneOf'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$OneOf']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_48: Path = (path + [ prop ]) if path is not None else None
        if prop == "oneOf":
            # handle must oneOf property
            must_count += 1
            # .'$OneOf'.oneOf
            res = json_model_4(pval, lpath_48 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$schemaArray\" [.'$OneOf'.oneOf]", lpath_48 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$OneOf'.oneOf]", lpath_48 if path is not None else None))
                return False
            continue
        if pfun := json_model_19_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_48 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$OneOf']", lpath_48 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$OneOf']", lpath_48 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "oneOf" in val:
                rep is None or rep.append(("missing mandatory prop <oneOf> [.'$OneOf']", path))
        return False
    return True

# check json_model_20_map_$anchor (.'$Enum'.'$anchor')
def _jm_f_193(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Enum'.'$anchor']", path))
    return res

# check json_model_20_map_$comment (.'$Enum'.'$comment')
def _jm_f_194(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Enum'.'$comment']", path))
    return res

# check json_model_20_map_$defs (.'$Enum'.'$defs')
def _jm_f_195(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Enum'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_53: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Enum'.'$defs'.''
        res = json_model_25(pval, lpath_53 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Enum'.'$defs'.'']", lpath_53 if path is not None else None))
            return False
    return True

# check json_model_20_map_$dynamicAnchor (.'$Enum'.'$dynamicAnchor')
def _jm_f_196(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Enum'.'$dynamicAnchor']", path))
    return res

# check json_model_20_map_$id (.'$Enum'.'$id')
def _jm_f_197(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Enum'.'$id']", path))
    return res

# check json_model_20_map_$schema (.'$Enum'.'$schema')
def _jm_f_198(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Enum'.'$schema']", path))
    return res

# check json_model_20_map_$vocabulary (.'$Enum'.'$vocabulary')
def _jm_f_199(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Enum'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_54: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_54 if path is not None else None, rep):
            # handle 1 key props
            # .'$Enum'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Enum'.'$vocabulary'.'$URI']", lpath_54 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Enum'.'$vocabulary']", lpath_54 if path is not None else None))
            return False
    return True

# check json_model_20_map_default (.'$Enum'.default)
def _jm_f_200(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.default
    return True

# check json_model_20_map_definitions (.'$Enum'.definitions)
def _jm_f_201(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Enum'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_55: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Enum'.definitions.''
        res = json_model_25(pval, lpath_55 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Enum'.definitions.'']", lpath_55 if path is not None else None))
            return False
    return True

# check json_model_20_map_deprecated (.'$Enum'.deprecated)
def _jm_f_202(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Enum'.deprecated]", path))
    return res

# check json_model_20_map_description (.'$Enum'.description)
def _jm_f_203(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Enum'.description]", path))
    return res

# check json_model_20_map_examples (.'$Enum'.examples)
def _jm_f_204(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Enum'.examples]", path))
    return res

# check json_model_20_map_id (.'$Enum'.id)
def _jm_f_205(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Enum'.id]", path))
    return res

# check json_model_20_map_readOnly (.'$Enum'.readOnly)
def _jm_f_206(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Enum'.readOnly]", path))
    return res

# check json_model_20_map_title (.'$Enum'.title)
def _jm_f_207(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Enum'.title]", path))
    return res

# check json_model_20_map_writeOnly (.'$Enum'.writeOnly)
def _jm_f_208(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Enum'.writeOnly]", path))
    return res


# check $Enum (.'$Enum')
def json_model_20(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Enum'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Enum']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_52: Path = (path + [ prop ]) if path is not None else None
        if prop == "enum":
            # handle must enum property
            must_count += 1
            # .'$Enum'.enum
            res = json_model_3(pval, lpath_52 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$enum\" [.'$Enum'.enum]", lpath_52 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Enum'.enum]", lpath_52 if path is not None else None))
                return False
            continue
        if pfun := json_model_20_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_52 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Enum']", lpath_52 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Enum']", lpath_52 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "enum" in val:
                rep is None or rep.append(("missing mandatory prop <enum> [.'$Enum']", path))
        return False
    return True

# check json_model_21_map_$anchor (.'$Const'.'$anchor')
def _jm_f_209(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Const'.'$anchor']", path))
    return res

# check json_model_21_map_$comment (.'$Const'.'$comment')
def _jm_f_210(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Const'.'$comment']", path))
    return res

# check json_model_21_map_$defs (.'$Const'.'$defs')
def _jm_f_211(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Const'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_57: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Const'.'$defs'.''
        res = json_model_25(pval, lpath_57 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Const'.'$defs'.'']", lpath_57 if path is not None else None))
            return False
    return True

# check json_model_21_map_$dynamicAnchor (.'$Const'.'$dynamicAnchor')
def _jm_f_212(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Const'.'$dynamicAnchor']", path))
    return res

# check json_model_21_map_$id (.'$Const'.'$id')
def _jm_f_213(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Const'.'$id']", path))
    return res

# check json_model_21_map_$schema (.'$Const'.'$schema')
def _jm_f_214(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Const'.'$schema']", path))
    return res

# check json_model_21_map_$vocabulary (.'$Const'.'$vocabulary')
def _jm_f_215(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Const'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_58: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_58 if path is not None else None, rep):
            # handle 1 key props
            # .'$Const'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Const'.'$vocabulary'.'$URI']", lpath_58 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Const'.'$vocabulary']", lpath_58 if path is not None else None))
            return False
    return True

# check json_model_21_map_default (.'$Const'.default)
def _jm_f_216(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.default
    return True

# check json_model_21_map_definitions (.'$Const'.definitions)
def _jm_f_217(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Const'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_59: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Const'.definitions.''
        res = json_model_25(pval, lpath_59 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Const'.definitions.'']", lpath_59 if path is not None else None))
            return False
    return True

# check json_model_21_map_deprecated (.'$Const'.deprecated)
def _jm_f_218(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Const'.deprecated]", path))
    return res

# check json_model_21_map_description (.'$Const'.description)
def _jm_f_219(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Const'.description]", path))
    return res

# check json_model_21_map_examples (.'$Const'.examples)
def _jm_f_220(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Const'.examples]", path))
    return res

# check json_model_21_map_id (.'$Const'.id)
def _jm_f_221(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Const'.id]", path))
    return res

# check json_model_21_map_readOnly (.'$Const'.readOnly)
def _jm_f_222(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Const'.readOnly]", path))
    return res

# check json_model_21_map_title (.'$Const'.title)
def _jm_f_223(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Const'.title]", path))
    return res

# check json_model_21_map_writeOnly (.'$Const'.writeOnly)
def _jm_f_224(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Const'.writeOnly]", path))
    return res


# check $Const (.'$Const')
def json_model_21(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Const'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Const']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_56: Path = (path + [ prop ]) if path is not None else None
        if prop == "const":
            # handle must const property
            must_count += 1
            # .'$Const'.const
            res = json_model_2(pval, lpath_56 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$const\" [.'$Const'.const]", lpath_56 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Const'.const]", lpath_56 if path is not None else None))
                return False
            continue
        if pfun := json_model_21_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_56 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Const']", lpath_56 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Const']", lpath_56 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "const" in val:
                rep is None or rep.append(("missing mandatory prop <const> [.'$Const']", path))
        return False
    return True

# check json_model_22_map_$anchor (.'$Ref'.'$anchor')
def _jm_f_225(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Ref'.'$anchor']", path))
    return res

# check json_model_22_map_$comment (.'$Ref'.'$comment')
def _jm_f_226(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Ref'.'$comment']", path))
    return res

# check json_model_22_map_$defs (.'$Ref'.'$defs')
def _jm_f_227(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Ref'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_61: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Ref'.'$defs'.''
        res = json_model_25(pval, lpath_61 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Ref'.'$defs'.'']", lpath_61 if path is not None else None))
            return False
    return True

# check json_model_22_map_$dynamicAnchor (.'$Ref'.'$dynamicAnchor')
def _jm_f_228(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Ref'.'$dynamicAnchor']", path))
    return res

# check json_model_22_map_$id (.'$Ref'.'$id')
def _jm_f_229(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Ref'.'$id']", path))
    return res

# check json_model_22_map_$schema (.'$Ref'.'$schema')
def _jm_f_230(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Ref'.'$schema']", path))
    return res

# check json_model_22_map_$vocabulary (.'$Ref'.'$vocabulary')
def _jm_f_231(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Ref'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_62: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_62 if path is not None else None, rep):
            # handle 1 key props
            # .'$Ref'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$Ref'.'$vocabulary'.'$URI']", lpath_62 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$Ref'.'$vocabulary']", lpath_62 if path is not None else None))
            return False
    return True

# check json_model_22_map_default (.'$Ref'.default)
def _jm_f_232(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.default
    return True

# check json_model_22_map_definitions (.'$Ref'.definitions)
def _jm_f_233(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Ref'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_63: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$Ref'.definitions.''
        res = json_model_25(pval, lpath_63 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$Ref'.definitions.'']", lpath_63 if path is not None else None))
            return False
    return True

# check json_model_22_map_deprecated (.'$Ref'.deprecated)
def _jm_f_234(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Ref'.deprecated]", path))
    return res

# check json_model_22_map_description (.'$Ref'.description)
def _jm_f_235(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Ref'.description]", path))
    return res

# check json_model_22_map_examples (.'$Ref'.examples)
def _jm_f_236(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$Ref'.examples]", path))
    return res

# check json_model_22_map_id (.'$Ref'.id)
def _jm_f_237(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Ref'.id]", path))
    return res

# check json_model_22_map_readOnly (.'$Ref'.readOnly)
def _jm_f_238(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Ref'.readOnly]", path))
    return res

# check json_model_22_map_title (.'$Ref'.title)
def _jm_f_239(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$Ref'.title]", path))
    return res

# check json_model_22_map_writeOnly (.'$Ref'.writeOnly)
def _jm_f_240(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Ref'.writeOnly]", path))
    return res


# check $Ref (.'$Ref')
def json_model_22(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Ref'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$Ref']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_60: Path = (path + [ prop ]) if path is not None else None
        if prop == "$ref":
            # handle must $ref property
            must_count += 1
            # .'$Ref'.'$ref'
            res = is_valid_url(pval, lpath_60 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$URI\" [.'$Ref'.'$ref']", lpath_60 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$Ref'.'$ref']", lpath_60 if path is not None else None))
                return False
            continue
        if pfun := json_model_22_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_60 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$Ref']", lpath_60 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$Ref']", lpath_60 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "$ref" in val:
                rep is None or rep.append(("missing mandatory prop <$ref> [.'$Ref']", path))
        return False
    return True

# check json_model_23_map_$anchor (.'$DynRef'.'$anchor')
def _jm_f_241(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.'$anchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$DynRef'.'$anchor']", path))
    return res

# check json_model_23_map_$comment (.'$DynRef'.'$comment')
def _jm_f_242(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.'$comment'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$DynRef'.'$comment']", path))
    return res

# check json_model_23_map_$defs (.'$DynRef'.'$defs')
def _jm_f_243(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.'$defs'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$DynRef'.'$defs']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_65: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$DynRef'.'$defs'.''
        res = json_model_25(pval, lpath_65 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$DynRef'.'$defs'.'']", lpath_65 if path is not None else None))
            return False
    return True

# check json_model_23_map_$dynamicAnchor (.'$DynRef'.'$dynamicAnchor')
def _jm_f_244(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.'$dynamicAnchor'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$DynRef'.'$dynamicAnchor']", path))
    return res

# check json_model_23_map_$id (.'$DynRef'.'$id')
def _jm_f_245(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.'$id'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$DynRef'.'$id']", path))
    return res

# check json_model_23_map_$schema (.'$DynRef'.'$schema')
def _jm_f_246(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.'$schema'
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$DynRef'.'$schema']", path))
    return res

# check json_model_23_map_$vocabulary (.'$DynRef'.'$vocabulary')
def _jm_f_247(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.'$vocabulary'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$DynRef'.'$vocabulary']", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_66: Path = (path + [ prop ]) if path is not None else None
        if is_valid_url(prop, lpath_66 if path is not None else None, rep):
            # handle 1 key props
            # .'$DynRef'.'$vocabulary'.'$URI'
            res = isinstance(pval, bool)
            if not res:
                rep is None or rep.append(("not a bool [.'$DynRef'.'$vocabulary'.'$URI']", lpath_66 if path is not None else None))
                return False
        else:
            rep is None or rep.append(("unexpected prop [.'$DynRef'.'$vocabulary']", lpath_66 if path is not None else None))
            return False
    return True

# check json_model_23_map_default (.'$DynRef'.default)
def _jm_f_248(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.default
    return True

# check json_model_23_map_definitions (.'$DynRef'.definitions)
def _jm_f_249(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.definitions
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$DynRef'.definitions]", path))
        return False
    res: bool
    for prop, pval in val.items():
        lpath_67: Path = (path + [ prop ]) if path is not None else None
        # handle other props
        # .'$DynRef'.definitions.''
        res = json_model_25(pval, lpath_67 if path is not None else None, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$Schema\" [.'$DynRef'.definitions.'']", lpath_67 if path is not None else None))
            return False
    return True

# check json_model_23_map_deprecated (.'$DynRef'.deprecated)
def _jm_f_250(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.deprecated
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$DynRef'.deprecated]", path))
    return res

# check json_model_23_map_description (.'$DynRef'.description)
def _jm_f_251(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.description
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$DynRef'.description]", path))
    return res

# check json_model_23_map_examples (.'$DynRef'.examples)
def _jm_f_252(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.examples
    res: bool = isinstance(val, list)
    if not res:
        rep is None or rep.append(("not array or unexpected array [.'$DynRef'.examples]", path))
    return res

# check json_model_23_map_id (.'$DynRef'.id)
def _jm_f_253(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.id
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$DynRef'.id]", path))
    return res

# check json_model_23_map_readOnly (.'$DynRef'.readOnly)
def _jm_f_254(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.readOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$DynRef'.readOnly]", path))
    return res

# check json_model_23_map_title (.'$DynRef'.title)
def _jm_f_255(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.title
    res: bool = isinstance(val, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$DynRef'.title]", path))
    return res

# check json_model_23_map_writeOnly (.'$DynRef'.writeOnly)
def _jm_f_256(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'.writeOnly
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$DynRef'.writeOnly]", path))
    return res


# check $DynRef (.'$DynRef')
def json_model_23(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$DynRef'
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$DynRef']", path))
        return False
    res: bool
    pfun: CheckFun
    must_count: int = 0
    for prop, pval in val.items():
        lpath_64: Path = (path + [ prop ]) if path is not None else None
        if prop == "$dynamicRef":
            # handle must $dynamicRef property
            must_count += 1
            # .'$DynRef'.'$dynamicRef'
            res = is_valid_url(pval, lpath_64 if path is not None else None, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$URI\" [.'$DynRef'.'$dynamicRef']", lpath_64 if path is not None else None))
                rep is None or rep.append(("invalid mandatory prop value [.'$DynRef'.'$dynamicRef']", lpath_64 if path is not None else None))
                return False
            continue
        if pfun := json_model_23_map.get(prop):
            # handle 16 may props
            if pfun != UNDEFINED and not pfun(pval, lpath_64 if path is not None else None, rep):
                rep is None or rep.append(("invalid optional prop value [.'$DynRef']", lpath_64 if path is not None else None))
                return False
            continue
        rep is None or rep.append(("unexpected prop [.'$DynRef']", lpath_64 if path is not None else None))
        return False
    if must_count != 1:
        if rep is not None:
            if not "$dynamicRef" in val:
                rep is None or rep.append(("missing mandatory prop <$dynamicRef> [.'$DynRef']", path))
        return False
    return True


# check $ObjectSchema (.'$ObjectSchema')
def json_model_24(val: Jsonable, path: Path, rep: Report) -> bool:
    # we could use ^ instead of | below
    # .'$ObjectSchema'
    res: bool = isinstance(val, dict)
    if res:
        tag_0: Jsonable
        if (tag_0 := val.get("type", UNDEFINED)) != UNDEFINED:
            fun_0: CheckFun = _jm_map_0.get(tag_0, UNDEFINED) if isinstance(tag_0, str) else UNDEFINED
            if fun_0 != UNDEFINED:
                res = fun_0(val, path, rep)
            else:
                res = False
                rep is None or rep.append(("tag <type> value not found [.'$ObjectSchema'.'|']", path))
        else:
            res = False
            rep is None or rep.append(("tag prop <type> is missing [.'$ObjectSchema'.'|']", path))
    else:
        rep is None or rep.append(("value is not an object [.'$ObjectSchema'.'|']", path))
    return res or json_model_9(val, path, rep) or json_model_21(val, path, rep) or json_model_20(val, path, rep) or json_model_17(val, path, rep) or json_model_18(val, path, rep) or json_model_19(val, path, rep) or json_model_22(val, path, rep) or json_model_23(val, path, rep)

# check $Schema (.'$Schema')
def json_model_25(val: Jsonable, path: Path, rep: Report) -> bool:
    # .'$Schema'
    # .'$Schema'.'|'.0
    res: bool = isinstance(val, bool)
    if not res:
        rep is None or rep.append(("not a bool [.'$Schema'.'|'.0]", path))
        # .'$Schema'.'|'.1
        res = json_model_24(val, path, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$ObjectSchema\" [.'$Schema'.'|'.1]", path))
    if res:
        rep is None or rep.clear()
    else:
        rep is None or rep.append(("no model matched [.'$Schema'.'|']", path))
    return res

# object .'$RootSchema'.'&'.0
def _jm_obj_2(val: Jsonable, path: Path, rep: Report) -> bool:
    # check open must/may only props
    if not isinstance(val, dict):
        rep is None or rep.append(("not an object [.'$RootSchema'.'&'.0]", path))
        return False
    lpath: Path
    pval: Jsonable
    if not ((pval := val.get("$schema", UNDEFINED)) != UNDEFINED):
        rep is None or rep.append(("missing mandatory prop <$schema> [.'$RootSchema'.'&'.0]", path))
        return False
    lpath = (path + [ "$schema" ]) if path is not None else None
    # .'$RootSchema'.'&'.0.'$schema'
    res: bool = isinstance(pval, str)
    if not res:
        rep is None or rep.append(("unexpected value for model \"\" [.'$RootSchema'.'&'.0.'$schema']", lpath if path is not None else None))
        rep is None or rep.append(("unexpected value for mandatory prop <$schema> [.'$RootSchema'.'&'.0]", lpath if path is not None else None))
        return False
    return True

# check $RootSchema (.'$RootSchema')
def json_model_26(val: Jsonable, path: Path, rep: Report) -> bool:
    # keyword $schema is mandatory at the root, and optional elsewhere
    # .'$RootSchema'
    res: bool = True
    if res:
        # .'$RootSchema'.'&'.0
        res = _jm_obj_2(val, path, rep)
        if res:
            # .'$RootSchema'.'&'.1
            res = json_model_24(val, path, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$ObjectSchema\" [.'$RootSchema'.'&'.1]", path))
        else:
            rep is None or rep.append(("unexpected element [.'$RootSchema'.'&'.0]", path))
    if res:
        rep is None or rep.clear()
    else:
        rep is None or rep.append(("not all model match [.'$RootSchema'.'&']", path))
    return res

# check $ (.)
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    # .
    res: bool = json_model_26(val, path, rep)
    if not res:
        rep is None or rep.append(("unexpected value for model \"$RootSchema\" [.]", path))
    return res


# initialization guard
initialized: bool = False

# differed module initializations
def check_model_init():
    global initialized
    if not initialized:
        initialized = True
        global _jm_cst_0
        _jm_cst_0 = {'byte', 'date', 'date-time', 'double', 'duration', 'email', 'hostname', 'idn-email', 'idn-hostname', 'int', 'int32', 'int64', 'ipv4', 'ipv6', 'iri', 'iri-reference', 'json-pointer', 'mime-type', 'regex', 'relative-json-pointer', 'time', 'uint', 'uint32', 'uint64', 'uri', 'uri-reference', 'uuid'}
        global json_model_9_map
        json_model_9_map = {
            "$anchor": _jm_f_0,
            "$comment": _jm_f_1,
            "$defs": _jm_f_2,
            "$dynamicAnchor": _jm_f_3,
            "$id": _jm_f_4,
            "$schema": _jm_f_5,
            "$vocabulary": _jm_f_6,
            "default": _jm_f_7,
            "definitions": _jm_f_8,
            "deprecated": _jm_f_9,
            "description": _jm_f_10,
            "examples": _jm_f_11,
            "id": _jm_f_12,
            "readOnly": _jm_f_13,
            "title": _jm_f_14,
            "writeOnly": _jm_f_15,
        }
        global _jm_cst_1
        _jm_cst_1 = {'byte', 'date', 'date-time', 'double', 'duration', 'email', 'hostname', 'idn-email', 'idn-hostname', 'int', 'int32', 'int64', 'ipv4', 'ipv6', 'iri', 'iri-reference', 'json-pointer', 'mime-type', 'regex', 'relative-json-pointer', 'time', 'uint', 'uint32', 'uint64', 'uri', 'uri-reference', 'uuid'}
        global json_model_10_map
        json_model_10_map = {
            "$anchor": _jm_f_16,
            "$comment": _jm_f_17,
            "$defs": _jm_f_18,
            "$dynamicAnchor": _jm_f_19,
            "$id": _jm_f_20,
            "$schema": _jm_f_21,
            "$vocabulary": _jm_f_22,
            "default": _jm_f_23,
            "definitions": _jm_f_24,
            "deprecated": _jm_f_25,
            "description": _jm_f_26,
            "examples": _jm_f_27,
            "format": _jm_f_28,
            "id": _jm_f_29,
            "maxLength": _jm_f_30,
            "minLength": _jm_f_31,
            "pattern": _jm_f_32,
            "readOnly": _jm_f_33,
            "title": _jm_f_34,
            "writeOnly": _jm_f_35,
        }
        global json_model_11_map
        json_model_11_map = {
            "$anchor": _jm_f_36,
            "$comment": _jm_f_37,
            "$defs": _jm_f_38,
            "$dynamicAnchor": _jm_f_39,
            "$id": _jm_f_40,
            "$schema": _jm_f_41,
            "$vocabulary": _jm_f_42,
            "additionalItems": json_model_25,
            "default": _jm_f_43,
            "definitions": _jm_f_44,
            "deprecated": _jm_f_45,
            "description": _jm_f_46,
            "examples": _jm_f_47,
            "id": _jm_f_48,
            "items": _jm_f_49,
            "maxItems": _jm_f_50,
            "minItems": _jm_f_51,
            "prefixItems": json_model_4,
            "readOnly": _jm_f_52,
            "title": _jm_f_53,
            "unevaluatedItems": json_model_25,
            "uniqueItems": _jm_f_54,
            "writeOnly": _jm_f_55,
        }
        global json_model_12_map
        json_model_12_map = {
            "$anchor": _jm_f_56,
            "$comment": _jm_f_57,
            "$defs": _jm_f_58,
            "$dynamicAnchor": _jm_f_59,
            "$id": _jm_f_60,
            "$schema": _jm_f_61,
            "$vocabulary": _jm_f_62,
            "additionalProperties": json_model_25,
            "default": _jm_f_63,
            "definitions": _jm_f_64,
            "deprecated": _jm_f_65,
            "description": _jm_f_66,
            "examples": _jm_f_67,
            "id": _jm_f_68,
            "maxProperties": _jm_f_69,
            "minProperties": _jm_f_70,
            "patternProperties": _jm_f_71,
            "properties": _jm_f_72,
            "propertyNames": json_model_24,
            "readOnly": _jm_f_73,
            "required": _jm_f_74,
            "title": _jm_f_75,
            "unevaluatedProperties": json_model_25,
            "writeOnly": _jm_f_76,
        }
        global json_model_13_map
        json_model_13_map = {
            "$anchor": _jm_f_77,
            "$comment": _jm_f_78,
            "$defs": _jm_f_79,
            "$dynamicAnchor": _jm_f_80,
            "$id": _jm_f_81,
            "$schema": _jm_f_82,
            "$vocabulary": _jm_f_83,
            "default": _jm_f_84,
            "definitions": _jm_f_85,
            "deprecated": _jm_f_86,
            "description": _jm_f_87,
            "examples": _jm_f_88,
            "id": _jm_f_89,
            "maximum": _jm_f_90,
            "minimum": _jm_f_91,
            "readOnly": _jm_f_92,
            "title": _jm_f_93,
            "writeOnly": _jm_f_94,
        }
        global json_model_14_map
        json_model_14_map = {
            "$anchor": _jm_f_95,
            "$comment": _jm_f_96,
            "$defs": _jm_f_97,
            "$dynamicAnchor": _jm_f_98,
            "$id": _jm_f_99,
            "$schema": _jm_f_100,
            "$vocabulary": _jm_f_101,
            "default": _jm_f_102,
            "definitions": _jm_f_103,
            "deprecated": _jm_f_104,
            "description": _jm_f_105,
            "examples": _jm_f_106,
            "id": _jm_f_107,
            "maximum": _jm_f_108,
            "minimum": _jm_f_109,
            "readOnly": _jm_f_110,
            "title": _jm_f_111,
            "writeOnly": _jm_f_112,
        }
        global json_model_15_map
        json_model_15_map = {
            "$anchor": _jm_f_113,
            "$comment": _jm_f_114,
            "$defs": _jm_f_115,
            "$dynamicAnchor": _jm_f_116,
            "$id": _jm_f_117,
            "$schema": _jm_f_118,
            "$vocabulary": _jm_f_119,
            "default": _jm_f_120,
            "definitions": _jm_f_121,
            "deprecated": _jm_f_122,
            "description": _jm_f_123,
            "examples": _jm_f_124,
            "id": _jm_f_125,
            "readOnly": _jm_f_126,
            "title": _jm_f_127,
            "writeOnly": _jm_f_128,
        }
        global json_model_16_map
        json_model_16_map = {
            "$anchor": _jm_f_129,
            "$comment": _jm_f_130,
            "$defs": _jm_f_131,
            "$dynamicAnchor": _jm_f_132,
            "$id": _jm_f_133,
            "$schema": _jm_f_134,
            "$vocabulary": _jm_f_135,
            "default": _jm_f_136,
            "definitions": _jm_f_137,
            "deprecated": _jm_f_138,
            "description": _jm_f_139,
            "examples": _jm_f_140,
            "id": _jm_f_141,
            "readOnly": _jm_f_142,
            "title": _jm_f_143,
            "writeOnly": _jm_f_144,
        }
        global json_model_17_map
        json_model_17_map = {
            "$anchor": _jm_f_145,
            "$comment": _jm_f_146,
            "$defs": _jm_f_147,
            "$dynamicAnchor": _jm_f_148,
            "$id": _jm_f_149,
            "$schema": _jm_f_150,
            "$vocabulary": _jm_f_151,
            "default": _jm_f_152,
            "definitions": _jm_f_153,
            "deprecated": _jm_f_154,
            "description": _jm_f_155,
            "examples": _jm_f_156,
            "id": _jm_f_157,
            "readOnly": _jm_f_158,
            "title": _jm_f_159,
            "writeOnly": _jm_f_160,
        }
        global json_model_18_map
        json_model_18_map = {
            "$anchor": _jm_f_161,
            "$comment": _jm_f_162,
            "$defs": _jm_f_163,
            "$dynamicAnchor": _jm_f_164,
            "$id": _jm_f_165,
            "$schema": _jm_f_166,
            "$vocabulary": _jm_f_167,
            "default": _jm_f_168,
            "definitions": _jm_f_169,
            "deprecated": _jm_f_170,
            "description": _jm_f_171,
            "examples": _jm_f_172,
            "id": _jm_f_173,
            "readOnly": _jm_f_174,
            "title": _jm_f_175,
            "writeOnly": _jm_f_176,
        }
        global json_model_19_map
        json_model_19_map = {
            "$anchor": _jm_f_177,
            "$comment": _jm_f_178,
            "$defs": _jm_f_179,
            "$dynamicAnchor": _jm_f_180,
            "$id": _jm_f_181,
            "$schema": _jm_f_182,
            "$vocabulary": _jm_f_183,
            "default": _jm_f_184,
            "definitions": _jm_f_185,
            "deprecated": _jm_f_186,
            "description": _jm_f_187,
            "examples": _jm_f_188,
            "id": _jm_f_189,
            "readOnly": _jm_f_190,
            "title": _jm_f_191,
            "writeOnly": _jm_f_192,
        }
        global json_model_20_map
        json_model_20_map = {
            "$anchor": _jm_f_193,
            "$comment": _jm_f_194,
            "$defs": _jm_f_195,
            "$dynamicAnchor": _jm_f_196,
            "$id": _jm_f_197,
            "$schema": _jm_f_198,
            "$vocabulary": _jm_f_199,
            "default": _jm_f_200,
            "definitions": _jm_f_201,
            "deprecated": _jm_f_202,
            "description": _jm_f_203,
            "examples": _jm_f_204,
            "id": _jm_f_205,
            "readOnly": _jm_f_206,
            "title": _jm_f_207,
            "writeOnly": _jm_f_208,
        }
        global json_model_21_map
        json_model_21_map = {
            "$anchor": _jm_f_209,
            "$comment": _jm_f_210,
            "$defs": _jm_f_211,
            "$dynamicAnchor": _jm_f_212,
            "$id": _jm_f_213,
            "$schema": _jm_f_214,
            "$vocabulary": _jm_f_215,
            "default": _jm_f_216,
            "definitions": _jm_f_217,
            "deprecated": _jm_f_218,
            "description": _jm_f_219,
            "examples": _jm_f_220,
            "id": _jm_f_221,
            "readOnly": _jm_f_222,
            "title": _jm_f_223,
            "writeOnly": _jm_f_224,
        }
        global json_model_22_map
        json_model_22_map = {
            "$anchor": _jm_f_225,
            "$comment": _jm_f_226,
            "$defs": _jm_f_227,
            "$dynamicAnchor": _jm_f_228,
            "$id": _jm_f_229,
            "$schema": _jm_f_230,
            "$vocabulary": _jm_f_231,
            "default": _jm_f_232,
            "definitions": _jm_f_233,
            "deprecated": _jm_f_234,
            "description": _jm_f_235,
            "examples": _jm_f_236,
            "id": _jm_f_237,
            "readOnly": _jm_f_238,
            "title": _jm_f_239,
            "writeOnly": _jm_f_240,
        }
        global json_model_23_map
        json_model_23_map = {
            "$anchor": _jm_f_241,
            "$comment": _jm_f_242,
            "$defs": _jm_f_243,
            "$dynamicAnchor": _jm_f_244,
            "$id": _jm_f_245,
            "$schema": _jm_f_246,
            "$vocabulary": _jm_f_247,
            "default": _jm_f_248,
            "definitions": _jm_f_249,
            "deprecated": _jm_f_250,
            "description": _jm_f_251,
            "examples": _jm_f_252,
            "id": _jm_f_253,
            "readOnly": _jm_f_254,
            "title": _jm_f_255,
            "writeOnly": _jm_f_256,
        }
        global _jm_map_0
        _jm_map_0 = {
            "null": json_model_16,
            "boolean": json_model_15,
            "integer": json_model_13,
            "number": json_model_14,
            "string": json_model_10,
            "array": json_model_11,
            "object": json_model_12,
        }
        global check_model_map
        check_model_map = {
            "": json_model_26,
            "const": json_model_2,
            "enum": json_model_3,
            "schemaArray": json_model_4,
            "stringKeywords": json_model_5,
            "arrayKeywords": json_model_6,
            "objectKeywords": json_model_7,
            "numberKeywords": json_model_8,
            "metas": json_model_9,
            "String": json_model_10,
            "Array": json_model_11,
            "Object": json_model_12,
            "Integer": json_model_13,
            "Number": json_model_14,
            "Bool": json_model_15,
            "Null": json_model_16,
            "AllOf": json_model_17,
            "AnyOf": json_model_18,
            "OneOf": json_model_19,
            "Enum": json_model_20,
            "Const": json_model_21,
            "Ref": json_model_22,
            "DynRef": json_model_23,
            "ObjectSchema": json_model_24,
            "Schema": json_model_25,
            "RootSchema": json_model_26,
        }

# differed module cleanup
def check_model_free():
    global initialized
    if initialized:
        initialized = False

if __name__ == "__main__":
    check_model_init()
    main(check_model_fun, check_model_map, __version__)
    check_model_free()
