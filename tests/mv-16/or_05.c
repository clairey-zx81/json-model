    ...
    if (res)
    {
        ...
        if (! res)
        {
            ...
            if (! res)
            {
                ...
                if (! res)
                {
                    if (rep) jm_report_add_entry(rep, "unexpected /[A-Z]/ [.'|'.2]", path);
                }
            }
        }
        if (res)
...
char *CHECK_init(void)
{
    if (!initialized)
    {
        ...
        _jm_re_1_code = pcre2_compile((PCRE2_SPTR) "[a-z]", PCRE2_ZERO_TERMINATED, PCRE2_UCP|PCRE2_UTF, &err_code, &err_offset, NULL);
        ...
        _jm_re_2_code = pcre2_compile((PCRE2_SPTR) "[0-9]", PCRE2_ZERO_TERMINATED, PCRE2_UCP|PCRE2_UTF, &err_code, &err_offset, NULL);
