...
final _jm_re_1_re = RegExp("[a-z]", caseSensitive: true, multiLine: false);
final _jm_re_2_re = RegExp("[0-9]", caseSensitive: true, multiLine: false);
...
bool json_model_1(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    ...
    if (res)
    {
        ...
        if (! res)
        {
            rep?.add(["unexpected /[0-9]/ [.'|'.0]", path]);
            // .'|'.1
            // "/[a-z]/"
            res = _jm_re_1(val, path, rep);
            if (! res)
            {
                rep?.add(["unexpected /[a-z]/ [.'|'.1]", path]);
                // .'|'.2
                // "/[A-Z]/"
                res = _jm_re_0(val, path, rep);
                if (! res)
                {
                    rep?.add(["unexpected /[A-Z]/ [.'|'.2]", path]);
                }
            }
        }
        if (res)
