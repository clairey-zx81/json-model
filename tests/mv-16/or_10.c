...
static bool _jm_obj_0(const json_t* val, Path* path, Report* rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (strcmp(prop, "name") == 0)
        {
            ...
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "unexpected string [.'|'.1.name]", (path ? &lpath_0 : NULL));
            }
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid mandatory property value [.'|'.1.name]", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else
    ...
    if (must_count != 1)
    {
        if (rep != NULL)
        {
            ...
            {
                if (rep) jm_report_add_entry(rep, "missing mandatory prop <name> [.'|'.1]", path);
            }
        }
        return false;
    }
