...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        ...
        if not res:
            rep is None or rep.append(("unexpected value for model \"/[0-9]/\" [.'|'.0]", path))
            # .'|'.1
            # "/[a-z]/"
            res = _jm_re_1(val, path, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"/[a-z]/\" [.'|'.1]", path))
                # .'|'.2
                # "/[A-Z]/"
                res = _jm_re_0(val, path, rep)
                if not res:
                    rep is None or rep.append(("unexpected value for model \"/[A-Z]/\" [.'|'.2]", path))
        if res:
