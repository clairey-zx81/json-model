//
// Generated by JSON Model Compiler version 2
// see https://github.com/clairey-zx81/json-model
//

import json_model.*;
import java.util.Map;
import java.util.Set;
import java.util.HashMap;
import java.util.HashSet;
import java.util.Iterator;
import java.util.regex.Pattern;
import java.util.regex.Matcher;

@SuppressWarnings("unchecked")
public class root_0 extends ModelChecker
{
    static public final String VERSION = "2";

    public Map<String, Checker> root_0_map_pmap;

    // object .'$Root'
    public boolean _jm_obj_0(Object val, Path path, Report rep)
    {
        // check close must only props
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$Root']", path);
            return false;
        }
        if (json.objectSize(val) != 2)
        {
            if (rep != null) rep.addEntry("bad property count [.'$Root']", path);
            return false;
        }
        Object pval;
        boolean res;
        if (! json.objectHasProp(val, "id"))
        {
            if (rep != null) rep.addEntry("missing mandatory prop <id> [.'$Root']", path);
            return false;
        }
        pval = json.objectValue(val, "id");
        // .'$Root'.id
        res = json.isInteger(pval) && json.asLong(pval) == 0;
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected =0 [.'$Root'.id]", path);
            if (rep != null) rep.addEntry("unexpected value for mandatory prop <id> [.'$Root']", path);
            return false;
        }
        if (! json.objectHasProp(val, "name"))
        {
            if (rep != null) rep.addEntry("missing mandatory prop <name> [.'$Root']", path);
            return false;
        }
        pval = json.objectValue(val, "name");
        // .'$Root'.name
        res = json.isString(pval);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected string [.'$Root'.name]", path);
            if (rep != null) rep.addEntry("unexpected value for mandatory prop <name> [.'$Root']", path);
            return false;
        }
        return true;
    }

    // check $Root (.'$Root')
    public boolean json_model_2(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Root'
        res = _jm_obj_0(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected element [.'$Root']", path);
        }
        return res;
    }

    // check $ (.)
    public boolean json_model_1(Object val, Path path, Report rep)
    {
        boolean res;
        // .
        res = _jm_obj_0(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected $Root [.]", path);
        }
        return res;
    }


    public void init(JSON json)
    {
        if (!initialized)
        {
            try {
            root_0_map_pmap = new HashMap<String, Checker>();
            root_0_map_pmap.put("", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_obj_0(o, p, r);} });
            root_0_map_pmap.put("Root", new Checker() { public boolean call(Object o, Path p, Report r) { return _jm_obj_0(o, p, r);} });
                super.init(json);
            }
            catch (Exception e) {
                throw new Error("cannot initialized model checker: " + e);
            }
        }
    }

    public void free()
    {
        if (initialized)
        {
            super.free();
            root_0_map_pmap = null;
        }
    }

    public Checker get(String name)
    {
        return root_0_map_pmap.get(name);
    }

    public Set<String> models()
    {
        return root_0_map_pmap.keySet();
    }

    static public void main(String[] args) throws Exception
    {
        ModelChecker checker = new root_0();
        Main.main("root_0", checker, VERSION, args);
    }
}
