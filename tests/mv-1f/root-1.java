//
// Generated by JSON Model Compiler version 2.0b1
// see https://github.com/clairey-zx81/json-model
//

import json_model.*;
import java.util.Map;
import java.util.Set;
import java.util.HashMap;
import java.util.HashSet;
import java.util.Iterator;
import java.util.regex.Pattern;
import java.util.regex.Matcher;

@SuppressWarnings("unchecked")
public class root_1 extends ModelChecker
{
    static public final String VERSION = "2";

    public Map<String, Checker> root_1_map_pmap;

    // check $root (.'$root')
    public boolean json_model_4(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$root'
        res = json_model_5(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected $Root [.'$root']", path);
        }
        return res;
    }
    
    // check $Root (.'$Root')
    public boolean json_model_3(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$Root'
        res = json_model_5(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected $root#Root [.'$Root']", path);
        }
        return res;
    }
    
    // check $ (.)
    public boolean json_model_1(Object val, Path path, Report rep)
    {
        boolean res;
        // .
        res = json_model_3(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected $Root [.]", path);
        }
        return res;
    }
    
    // object .'$root#Root'
    public boolean _jm_obj_0(Object val, Path path, Report rep)
    {
        if (! json.isObject(val))
        {
            if (rep != null) rep.addEntry("not an object [.'$root#Root']", path);
            return false;
        }
        boolean res;
        long must_count = 0;
        Iterator<String> prop_loop = json.objectIterator(val);
        while (prop_loop.hasNext())
        {
            String prop = prop_loop.next();
            Object pval = json.objectValue(val, prop);
            Path lpath_0 = new Path(prop, path);
            if (prop.compareTo("id") == 0)
            {
                // handle must id property
                must_count += 1;
                // .'$root#Root'.id
                res = json.isInteger(pval) && json.asLong(pval) == 1;
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected =1 [.'$root#Root'.id]", (path != null ? lpath_0 : null));
                }
                if (! res)
                {
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$root#Root'.id]", (path != null ? lpath_0 : null));
                    return false;
                }
            }
            else if (prop.compareTo("name") == 0)
            {
                // handle must name property
                must_count += 1;
                // .'$root#Root'.name
                res = json.isString(pval);
                if (! res)
                {
                    if (rep != null) rep.addEntry("unexpected string [.'$root#Root'.name]", (path != null ? lpath_0 : null));
                }
                if (! res)
                {
                    if (rep != null) rep.addEntry("invalid mandatory prop value [.'$root#Root'.name]", (path != null ? lpath_0 : null));
                    return false;
                }
            }
            else
            {
                if (rep != null) rep.addEntry("unexpected prop [.'$root#Root']", (path != null ? lpath_0 : null));
                return false;
            }
        }
        if (must_count != 2)
        {
            if (rep != null)
            {
                if (! json.objectHasProp(val, "id"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <id> [.'$root#Root']", path);
                }
                if (! json.objectHasProp(val, "name"))
                {
                    if (rep != null) rep.addEntry("missing mandatory prop <name> [.'$root#Root']", path);
                }
            }
            return false;
        }
        return true;
    }
    
    // check $root#Root (.'$root#Root')
    public boolean json_model_5(Object val, Path path, Report rep)
    {
        boolean res;
        // .'$root#Root'
        res = _jm_obj_0(val, path, rep);
        if (! res)
        {
            if (rep != null) rep.addEntry("unexpected element [.'$root#Root']", path);
        }
        return res;
    }
    

    public void init(JSON json)
    {
        if (!initialized)
        {
            try {
            root_1_map_pmap = new HashMap<String, Checker>();
            root_1_map_pmap.put("", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_1(o, p, r);} });
            root_1_map_pmap.put("root", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_4(o, p, r);} });
            root_1_map_pmap.put("Root", new Checker() { public boolean call(Object o, Path p, Report r) { return json_model_3(o, p, r);} });
                super.init(json);
            }
            catch (Exception e) {
                throw new Error("cannot initialized model checker: " + e);
            }
        }
    }

    public void free()
    {
        if (initialized)
        {
            super.free();
            root_1_map_pmap = null;
        }
    }

    public Checker get(String name)
    {
        return root_1_map_pmap.get(name);
    }

    public Set<String> models()
    {
        return root_1_map_pmap.keySet();
    }

    static public void main(String[] args) throws Exception
    {
        ModelChecker checker = new root_1();
        Main.main("root_1", checker, VERSION, args);
    }
}
