...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    for prop, pval in val.items():
        ...
        if prop == "foo":
            ...
            if not res:
                rep is None or rep.append(("unexpected value for model \"$root\" [.foo.'|'.0]", lpath_0 if path is not None else None))
                # .foo.'|'.1
                ...
                if res:
                    for arr_0_idx, arr_0_item in enumerate(pval):
                        ...
                        if not res:
                            rep is None or rep.append(("unexpected value for model \"$root\" [.foo.'|'.1.0]", arr_0_lpath if (lpath_0 if path is not None else None) is not None else None))
                            break
                if not res:
