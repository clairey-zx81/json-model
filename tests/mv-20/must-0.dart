...
bool json_model_1(dynamic val, List<dynamic>? path, List<dynamic>? rep)
{
    ...
    if (! val.containsKey("born"))
    {
        rep?.add(["missing mandatory prop <born> [.]", path]);
        return false;
    }
    lpath = (path != null ? [...?path, "born"] : null);
    pval = val["born"];
    // .born
    res = runtime.jm_is_valid_date(pval, (path != null ? lpath : null), rep);
    if (! res)
    {
        rep?.add(["unexpected \$DATE [.born]", (path != null ? lpath : null)]);
        rep?.add(["unexpected value for mandatory prop <born> [.]", (path != null ? lpath : null)]);
        return false;
    }
    return true;
}
