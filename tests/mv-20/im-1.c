...
{
    // .'$c'
    bool res = json_is_string(val) && jm_str_eq_2(json_string_value(val), 0x00000063);
    if (unlikely(! res))
    {
        if (rep) jm_report_add_entry(rep, "unexpected value for model \"_c\" [.'$c']", path);
    }
    return res;
}
...
{
    // .'$a'
    bool res = json_is_string(val) && jm_str_eq_2(json_string_value(val), 0x00000061);
    if (unlikely(! res))
    {
...
const char *check_model_init(void)
{
    if (!initialized)
    {
        ...
        check_model_map_tab[1] = (jm_propmap_t) { "c", json_model_2 };
        check_model_map_tab[2] = (jm_propmap_t) { "a", json_model_4 };
        check_model_map_tab[3] = (jm_propmap_t) { "b", json_model_5 };
