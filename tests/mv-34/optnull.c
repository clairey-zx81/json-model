...
static INLINE bool _jm_cst_0_test(const json_t *val)
{
  jm_constant_t cst;
  jm_set_cst(&cst, val);
  return jm_search_cst(&cst, _jm_cst_0, 3);
}
...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    {
        if (likely(jm_str_eq_5(prop, 0x000000006c6c756eLL)))
        {
            res = jm_json_is_scalar(pval) && _jm_cst_0_test(pval);
            if (unlikely(! res))
                return false;
            continue;
        }
...
const char *check_model_init(void)
{
    if (!initialized)
    {
        ...
        _jm_cst_0[1] = (jm_constant_t) { 5, { .s = "NULL" } };
        _jm_cst_0[2] = (jm_constant_t) { 5, { .s = "NOPE" } };
