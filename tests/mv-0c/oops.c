...
static INLINE bool _jm_obj_0(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (likely(jm_str_eq_2(prop, 0x00000063)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.'$Ax'.'^'.1.c]", (path ? &lpath_0 : NULL));
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'$Ax'.'^'.1.c]", (path ? &lpath_0 : NULL));
                return false;
            }
            continue;
        }
        if (rep) jm_report_add_entry(rep, "unexpected prop [.'$Ax'.'^'.1]", (path ? &lpath_0 : NULL));
        return false;
    }
    return true;
}
...
static bool json_model_2(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    // not-case xor list
    // .'$Ax'.'^'.1
    bool is_0 = _jm_obj_0(val, path, rep);
    if (unlikely(! is_0))
    {
