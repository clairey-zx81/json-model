        ...
        if not res:
            ...
            if res:
                for arr_0_idx, arr_0_item in enumerate(pval):
                    ...
                    if not res:
                        rep is None or rep.append(("unexpected value for model \"\" [.'$schema#ObjectSchema'.dependencies.''.'|'.1.0]", arr_0_lpath if (lpath_2 if path is not None else None) is not None else None))
                        break
            if not res:
...
def _jm_f_28(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        for arr_1_idx, arr_1_item in enumerate(val):
            ...
            if not res:
                rep is None or rep.append(("unexpected value for model \"\" [.'$schema#ObjectSchema'.required.0]", arr_1_lpath if path is not None else None))
                break
    if not res:
...
def _jm_f_30(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if not res:
        ...
        if not res:
            rep is None or rep.append(("unexpected value for model \"$arrayTypes\" [.'$schema#ObjectSchema'.type.'|'.1]", path))
    if res:
        rep is None or rep.clear()
    else:
...
def json_model_7(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        for arr_3_idx, arr_3_item in enumerate(val):
            ...
            if not res:
                rep is None or rep.append(("unexpected value for model \"$simpleTypes\" [.'$schema#arrayTypes'.'@'.0]", arr_3_lpath if path is not None else None))
                break
    if res:
        ival_2: int = len(val)
