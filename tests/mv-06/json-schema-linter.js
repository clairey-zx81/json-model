#! /bin/env node
// we may need require to load re2
import { createRequire } from 'node:module';
const require = createRequire(import.meta.url);
//
// Generated by JSON Model Compiler version 2
// see https://github.com/clairey-zx81/json-model
//
import * as runtime from "json_model_runtime"
const JSON_MODEL_VERSION = "2";

let _jm_cst_0 = new Set()
var json_model_6_map = new Map()
var json_model_9_map = new Map()
const _jm_re_0_re = new runtime.RX("^x-.*", "")
let _jm_cst_1 = new Set()
var json_model_10_map = new Map()
var json_model_11_map = new Map()
var json_model_12_map = new Map()
var json_model_13_map = new Map()
var json_model_14_map = new Map()
var json_model_15_map = new Map()
var json_model_16_map = new Map()
var json_model_17_map = new Map()
var json_model_18_map = new Map()
var json_model_19_map = new Map()
var json_model_20_map = new Map()
var json_model_21_map = new Map()
var json_model_22_map = new Map()
let _jm_map_0 = new Map()
var check_model_map = new Map()

// check $const (.'$const')
function json_model_2(val, path, rep)
{
    let res;
    // .'$const'
    // .'$const'.'|'.0
    res = val === null;
    if (! res)
    {
        rep !== null && rep.push(["not null [.'$const'.'|'.0]", path])
    }
    if (! res)
    {
        // .'$const'.'|'.1
        res = (typeof val === 'boolean' || val instanceof Boolean);
        if (! res)
        {
            rep !== null && rep.push(["not a bool [.'$const'.'|'.1]", path])
        }
        if (! res)
        {
            // .'$const'.'|'.2
            res = (typeof val === 'number' || val instanceof Number) && Number.isInteger(val);
            if (! res)
            {
                rep !== null && rep.push(["not a -1 strict int [.'$const'.'|'.2]", path])
            }
            if (! res)
            {
                // .'$const'.'|'.3
                res = (typeof val === 'number' || val instanceof Number);
                if (! res)
                {
                    rep !== null && rep.push(["not a -1.0 strict float [.'$const'.'|'.3]", path])
                }
                if (! res)
                {
                    // .'$const'.'|'.4
                    res = (typeof val === 'string' || val instanceof String);
                    if (! res)
                    {
                        rep !== null && rep.push(["unexpected string [.'$const'.'|'.4]", path])
                    }
                }
            }
        }
    }
    if (res)
    {
        if (rep !== null) rep.length = 0
    }
    else
    {
        rep !== null && rep.push(["no model matched [.'$const'.'|']", path])
    }
    return res;
}

// check $enum (.'$enum')
function json_model_3(val, path, rep)
{
    let res;
    // .'$enum'
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["unexpected type [.'$enum'.'|']", path])
    }
    if (res)
    {
        // .'$enum'.'|'.0
        for (let arr_2_idx = 0; arr_2_idx < val.length; arr_2_idx++)
        {
            let arr_2_item = val[arr_2_idx]
            let arr_2_lpath = path ? path.concat([arr_2_idx]) : null;
            // .'$enum'.'|'.0.0
            res = (typeof arr_2_item === 'string' || arr_2_item instanceof String);
            if (! res)
            {
                rep !== null && rep.push(["unexpected string [.'$enum'.'|'.0.0]", (path ? arr_2_lpath : null)])
                break;
            }
        }
        if (! res)
        {
            rep !== null && rep.push(["not array or unexpected array [.'$enum'.'|'.0]", path])
        }
        if (! res)
        {
            // .'$enum'.'|'.1
            for (let arr_1_idx = 0; arr_1_idx < val.length; arr_1_idx++)
            {
                let arr_1_item = val[arr_1_idx]
                let arr_1_lpath = path ? path.concat([arr_1_idx]) : null;
                // .'$enum'.'|'.1.0
                res = (typeof arr_1_item === 'number' || arr_1_item instanceof Number) && Number.isInteger(arr_1_item);
                if (! res)
                {
                    rep !== null && rep.push(["not a -1 strict int [.'$enum'.'|'.1.0]", (path ? arr_1_lpath : null)])
                    break;
                }
            }
            if (! res)
            {
                rep !== null && rep.push(["not array or unexpected array [.'$enum'.'|'.1]", path])
            }
            if (! res)
            {
                // .'$enum'.'|'.2
                for (let arr_0_idx = 0; arr_0_idx < val.length; arr_0_idx++)
                {
                    let arr_0_item = val[arr_0_idx]
                    let arr_0_lpath = path ? path.concat([arr_0_idx]) : null;
                    // .'$enum'.'|'.2.0
                    res = (typeof arr_0_item === 'boolean' || arr_0_item instanceof Boolean);
                    if (! res)
                    {
                        rep !== null && rep.push(["not a bool [.'$enum'.'|'.2.0]", (path ? arr_0_lpath : null)])
                        break;
                    }
                }
                if (! res)
                {
                    rep !== null && rep.push(["not array or unexpected array [.'$enum'.'|'.2]", path])
                }
            }
        }
        if (res)
        {
            if (rep !== null) rep.length = 0
        }
        else
        {
            rep !== null && rep.push(["no model matched [.'$enum'.'|']", path])
        }
    }
    return res;
}

// check $schemaArray (.'$schemaArray')
function json_model_4(val, path, rep)
{
    let res;
    // .'$schemaArray'
    res = Array.isArray(val);
    if (res)
    {
        for (let arr_3_idx = 0; arr_3_idx < val.length; arr_3_idx++)
        {
            let arr_3_item = val[arr_3_idx]
            let arr_3_lpath = path ? path.concat([arr_3_idx]) : null;
            // .'$schemaArray'.0
            res = json_model_24(arr_3_item, (path ? arr_3_lpath : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $Schema [.'$schemaArray'.0]", (path ? arr_3_lpath : null)])
                break;
            }
        }
    }
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$schemaArray']", path])
    }
    return res;
}


// check $stringKeywords (.'$stringKeywords')
function json_model_5(val, path, rep)
{
    // .'$stringKeywords'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$stringKeywords']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_0 = path ? path.concat([prop]) : null;
        if (prop == "pattern")
        {
            // handle may pattern property
            // .'$stringKeywords'.pattern
            res = runtime.jm_is_valid_regex(pval, false);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $REGEX [.'$stringKeywords'.pattern]", (path ? lpath_0 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$stringKeywords'.pattern]", (path ? lpath_0 : null)])
                return false;
            }
        }
        else if (prop == "minLength")
        {
            // handle may minLength property
            // .'$stringKeywords'.minLength
            res = ((typeof pval === 'number' || pval instanceof Number) && Number.isInteger(pval)) && pval >= 0;
            if (! res)
            {
                rep !== null && rep.push(["not a 0 strict int [.'$stringKeywords'.minLength]", (path ? lpath_0 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$stringKeywords'.minLength]", (path ? lpath_0 : null)])
                return false;
            }
        }
        else if (prop == "maxLength")
        {
            // handle may maxLength property
            // .'$stringKeywords'.maxLength
            res = ((typeof pval === 'number' || pval instanceof Number) && Number.isInteger(pval)) && pval >= 0;
            if (! res)
            {
                rep !== null && rep.push(["not a 0 strict int [.'$stringKeywords'.maxLength]", (path ? lpath_0 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$stringKeywords'.maxLength]", (path ? lpath_0 : null)])
                return false;
            }
        }
        else if (prop == "format")
        {
            // handle may format property
            // .'$stringKeywords'.format
            res = ((pval === null || (typeof pval === 'number' || pval instanceof Number) || (typeof pval === 'boolean' || pval instanceof Boolean) || (typeof pval === 'string' || pval instanceof String))) && _jm_cst_0.has(pval);
            if (! res)
            {
                rep !== null && rep.push(["value not in enum [.'$stringKeywords'.format.'|']", (path ? lpath_0 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$stringKeywords'.format]", (path ? lpath_0 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$stringKeywords']", (path ? lpath_0 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_6_map_additionalItems (.'$arrayKeywords'.additionalItems)
function _jm_f_0(val, path, rep)
{
    let res;
    // .'$arrayKeywords'.additionalItems
    res = json_model_24(val, path, rep);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $Schema [.'$arrayKeywords'.additionalItems]", path])
    }
    return res;
}

// check json_model_6_map_items (.'$arrayKeywords'.items)
function _jm_f_1(val, path, rep)
{
    let res;
    // .'$arrayKeywords'.items
    // .'$arrayKeywords'.items.'|'.0
    res = json_model_24(val, path, rep);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $Schema [.'$arrayKeywords'.items.'|'.0]", path])
    }
    if (! res)
    {
        // .'$arrayKeywords'.items.'|'.1
        res = json_model_4(val, path, rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $schemaArray [.'$arrayKeywords'.items.'|'.1]", path])
        }
    }
    if (res)
    {
        if (rep !== null) rep.length = 0
    }
    else
    {
        rep !== null && rep.push(["no model matched [.'$arrayKeywords'.items.'|']", path])
    }
    return res;
}

// check json_model_6_map_maxItems (.'$arrayKeywords'.maxItems)
function _jm_f_2(val, path, rep)
{
    let res;
    // .'$arrayKeywords'.maxItems
    res = ((typeof val === 'number' || val instanceof Number) && Number.isInteger(val)) && val >= 0;
    if (! res)
    {
        rep !== null && rep.push(["not a 0 strict int [.'$arrayKeywords'.maxItems]", path])
    }
    return res;
}

// check json_model_6_map_minItems (.'$arrayKeywords'.minItems)
function _jm_f_3(val, path, rep)
{
    let res;
    // .'$arrayKeywords'.minItems
    res = ((typeof val === 'number' || val instanceof Number) && Number.isInteger(val)) && val >= 0;
    if (! res)
    {
        rep !== null && rep.push(["not a 0 strict int [.'$arrayKeywords'.minItems]", path])
    }
    return res;
}

// check json_model_6_map_prefixItems (.'$arrayKeywords'.prefixItems)
function _jm_f_4(val, path, rep)
{
    let res;
    // .'$arrayKeywords'.prefixItems
    res = json_model_4(val, path, rep);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $schemaArray [.'$arrayKeywords'.prefixItems]", path])
    }
    return res;
}

// check json_model_6_map_uniqueItems (.'$arrayKeywords'.uniqueItems)
function _jm_f_5(val, path, rep)
{
    let res;
    // .'$arrayKeywords'.uniqueItems
    res = (typeof val === 'boolean' || val instanceof Boolean);
    if (! res)
    {
        rep !== null && rep.push(["not a bool [.'$arrayKeywords'.uniqueItems]", path])
    }
    return res;
}


// check $arrayKeywords (.'$arrayKeywords')
function json_model_6(val, path, rep)
{
    // not included: contains…
    // .'$arrayKeywords'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$arrayKeywords']", path])
        return false;
    }
    let pfun;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_1 = path ? path.concat([prop]) : null;
        if ((pfun = json_model_6_map.get(prop)))
        {
            // handle 6 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_1 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$arrayKeywords']", (path ? lpath_1 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$arrayKeywords']", (path ? lpath_1 : null)])
            return false;
        }
    }
    return true;
}

// object .'$objectKeywords'.properties
function _jm_obj_0(val, path, rep)
{
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$objectKeywords'.properties]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_3 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$objectKeywords'.properties.''
        res = json_model_24(pval, (path ? lpath_3 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$objectKeywords'.properties.'']", (path ? lpath_3 : null)])
            return false;
        }
    }
    return true;
}

// object .'$objectKeywords'.patternProperties
function _jm_obj_1(val, path, rep)
{
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$objectKeywords'.patternProperties]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_4 = path ? path.concat([prop]) : null;
        if (runtime.jm_is_valid_regex(prop, false))
        {
            // handle 1 key props
            // .'$objectKeywords'.patternProperties.'$REGEX'
            res = json_model_24(pval, (path ? lpath_4 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $Schema [.'$objectKeywords'.patternProperties.'$REGEX']", (path ? lpath_4 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$objectKeywords'.patternProperties]", (path ? lpath_4 : null)])
            return false;
        }
    }
    return true;
}

// check $objectKeywords (.'$objectKeywords')
function json_model_7(val, path, rep)
{
    // not included: dependentRequired, min/maxProps…
    // .'$objectKeywords'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$objectKeywords']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_2 = path ? path.concat([prop]) : null;
        if (prop == "properties")
        {
            // handle may properties property
            // .'$objectKeywords'.properties
            res = _jm_obj_0(pval, (path ? lpath_2 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected element [.'$objectKeywords'.properties]", (path ? lpath_2 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$objectKeywords'.properties]", (path ? lpath_2 : null)])
                return false;
            }
        }
        else if (prop == "required")
        {
            // handle may required property
            // .'$objectKeywords'.required
            res = Array.isArray(pval);
            if (res)
            {
                for (let arr_4_idx = 0; arr_4_idx < pval.length; arr_4_idx++)
                {
                    let arr_4_item = pval[arr_4_idx]
                    let arr_4_lpath = (path ? lpath_2 : null) ? (path ? lpath_2 : null).concat([arr_4_idx]) : null;
                    // .'$objectKeywords'.required.0
                    res = (typeof arr_4_item === 'string' || arr_4_item instanceof String);
                    if (! res)
                    {
                        rep !== null && rep.push(["unexpected string [.'$objectKeywords'.required.0]", ((path ? lpath_2 : null) ? arr_4_lpath : null)])
                        break;
                    }
                }
            }
            if (! res)
            {
                rep !== null && rep.push(["not array or unexpected array [.'$objectKeywords'.required]", (path ? lpath_2 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$objectKeywords'.required]", (path ? lpath_2 : null)])
                return false;
            }
        }
        else if (prop == "additionalProperties")
        {
            // handle may additionalProperties property
            // .'$objectKeywords'.additionalProperties
            res = json_model_24(pval, (path ? lpath_2 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $Schema [.'$objectKeywords'.additionalProperties]", (path ? lpath_2 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$objectKeywords'.additionalProperties]", (path ? lpath_2 : null)])
                return false;
            }
        }
        else if (prop == "patternProperties")
        {
            // handle may patternProperties property
            // .'$objectKeywords'.patternProperties
            res = _jm_obj_1(pval, (path ? lpath_2 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected element [.'$objectKeywords'.patternProperties]", (path ? lpath_2 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$objectKeywords'.patternProperties]", (path ? lpath_2 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$objectKeywords']", (path ? lpath_2 : null)])
            return false;
        }
    }
    return true;
}

// check $numberKeywords (.'$numberKeywords')
function json_model_8(val, path, rep)
{
    // not included: exclusive*, multipleOf…
    // .'$numberKeywords'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$numberKeywords']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_5 = path ? path.concat([prop]) : null;
        if (prop == "minimum")
        {
            // handle may minimum property
            // .'$numberKeywords'.minimum
            res = (typeof pval === 'number' || pval instanceof Number);
            if (! res)
            {
                rep !== null && rep.push(["not a -1.0 strict float [.'$numberKeywords'.minimum]", (path ? lpath_5 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$numberKeywords'.minimum]", (path ? lpath_5 : null)])
                return false;
            }
        }
        else if (prop == "maximum")
        {
            // handle may maximum property
            // .'$numberKeywords'.maximum
            res = (typeof pval === 'number' || pval instanceof Number);
            if (! res)
            {
                rep !== null && rep.push(["not a -1.0 strict float [.'$numberKeywords'.maximum]", (path ? lpath_5 : null)])
                rep !== null && rep.push(["invalid optional prop value [.'$numberKeywords'.maximum]", (path ? lpath_5 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$numberKeywords']", (path ? lpath_5 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_9_map_$comment (.'$metas'.'$comment')
function _jm_f_6(val, path, rep)
{
    let res;
    // .'$metas'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$metas'.'$comment']", path])
    }
    return res;
}

// check json_model_9_map_$defs (.'$metas'.'$defs')
function _jm_f_7(val, path, rep)
{
    // .'$metas'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$metas'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_7 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$metas'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_7 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$metas'.'$defs'.'']", (path ? lpath_7 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_9_map_$id (.'$metas'.'$id')
function _jm_f_8(val, path, rep)
{
    let res;
    // .'$metas'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$metas'.'$id']", path])
    }
    return res;
}

// check json_model_9_map_$schema (.'$metas'.'$schema')
function _jm_f_9(val, path, rep)
{
    let res;
    // .'$metas'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$metas'.'$schema']", path])
    }
    return res;
}

// check json_model_9_map_default (.'$metas'.default)
function _jm_f_10(val, path, rep)
{
    let res;
    // .'$metas'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$metas'.default]", path])
    }
    return res;
}

// check json_model_9_map_definitions (.'$metas'.definitions)
function _jm_f_11(val, path, rep)
{
    // .'$metas'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$metas'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_8 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$metas'.definitions.''
        res = json_model_24(pval, (path ? lpath_8 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$metas'.definitions.'']", (path ? lpath_8 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_9_map_description (.'$metas'.description)
function _jm_f_12(val, path, rep)
{
    let res;
    // .'$metas'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$metas'.description]", path])
    }
    return res;
}

// check json_model_9_map_examples (.'$metas'.examples)
function _jm_f_13(val, path, rep)
{
    let res;
    // .'$metas'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$metas'.examples]", path])
    }
    return res;
}

// check json_model_9_map_id (.'$metas'.id)
function _jm_f_14(val, path, rep)
{
    let res;
    // .'$metas'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$metas'.id]", path])
    }
    return res;
}

// check json_model_9_map_title (.'$metas'.title)
function _jm_f_15(val, path, rep)
{
    let res;
    // .'$metas'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$metas'.title]", path])
    }
    return res;
}


const _jm_re_0 = (s) => _jm_re_0_re.exec(s) !== null

// check $metas (.'$metas')
function json_model_9(val, path, rep)
{
    // there is a trick with $schema… in RootSchema below
    // .'$metas'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$metas']", path])
        return false;
    }
    let res;
    let pfun;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_6 = path ? path.concat([prop]) : null;
        if ((pfun = json_model_9_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_6 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$metas']", (path ? lpath_6 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$metas'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$metas'.'/^x-.*/']", (path ? lpath_6 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$metas']", (path ? lpath_6 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_10_map_$comment (.'$String'.'$comment')
function _jm_f_16(val, path, rep)
{
    let res;
    // .'$String'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$String'.'$comment']", path])
    }
    return res;
}

// check json_model_10_map_$defs (.'$String'.'$defs')
function _jm_f_17(val, path, rep)
{
    // .'$String'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$String'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_10 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$String'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_10 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$String'.'$defs'.'']", (path ? lpath_10 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_10_map_$id (.'$String'.'$id')
function _jm_f_18(val, path, rep)
{
    let res;
    // .'$String'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$String'.'$id']", path])
    }
    return res;
}

// check json_model_10_map_$schema (.'$String'.'$schema')
function _jm_f_19(val, path, rep)
{
    let res;
    // .'$String'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$String'.'$schema']", path])
    }
    return res;
}

// check json_model_10_map_default (.'$String'.default)
function _jm_f_20(val, path, rep)
{
    let res;
    // .'$String'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$String'.default]", path])
    }
    return res;
}

// check json_model_10_map_definitions (.'$String'.definitions)
function _jm_f_21(val, path, rep)
{
    // .'$String'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$String'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_11 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$String'.definitions.''
        res = json_model_24(pval, (path ? lpath_11 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$String'.definitions.'']", (path ? lpath_11 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_10_map_description (.'$String'.description)
function _jm_f_22(val, path, rep)
{
    let res;
    // .'$String'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$String'.description]", path])
    }
    return res;
}

// check json_model_10_map_examples (.'$String'.examples)
function _jm_f_23(val, path, rep)
{
    let res;
    // .'$String'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$String'.examples]", path])
    }
    return res;
}


// check json_model_10_map_format (.'$String'.format)
function _jm_f_24(val, path, rep)
{
    let res;
    // .'$String'.format
    res = ((val === null || (typeof val === 'number' || val instanceof Number) || (typeof val === 'boolean' || val instanceof Boolean) || (typeof val === 'string' || val instanceof String))) && _jm_cst_1.has(val);
    if (! res)
    {
        rep !== null && rep.push(["value not in enum [.'$String'.format.'|']", path])
    }
    return res;
}

// check json_model_10_map_id (.'$String'.id)
function _jm_f_25(val, path, rep)
{
    let res;
    // .'$String'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$String'.id]", path])
    }
    return res;
}

// check json_model_10_map_maxLength (.'$String'.maxLength)
function _jm_f_26(val, path, rep)
{
    let res;
    // .'$String'.maxLength
    res = ((typeof val === 'number' || val instanceof Number) && Number.isInteger(val)) && val >= 0;
    if (! res)
    {
        rep !== null && rep.push(["not a 0 strict int [.'$String'.maxLength]", path])
    }
    return res;
}

// check json_model_10_map_minLength (.'$String'.minLength)
function _jm_f_27(val, path, rep)
{
    let res;
    // .'$String'.minLength
    res = ((typeof val === 'number' || val instanceof Number) && Number.isInteger(val)) && val >= 0;
    if (! res)
    {
        rep !== null && rep.push(["not a 0 strict int [.'$String'.minLength]", path])
    }
    return res;
}

// check json_model_10_map_pattern (.'$String'.pattern)
function _jm_f_28(val, path, rep)
{
    let res;
    // .'$String'.pattern
    res = runtime.jm_is_valid_regex(val, false);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $REGEX [.'$String'.pattern]", path])
    }
    return res;
}

// check json_model_10_map_title (.'$String'.title)
function _jm_f_29(val, path, rep)
{
    let res;
    // .'$String'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$String'.title]", path])
    }
    return res;
}


// check $String (.'$String')
function json_model_10(val, path, rep)
{
    // .'$String'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$String']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_9 = path ? path.concat([prop]) : null;
        if (prop == "type")
        {
            // handle must type property
            must_count += 1;
            // .'$String'.type
            res = ((typeof pval === 'string' || pval instanceof String)) && pval == "string";
            if (! res)
            {
                rep !== null && rep.push(["unexpected string [.'$String'.type]", (path ? lpath_9 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$String'.type]", (path ? lpath_9 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_10_map.get(prop)))
        {
            // handle 14 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_9 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$String']", (path ? lpath_9 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$String'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$String'.'/^x-.*/']", (path ? lpath_9 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$String']", (path ? lpath_9 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("type"))
            {
                rep !== null && rep.push(["missing mandatory prop <type> [.'$String']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_11_map_$comment (.'$Array'.'$comment')
function _jm_f_30(val, path, rep)
{
    let res;
    // .'$Array'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Array'.'$comment']", path])
    }
    return res;
}

// check json_model_11_map_$defs (.'$Array'.'$defs')
function _jm_f_31(val, path, rep)
{
    // .'$Array'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Array'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_13 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Array'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_13 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Array'.'$defs'.'']", (path ? lpath_13 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_11_map_$id (.'$Array'.'$id')
function _jm_f_32(val, path, rep)
{
    let res;
    // .'$Array'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Array'.'$id']", path])
    }
    return res;
}

// check json_model_11_map_$schema (.'$Array'.'$schema')
function _jm_f_33(val, path, rep)
{
    let res;
    // .'$Array'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Array'.'$schema']", path])
    }
    return res;
}

// check json_model_11_map_additionalItems (.'$Array'.additionalItems)
function _jm_f_34(val, path, rep)
{
    let res;
    // .'$Array'.additionalItems
    res = json_model_24(val, path, rep);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $Schema [.'$Array'.additionalItems]", path])
    }
    return res;
}

// check json_model_11_map_default (.'$Array'.default)
function _jm_f_35(val, path, rep)
{
    let res;
    // .'$Array'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Array'.default]", path])
    }
    return res;
}

// check json_model_11_map_definitions (.'$Array'.definitions)
function _jm_f_36(val, path, rep)
{
    // .'$Array'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Array'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_14 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Array'.definitions.''
        res = json_model_24(pval, (path ? lpath_14 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Array'.definitions.'']", (path ? lpath_14 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_11_map_description (.'$Array'.description)
function _jm_f_37(val, path, rep)
{
    let res;
    // .'$Array'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Array'.description]", path])
    }
    return res;
}

// check json_model_11_map_examples (.'$Array'.examples)
function _jm_f_38(val, path, rep)
{
    let res;
    // .'$Array'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Array'.examples]", path])
    }
    return res;
}

// check json_model_11_map_id (.'$Array'.id)
function _jm_f_39(val, path, rep)
{
    let res;
    // .'$Array'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Array'.id]", path])
    }
    return res;
}

// check json_model_11_map_items (.'$Array'.items)
function _jm_f_40(val, path, rep)
{
    let res;
    // .'$Array'.items
    // .'$Array'.items.'|'.0
    res = json_model_24(val, path, rep);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $Schema [.'$Array'.items.'|'.0]", path])
    }
    if (! res)
    {
        // .'$Array'.items.'|'.1
        res = json_model_4(val, path, rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $schemaArray [.'$Array'.items.'|'.1]", path])
        }
    }
    if (res)
    {
        if (rep !== null) rep.length = 0
    }
    else
    {
        rep !== null && rep.push(["no model matched [.'$Array'.items.'|']", path])
    }
    return res;
}

// check json_model_11_map_maxItems (.'$Array'.maxItems)
function _jm_f_41(val, path, rep)
{
    let res;
    // .'$Array'.maxItems
    res = ((typeof val === 'number' || val instanceof Number) && Number.isInteger(val)) && val >= 0;
    if (! res)
    {
        rep !== null && rep.push(["not a 0 strict int [.'$Array'.maxItems]", path])
    }
    return res;
}

// check json_model_11_map_minItems (.'$Array'.minItems)
function _jm_f_42(val, path, rep)
{
    let res;
    // .'$Array'.minItems
    res = ((typeof val === 'number' || val instanceof Number) && Number.isInteger(val)) && val >= 0;
    if (! res)
    {
        rep !== null && rep.push(["not a 0 strict int [.'$Array'.minItems]", path])
    }
    return res;
}

// check json_model_11_map_prefixItems (.'$Array'.prefixItems)
function _jm_f_43(val, path, rep)
{
    let res;
    // .'$Array'.prefixItems
    res = json_model_4(val, path, rep);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $schemaArray [.'$Array'.prefixItems]", path])
    }
    return res;
}

// check json_model_11_map_title (.'$Array'.title)
function _jm_f_44(val, path, rep)
{
    let res;
    // .'$Array'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Array'.title]", path])
    }
    return res;
}

// check json_model_11_map_uniqueItems (.'$Array'.uniqueItems)
function _jm_f_45(val, path, rep)
{
    let res;
    // .'$Array'.uniqueItems
    res = (typeof val === 'boolean' || val instanceof Boolean);
    if (! res)
    {
        rep !== null && rep.push(["not a bool [.'$Array'.uniqueItems]", path])
    }
    return res;
}


// check $Array (.'$Array')
function json_model_11(val, path, rep)
{
    // .'$Array'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Array']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_12 = path ? path.concat([prop]) : null;
        if (prop == "type")
        {
            // handle must type property
            must_count += 1;
            // .'$Array'.type
            res = ((typeof pval === 'string' || pval instanceof String)) && pval == "array";
            if (! res)
            {
                rep !== null && rep.push(["unexpected array [.'$Array'.type]", (path ? lpath_12 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Array'.type]", (path ? lpath_12 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_11_map.get(prop)))
        {
            // handle 16 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_12 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Array']", (path ? lpath_12 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Array'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Array'.'/^x-.*/']", (path ? lpath_12 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Array']", (path ? lpath_12 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("type"))
            {
                rep !== null && rep.push(["missing mandatory prop <type> [.'$Array']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_12_map_$comment (.'$Object'.'$comment')
function _jm_f_46(val, path, rep)
{
    let res;
    // .'$Object'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Object'.'$comment']", path])
    }
    return res;
}

// check json_model_12_map_$defs (.'$Object'.'$defs')
function _jm_f_47(val, path, rep)
{
    // .'$Object'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Object'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_16 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Object'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_16 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Object'.'$defs'.'']", (path ? lpath_16 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_12_map_$id (.'$Object'.'$id')
function _jm_f_48(val, path, rep)
{
    let res;
    // .'$Object'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Object'.'$id']", path])
    }
    return res;
}

// check json_model_12_map_$schema (.'$Object'.'$schema')
function _jm_f_49(val, path, rep)
{
    let res;
    // .'$Object'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Object'.'$schema']", path])
    }
    return res;
}

// check json_model_12_map_additionalProperties (.'$Object'.additionalProperties)
function _jm_f_50(val, path, rep)
{
    let res;
    // .'$Object'.additionalProperties
    res = json_model_24(val, path, rep);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $Schema [.'$Object'.additionalProperties]", path])
    }
    return res;
}

// check json_model_12_map_default (.'$Object'.default)
function _jm_f_51(val, path, rep)
{
    let res;
    // .'$Object'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Object'.default]", path])
    }
    return res;
}

// check json_model_12_map_definitions (.'$Object'.definitions)
function _jm_f_52(val, path, rep)
{
    // .'$Object'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Object'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_17 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Object'.definitions.''
        res = json_model_24(pval, (path ? lpath_17 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Object'.definitions.'']", (path ? lpath_17 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_12_map_description (.'$Object'.description)
function _jm_f_53(val, path, rep)
{
    let res;
    // .'$Object'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Object'.description]", path])
    }
    return res;
}

// check json_model_12_map_examples (.'$Object'.examples)
function _jm_f_54(val, path, rep)
{
    let res;
    // .'$Object'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Object'.examples]", path])
    }
    return res;
}

// check json_model_12_map_id (.'$Object'.id)
function _jm_f_55(val, path, rep)
{
    let res;
    // .'$Object'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Object'.id]", path])
    }
    return res;
}

// check json_model_12_map_patternProperties (.'$Object'.patternProperties)
function _jm_f_56(val, path, rep)
{
    // .'$Object'.patternProperties
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Object'.patternProperties]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_18 = path ? path.concat([prop]) : null;
        if (runtime.jm_is_valid_regex(prop, false))
        {
            // handle 1 key props
            // .'$Object'.patternProperties.'$REGEX'
            res = json_model_24(pval, (path ? lpath_18 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $Schema [.'$Object'.patternProperties.'$REGEX']", (path ? lpath_18 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Object'.patternProperties]", (path ? lpath_18 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_12_map_properties (.'$Object'.properties)
function _jm_f_57(val, path, rep)
{
    // .'$Object'.properties
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Object'.properties]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_19 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Object'.properties.''
        res = json_model_24(pval, (path ? lpath_19 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Object'.properties.'']", (path ? lpath_19 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_12_map_required (.'$Object'.required)
function _jm_f_58(val, path, rep)
{
    let res;
    // .'$Object'.required
    res = Array.isArray(val);
    if (res)
    {
        for (let arr_5_idx = 0; arr_5_idx < val.length; arr_5_idx++)
        {
            let arr_5_item = val[arr_5_idx]
            let arr_5_lpath = path ? path.concat([arr_5_idx]) : null;
            // .'$Object'.required.0
            res = (typeof arr_5_item === 'string' || arr_5_item instanceof String);
            if (! res)
            {
                rep !== null && rep.push(["unexpected string [.'$Object'.required.0]", (path ? arr_5_lpath : null)])
                break;
            }
        }
    }
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Object'.required]", path])
    }
    return res;
}

// check json_model_12_map_title (.'$Object'.title)
function _jm_f_59(val, path, rep)
{
    let res;
    // .'$Object'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Object'.title]", path])
    }
    return res;
}


// check $Object (.'$Object')
function json_model_12(val, path, rep)
{
    // .'$Object'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Object']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_15 = path ? path.concat([prop]) : null;
        if (prop == "type")
        {
            // handle must type property
            must_count += 1;
            // .'$Object'.type
            res = ((typeof pval === 'string' || pval instanceof String)) && pval == "object";
            if (! res)
            {
                rep !== null && rep.push(["unexpected object [.'$Object'.type]", (path ? lpath_15 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Object'.type]", (path ? lpath_15 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_12_map.get(prop)))
        {
            // handle 14 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_15 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Object']", (path ? lpath_15 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Object'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Object'.'/^x-.*/']", (path ? lpath_15 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Object']", (path ? lpath_15 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("type"))
            {
                rep !== null && rep.push(["missing mandatory prop <type> [.'$Object']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_13_map_$comment (.'$Integer'.'$comment')
function _jm_f_60(val, path, rep)
{
    let res;
    // .'$Integer'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Integer'.'$comment']", path])
    }
    return res;
}

// check json_model_13_map_$defs (.'$Integer'.'$defs')
function _jm_f_61(val, path, rep)
{
    // .'$Integer'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Integer'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_21 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Integer'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_21 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Integer'.'$defs'.'']", (path ? lpath_21 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_13_map_$id (.'$Integer'.'$id')
function _jm_f_62(val, path, rep)
{
    let res;
    // .'$Integer'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Integer'.'$id']", path])
    }
    return res;
}

// check json_model_13_map_$schema (.'$Integer'.'$schema')
function _jm_f_63(val, path, rep)
{
    let res;
    // .'$Integer'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Integer'.'$schema']", path])
    }
    return res;
}

// check json_model_13_map_default (.'$Integer'.default)
function _jm_f_64(val, path, rep)
{
    let res;
    // .'$Integer'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Integer'.default]", path])
    }
    return res;
}

// check json_model_13_map_definitions (.'$Integer'.definitions)
function _jm_f_65(val, path, rep)
{
    // .'$Integer'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Integer'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_22 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Integer'.definitions.''
        res = json_model_24(pval, (path ? lpath_22 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Integer'.definitions.'']", (path ? lpath_22 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_13_map_description (.'$Integer'.description)
function _jm_f_66(val, path, rep)
{
    let res;
    // .'$Integer'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Integer'.description]", path])
    }
    return res;
}

// check json_model_13_map_examples (.'$Integer'.examples)
function _jm_f_67(val, path, rep)
{
    let res;
    // .'$Integer'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Integer'.examples]", path])
    }
    return res;
}

// check json_model_13_map_id (.'$Integer'.id)
function _jm_f_68(val, path, rep)
{
    let res;
    // .'$Integer'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Integer'.id]", path])
    }
    return res;
}

// check json_model_13_map_maximum (.'$Integer'.maximum)
function _jm_f_69(val, path, rep)
{
    let res;
    // .'$Integer'.maximum
    res = (typeof val === 'number' || val instanceof Number);
    if (! res)
    {
        rep !== null && rep.push(["not a -1.0 strict float [.'$Integer'.maximum]", path])
    }
    return res;
}

// check json_model_13_map_minimum (.'$Integer'.minimum)
function _jm_f_70(val, path, rep)
{
    let res;
    // .'$Integer'.minimum
    res = (typeof val === 'number' || val instanceof Number);
    if (! res)
    {
        rep !== null && rep.push(["not a -1.0 strict float [.'$Integer'.minimum]", path])
    }
    return res;
}

// check json_model_13_map_title (.'$Integer'.title)
function _jm_f_71(val, path, rep)
{
    let res;
    // .'$Integer'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Integer'.title]", path])
    }
    return res;
}


// check $Integer (.'$Integer')
function json_model_13(val, path, rep)
{
    // .'$Integer'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Integer']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_20 = path ? path.concat([prop]) : null;
        if (prop == "type")
        {
            // handle must type property
            must_count += 1;
            // .'$Integer'.type
            res = ((typeof pval === 'string' || pval instanceof String)) && pval == "integer";
            if (! res)
            {
                rep !== null && rep.push(["unexpected integer [.'$Integer'.type]", (path ? lpath_20 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Integer'.type]", (path ? lpath_20 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_13_map.get(prop)))
        {
            // handle 12 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_20 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Integer']", (path ? lpath_20 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Integer'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Integer'.'/^x-.*/']", (path ? lpath_20 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Integer']", (path ? lpath_20 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("type"))
            {
                rep !== null && rep.push(["missing mandatory prop <type> [.'$Integer']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_14_map_$comment (.'$Number'.'$comment')
function _jm_f_72(val, path, rep)
{
    let res;
    // .'$Number'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Number'.'$comment']", path])
    }
    return res;
}

// check json_model_14_map_$defs (.'$Number'.'$defs')
function _jm_f_73(val, path, rep)
{
    // .'$Number'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Number'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_24 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Number'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_24 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Number'.'$defs'.'']", (path ? lpath_24 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_14_map_$id (.'$Number'.'$id')
function _jm_f_74(val, path, rep)
{
    let res;
    // .'$Number'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Number'.'$id']", path])
    }
    return res;
}

// check json_model_14_map_$schema (.'$Number'.'$schema')
function _jm_f_75(val, path, rep)
{
    let res;
    // .'$Number'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Number'.'$schema']", path])
    }
    return res;
}

// check json_model_14_map_default (.'$Number'.default)
function _jm_f_76(val, path, rep)
{
    let res;
    // .'$Number'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Number'.default]", path])
    }
    return res;
}

// check json_model_14_map_definitions (.'$Number'.definitions)
function _jm_f_77(val, path, rep)
{
    // .'$Number'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Number'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_25 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Number'.definitions.''
        res = json_model_24(pval, (path ? lpath_25 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Number'.definitions.'']", (path ? lpath_25 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_14_map_description (.'$Number'.description)
function _jm_f_78(val, path, rep)
{
    let res;
    // .'$Number'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Number'.description]", path])
    }
    return res;
}

// check json_model_14_map_examples (.'$Number'.examples)
function _jm_f_79(val, path, rep)
{
    let res;
    // .'$Number'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Number'.examples]", path])
    }
    return res;
}

// check json_model_14_map_id (.'$Number'.id)
function _jm_f_80(val, path, rep)
{
    let res;
    // .'$Number'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Number'.id]", path])
    }
    return res;
}

// check json_model_14_map_maximum (.'$Number'.maximum)
function _jm_f_81(val, path, rep)
{
    let res;
    // .'$Number'.maximum
    res = (typeof val === 'number' || val instanceof Number);
    if (! res)
    {
        rep !== null && rep.push(["not a -1.0 strict float [.'$Number'.maximum]", path])
    }
    return res;
}

// check json_model_14_map_minimum (.'$Number'.minimum)
function _jm_f_82(val, path, rep)
{
    let res;
    // .'$Number'.minimum
    res = (typeof val === 'number' || val instanceof Number);
    if (! res)
    {
        rep !== null && rep.push(["not a -1.0 strict float [.'$Number'.minimum]", path])
    }
    return res;
}

// check json_model_14_map_title (.'$Number'.title)
function _jm_f_83(val, path, rep)
{
    let res;
    // .'$Number'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Number'.title]", path])
    }
    return res;
}


// check $Number (.'$Number')
function json_model_14(val, path, rep)
{
    // .'$Number'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Number']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_23 = path ? path.concat([prop]) : null;
        if (prop == "type")
        {
            // handle must type property
            must_count += 1;
            // .'$Number'.type
            res = ((typeof pval === 'string' || pval instanceof String)) && pval == "number";
            if (! res)
            {
                rep !== null && rep.push(["unexpected number [.'$Number'.type]", (path ? lpath_23 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Number'.type]", (path ? lpath_23 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_14_map.get(prop)))
        {
            // handle 12 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_23 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Number']", (path ? lpath_23 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Number'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Number'.'/^x-.*/']", (path ? lpath_23 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Number']", (path ? lpath_23 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("type"))
            {
                rep !== null && rep.push(["missing mandatory prop <type> [.'$Number']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_15_map_$comment (.'$Bool'.'$comment')
function _jm_f_84(val, path, rep)
{
    let res;
    // .'$Bool'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Bool'.'$comment']", path])
    }
    return res;
}

// check json_model_15_map_$defs (.'$Bool'.'$defs')
function _jm_f_85(val, path, rep)
{
    // .'$Bool'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Bool'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_27 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Bool'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_27 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Bool'.'$defs'.'']", (path ? lpath_27 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_15_map_$id (.'$Bool'.'$id')
function _jm_f_86(val, path, rep)
{
    let res;
    // .'$Bool'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Bool'.'$id']", path])
    }
    return res;
}

// check json_model_15_map_$schema (.'$Bool'.'$schema')
function _jm_f_87(val, path, rep)
{
    let res;
    // .'$Bool'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Bool'.'$schema']", path])
    }
    return res;
}

// check json_model_15_map_default (.'$Bool'.default)
function _jm_f_88(val, path, rep)
{
    let res;
    // .'$Bool'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Bool'.default]", path])
    }
    return res;
}

// check json_model_15_map_definitions (.'$Bool'.definitions)
function _jm_f_89(val, path, rep)
{
    // .'$Bool'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Bool'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_28 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Bool'.definitions.''
        res = json_model_24(pval, (path ? lpath_28 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Bool'.definitions.'']", (path ? lpath_28 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_15_map_description (.'$Bool'.description)
function _jm_f_90(val, path, rep)
{
    let res;
    // .'$Bool'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Bool'.description]", path])
    }
    return res;
}

// check json_model_15_map_examples (.'$Bool'.examples)
function _jm_f_91(val, path, rep)
{
    let res;
    // .'$Bool'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Bool'.examples]", path])
    }
    return res;
}

// check json_model_15_map_id (.'$Bool'.id)
function _jm_f_92(val, path, rep)
{
    let res;
    // .'$Bool'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Bool'.id]", path])
    }
    return res;
}

// check json_model_15_map_title (.'$Bool'.title)
function _jm_f_93(val, path, rep)
{
    let res;
    // .'$Bool'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Bool'.title]", path])
    }
    return res;
}


// check $Bool (.'$Bool')
function json_model_15(val, path, rep)
{
    // .'$Bool'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Bool']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_26 = path ? path.concat([prop]) : null;
        if (prop == "type")
        {
            // handle must type property
            must_count += 1;
            // .'$Bool'.type
            res = ((typeof pval === 'string' || pval instanceof String)) && pval == "boolean";
            if (! res)
            {
                rep !== null && rep.push(["unexpected boolean [.'$Bool'.type]", (path ? lpath_26 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Bool'.type]", (path ? lpath_26 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_15_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_26 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Bool']", (path ? lpath_26 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Bool'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Bool'.'/^x-.*/']", (path ? lpath_26 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Bool']", (path ? lpath_26 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("type"))
            {
                rep !== null && rep.push(["missing mandatory prop <type> [.'$Bool']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_16_map_$comment (.'$Null'.'$comment')
function _jm_f_94(val, path, rep)
{
    let res;
    // .'$Null'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Null'.'$comment']", path])
    }
    return res;
}

// check json_model_16_map_$defs (.'$Null'.'$defs')
function _jm_f_95(val, path, rep)
{
    // .'$Null'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Null'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_30 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Null'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_30 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Null'.'$defs'.'']", (path ? lpath_30 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_16_map_$id (.'$Null'.'$id')
function _jm_f_96(val, path, rep)
{
    let res;
    // .'$Null'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Null'.'$id']", path])
    }
    return res;
}

// check json_model_16_map_$schema (.'$Null'.'$schema')
function _jm_f_97(val, path, rep)
{
    let res;
    // .'$Null'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Null'.'$schema']", path])
    }
    return res;
}

// check json_model_16_map_default (.'$Null'.default)
function _jm_f_98(val, path, rep)
{
    let res;
    // .'$Null'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Null'.default]", path])
    }
    return res;
}

// check json_model_16_map_definitions (.'$Null'.definitions)
function _jm_f_99(val, path, rep)
{
    // .'$Null'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Null'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_31 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Null'.definitions.''
        res = json_model_24(pval, (path ? lpath_31 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Null'.definitions.'']", (path ? lpath_31 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_16_map_description (.'$Null'.description)
function _jm_f_100(val, path, rep)
{
    let res;
    // .'$Null'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Null'.description]", path])
    }
    return res;
}

// check json_model_16_map_examples (.'$Null'.examples)
function _jm_f_101(val, path, rep)
{
    let res;
    // .'$Null'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Null'.examples]", path])
    }
    return res;
}

// check json_model_16_map_id (.'$Null'.id)
function _jm_f_102(val, path, rep)
{
    let res;
    // .'$Null'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Null'.id]", path])
    }
    return res;
}

// check json_model_16_map_title (.'$Null'.title)
function _jm_f_103(val, path, rep)
{
    let res;
    // .'$Null'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Null'.title]", path])
    }
    return res;
}


// check $Null (.'$Null')
function json_model_16(val, path, rep)
{
    // .'$Null'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Null']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_29 = path ? path.concat([prop]) : null;
        if (prop == "type")
        {
            // handle must type property
            must_count += 1;
            // .'$Null'.type
            res = ((typeof pval === 'string' || pval instanceof String)) && pval == "null";
            if (! res)
            {
                rep !== null && rep.push(["unexpected null [.'$Null'.type]", (path ? lpath_29 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Null'.type]", (path ? lpath_29 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_16_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_29 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Null']", (path ? lpath_29 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Null'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Null'.'/^x-.*/']", (path ? lpath_29 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Null']", (path ? lpath_29 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("type"))
            {
                rep !== null && rep.push(["missing mandatory prop <type> [.'$Null']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_17_map_$comment (.'$AllOf'.'$comment')
function _jm_f_104(val, path, rep)
{
    let res;
    // .'$AllOf'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AllOf'.'$comment']", path])
    }
    return res;
}

// check json_model_17_map_$defs (.'$AllOf'.'$defs')
function _jm_f_105(val, path, rep)
{
    // .'$AllOf'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$AllOf'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_33 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$AllOf'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_33 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$AllOf'.'$defs'.'']", (path ? lpath_33 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_17_map_$id (.'$AllOf'.'$id')
function _jm_f_106(val, path, rep)
{
    let res;
    // .'$AllOf'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AllOf'.'$id']", path])
    }
    return res;
}

// check json_model_17_map_$schema (.'$AllOf'.'$schema')
function _jm_f_107(val, path, rep)
{
    let res;
    // .'$AllOf'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AllOf'.'$schema']", path])
    }
    return res;
}

// check json_model_17_map_default (.'$AllOf'.default)
function _jm_f_108(val, path, rep)
{
    let res;
    // .'$AllOf'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$AllOf'.default]", path])
    }
    return res;
}

// check json_model_17_map_definitions (.'$AllOf'.definitions)
function _jm_f_109(val, path, rep)
{
    // .'$AllOf'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$AllOf'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_34 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$AllOf'.definitions.''
        res = json_model_24(pval, (path ? lpath_34 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$AllOf'.definitions.'']", (path ? lpath_34 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_17_map_description (.'$AllOf'.description)
function _jm_f_110(val, path, rep)
{
    let res;
    // .'$AllOf'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AllOf'.description]", path])
    }
    return res;
}

// check json_model_17_map_examples (.'$AllOf'.examples)
function _jm_f_111(val, path, rep)
{
    let res;
    // .'$AllOf'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$AllOf'.examples]", path])
    }
    return res;
}

// check json_model_17_map_id (.'$AllOf'.id)
function _jm_f_112(val, path, rep)
{
    let res;
    // .'$AllOf'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AllOf'.id]", path])
    }
    return res;
}

// check json_model_17_map_title (.'$AllOf'.title)
function _jm_f_113(val, path, rep)
{
    let res;
    // .'$AllOf'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AllOf'.title]", path])
    }
    return res;
}


// check $AllOf (.'$AllOf')
function json_model_17(val, path, rep)
{
    // .'$AllOf'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$AllOf']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_32 = path ? path.concat([prop]) : null;
        if (prop == "allOf")
        {
            // handle must allOf property
            must_count += 1;
            // .'$AllOf'.allOf
            res = json_model_4(pval, (path ? lpath_32 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $schemaArray [.'$AllOf'.allOf]", (path ? lpath_32 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$AllOf'.allOf]", (path ? lpath_32 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_17_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_32 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$AllOf']", (path ? lpath_32 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$AllOf'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$AllOf'.'/^x-.*/']", (path ? lpath_32 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$AllOf']", (path ? lpath_32 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("allOf"))
            {
                rep !== null && rep.push(["missing mandatory prop <allOf> [.'$AllOf']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_18_map_$comment (.'$AnyOf'.'$comment')
function _jm_f_114(val, path, rep)
{
    let res;
    // .'$AnyOf'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AnyOf'.'$comment']", path])
    }
    return res;
}

// check json_model_18_map_$defs (.'$AnyOf'.'$defs')
function _jm_f_115(val, path, rep)
{
    // .'$AnyOf'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$AnyOf'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_36 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$AnyOf'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_36 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$AnyOf'.'$defs'.'']", (path ? lpath_36 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_18_map_$id (.'$AnyOf'.'$id')
function _jm_f_116(val, path, rep)
{
    let res;
    // .'$AnyOf'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AnyOf'.'$id']", path])
    }
    return res;
}

// check json_model_18_map_$schema (.'$AnyOf'.'$schema')
function _jm_f_117(val, path, rep)
{
    let res;
    // .'$AnyOf'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AnyOf'.'$schema']", path])
    }
    return res;
}

// check json_model_18_map_default (.'$AnyOf'.default)
function _jm_f_118(val, path, rep)
{
    let res;
    // .'$AnyOf'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$AnyOf'.default]", path])
    }
    return res;
}

// check json_model_18_map_definitions (.'$AnyOf'.definitions)
function _jm_f_119(val, path, rep)
{
    // .'$AnyOf'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$AnyOf'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_37 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$AnyOf'.definitions.''
        res = json_model_24(pval, (path ? lpath_37 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$AnyOf'.definitions.'']", (path ? lpath_37 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_18_map_description (.'$AnyOf'.description)
function _jm_f_120(val, path, rep)
{
    let res;
    // .'$AnyOf'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AnyOf'.description]", path])
    }
    return res;
}

// check json_model_18_map_examples (.'$AnyOf'.examples)
function _jm_f_121(val, path, rep)
{
    let res;
    // .'$AnyOf'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$AnyOf'.examples]", path])
    }
    return res;
}

// check json_model_18_map_id (.'$AnyOf'.id)
function _jm_f_122(val, path, rep)
{
    let res;
    // .'$AnyOf'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AnyOf'.id]", path])
    }
    return res;
}

// check json_model_18_map_title (.'$AnyOf'.title)
function _jm_f_123(val, path, rep)
{
    let res;
    // .'$AnyOf'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$AnyOf'.title]", path])
    }
    return res;
}


// check $AnyOf (.'$AnyOf')
function json_model_18(val, path, rep)
{
    // .'$AnyOf'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$AnyOf']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_35 = path ? path.concat([prop]) : null;
        if (prop == "anyOf")
        {
            // handle must anyOf property
            must_count += 1;
            // .'$AnyOf'.anyOf
            res = json_model_4(pval, (path ? lpath_35 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $schemaArray [.'$AnyOf'.anyOf]", (path ? lpath_35 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$AnyOf'.anyOf]", (path ? lpath_35 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_18_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_35 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$AnyOf']", (path ? lpath_35 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$AnyOf'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$AnyOf'.'/^x-.*/']", (path ? lpath_35 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$AnyOf']", (path ? lpath_35 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("anyOf"))
            {
                rep !== null && rep.push(["missing mandatory prop <anyOf> [.'$AnyOf']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_19_map_$comment (.'$OneOf'.'$comment')
function _jm_f_124(val, path, rep)
{
    let res;
    // .'$OneOf'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$OneOf'.'$comment']", path])
    }
    return res;
}

// check json_model_19_map_$defs (.'$OneOf'.'$defs')
function _jm_f_125(val, path, rep)
{
    // .'$OneOf'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$OneOf'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_39 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$OneOf'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_39 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$OneOf'.'$defs'.'']", (path ? lpath_39 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_19_map_$id (.'$OneOf'.'$id')
function _jm_f_126(val, path, rep)
{
    let res;
    // .'$OneOf'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$OneOf'.'$id']", path])
    }
    return res;
}

// check json_model_19_map_$schema (.'$OneOf'.'$schema')
function _jm_f_127(val, path, rep)
{
    let res;
    // .'$OneOf'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$OneOf'.'$schema']", path])
    }
    return res;
}

// check json_model_19_map_default (.'$OneOf'.default)
function _jm_f_128(val, path, rep)
{
    let res;
    // .'$OneOf'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$OneOf'.default]", path])
    }
    return res;
}

// check json_model_19_map_definitions (.'$OneOf'.definitions)
function _jm_f_129(val, path, rep)
{
    // .'$OneOf'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$OneOf'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_40 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$OneOf'.definitions.''
        res = json_model_24(pval, (path ? lpath_40 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$OneOf'.definitions.'']", (path ? lpath_40 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_19_map_description (.'$OneOf'.description)
function _jm_f_130(val, path, rep)
{
    let res;
    // .'$OneOf'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$OneOf'.description]", path])
    }
    return res;
}

// check json_model_19_map_examples (.'$OneOf'.examples)
function _jm_f_131(val, path, rep)
{
    let res;
    // .'$OneOf'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$OneOf'.examples]", path])
    }
    return res;
}

// check json_model_19_map_id (.'$OneOf'.id)
function _jm_f_132(val, path, rep)
{
    let res;
    // .'$OneOf'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$OneOf'.id]", path])
    }
    return res;
}

// check json_model_19_map_title (.'$OneOf'.title)
function _jm_f_133(val, path, rep)
{
    let res;
    // .'$OneOf'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$OneOf'.title]", path])
    }
    return res;
}


// check $OneOf (.'$OneOf')
function json_model_19(val, path, rep)
{
    // .'$OneOf'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$OneOf']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_38 = path ? path.concat([prop]) : null;
        if (prop == "oneOf")
        {
            // handle must oneOf property
            must_count += 1;
            // .'$OneOf'.oneOf
            res = json_model_4(pval, (path ? lpath_38 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $schemaArray [.'$OneOf'.oneOf]", (path ? lpath_38 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$OneOf'.oneOf]", (path ? lpath_38 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_19_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_38 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$OneOf']", (path ? lpath_38 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$OneOf'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$OneOf'.'/^x-.*/']", (path ? lpath_38 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$OneOf']", (path ? lpath_38 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("oneOf"))
            {
                rep !== null && rep.push(["missing mandatory prop <oneOf> [.'$OneOf']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_20_map_$comment (.'$Enum'.'$comment')
function _jm_f_134(val, path, rep)
{
    let res;
    // .'$Enum'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Enum'.'$comment']", path])
    }
    return res;
}

// check json_model_20_map_$defs (.'$Enum'.'$defs')
function _jm_f_135(val, path, rep)
{
    // .'$Enum'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Enum'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_42 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Enum'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_42 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Enum'.'$defs'.'']", (path ? lpath_42 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_20_map_$id (.'$Enum'.'$id')
function _jm_f_136(val, path, rep)
{
    let res;
    // .'$Enum'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Enum'.'$id']", path])
    }
    return res;
}

// check json_model_20_map_$schema (.'$Enum'.'$schema')
function _jm_f_137(val, path, rep)
{
    let res;
    // .'$Enum'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Enum'.'$schema']", path])
    }
    return res;
}

// check json_model_20_map_default (.'$Enum'.default)
function _jm_f_138(val, path, rep)
{
    let res;
    // .'$Enum'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Enum'.default]", path])
    }
    return res;
}

// check json_model_20_map_definitions (.'$Enum'.definitions)
function _jm_f_139(val, path, rep)
{
    // .'$Enum'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Enum'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_43 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Enum'.definitions.''
        res = json_model_24(pval, (path ? lpath_43 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Enum'.definitions.'']", (path ? lpath_43 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_20_map_description (.'$Enum'.description)
function _jm_f_140(val, path, rep)
{
    let res;
    // .'$Enum'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Enum'.description]", path])
    }
    return res;
}

// check json_model_20_map_examples (.'$Enum'.examples)
function _jm_f_141(val, path, rep)
{
    let res;
    // .'$Enum'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Enum'.examples]", path])
    }
    return res;
}

// check json_model_20_map_id (.'$Enum'.id)
function _jm_f_142(val, path, rep)
{
    let res;
    // .'$Enum'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Enum'.id]", path])
    }
    return res;
}

// check json_model_20_map_title (.'$Enum'.title)
function _jm_f_143(val, path, rep)
{
    let res;
    // .'$Enum'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Enum'.title]", path])
    }
    return res;
}


// check $Enum (.'$Enum')
function json_model_20(val, path, rep)
{
    // .'$Enum'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Enum']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_41 = path ? path.concat([prop]) : null;
        if (prop == "enum")
        {
            // handle must enum property
            must_count += 1;
            // .'$Enum'.enum
            res = json_model_3(pval, (path ? lpath_41 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $enum [.'$Enum'.enum]", (path ? lpath_41 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Enum'.enum]", (path ? lpath_41 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_20_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_41 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Enum']", (path ? lpath_41 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Enum'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Enum'.'/^x-.*/']", (path ? lpath_41 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Enum']", (path ? lpath_41 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("enum"))
            {
                rep !== null && rep.push(["missing mandatory prop <enum> [.'$Enum']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_21_map_$comment (.'$Const'.'$comment')
function _jm_f_144(val, path, rep)
{
    let res;
    // .'$Const'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Const'.'$comment']", path])
    }
    return res;
}

// check json_model_21_map_$defs (.'$Const'.'$defs')
function _jm_f_145(val, path, rep)
{
    // .'$Const'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Const'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_45 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Const'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_45 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Const'.'$defs'.'']", (path ? lpath_45 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_21_map_$id (.'$Const'.'$id')
function _jm_f_146(val, path, rep)
{
    let res;
    // .'$Const'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Const'.'$id']", path])
    }
    return res;
}

// check json_model_21_map_$schema (.'$Const'.'$schema')
function _jm_f_147(val, path, rep)
{
    let res;
    // .'$Const'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Const'.'$schema']", path])
    }
    return res;
}

// check json_model_21_map_default (.'$Const'.default)
function _jm_f_148(val, path, rep)
{
    let res;
    // .'$Const'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Const'.default]", path])
    }
    return res;
}

// check json_model_21_map_definitions (.'$Const'.definitions)
function _jm_f_149(val, path, rep)
{
    // .'$Const'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Const'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_46 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Const'.definitions.''
        res = json_model_24(pval, (path ? lpath_46 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Const'.definitions.'']", (path ? lpath_46 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_21_map_description (.'$Const'.description)
function _jm_f_150(val, path, rep)
{
    let res;
    // .'$Const'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Const'.description]", path])
    }
    return res;
}

// check json_model_21_map_examples (.'$Const'.examples)
function _jm_f_151(val, path, rep)
{
    let res;
    // .'$Const'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Const'.examples]", path])
    }
    return res;
}

// check json_model_21_map_id (.'$Const'.id)
function _jm_f_152(val, path, rep)
{
    let res;
    // .'$Const'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Const'.id]", path])
    }
    return res;
}

// check json_model_21_map_title (.'$Const'.title)
function _jm_f_153(val, path, rep)
{
    let res;
    // .'$Const'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Const'.title]", path])
    }
    return res;
}


// check $Const (.'$Const')
function json_model_21(val, path, rep)
{
    // .'$Const'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Const']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_44 = path ? path.concat([prop]) : null;
        if (prop == "const")
        {
            // handle must const property
            must_count += 1;
            // .'$Const'.const
            res = json_model_2(pval, (path ? lpath_44 : null), rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $const [.'$Const'.const]", (path ? lpath_44 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Const'.const]", (path ? lpath_44 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_21_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_44 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Const']", (path ? lpath_44 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Const'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Const'.'/^x-.*/']", (path ? lpath_44 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Const']", (path ? lpath_44 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("const"))
            {
                rep !== null && rep.push(["missing mandatory prop <const> [.'$Const']", path])
            }
        }
        return false;
    }
    return true;
}

// check json_model_22_map_$comment (.'$Ref'.'$comment')
function _jm_f_154(val, path, rep)
{
    let res;
    // .'$Ref'.'$comment'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Ref'.'$comment']", path])
    }
    return res;
}

// check json_model_22_map_$defs (.'$Ref'.'$defs')
function _jm_f_155(val, path, rep)
{
    // .'$Ref'.'$defs'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Ref'.'$defs']", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_48 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Ref'.'$defs'.''
        res = json_model_24(pval, (path ? lpath_48 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Ref'.'$defs'.'']", (path ? lpath_48 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_22_map_$id (.'$Ref'.'$id')
function _jm_f_156(val, path, rep)
{
    let res;
    // .'$Ref'.'$id'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Ref'.'$id']", path])
    }
    return res;
}

// check json_model_22_map_$schema (.'$Ref'.'$schema')
function _jm_f_157(val, path, rep)
{
    let res;
    // .'$Ref'.'$schema'
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Ref'.'$schema']", path])
    }
    return res;
}

// check json_model_22_map_default (.'$Ref'.default)
function _jm_f_158(val, path, rep)
{
    let res;
    // .'$Ref'.default
    res = true;
    if (! res)
    {
        rep !== null && rep.push(["unexpected $ANY [.'$Ref'.default]", path])
    }
    return res;
}

// check json_model_22_map_definitions (.'$Ref'.definitions)
function _jm_f_159(val, path, rep)
{
    // .'$Ref'.definitions
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Ref'.definitions]", path])
        return false;
    }
    let res;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_49 = path ? path.concat([prop]) : null;
        // handle other props
        // .'$Ref'.definitions.''
        res = json_model_24(pval, (path ? lpath_49 : null), rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $Schema [.'$Ref'.definitions.'']", (path ? lpath_49 : null)])
            return false;
        }
    }
    return true;
}

// check json_model_22_map_description (.'$Ref'.description)
function _jm_f_160(val, path, rep)
{
    let res;
    // .'$Ref'.description
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Ref'.description]", path])
    }
    return res;
}

// check json_model_22_map_examples (.'$Ref'.examples)
function _jm_f_161(val, path, rep)
{
    let res;
    // .'$Ref'.examples
    res = Array.isArray(val);
    if (! res)
    {
        rep !== null && rep.push(["not array or unexpected array [.'$Ref'.examples]", path])
    }
    return res;
}

// check json_model_22_map_id (.'$Ref'.id)
function _jm_f_162(val, path, rep)
{
    let res;
    // .'$Ref'.id
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Ref'.id]", path])
    }
    return res;
}

// check json_model_22_map_title (.'$Ref'.title)
function _jm_f_163(val, path, rep)
{
    let res;
    // .'$Ref'.title
    res = (typeof val === 'string' || val instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$Ref'.title]", path])
    }
    return res;
}


// check $Ref (.'$Ref')
function json_model_22(val, path, rep)
{
    // .'$Ref'
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$Ref']", path])
        return false;
    }
    let res;
    let pfun;
    let must_count = 0;
    for (const [prop, pval] of Object.entries(val))
    {
        let lpath_47 = path ? path.concat([prop]) : null;
        if (prop == "$ref")
        {
            // handle must $ref property
            must_count += 1;
            // .'$Ref'.'$ref'
            res = runtime.jm_is_valid_url(pval);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $URL [.'$Ref'.'$ref']", (path ? lpath_47 : null)])
                rep !== null && rep.push(["invalid mandatory prop value [.'$Ref'.'$ref']", (path ? lpath_47 : null)])
                return false;
            }
        }
        else if ((pfun = json_model_22_map.get(prop)))
        {
            // handle 10 may props
            if (pfun !== undefined && ! pfun(pval, (path ? lpath_47 : null), rep))
            {
                rep !== null && rep.push(["invalid optional prop value [.'$Ref']", (path ? lpath_47 : null)])
                return false;
            }
        }
        else if (_jm_re_0(prop, path, rep))
        {
            // handle 1 re props
            // .'$Ref'.'/^x-.*/'
            res = true;
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ANY [.'$Ref'.'/^x-.*/']", (path ? lpath_47 : null)])
                return false;
            }
        }
        else
        {
            rep !== null && rep.push(["unexpected prop [.'$Ref']", (path ? lpath_47 : null)])
            return false;
        }
    }
    if (must_count != 1)
    {
        if (rep !== null)
        {
            if (! val.hasOwnProperty("$ref"))
            {
                rep !== null && rep.push(["missing mandatory prop <$ref> [.'$Ref']", path])
            }
        }
        return false;
    }
    return true;
}


// check $ObjectSchema (.'$ObjectSchema')
function json_model_23(val, path, rep)
{
    let res;
    // we could use ^ instead of | below
    // .'$ObjectSchema'
    let iso_0 = Object.prototype.toString.call(val) === '[object Object]';
    res = iso_0;
    if (res)
    {
        if (val.hasOwnProperty("type"))
        {
            let tag_0 = val["type"];
            let fun_0 = _jm_map_0.get(tag_0);
            if (fun_0 !== undefined)
            {
                res = fun_0(val, path, rep);
            }
            else
            {
                res = false;
                rep !== null && rep.push(["tag <type> value not found [.'$ObjectSchema'.'|']", path])
            }
        }
        else
        {
            res = false;
            rep !== null && rep.push(["tag prop <type> is missing [.'$ObjectSchema'.'|']", path])
        }
    }
    else
    {
        rep !== null && rep.push(["value is not an object [.'$ObjectSchema'.'|']", path])
    }
    if (! res)
    {
        res = json_model_9(val, path, rep);
        if (! res)
        {
            res = json_model_21(val, path, rep);
            if (! res)
            {
                res = json_model_20(val, path, rep);
                if (! res)
                {
                    res = json_model_17(val, path, rep);
                    if (! res)
                    {
                        res = json_model_18(val, path, rep);
                        if (! res)
                        {
                            res = json_model_19(val, path, rep);
                            if (! res)
                            {
                                res = json_model_22(val, path, rep);
                            }
                        }
                    }
                }
            }
        }
    }
    return res;
}

// check $Schema (.'$Schema')
function json_model_24(val, path, rep)
{
    let res;
    // .'$Schema'
    // .'$Schema'.'|'.0
    res = (typeof val === 'boolean' || val instanceof Boolean);
    if (! res)
    {
        rep !== null && rep.push(["not a bool [.'$Schema'.'|'.0]", path])
    }
    if (! res)
    {
        // .'$Schema'.'|'.1
        res = json_model_23(val, path, rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected $ObjectSchema [.'$Schema'.'|'.1]", path])
        }
    }
    if (res)
    {
        if (rep !== null) rep.length = 0
    }
    else
    {
        rep !== null && rep.push(["no model matched [.'$Schema'.'|']", path])
    }
    return res;
}

// object .'$RootSchema'.'&'.0
function _jm_obj_2(val, path, rep)
{
    // check open must/may only props
    if (! (Object.prototype.toString.call(val) === '[object Object]'))
    {
        rep !== null && rep.push(["not an object [.'$RootSchema'.'&'.0]", path])
        return false;
    }
    let lpath;
    let pval;
    let res;
    if (! val.hasOwnProperty("$schema"))
    {
        rep !== null && rep.push(["missing mandatory prop <$schema> [.'$RootSchema'.'&'.0]", path])
        return false;
    }
    lpath = path ? path.concat(["$schema"]) : null;
    pval = val["$schema"];
    // .'$RootSchema'.'&'.0.'$schema'
    res = (typeof pval === 'string' || pval instanceof String);
    if (! res)
    {
        rep !== null && rep.push(["unexpected string [.'$RootSchema'.'&'.0.'$schema']", (path ? lpath : null)])
        rep !== null && rep.push(["unexpected value for mandatory prop <$schema> [.'$RootSchema'.'&'.0]", (path ? lpath : null)])
        return false;
    }
    return true;
}

// check $RootSchema (.'$RootSchema')
function json_model_25(val, path, rep)
{
    let res;
    // keyword $schema is mandatory at the root, and optional elsewhere
    // .'$RootSchema'
    res = true;
    if (res)
    {
        // .'$RootSchema'.'&'.0
        res = _jm_obj_2(val, path, rep);
        if (! res)
        {
            rep !== null && rep.push(["unexpected element [.'$RootSchema'.'&'.0]", path])
        }
        if (res)
        {
            // .'$RootSchema'.'&'.1
            res = json_model_23(val, path, rep);
            if (! res)
            {
                rep !== null && rep.push(["unexpected $ObjectSchema [.'$RootSchema'.'&'.1]", path])
            }
        }
    }
    if (res)
    {
        if (rep !== null) rep.length = 0
    }
    else
    {
        rep !== null && rep.push(["not all model match [.'$RootSchema'.'&']", path])
    }
    return res;
}

// check $ (.)
function json_model_1(val, path, rep)
{
    let res;
    // .
    res = json_model_25(val, path, rep);
    if (! res)
    {
        rep !== null && rep.push(["unexpected $RootSchema [.]", path])
    }
    return res;
}


var initialized = false

// differed module initializations
export function check_model_init()
{
    if (! initialized)
    {
        initialized = true;
        runtime.jm_set_rx(RegExp)
        _jm_cst_0.add("uri")
        _jm_cst_0.add("uri-reference")
        _jm_cst_0.add("iri")
        _jm_cst_0.add("iri-reference")
        _jm_cst_0.add("date")
        _jm_cst_0.add("time")
        _jm_cst_0.add("date-time")
        _jm_cst_0.add("ipv4")
        _jm_cst_0.add("ipv6")
        _jm_cst_0.add("regex")
        _jm_cst_0.add("uuid")
        _jm_cst_0.add("hostname")
        _jm_cst_0.add("email")
        _jm_cst_0.add("idn-hostname")
        _jm_cst_0.add("idn-email")
        _jm_cst_0.add("duration")
        _jm_cst_0.add("int32")
        _jm_cst_0.add("int64")
        _jm_cst_0.add("uint")
        _jm_cst_0.add("uint32")
        _jm_cst_0.add("uint64")
        _jm_cst_0.add("json-pointer")
        _jm_cst_0.add("relative-json-pointer")
        _jm_cst_0.add("byte")
        _jm_cst_0.add("double")
        _jm_cst_0.add("int")
        _jm_cst_0.add("mime-type")
        json_model_6_map.set("additionalItems", _jm_f_0)
        json_model_6_map.set("items", _jm_f_1)
        json_model_6_map.set("maxItems", _jm_f_2)
        json_model_6_map.set("minItems", _jm_f_3)
        json_model_6_map.set("prefixItems", _jm_f_4)
        json_model_6_map.set("uniqueItems", _jm_f_5)
        json_model_9_map.set("$comment", _jm_f_6)
        json_model_9_map.set("$defs", _jm_f_7)
        json_model_9_map.set("$id", _jm_f_8)
        json_model_9_map.set("$schema", _jm_f_9)
        json_model_9_map.set("default", _jm_f_10)
        json_model_9_map.set("definitions", _jm_f_11)
        json_model_9_map.set("description", _jm_f_12)
        json_model_9_map.set("examples", _jm_f_13)
        json_model_9_map.set("id", _jm_f_14)
        json_model_9_map.set("title", _jm_f_15)
        _jm_cst_1.add("uri")
        _jm_cst_1.add("uri-reference")
        _jm_cst_1.add("iri")
        _jm_cst_1.add("iri-reference")
        _jm_cst_1.add("date")
        _jm_cst_1.add("time")
        _jm_cst_1.add("date-time")
        _jm_cst_1.add("ipv4")
        _jm_cst_1.add("ipv6")
        _jm_cst_1.add("regex")
        _jm_cst_1.add("uuid")
        _jm_cst_1.add("hostname")
        _jm_cst_1.add("email")
        _jm_cst_1.add("idn-hostname")
        _jm_cst_1.add("idn-email")
        _jm_cst_1.add("duration")
        _jm_cst_1.add("int32")
        _jm_cst_1.add("int64")
        _jm_cst_1.add("uint")
        _jm_cst_1.add("uint32")
        _jm_cst_1.add("uint64")
        _jm_cst_1.add("json-pointer")
        _jm_cst_1.add("relative-json-pointer")
        _jm_cst_1.add("byte")
        _jm_cst_1.add("double")
        _jm_cst_1.add("int")
        _jm_cst_1.add("mime-type")
        json_model_10_map.set("$comment", _jm_f_16)
        json_model_10_map.set("$defs", _jm_f_17)
        json_model_10_map.set("$id", _jm_f_18)
        json_model_10_map.set("$schema", _jm_f_19)
        json_model_10_map.set("default", _jm_f_20)
        json_model_10_map.set("definitions", _jm_f_21)
        json_model_10_map.set("description", _jm_f_22)
        json_model_10_map.set("examples", _jm_f_23)
        json_model_10_map.set("format", _jm_f_24)
        json_model_10_map.set("id", _jm_f_25)
        json_model_10_map.set("maxLength", _jm_f_26)
        json_model_10_map.set("minLength", _jm_f_27)
        json_model_10_map.set("pattern", _jm_f_28)
        json_model_10_map.set("title", _jm_f_29)
        json_model_11_map.set("$comment", _jm_f_30)
        json_model_11_map.set("$defs", _jm_f_31)
        json_model_11_map.set("$id", _jm_f_32)
        json_model_11_map.set("$schema", _jm_f_33)
        json_model_11_map.set("additionalItems", _jm_f_34)
        json_model_11_map.set("default", _jm_f_35)
        json_model_11_map.set("definitions", _jm_f_36)
        json_model_11_map.set("description", _jm_f_37)
        json_model_11_map.set("examples", _jm_f_38)
        json_model_11_map.set("id", _jm_f_39)
        json_model_11_map.set("items", _jm_f_40)
        json_model_11_map.set("maxItems", _jm_f_41)
        json_model_11_map.set("minItems", _jm_f_42)
        json_model_11_map.set("prefixItems", _jm_f_43)
        json_model_11_map.set("title", _jm_f_44)
        json_model_11_map.set("uniqueItems", _jm_f_45)
        json_model_12_map.set("$comment", _jm_f_46)
        json_model_12_map.set("$defs", _jm_f_47)
        json_model_12_map.set("$id", _jm_f_48)
        json_model_12_map.set("$schema", _jm_f_49)
        json_model_12_map.set("additionalProperties", _jm_f_50)
        json_model_12_map.set("default", _jm_f_51)
        json_model_12_map.set("definitions", _jm_f_52)
        json_model_12_map.set("description", _jm_f_53)
        json_model_12_map.set("examples", _jm_f_54)
        json_model_12_map.set("id", _jm_f_55)
        json_model_12_map.set("patternProperties", _jm_f_56)
        json_model_12_map.set("properties", _jm_f_57)
        json_model_12_map.set("required", _jm_f_58)
        json_model_12_map.set("title", _jm_f_59)
        json_model_13_map.set("$comment", _jm_f_60)
        json_model_13_map.set("$defs", _jm_f_61)
        json_model_13_map.set("$id", _jm_f_62)
        json_model_13_map.set("$schema", _jm_f_63)
        json_model_13_map.set("default", _jm_f_64)
        json_model_13_map.set("definitions", _jm_f_65)
        json_model_13_map.set("description", _jm_f_66)
        json_model_13_map.set("examples", _jm_f_67)
        json_model_13_map.set("id", _jm_f_68)
        json_model_13_map.set("maximum", _jm_f_69)
        json_model_13_map.set("minimum", _jm_f_70)
        json_model_13_map.set("title", _jm_f_71)
        json_model_14_map.set("$comment", _jm_f_72)
        json_model_14_map.set("$defs", _jm_f_73)
        json_model_14_map.set("$id", _jm_f_74)
        json_model_14_map.set("$schema", _jm_f_75)
        json_model_14_map.set("default", _jm_f_76)
        json_model_14_map.set("definitions", _jm_f_77)
        json_model_14_map.set("description", _jm_f_78)
        json_model_14_map.set("examples", _jm_f_79)
        json_model_14_map.set("id", _jm_f_80)
        json_model_14_map.set("maximum", _jm_f_81)
        json_model_14_map.set("minimum", _jm_f_82)
        json_model_14_map.set("title", _jm_f_83)
        json_model_15_map.set("$comment", _jm_f_84)
        json_model_15_map.set("$defs", _jm_f_85)
        json_model_15_map.set("$id", _jm_f_86)
        json_model_15_map.set("$schema", _jm_f_87)
        json_model_15_map.set("default", _jm_f_88)
        json_model_15_map.set("definitions", _jm_f_89)
        json_model_15_map.set("description", _jm_f_90)
        json_model_15_map.set("examples", _jm_f_91)
        json_model_15_map.set("id", _jm_f_92)
        json_model_15_map.set("title", _jm_f_93)
        json_model_16_map.set("$comment", _jm_f_94)
        json_model_16_map.set("$defs", _jm_f_95)
        json_model_16_map.set("$id", _jm_f_96)
        json_model_16_map.set("$schema", _jm_f_97)
        json_model_16_map.set("default", _jm_f_98)
        json_model_16_map.set("definitions", _jm_f_99)
        json_model_16_map.set("description", _jm_f_100)
        json_model_16_map.set("examples", _jm_f_101)
        json_model_16_map.set("id", _jm_f_102)
        json_model_16_map.set("title", _jm_f_103)
        json_model_17_map.set("$comment", _jm_f_104)
        json_model_17_map.set("$defs", _jm_f_105)
        json_model_17_map.set("$id", _jm_f_106)
        json_model_17_map.set("$schema", _jm_f_107)
        json_model_17_map.set("default", _jm_f_108)
        json_model_17_map.set("definitions", _jm_f_109)
        json_model_17_map.set("description", _jm_f_110)
        json_model_17_map.set("examples", _jm_f_111)
        json_model_17_map.set("id", _jm_f_112)
        json_model_17_map.set("title", _jm_f_113)
        json_model_18_map.set("$comment", _jm_f_114)
        json_model_18_map.set("$defs", _jm_f_115)
        json_model_18_map.set("$id", _jm_f_116)
        json_model_18_map.set("$schema", _jm_f_117)
        json_model_18_map.set("default", _jm_f_118)
        json_model_18_map.set("definitions", _jm_f_119)
        json_model_18_map.set("description", _jm_f_120)
        json_model_18_map.set("examples", _jm_f_121)
        json_model_18_map.set("id", _jm_f_122)
        json_model_18_map.set("title", _jm_f_123)
        json_model_19_map.set("$comment", _jm_f_124)
        json_model_19_map.set("$defs", _jm_f_125)
        json_model_19_map.set("$id", _jm_f_126)
        json_model_19_map.set("$schema", _jm_f_127)
        json_model_19_map.set("default", _jm_f_128)
        json_model_19_map.set("definitions", _jm_f_129)
        json_model_19_map.set("description", _jm_f_130)
        json_model_19_map.set("examples", _jm_f_131)
        json_model_19_map.set("id", _jm_f_132)
        json_model_19_map.set("title", _jm_f_133)
        json_model_20_map.set("$comment", _jm_f_134)
        json_model_20_map.set("$defs", _jm_f_135)
        json_model_20_map.set("$id", _jm_f_136)
        json_model_20_map.set("$schema", _jm_f_137)
        json_model_20_map.set("default", _jm_f_138)
        json_model_20_map.set("definitions", _jm_f_139)
        json_model_20_map.set("description", _jm_f_140)
        json_model_20_map.set("examples", _jm_f_141)
        json_model_20_map.set("id", _jm_f_142)
        json_model_20_map.set("title", _jm_f_143)
        json_model_21_map.set("$comment", _jm_f_144)
        json_model_21_map.set("$defs", _jm_f_145)
        json_model_21_map.set("$id", _jm_f_146)
        json_model_21_map.set("$schema", _jm_f_147)
        json_model_21_map.set("default", _jm_f_148)
        json_model_21_map.set("definitions", _jm_f_149)
        json_model_21_map.set("description", _jm_f_150)
        json_model_21_map.set("examples", _jm_f_151)
        json_model_21_map.set("id", _jm_f_152)
        json_model_21_map.set("title", _jm_f_153)
        json_model_22_map.set("$comment", _jm_f_154)
        json_model_22_map.set("$defs", _jm_f_155)
        json_model_22_map.set("$id", _jm_f_156)
        json_model_22_map.set("$schema", _jm_f_157)
        json_model_22_map.set("default", _jm_f_158)
        json_model_22_map.set("definitions", _jm_f_159)
        json_model_22_map.set("description", _jm_f_160)
        json_model_22_map.set("examples", _jm_f_161)
        json_model_22_map.set("id", _jm_f_162)
        json_model_22_map.set("title", _jm_f_163)
        _jm_map_0.set("null", json_model_16)
        _jm_map_0.set("boolean", json_model_15)
        _jm_map_0.set("integer", json_model_13)
        _jm_map_0.set("number", json_model_14)
        _jm_map_0.set("string", json_model_10)
        _jm_map_0.set("array", json_model_11)
        _jm_map_0.set("object", json_model_12)
        check_model_map.set("", json_model_25)
        check_model_map.set("const", json_model_2)
        check_model_map.set("enum", json_model_3)
        check_model_map.set("schemaArray", json_model_4)
        check_model_map.set("stringKeywords", json_model_5)
        check_model_map.set("arrayKeywords", json_model_6)
        check_model_map.set("objectKeywords", json_model_7)
        check_model_map.set("numberKeywords", json_model_8)
        check_model_map.set("metas", json_model_9)
        check_model_map.set("String", json_model_10)
        check_model_map.set("Array", json_model_11)
        check_model_map.set("Object", json_model_12)
        check_model_map.set("Integer", json_model_13)
        check_model_map.set("Number", json_model_14)
        check_model_map.set("Bool", json_model_15)
        check_model_map.set("Null", json_model_16)
        check_model_map.set("AllOf", json_model_17)
        check_model_map.set("AnyOf", json_model_18)
        check_model_map.set("OneOf", json_model_19)
        check_model_map.set("Enum", json_model_20)
        check_model_map.set("Const", json_model_21)
        check_model_map.set("Ref", json_model_22)
        check_model_map.set("ObjectSchema", json_model_23)
        check_model_map.set("Schema", json_model_24)
        check_model_map.set("RootSchema", json_model_25)
    }
}

// differed module cleanup
export function check_model_free()
{
    if (initialized)
    {
        initialized = false;
    }
}

export function check_model(val, name, rep)
{
    let checker = check_model_map.get(name)
    if (checker === undefined)
        throw `no checker for "${name}"`
    let path = rep !== null ? [] : null

    return checker(val, path, rep)
}

// possibly run as main based on a guess
import main from "json_model_runtime/main.js"

if (import.meta.url.endsWith(process.argv[1]))
    main(check_model_init, check_model, check_model_free)
