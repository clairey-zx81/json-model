...
sub _jm_f_4($$$)
{
    ...
    while (my ($prop, $pval) = each %$val)
    {
        ...
        $res = json_model_8($pval, undef, $rep) || json_model_6($pval, undef, $rep);
        if (! $res)
        {
            return 0;
        }
...
sub _jm_f_11($$$)
{
    ...
    return json_model_8($val, $path, $rep) || json_model_3($val, $path, $rep);
}
...
sub json_model_8($$$)
{
    ...
    # .'$Schema'.'|'.0
    # .'$Schema'.'|'.1
    return jm_is_boolean($val) || json_model_7($val, $path, $rep);
}
