...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (hash_0 <= 0x2a746f64)
        {
            ...
            else if (jm_str_eq_5(prop, 0x000000002a746f64LL))
            {
                res = json_is_string(pval) && _jm_re_3(json_string_value(pval), NULL, NULL);
                if (unlikely(! res))
                    return false;
                continue;
            }
            else if (jm_str_eq_6(prop, 0x000000732a746f64LL))
            {
