...
static INLINE bool _jm_re_0(const char *s, jm_path_t *path, jm_report_t *rep)
{
    return
        ...
        jm_str_eq_6(s, 0x0000006f6c6c6548LL) ||
        jm_str_eq_6(s, 0x0000006569737553LL) ||
        jm_str_eq_6(s, 0x000000646c726f57LL)
    ;
}
...
{
    // .
    // "/^(Hello|World|Susie|Calvin)$/"
    bool res = json_is_string(val) && _jm_re_0(json_string_value(val), path, rep);
    if (unlikely(! res))
    {
        if (rep) jm_report_add_entry(rep, "unexpected value for model \"/^(Hello|World|Susie|Calvin)$/\" [.]", path);
    }
    return res;
}
