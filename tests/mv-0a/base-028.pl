...
sub json_model_1($$$)
{
    ...
    for my $prop (sort keys %$val)
    {
        ...
        if ($prop eq "country")
        {
            ...
            unless ($res)
            {
                push @$rep, ["unexpected value for model \"_South Africa\" [.country]", defined $path ? $lpath_0 : undef] if defined $rep;
                push @$rep, ["invalid optional prop value [.country]", defined $path ? $lpath_0 : undef] if defined $rep;
                return 0;
            }
            next;
        }
