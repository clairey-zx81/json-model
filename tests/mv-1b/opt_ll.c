...
static bool _jm_obj_0(const json_t* val, Path* path, Report* rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (strcmp(prop, "i") == 0)
        {
            ...
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid mandatory property value [.i]", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else if (strcmp(prop, "f") == 0)
        {
            ...
            if (! res)
            {
                if (rep) jm_report_add_entry(rep, "invalid mandatory property value [.f]", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else
