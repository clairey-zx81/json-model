...
sub json_model_7($$$)
{
    ...
    $pval = $$val{"method"};
    # .'$Notification'.method
    return jm_is_string($pval) && $pval eq "notifications/initialized";
}
...
sub json_model_1($$$)
{
    ...
    return $res || json_model_6($val, undef, undef) || json_model_9($val, undef, undef) || json_model_11($val, undef, undef);
}
