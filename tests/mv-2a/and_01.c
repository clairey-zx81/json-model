...
static INLINE bool _jm_obj_0(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (unlikely(jm_str_eq_1(prop, 0x73)))
        {
            // handle 2 re props
            // .'&'.1.'/^s/'
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "not a bool [.'&'.1.'/^s/']", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else if (unlikely(jm_str_eq_1(prop, 0x69)))
        {
            // handle 2 re props
            // .'&'.1.'/^i/'
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "not a 1 strict int [.'&'.1.'/^i/']", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else
...
static INLINE bool _jm_obj_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (unlikely(jm_str_eq_2(prop, 0x00000069)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "not a 1 strict int [.'&'.0.i]", (path ? &lpath_1 : NULL));
                if (rep) jm_report_add_entry(rep, "invalid mandatory prop value [.'&'.0.i]", (path ? &lpath_1 : NULL));
                return false;
            }
            continue;
        }
        ...
        if (unlikely(jm_str_eq_3(prop, 0x00007373)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.'&'.0.ss]", (path ? &lpath_1 : NULL));
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'&'.0.ss]", (path ? &lpath_1 : NULL));
                return false;
            }
            continue;
        }
    ...
    if (unlikely(must_count != 2))
    {
        if (likely(rep != NULL))
        {
            ...
            {
                if (rep) jm_report_add_entry(rep, "missing mandatory prop <i> [.'&'.0.'']", path);
            }
        }
        return false;
    }
