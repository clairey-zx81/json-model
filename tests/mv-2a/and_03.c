...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (unlikely(jm_str_eq_2(prop, 0x00000073)))
        {
            ...
            if (unlikely(! res))
            {
                ...
                if (rep) jm_report_add_entry(rep, "invalid mandatory prop value [.s]", (path ? &lpath_0 : NULL));
                return false;
            }
            continue;
        }
        ...
        if (unlikely(jm_str_eq_2(prop, 0x00007378)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.'/^xs/']", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else if (unlikely(jm_str_eq_2(prop, 0x00006278)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "not a bool [.'/^xb/']", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else if (unlikely(jm_str_eq_2(prop, 0x00006978)))
        {
            // handle 4 re props
            // .'/^xi/'
            res = json_is_integer(pval) && json_integer_value(pval) >= 1;
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "not a 1 strict int [.'/^xi/']", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else if (unlikely(jm_str_eq_2(prop, 0x00006678)))
        {
            // handle 4 re props
            // .'/^xf/'
            res = json_is_real(pval) && json_real_value(pval) > 0.0;
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "not a 1.0 strict float [.'/^xf/']", (path ? &lpath_0 : NULL));
                return false;
            }
        }
        else
    ...
    if (unlikely(must_count != 1))
    {
        if (likely(rep != NULL))
        {
            ...
            {
                if (rep) jm_report_add_entry(rep, "missing mandatory prop <s> [.'']", path);
            }
        }
        return false;
    }
