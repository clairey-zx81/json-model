    ...
    if (likely(res))
    {
        ...
        json_array_foreach(val, arr_0_idx, arr_0_item)
        {
            ...
            if (likely(res))
            {
                ...
                // .0.0
                res = json_is_string(json_array_get(arr_0_item, 0));
                if (likely(res))
                {
                    ...
                    if (unlikely(! res))
                    {
                        if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.0.1]", ((path ? &arr_0_lpath : NULL) ? &lpath_0 : NULL));
                    }
                }
                else
                {
                    if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.0.0]", ((path ? &arr_0_lpath : NULL) ? &lpath_0 : NULL));
                }
            }
            if (unlikely(! res))
            {
