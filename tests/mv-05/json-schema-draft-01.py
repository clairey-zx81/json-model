    ...
    if not res:
        ...
        if res:
            for arr_0_idx, arr_0_item in enumerate(val):
                ...
                if not res:
                    rep is None or rep.append(("unexpected string [.disallow.'|'.1.0]", arr_0_lpath if path is not None else None))
                if not res:
                    break
        if not res:
    ...
    if not res:
        ...
        if res:
            ...
            for arr_3_idx, arr_3_item in enumerate(val):
                ...
                if not res:
                    rep is None or rep.append(("unexpected string [.type.'|'.0.0.'|'.0]", arr_3_lpath if path is not None else None))
                if not res:
                    # .type.'|'.0.0.'|'.1
                    res = json_model_1(arr_3_item, arr_3_lpath if path is not None else None, rep)
                    if not res:
                        rep is None or rep.append(("unexpected $Schema [.type.'|'.0.0.'|'.1]", arr_3_lpath if path is not None else None))
                if res:
                    rep is None or rep.clear()
                else:
                    rep is None or rep.append(("no model matched [.type.'|'.0.0.'|']", arr_3_lpath if path is not None else None))
                if not res:
                    break
            if not res:
