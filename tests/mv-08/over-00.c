...
static bool json_model_2(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (likely(jm_str_eq_4(prop, 0x006f6f66)))
        {
            ...
            // .'$Foo'.foo
            const char *sval_0;
            res = json_is_string(pval) && ((sval_0 = json_string_value(pval)), jm_str_eq_8(sval_0, 0x206c616974696e69LL) && jm_str_eq_4(sval_0 + 8, 0x006f6f66));
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"_initial foo\" [.'$Foo'.foo]", (path ? &lpath_0 : NULL));
