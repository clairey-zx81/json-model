...
def json_model_2(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        for arr_0_idx, arr_0_item in enumerate(val):
            ...
            if not res:
                rep is None or rep.append(("unexpected value for model \"$self\" [.'$array'.0]", arr_0_lpath if path is not None else None))
                break
    if not res:
...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    for prop, pval in val.items():
        ...
        if prop == "prop":
            ...
            if not res:
                rep is None or rep.append(("unexpected value for model \"$self\" [.prop]", lpath_0 if path is not None else None))
