...
def json_model_3(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        for arr_1_idx, arr_1_item in enumerate(val):
            ...
            if not res:
                rep is None or rep.append(("unexpected value for model \"$URL\" [.'$ls0'.0]", arr_1_lpath if path is not None else None))
                break
    if not res:
...
def json_model_4(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if res:
        for arr_2_idx, arr_2_item in enumerate(val):
            ...
            if not res:
                rep is None or rep.append(("unexpected value for model \"$BOOL\" [.'$lb0'.0]", arr_2_lpath if path is not None else None))
                break
    if not res:
...
def json_model_1(val: Jsonable, path: Path, rep: Report) -> bool:
    ...
    if not res:
        rep is None or rep.append(("unexpected value for model \"$li0\" [.'|'.0]", path))
        # .'|'.1
        res = json_model_3(val, path, rep)
        if not res:
            rep is None or rep.append(("unexpected value for model \"$ls0\" [.'|'.1]", path))
            # .'|'.2
            res = json_model_4(val, path, rep)
            if not res:
                rep is None or rep.append(("unexpected value for model \"$lb0\" [.'|'.2]", path))
                # .'|'.3
                res = json_model_5(val, path, rep)
                if not res:
                    rep is None or rep.append(("unexpected value for model \"$lf0\" [.'|'.3]", path))
    if res:
