#! /usr/bin/perl
#
# Generated by JSON Model Compiler version 2
# see https://github.com/clairey-zx81/json-model
#
use utf8;
use strict;
use warnings;
no warnings "uninitialized";
use re::engine::RE2;
use JSON::JsonModel;
use constant JMC_VERSION => "2";


sub json_model_1($$$);
my %check_model_map;

# check $ (.)
sub json_model_1($$$)
{
    my ($val, $path, $rep) = @_;
    # .
    my $res = jm_is_array($val) && scalar @$val == 4;
    if ($res)
    {
        my $lpath_0 = defined $path ? [@{$path}, 0] : undef;
        # .0
        $res = jm_is_boolean($$val[0]);
        if ($res)
        {
            $lpath_0 = defined $path ? [@{$path}, 1] : undef;
            # .1
            $res = jm_is_integer($$val[1]) && $$val[1] >= 0;
            if ($res)
            {
                $lpath_0 = defined $path ? [@{$path}, 2] : undef;
                # .2
                $res = jm_is_numeric($$val[2]) && $$val[2] >= 0.0;
                if ($res)
                {
                    $lpath_0 = defined $path ? [@{$path}, 3] : undef;
                    # .3
                    $res = jm_is_string($$val[3]);
                    push @$rep, ["unexpected value for model \"\" [.3]", defined $path ? $lpath_0 : undef] if defined $rep and not $res;
                }
                else
                {
                    push @$rep, ["not a 0.0 strict float [.2]", defined $path ? $lpath_0 : undef] if defined $rep;
                }
            }
            else
            {
                push @$rep, ["not a 0 strict int [.1]", defined $path ? $lpath_0 : undef] if defined $rep;
            }
        }
        else
        {
            push @$rep, ["not a bool [.0]", defined $path ? $lpath_0 : undef] if defined $rep;
        }
    }
    push @$rep, ["not array or unexpected array [.]", $path] if defined $rep and not $res;
    return $res;
}


# initialization of global variables

our $initialized = 0;

sub check_model_init()
{
    if (!$initialized)
    {
        $initialized = 1;
        %check_model_map = (
            "" => \&json_model_1,
        );
    }
}

sub check_model_free()
{
    if ($initialized)
    {
        $initialized = 0;
        %check_model_map = ();
    }
}

sub check_model_mapper($)
{
    my ($name) = @_;
    die "unexpected model name \"$name\"" unless exists $check_model_map{$name};
    return $check_model_map{$name};
}

sub check_model($$$)
{
    my ($json, $name, $rep) = @_;
    die "unexpected model name \"$name\"" unless exists $check_model_map{$name};
    my $path = defined $rep ? [] : undef;
    return $check_model_map{$name}($json, $path, $rep);
}

#
# SCRIPT EXECUTION
#
check_model_init();
jm_main(\&check_model, \%check_model_map, JMC_VERSION);
check_model_free();
