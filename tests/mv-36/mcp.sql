...
BEGIN
  ...
  FOR prop, pval IN SELECT * FROM JSONB_EACH(val) LOOP
    ...
    ELSEIF prop = 'method' THEN
      ...
      -- .'$Notification'.method
      -- "/^notifications/"
      res := JSONB_TYPEOF(pval) = 'string' AND STARTS_WITH(JSON_VALUE(pval, '$' RETURNING TEXT), 'notifications');
      IF NOT res THEN
        RETURN FALSE;
      END IF;
      CONTINUE;
...
BEGIN
  ...
  IF res THEN
    ...
    END IF;
  END IF;
  RETURN res OR json_model_6(val, NULL, NULL) OR json_model_7(val, NULL, NULL) OR json_model_9(val, NULL, NULL) OR json_model_11(val, NULL, NULL);
END;
$$ LANGUAGE PLpgSQL;
