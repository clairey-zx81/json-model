...
// process one value
function processing(source, value, checker, expected, report, times, overhead)
{
...
export default async function main(validate)
{
    const options = {
      // TODO --help --list
        ...
        try {
            ...
            if (args.values.test) {
                ...
                for (const item of value)
                {
                    ...
                    errors += processing(display, val, validate, expect, args.values.report, times, overhead)

                    index += 1
                }
            }
            else
                errors += processing(fname, value, validate, null, args.values.report, times, overhead)
        }
        catch (e) {
            errors++
