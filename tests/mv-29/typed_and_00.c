...
static bool json_model_2(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (likely(jm_str_eq_4(prop, 0x00616c62)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "not a 1 strict int [.'$foo'.bla]", (path ? &lpath_0 : NULL));
                if (rep) jm_report_add_entry(rep, "invalid mandatory prop value [.'$foo'.bla]", (path ? &lpath_0 : NULL));
                return false;
            }
            continue;
        }
    ...
    if (unlikely(must_count != 1))
    {
        if (likely(rep != NULL))
        {
            ...
            {
                if (rep) jm_report_add_entry(rep, "missing mandatory prop <bla> [.'$foo']", path);
            }
        }
        return false;
    }
