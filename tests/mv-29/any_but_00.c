...
static INLINE bool _jm_obj_0(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    json_object_foreach((json_t *) val, prop, pval)
    {
        ...
        if (likely(jm_str_eq_2(prop, 0x00000061)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.'|'.6.a]", (path ? &lpath_0 : NULL));
                if (rep) jm_report_add_entry(rep, "invalid mandatory prop value [.'|'.6.a]", (path ? &lpath_0 : NULL));
                return false;
            }
            continue;
        }
        if (likely(jm_str_eq_2(prop, 0x00000062)))
        {
            ...
            if (unlikely(! res))
            {
                if (rep) jm_report_add_entry(rep, "unexpected value for model \"\" [.'|'.6.b]", (path ? &lpath_0 : NULL));
                if (rep) jm_report_add_entry(rep, "invalid optional prop value [.'|'.6.b]", (path ? &lpath_0 : NULL));
                return false;
            }
            continue;
        }
        if (rep) jm_report_add_entry(rep, "unexpected prop [.'|'.6]", (path ? &lpath_0 : NULL));
        return false;
    }
    if (unlikely(must_count != 1))
    {
        if (likely(rep != NULL))
        {
            ...
            {
                if (rep) jm_report_add_entry(rep, "missing mandatory prop <a> [.'|'.6]", path);
            }
        }
        return false;
    }
...
static bool json_model_1(const json_t *val, jm_path_t *path, jm_report_t *rep)
{
    ...
    if (likely(res))
    {
        ...
        if (unlikely(! res))
        {
            if (rep) jm_report_add_entry(rep, "unexpected element [.'|'.6]", path);
        }
    }
    else
        res = true;
