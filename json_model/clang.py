...
class CLangJansson(Language):
        ...
        if self._regex_opt and (regex, opts) in _COMPILED_RE:
            return [
                f"#define {name}(s, p, r) {_COMPILED_RE[(regex, opts)]}(s)"
                    ...
                    rx.simple_re(regex, opts) or
                    rx.ic_str_cmp_re(regex, opts, self._max_strcmp_cset) or
                    rx.str_len_bounds(regex, opts)
                ):
            return [
    ...
    def sub_re(self, name: str, regex: str, opts: str) -> Block:
        if self._regex_opt and (regex, opts) in _COMPILED_RE:
            return []
        elif self._regex_opt and (bounds := rx.str_len_bounds(regex, opts)):
            return _compile_str_len_bounds(name, *bounds, self._inline, self._count)
        elif self._regex_opt and (simple := rx.simple_re(regex, opts)):
            return _compile_simple_re(name, *simple, self._byte_order == "le", self._inline, self._count)
        elif self._regex_opt and (ls_ic := rx.ic_str_cmp_re(regex, opts, self._max_strcmp_cset)):
