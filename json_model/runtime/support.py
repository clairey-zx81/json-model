...
# ssh, telnel, mailto
DEST_ADDRESS_RX = re.compile(r"^\w+:\w[-\w.]*@\w[-\w]*(\.\w[-\w]*)*$")
...
def is_valid_ip6(value: Jsonable, path: Path, rep: Report = None) -> bool:
    valid = (
        ...
        # NOTE may include a /%n$/ zone id suffix
        validators.ipv6(value, cidr=False) is True and
        "%" not in value
    )
    if not valid:
...
def main(jm_fun, jm_map, jmc_version):
    ...
    ap.add_argument("--parse", action="store_true", default=False, help="show jsonl parse time")
            ...
            if args.jsonl:
                ...
                if args.parse:
                    ...
                    for _ in range(args.time):
                        ...
                        del values
                        delay = 1000000.0 * (time.clock_gettime(0) - start) - empty
                        psum1 += delay
                        psum2 += delay * delay
                    pavg = psum1 / args.time
                    pstd = math.sqrt(psum2 / args.time - pavg * pavg)
                    print(f"{fn} {len(lines)}: PARSE "
                          f"{pavg:.03f} ± {pstd:.03f} µs ({empty:.03f})",
                          file=sys.stderr)
                values = [json.loads(r) for r in lines]
            else:
                with open(fn) as f:
