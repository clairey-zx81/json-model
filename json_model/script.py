...
def jmc_script(xargs: list[str]|None = None) -> int:
    ...
    # TODO cpp ts rs go…
    arg("--format", "-F", choices=["json", "yaml", "py", "c", "js", "plpgsql", "pl", "java", "ts"],
    ...
    if args.op in "PUJNVIA":
        ...
        args.format = args.format or "json"
        if args.format not in ("json", "yaml", "py", "ts"):
            log.error(f"unexpected format {args.format} for operation {args.op}")
            return 1
    elif args.op == "C":
        args.format = args.format or "py"
    ...
    if args.op == "J":  # json dump
        ...
        if args.format in ("json", "yaml"):
            ...
            try:
                interface = model2tsinterface(model, root=args.name or "RootModel")
            except Exception as e:
                log.error(e, exc_info=args.debug)
                return 1
            code = [
                ...
                "// see https://json-model.org/",
                ""
            ] + model2tsinterface(model, root=args.name or "RootModel")
            print("\n".join(code), file=output)
    else:
