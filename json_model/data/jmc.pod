...
=head1 SYNOPSIS
 ...
 jmc [-h] [--version] [--debug] [--verbose] [--quiet] [--doc {pod,synopsis,help,man}]
...
JSON Model Compiler command processes a L<JSON Model|https://json-model.org/> to
...
=item B<--op {P,U,J,N,E,C}>

operation to run (preproc, dump, jdump, nope, export, compile).

=item B<--{loose,strict}-{int,float,number}>

whether to assume loose JSON numbers,
i.e. accept I<42.0> as an integer or I<42> as a float. Default is strict, but
